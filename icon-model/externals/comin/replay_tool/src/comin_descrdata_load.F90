...
CONTAINS
  ...
  SUBROUTINE comin_descrdata_load_domain(ncid, descrdata_obj)
    ...
    ENDIF
    CALL nf90(nf90_get_att(ncid, NF90_GLOBAL, "number_of_grid_used", descrdata_obj%number_of_grid_used))
    ...
    ENDIF
    status = nf90_inquire_attribute(ncid, NF90_GLOBAL, "parent_id", len=shap(1))
    IF(status == NF90_NOERR) THEN
      ALLOCATE(descrdata_obj%parent_id)
      CALL nf90(nf90_get_att(ncid, NF90_GLOBAL, "parent_id", descrdata_obj%parent_id))
    ENDIF
    status = nf90_inq_varid(ncid, "child_id", var_id)
    IF (status == NF90_NOERR) THEN
      shap(1:1) = nf90_utils_get_shape(ncid, "child_id", 1)
      ALLOCATE(descrdata_obj%child_id(shap(1)))
      CALL nf90(nf90_get_var(ncid, var_id, descrdata_obj%child_id))
    ENDIF
