...
CONTAINS
  ...
  SUBROUTINE comin_descrdata_save_domain(ncid, descrdata_obj)
    ...
    ENDIF
    CALL nf90(nf90_put_att(ncid, NF90_GLOBAL, "number_of_grid_used", descrdata_obj%number_of_grid_used))
    IF (ASSOCIATED(descrdata_obj%id)) THEN
      CALL nf90(nf90_put_att(ncid, NF90_GLOBAL, "id", descrdata_obj%id))
    ENDIF
    IF (ASSOCIATED(descrdata_obj%parent_id)) THEN
      CALL nf90(nf90_put_att(ncid, NF90_GLOBAL, "parent_id", descrdata_obj%parent_id))
    ENDIF
    IF (ASSOCIATED(descrdata_obj%child_id)) THEN
      var_id = nf90_utils_def_var(ncid, "child_id", NF90_INT, SHAPE(descrdata_obj%child_id))
      CALL nf90(nf90_put_var(ncid, var_id, descrdata_obj%child_id))
    ENDIF
