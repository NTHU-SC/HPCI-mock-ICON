...
MODULE mo_math_utilities
  ...
  USE mo_lib_grid_geometry_info, ONLY: t_grid_geometry_info, planar_torus_geometry, sphere_geometry
...
!   PUBLIC :: sphere_cell_mean_char_length
  ...
  INTERFACE check_orientation
    MODULE PROCEDURE check_orientation_dp
  ...
  TYPE t_value_set
    ...
    REAL(dp), ALLOCATABLE :: values(:) !< sorted set of values.
...
CONTAINS
  ...
  INTEGER FUNCTION check_orientation_dp(lonc, lon, lat, n)
    ...
    REAL(dp), INTENT(in) :: lon(n), lat(n)

    REAL(dp) :: lonl(n), latl(n)
    REAL(dp) :: area
    ...
    DO i = 1, n
      lonl(i) = lonl(i) - lonc
      IF (lonl(i) < -pi) THEN
        lonl(i) = pi + MOD(lonl(i), REAL(pi, kind=dp))
      END IF
      IF (lonl(i) > pi) THEN
        lonl(i) = -pi + MOD(lonl(i), REAL(pi, kind=dp))
      END IF
    ...
    IF (area >= 0.0_dp) THEN
      check_orientation_dp = +1
    ELSE
      check_orientation_dp = -1
    END IF

  END FUNCTION check_orientation_dp

  !-------------------------------------------------------------------------
  ! Variant for working-precision=sp lon+lat in ICON
  INTEGER FUNCTION check_orientation_sp(lonc, lon, lat, n)
    INTEGER, INTENT(in) :: n
    REAL(sp), INTENT(in) :: lonc
    REAL(sp), INTENT(in) :: lon(n), lat(n)

    REAL(sp) :: lonl(n), latl(n)
    REAL(sp) :: area
    ...
    DO i = 1, n
      lonl(i) = lonl(i) - lonc
      IF (lonl(i) < -pi) THEN
        lonl(i) = pi + MOD(lonl(i), REAL(pi, kind=sp))
      END IF
      IF (lonl(i) > pi) THEN
        lonl(i) = -pi + MOD(lonl(i), REAL(pi, kind=sp))
      END IF
    ...
    IF (area >= 0.0_sp) THEN
      check_orientation_sp = +1
    ELSE
      check_orientation_sp = -1
    END IF

  END FUNCTION check_orientation_sp
...
!$OMP END PARALLEL
  ...
  !! for the tangent plane at the sphere on the given point (lon,lat)
...
!$OMP END PARALLEL DO
  ...
  !!  north pole direction (z-direction), second around new (y-direction).
  ...
  !!  <i>gc2cc</i>
  !!  <li>  Apply rotation matrix @f$R(\lambda_{in}^{ref}, \varphi_{in}^{ref})@f$
  ...
  !! conventions and formalism as "disp_new". The algorithm is as
  !! follows:
  ...
  !! Not used since the characteristic length os now part of the grid_geometry_info. LL, MPI-M (2012-12)
...
!   END SUBROUTINE sphere_cell_mean_char_length
  ...
  !-------------------------------------------------------------------------
  !>
  !! Checking turn when traveling along three points
  !!
...
! solve for vectors c-prime and d-prime
    DO i = 2, n
      m = b(i) - c_p(i - 1)*a(i)
      c_p(i) = c(i)/m
      d_p(i) = (d(i) - d_p(i - 1)*a(i))/m
    END DO
! initialize varout
    varout(n) = d_p(n)
! solve for varout from the vectors c-prime and d-prime
    DO i = n - 1, 1, -1
      varout(i) = d_p(i) - c_p(i)*varout(i + 1)
    END DO
...
!NEC$ outerloop_unroll(4)
  ...
  !-------------------------------------------------------------------------
  !> Merges a list of REAL(dp) numbers into another list, yielding the
  ...
  SUBROUTINE merge_values_into_set(nin_values, in_values, value_set, opt_tol)
    ...
    REAL(dp), INTENT(IN)           :: in_values(:) !< list of input values
    TYPE(t_value_set), INTENT(INOUT)        :: value_set !< result values (union set)
    REAL(dp), INTENT(IN), OPTIONAL :: opt_tol !< (optional:) tolerance for floating-point equality
    ! local variables
    CHARACTER(LEN=*), PARAMETER :: routine = '::merge_values_into_set'
    REAL(dp), PARAMETER :: default_tol = EPSILON(1._dp)*1e3_dp
    REAL(dp) :: tol
  ...
  !-------------------------------------------------------------------------
  !> Compares a list of REAL(dp) numbers with the contents of a
  ...
  !> Resizes the REAL(dp) data structures of a "t_value_set" derived type object.
  !
  SUBROUTINE resize_set(value_set, nadditional_values)
    ...
    REAL(dp), ALLOCATABLE :: tmp_rbuf(:)

    ! increase size of REAL(dp) buffer
    oldsize = 0
