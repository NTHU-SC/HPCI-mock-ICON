...
MODULE mo_lib_interpolation_scalar
  !-------------------------------------------------------------------------
  !
  USE mo_iconlib_kind, ONLY: wp, vp, dp, sp
  ...
  INTERFACE cells2edges_scalar_lib
    MODULE PROCEDURE cells2edges_scalar_dp_lib, cells2edges_scalar_sp_lib
    MODULE PROCEDURE cells2edges_scalar_sp2dp_lib
  END INTERFACE cells2edges_scalar_lib
CONTAINS
...
!!
  SUBROUTINE verts2edges_scalar_lib(p_vertex_in, edge_vertex_idx, edge_vertex_blk, &
    &                               coeff_int, p_edge_out, i_startblk, i_endblk, i_startidx_in, i_endidx_in, &
...
!$ACC DATA COPYIN(p_vertex_in, coeff_int) PRESENT(p_edge_out, iidx, iblk) IF(lzacc)
    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jk = slev, elev
        DO je = i_startidx, i_endidx
          ...
          p_edge_out(je, jk, jb) =  &
            coeff_int(je, 1, jb)*p_vertex_in(iidx(je, jb, 1), jk, iblk(je, jb, 1)) + &
            coeff_int(je, 2, jb)*p_vertex_in(iidx(je, jb, 2), jk, iblk(je, jb, 2))
...
!!  velocity points.
!!
  SUBROUTINE cells2edges_scalar_dp_lib(p_cell_in, edge_cell_idx, edge_cell_blk, coeff_int, p_edge_out, &
    ...
    REAL(dp), INTENT(IN) :: coeff_int(:, :, :)

    ! edge based scalar output field, dim: (nproma,nlev,nblks_e)
    REAL(dp), INTENT(INOUT) :: p_edge_out(:, :, :)
...
!$ACC DATA NO_CREATE(iidx, iblk, coeff_int, p_cell_in, p_edge_out) IF(lzacc)
      ...
      DO jb = i_startblk, i_endblk
        ...
        DO je = i_startidx, i_endidx
          ...
          ELSE IF (iidx(je, jb, 2) >= 1 .AND. iblk(je, jb, 2) >= 1) THEN
            ...
            END DO
          ELSE
            CALL finish('mo_interpolation:cells2edges_scalar_dp',  &
    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jk = slev, elev
        DO je = i_startidx, i_endidx
          ...
          p_edge_out(je, jk, jb) =  &
            coeff_int(je, 1, jb)*p_cell_in(iidx(je, jb, 1), jk, iblk(je, jb, 1)) +  &
            coeff_int(je, 2, jb)*p_cell_in(iidx(je, jb, 2), jk, iblk(je, jb, 2))
...
!$ACC END DATA
  ...
  END SUBROUTINE cells2edges_scalar_dp_lib
!-----------------------------------------
  SUBROUTINE cells2edges_scalar_sp_lib(p_cell_in, edge_cell_idx, edge_cell_blk, coeff_int, p_edge_out, &
    &                               i_startblk_in, i_endblk_in, i_startidx_in, i_endidx_in, &
    &                               slev, elev, nproma, patch_id, l_limited_area, lfill_latbc, lacc)
    ...
    ! coefficients for linear interpolation, dim: (nproma,2,nblks_e)
    REAL(sp), INTENT(IN) :: coeff_int(:, :, :)

    ! edge based scalar output field, dim: (nproma,nlev,nblks_e)
    REAL(sp), INTENT(INOUT) :: p_edge_out(:, :, :)
      ...
      DO jb = i_startblk, i_endblk
        ...
        DO je = i_startidx, i_endidx
          ...
          ELSE IF (iidx(je, jb, 2) >= 1 .AND. iblk(je, jb, 2) >= 1) THEN
            ...
            END DO
          ELSE
            CALL finish('mo_interpolation:cells2edges_scalar_sp',  &
              &          'error in lateral boundary filling')
          END IF
        END DO
      END DO
...
!$ACC END DATA
  ...
  END SUBROUTINE cells2edges_scalar_sp_lib

  SUBROUTINE cells2edges_scalar_sp2dp_lib(p_cell_in, edge_cell_idx, edge_cell_blk, coeff_int, p_edge_out, &
    &                               i_startblk_in, i_endblk_in, i_startidx_in, i_endidx_in, &
    &                               slev, elev, nproma, patch_id, l_limited_area, lfill_latbc, lacc)
      ...
      DO jb = i_startblk, i_endblk
        ...
        DO je = i_startidx, i_endidx
          ...
          ELSE IF (iidx(je, jb, 2) >= 1 .AND. iblk(je, jb, 2) >= 1) THEN
            ...
            END DO
          ELSE
            CALL finish('mo_interpolation:cells2edges_scalar_sp2dp_lib',  &
              &          'error in lateral boundary filling')
          END IF
        END DO
      END DO
...
!$ACC END DATA
  ...
  END SUBROUTINE cells2edges_scalar_sp2dp_lib
!------------------------------------------------------------------------
!
!>
...
!!
  SUBROUTINE edges2cells_scalar_dp_lib(p_edge_in, edge_idx, edge_blk, coeff_int, p_cell_out, &
    &                                  i_startblk, i_endblk, i_startidx_in, i_endidx_in, &
    &                                  slev, elev, nproma, lacc)
...
!$ACC DATA PRESENT(coeff_int, p_edge_in, p_cell_out, iidx, iblk) IF(lzacc)
    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jk = slev, elev
        DO jc = i_startidx, i_endidx
          ...
          p_cell_out(jc, jk, jb) = &
            coeff_int(jc, 1, jb)*p_edge_in(iidx(jc, jb, 1), jk, iblk(jc, jb, 1)) + &
            coeff_int(jc, 2, jb)*p_edge_in(iidx(jc, jb, 2), jk, iblk(jc, jb, 2)) + &
            coeff_int(jc, 3, jb)*p_edge_in(iidx(jc, jb, 3), jk, iblk(jc, jb, 3))
...
!!  cell centers via given interpolation weights
!!
  SUBROUTINE edges2cells_scalar_sp_lib(p_edge_in, cell_edge_idx, cell_edge_blk, coeff_int, p_cell_out, &
    &                                  i_startblk, i_endblk, i_startidx_in, i_endidx_in,  &
    &                                  slev, elev, nproma, lacc)
...
!!
  SUBROUTINE cells2verts_scalar_dp_lib(p_cell_in, vert_cell_idx, vert_cell_blk, coeff_int, p_vert_out, &
    &                                  i_startblk, i_endblk, i_startidx_in, i_endidx_in, &
...
!$ACC DATA NO_CREATE(iidx, iblk, p_cell_in, coeff_int, p_vert_out) IF(lzacc)
...
!$NEC outerloop_unroll(4)
      DO jk = slev, elev
        DO jv = i_startidx, i_endidx
          ...
          p_vert_out(jv, jk, jb) = &
            coeff_int(jv, 1, jb)*p_cell_in(iidx(jv, jb, 1), jk, iblk(jv, jb, 1)) + &
            coeff_int(jv, 2, jb)*p_cell_in(iidx(jv, jb, 2), jk, iblk(jv, jb, 2)) + &
            coeff_int(jv, 3, jb)*p_cell_in(iidx(jv, jb, 3), jk, iblk(jv, jb, 3)) + &
            coeff_int(jv, 4, jb)*p_cell_in(iidx(jv, jb, 4), jk, iblk(jv, jb, 4)) + &
            coeff_int(jv, 5, jb)*p_cell_in(iidx(jv, jb, 5), jk, iblk(jv, jb, 5)) + &
            coeff_int(jv, 6, jb)*p_cell_in(iidx(jv, jb, 6), jk, iblk(jv, jb, 6))
...
!!  Computes  average of scalar fields from centers of cells to vertices.
!!
  SUBROUTINE cells2verts_scalar_sp_lib(p_cell_in, vert_cell_idx, vert_cell_blk, coeff_int, p_vert_out,&
    &                                  i_startblk, i_endblk, i_startidx_in, i_endidx_in, &
    &                                  slev, elev, nproma, lacc, acc_async)
...
!$ACC DATA NO_CREATE(p_cell_in, coeff_int, p_vert_out, iidx, iblk) IF(lzacc)
...
!!  Computes  average of scalar fields from centers of cells to vertices.
!!
  SUBROUTINE cells2verts_scalar_sp2dp_lib(p_cell_in, vert_cell_idx, vert_cell_blk, coeff_int, p_vert_out, &
    &                                     i_startblk, i_endblk, i_startidx_in, i_endidx_in, &
    &                                     slev, elev, nproma, lacc, acc_async)
...
!CDIR UNROLL=6
      DO jk = slev, elev
        DO jv = i_startidx, i_endidx
          ...
          p_vert_out(jv, jk, jb) = &
            coeff_int(jv, 1, jb)*REAL(p_cell_in(iidx(jv, jb, 1), jk, iblk(jv, jb, 1)), dp) + &
            coeff_int(jv, 2, jb)*REAL(p_cell_in(iidx(jv, jb, 2), jk, iblk(jv, jb, 2)), dp) + &
            coeff_int(jv, 3, jb)*REAL(p_cell_in(iidx(jv, jb, 3), jk, iblk(jv, jb, 3)), dp) + &
            coeff_int(jv, 4, jb)*REAL(p_cell_in(iidx(jv, jb, 4), jk, iblk(jv, jb, 4)), dp) + &
            coeff_int(jv, 5, jb)*REAL(p_cell_in(iidx(jv, jb, 5), jk, iblk(jv, jb, 5)), dp) + &
            coeff_int(jv, 6, jb)*REAL(p_cell_in(iidx(jv, jb, 6), jk, iblk(jv, jb, 6)), dp)
...
!!
!!
  SUBROUTINE cells2verts_scalar_ri_lib(p_cell_in, vert_cell_idx, vert_cell_blk, coeff_int, p_vert_out, &
    &                                  i_startblk, i_endblk, i_startidx_in, i_endidx_in, &
    &                                  slev, elev, nproma, lacc, acc_async)
...
!!
  SUBROUTINE verts2cells_scalar_lib(p_vert_in, cell_vertex_idx, cell_vertex_blk, &
    ...
    ! chunk length in last block for the cells: t_patch%npromz_c
...
!$ACC DATA PRESENT(p_vert_in, coeff_int, p_cell_out, iidx, iblk) IF(lzacc)
...
!$NEC outerloop_unroll(4)
      DO jk = slev, elev
        DO jc = 1, nlen
          ...
          p_cell_out(jc, jk, jb) = &
            coeff_int(jc, 1, jb)*p_vert_in(iidx(jc, jb, 1), jk, iblk(jc, jb, 1)) + &
            coeff_int(jc, 2, jb)*p_vert_in(iidx(jc, jb, 2), jk, iblk(jc, jb, 2)) + &
            coeff_int(jc, 3, jb)*p_vert_in(iidx(jc, jb, 3), jk, iblk(jc, jb, 3))
