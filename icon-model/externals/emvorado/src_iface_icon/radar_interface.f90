...
!==============================================================================
  ...
  REAL(kind=dp) :: &
       ...
       dom_minimu_lon(ndoms_max) = HUGE(1.0_dp), &
       dom_maximu_lon(ndoms_max) = -HUGE(1.0_dp), &
       dom_minimu_lat(ndoms_max) = HUGE(1.0_dp), &
       dom_maximu_lat(ndoms_max) = -HUGE(1.0_dp)
#endif
...
!==============================================================================
  ...
  ! Flag for inner domain grid points. Will be set false for inner and outer halo points:
  LOGICAL, ALLOCATABLE :: is_inner_domain(:,:)
...
!==============================================================================
  ...
  PUBLIC :: abort_run, setup_auxgrid_for_cellindex, &
...
!**************************************************************************************************
  ...
  SUBROUTINE get_model_config_for_radar ( idom )
    ...
    ! Set up domain decomposition of ICON:
  ...
  SUBROUTINE get_model_hydrometeors (ntlev, idom, rain2mom_mu_incloud)
    ...
    REAL(kind=dp), INTENT(in) :: rain2mom_mu_incloud
    ...
    IF (ALLOCATED(p_nh_state)) THEN  
...
#ifdef _OPENACC
      IF (ltimer) CALL timer_start(timer_radar_acc_data_copies)
      CALL radar_d2h_hydrometeors(ntlev, idom, .TRUE.)
      IF (ltimer) CALL timer_stop(timer_radar_acc_data_copies)
#endif
      ...
      IF (ldebug_radsim .AND. my_radar_id == 0) THEN
        ...
        END IF
      END IF
      CALL update_mph(rain2mom_mu_incloud,itype_gscp_fwo)
  ...
  SUBROUTINE init_2mom_types ()
    ...
    TYPE(particle) :: cloud_2mom, rain_2mom, ice_2mom, snow_2mom, graupel_2mom, hail_2mom
...
! - unknown/irrelevant parameters set to fill value
  ...
  SUBROUTINE update_mph(rain2mom_mu_incloud, itype_gscp_fwo)
    IMPLICIT NONE
    ...
    ! Local variables:
    CHARACTER(LEN=*), PARAMETER :: yzroutine = 'update_mph'

    IF (rain2mom_mu_incloud .NE. unused_value) THEN
      IF (itype_gscp_fwo >= 200) THEN
        IF (rain2mom_mu_incloud > -1.0) THEN
          IF (my_radar_id == 0) THEN
            WRITE (*,'(a,2(e12.5,a))') &
                   'INFO EMVORADO '//TRIM(yzroutine)// &
                   ': rain2mom_mu_incloud modified by user (',rain%mu, &
                                                       ' -> ',rain2mom_mu_incloud,')'
          END IF
          rain%mu = rain2mom_mu_incloud
        ELSE
          IF (my_radar_id == 0) THEN
            WRITE (*,'(a)') &
                   'INFO EMVORADO '//TRIM(yzroutine)// &
                   ': namelist-modified rain2mom_mu_incloud NOT applied (due to mu <= -1.0)'
          END IF
        END IF
      ELSE
        IF (my_radar_id == 0) THEN
          WRITE (*,'(a)') &
                 'INFO EMVORADO '//TRIM(yzroutine)// &
                 ': namelist-modified rain2mom_mu_incloud NOT applied (due to 1mom scheme)'
        END IF
      END IF
    END IF
  END SUBROUTINE update_mph
    ...
    IF (ALLOCATED(p_nh_state)) THEN
      ...
      IF (p_patch(idom)%ldom_active) THEN
        IF (get_model_time_sec() /= time_mod_lastcall .OR. idom_lastcall /= idom .OR. &
          ...
          CALL diagnose_and_store_uv_pres_temp (ntlev_dyn, ntlev_qx, idom) ! internal subroutine, which is GPU aware with lacc=.TRUE.
...
#ifdef _OPENACC
      IF (ltimer) CALL timer_start(timer_radar_acc_data_copies)
      ! Copy all required data from GPU onto CPU
      CALL radar_d2h_model_variables(ntlev_dyn, idom, lacc=.TRUE.) ! TODO for upcoming GPU port: please improve the down-passing of `lacc`.
      IF (ltimer) CALL timer_stop(timer_radar_acc_data_copies)
#endif
  ...
  SUBROUTINE setup_auxgrid_for_cellindex (idom, nradsta, &
    ...
    INTEGER           :: idom_fwo, i, ii, iidx, k, kk, iblk, i_startblk, i_endblk, is, ie, in, jn, &
         ...
         &               i_rlstart_noouterhalo, i_rlstart_withouterhalo, i_rlend_noinnerhalo, i_rlend_withinnerhalo, &
         &               i_rlstart_here, i_rlend_here
    ...
    ! JM250606:
    ! This distinction is unnecessary, and the SH pollat formula in particular is wrong.
    !   While when interpreting pollon/lat literally as pole coordinates, lat>90° might
    !   confuse, but these are actually rotation angles and for these >90° is perfectly ok.
    ...
    ! 0) Preparation of loops over cells: general config of idx and blk loop boundaries:
    !    -------------------------------------------------------------------------------
    i_rlstart_noouterhalo   = grf_bdywidth_c+1
    i_rlstart_withouterhalo = 1
    i_rlend_noinnerhalo     = min_rlcell_int
    i_rlend_withinnerhalo   = min_rlcell

    ! 1b) Store and exchange the grid cell center lon/lats:
    !     -------------------------------------------------

    i_rlstart_here = i_rlstart_withouterhalo   ! including outer halo cells
    i_rlend_here   = i_rlend_noinnerhalo       ! excluding interior halo cells

    i_startblk = p_patch(idom) % cells % start_block(i_rlstart_here)
    i_endblk   = p_patch(idom) % cells % end_block(i_rlend_here)
...
!$OMP DO
    DO k = i_startblk, i_endblk
      ...
      CALL get_indices_c(p_patch(idom), k, i_startblk, i_endblk, is, ie, i_rlstart_here, i_rlend_here) ! excluding halo cells
...
!$OMP END PARALLEL
    ...
    i_rlstart_here = i_rlstart_withouterhalo
    i_rlend_here   = i_rlend_withinnerhalo

    i_startblk = p_patch(idom) % cells % start_block(i_rlstart_here)
    i_endblk   = p_patch(idom) % cells % end_block(i_rlend_here)       ! including halo cells
...
!$OMP&   REDUCTION(max:clon_max,clat_max)
    DO k = i_startblk, i_endblk
      ...
      CALL get_indices_c(p_patch(idom), k, i_startblk, i_endblk, is, ie, i_rlstart_here, i_rlend_here)
      ...
      DO i = is, ie
        ...
        IF (clon(i,k) > miss_threshold) THEN
...
!$OMP END PARALLEL
    ...
    i_rlstart_here = i_rlstart_withouterhalo
    i_rlend_here   = i_rlend_noinnerhalo
...
!$OMP END PARALLEL
    ...
    i_rlstart_here = i_rlstart_withouterhalo
    i_rlend_here   = i_rlend_noinnerhalo  ! exluding the inner halo cells
...
!$OMP END PARALLEL
    ...
    IF (cindex_grid(idom_fwo)%nlon > 0 .AND. cindex_grid(idom_fwo)%nlat > 0) THEN
      ...
      i_rlstart_here = i_rlstart_withouterhalo
      i_rlend_here   = i_rlend_withinnerhalo  ! inluding the halo cells
...
!      i_rlstart_here = i_rlstart_noouterhalo
      i_rlstart_here = i_rlstart_withouterhalo
      i_rlend_here   = i_rlend_withinnerhalo  ! inluding the halo cells
...
!NEC$ ivdep
            DO i = is, ie
              ...
              IF (inear(i,k) > -HUGE(1)) THEN
                ...
                ! JM251008: Replacement by cleaner formulation according to suggesting by UB
                !IF ( ( in /= inear(i,k) .AND. in >= 1 .AND. in <= cindex_grid(idom_fwo)%nlon ) .OR. &
                !     ( jn /= jnear(i,k) .AND. jn >= 1 .AND. jn <= cindex_grid(idom_fwo)%nlat ) ) THEN
                IF ( ( in /= inear(i,k) .OR. jn /= jnear(i,k) ) .AND. &
                     ( in >= 1 .AND. in <= cindex_grid(idom_fwo)%nlon .AND. &
                       jn >= 1 .AND. jn <= cindex_grid(idom_fwo)%nlat ) ) THEN
...
!$OMP END PARALLEL
    ...
    IF (cindex_grid(idom_fwo)%nlon > 0 .AND. cindex_grid(idom_fwo)%nlat > 0 .AND. ldebug) THEN
      ...
      testfile(:) = ' '
      WRITE (testfile, '(i5)') cindex_grid(idom_fwo)%nlon
      testfile = ADJUSTL(testfile)
      DO k = 1, cindex_grid(idom_fwo)%nlat
        WRITE (350+my_cart_id_fwo, '('//TRIM(testfile)//'(i6))') &
             (MAX(0,cindex_grid(idom_fwo)%cind(i,k)), i=1,cindex_grid(idom_fwo)%nlon)
      END DO
      ...
      testfile = ADJUSTL(testfile)
      DO k = 1, cindex_grid(idom_fwo)%nlat
        WRITE (350+my_cart_id_fwo, '('//TRIM(testfile)//'(i8))') &
             (NINT(MIN(MAX(-999.0_dp, cindex_grid(idom_fwo)%dist(i,k)),1e6_dp)), i=1,cindex_grid(idom_fwo)%nlon)
      END DO
      CLOSE(350+my_cart_id_fwo)
      ...
      testfile(:) = ' '
      WRITE (testfile, '(i5)') MIN(cindex_grid(idom_fwo)%nlon, HUGE(1))
      testfile = ADJUSTL(testfile)
      DO k = 1, cindex_grid(idom_fwo)%nlat
...
! Use reduced domain in any case for ICON: no interior or outer halo cells:
  ...
  FUNCTION get_domain_starttime_in_sec (idom) RESULT (time)
    ...
    IF (idom == 1 .AND. ANY((/MODE_IAU,MODE_IAU_OLD/)==init_mode)) THEN
      time = time_config%timeshift%dt_shift   ! [s] since tc_exp_startdate
    ELSE
  ...
  SUBROUTINE alloc_aux_model_variables (idom, lonline)
    ...
    INTEGER             :: i_startblk, i_endblk, is, ie
    ...
    IF (.NOT.ALLOCATED(is_inner_domain)) ALLOCATE(is_inner_domain(ni,nk))
    ...
    ! Mask of inner domain points (without any halo points):
    is_inner_domain(:,:) = .FALSE.

    i_startblk = p_patch(idom) % cells % start_block(grf_bdywidth_c+1) ! no outer halo points
    i_endblk   = p_patch(idom) % cells % end_block(min_rlcell_int)     ! no inner halo points

    DO k = i_startblk, i_endblk
      CALL get_indices_c(p_patch(idom), k, i_startblk, i_endblk, is, ie, grf_bdywidth_c+1, min_rlcell_int)
      DO i = is, ie
        is_inner_domain(i,k) = .TRUE.
      END DO
    END DO
  ...
  SUBROUTINE dealloc_aux_model_variables ()
    ...
    IF (ALLOCATED(is_inner_domain)) DEALLOCATE(is_inner_domain)
...
!================================================================================
  ...
  SUBROUTINE get_domaincenter_global (idom_model, ldebug)
    ...
    IF ( lcompute_pe_fwo) THEN
      ...
      i_startblk = p_patch(idom_model) % cells % start_block(1)        ! including outer halo cells
      i_endblk   = p_patch(idom_model) % cells % end_block(min_rlcell) ! including innter halo cells

      DO k = i_startblk, i_endblk

        CALL get_indices_c(p_patch(idom_model), k, i_startblk, i_endblk, is, ie, 1, min_rlcell)
    ...
    ! Also keep domain max extent for re-use elsewhere
    ! (seems more appropriate to derive these only initially and maybe store in a derived
    !  type, then have a subroutine to derive the center from them)
    dom_minimu_lon(idom_fwo) = clon_min
    dom_maximu_lon(idom_fwo) = clon_max
    dom_minimu_lat(idom_fwo) = clat_min
    dom_maximu_lat(idom_fwo) = clat_max
  ...
  SUBROUTINE setup_model2radarbins_vec (idom_model, rs_meta, nobsmax, lon_r, lat_r, alt_r, &
    ...
    CALL geo2cell_index2d_vec (cindex_grid(list_domains_for_radar(idom_model)), p_patch(idom_model), &
                               is_inner_domain(:,:), lon_r(:), lat_r(:), idx(:), blk(:))
#else
!$omp parallel do
    DO irp = 1, nobsmax
      CALL geo2cell_index2d (cindex_grid(list_domains_for_radar(idom_model)), p_patch(idom_model), &
                             is_inner_domain(:,:), lon_r(irp), lat_r(irp), idx(irp), blk(irp))
    ENDDO
!$omp end parallel do
#endif
...
!#ifdef __SX__
!      CALL geo2cell_index2d_vec (cindex_grid(list_domains_for_radar(idom_model)), p_patch(idom_model), &
!                                 is_inner_domain(:,:), lon_g(:,n), lat_g(:,n), idx(:), blk(:))
!
...
!#else
      DO m = 1, rs_grid%naz_nbl  ! loop over azimuths
        ...
        CALL geo2cell_index2d (cindex_grid(list_domains_for_radar(idom_model)), p_patch(idom_model), &
                               is_inner_domain(:,:), lon_g(m,n), lat_g(m,n), idx(m), blk(m))
!#endif
...
!==============================================================================
  ...
  SUBROUTINE geo2cell_index2d (cidx_grid, p_patch_dom, is_inner_point, lon_geo, lat_geo, idx, blk)

    IMPLICIT NONE
    TYPE(t_cindex_grid), INTENT(in) :: cidx_grid
    TYPE(t_patch)      , INTENT(in) :: p_patch_dom
    LOGICAL, INTENT(in)             :: is_inner_point(:,:)   ! Flag for inner domain points (no halo), dim(n_idx,n_blk)
    ...
    INTEGER                         :: i, k, inb, knb, idxtmp, blktmp, idx_orig, blk_orig, idx_v, blk_v

    IF ( ALLOCATED(cidx_grid%idx) ) THEN
      ...
      IF ( i > 0 .AND. i <= cidx_grid%nlon .AND. &
        ...
        idx_orig = cidx_grid%idx(i,k)
        blk_orig = cidx_grid%blk(i,k)

        idx = idx_orig
        blk = blk_orig
        
        IF (idx_orig > -HUGE(1)) THEN

          ! distance of the original candidate cell to the point of interest:
          tmplon = p_patch_dom % cells % center(idx_orig,blk_orig) % lon * raddeg
          tmplat = p_patch_dom % cells % center(idx_orig,blk_orig) % lat * raddeg
          dist = geo_dist(lon_geo, lat_geo, tmplon, tmplat, cidx_grid%r_earth, 0.0_dp)

          ! search in the neighbour cells of the original candidate if any of them might be closer:
          DO knb = 1, SIZE(p_patch_dom % cells % vertex_idx, dim=3)   ! loop over the 3 vertices

            ! The indices of the vertices of the n-th cell (original candidate):
            idx_v = p_patch_dom % cells % vertex_idx(idx_orig,blk_orig,knb)
            blk_v = p_patch_dom % cells % vertex_blk(idx_orig,blk_orig,knb)

            DO inb = 1, SIZE(p_patch_dom % verts % cell_idx, dim=3) ! loop over the 6 neighbours of each vertex
           
              ! The indices of the neighbouring cells of the vertex:
              idxtmp = p_patch_dom % verts % cell_idx(idx_v,blk_v,inb)
              blktmp = p_patch_dom % verts % cell_blk(idx_v,blk_v,inb)

              ! exclude the original candidate cell from the search:
              IF (idxtmp /= idx_orig .OR. blktmp /= blk_orig) THEN
                tmplon = p_patch_dom % cells % center(idxtmp,blktmp) % lon * raddeg
                tmplat = p_patch_dom % cells % center(idxtmp,blktmp) % lat * raddeg
          ...
          ! if the identified nearest cell is not among the interior cells,
          !  it will be found on another PE and is flagged as not found here:
          IF (.NOT. is_inner_point(idx,blk)) THEN
            idx = -HUGE(1)
            blk = -HUGE(1)
          END IF
  ...
  SUBROUTINE geo2cell_index2d_vec (cidx_grid, p_patch_dom, is_inner_point, lon_geo, lat_geo, idx, blk)
    ...
    INTEGER                         :: n, ii,kk, inb,knb, idxtmp, blktmp, size_vec, idx_v, blk_v
    INTEGER, ALLOCATABLE            :: i(:), k(:), idx_orig(:), blk_orig(:)
    ...
    IF (size_vec > 0) THEN
      ...
      IF ( ALLOCATED(cidx_grid%idx) ) THEN
        ...
        ALLOCATE (idx_orig(size_vec), blk_orig(size_vec))

        ! Prepare search in the cells neighbours,
        !  if any of them might be closer:
        ! -----------------------------------------------------------
        ...
        DO  n = 1, size_vec
          ...
          IF ( i(n) > 0 .AND. i(n) <= cidx_grid%nlon .AND. &
            ...
            IF (idx(n) > -HUGE(1)) THEN
              ...
              tmplat = p_patch_dom % cells % center(idx(n),blk(n)) % lat * raddeg
        ...
        ! Now do a search among the neighbouring triangles and check if any of them is
        !  closer to the n-th geo point than the original candidate:
        ! The neighbours can be found among the 6 (5) neighbours of the 3 cell vertices.
        ! -------------------------------------------------------------------
        ...
        DO knb = 1, SIZE(p_patch_dom % cells % vertex_idx, dim=3)  ! loop over the 3 vertices
          DO inb = 1, SIZE(p_patch_dom % verts % cell_idx, dim=3)  ! loop over the 6 neighbours of each vertex
            ...
            DO  n = 1, size_vec
              ...
              IF (idx_orig(n) > -HUGE(1)) THEN

                ! The indices of the vertices of the n-th cell (original candidate):
                idx_v = p_patch_dom % cells % vertex_idx(idx_orig(n),blk_orig(n),knb)
                blk_v = p_patch_dom % cells % vertex_blk(idx_orig(n),blk_orig(n),knb)
                ...
                ! exclude the original candidate cell from the search:
                IF (idxtmp /= idx_orig(n) .OR. blktmp /= blk_orig(n)) THEN
            
                  ! The geo coords of the neighour cell
                  !  and its distance to the n-th geo point:
                  tmplon = p_patch_dom % cells % center(idxtmp,blktmp) % lon * raddeg
                  tmplat = p_patch_dom % cells % center(idxtmp,blktmp) % lat * raddeg
                  disttmp = geo_dist(lon_geo(n), lat_geo(n), tmplon, tmplat, cidx_grid%r_earth, 0.0_dp)
                  ! If this neighbour is nearer to the n-th geo point, it will become
                  !  the new candidate:
        ...
        DO n=1, size_vec
          IF (idx(n) > -HUGE(1)) THEN
            IF (.NOT. is_inner_point(idx(n),blk(n))) THEN
              idx(n) = -HUGE(1)
              blk(n) = -HUGE(1)
            END IF
          END IF
        ...
        DEALLOCATE (dist, i, k, idx_orig, blk_orig)
  ...
  ! Global 1D-index of the enclosing ICON cell for a geographic location 
  FUNCTION geo2cell_index1d (cidx_grid, p_patch_dom, is_inner_point, lon_geo, lat_geo) RESULT (index1d)

    IMPLICIT NONE
    TYPE(t_cindex_grid), INTENT(in) :: cidx_grid
    TYPE(t_patch)      , INTENT(in) :: p_patch_dom
    LOGICAL, INTENT(in)             :: is_inner_point(:,:)
    ...
    IF ( ALLOCATED(cidx_grid%idx) ) THEN
      ...
      CALL geo2cell_index2d (cidx_grid, p_patch_dom, is_inner_point, lon_geo, lat_geo, idx, blk)

      IF (idx > -HUGE(1)) THEN
        index1d = idx_1d(idx,blk)
      ELSE
        index1d = -HUGE(1)
      END IF
  ...
  FUNCTION geo2cell_index1d_glob (cidx_grid,  p_patch_dom, is_inner_point, lon_geo, lat_geo) RESULT (index1d)
    ...
    IF ( ALLOCATED(cidx_grid%idx) ) THEN
      ...
      IF (idx > -HUGE(1)) THEN
        i = idx_1d(idx,blk)
        index1d = p_patch_dom % cells % decomp_info % glb_index(i)
      ELSE
        index1d = -HUGE(1)
      END IF
  ...
  SUBROUTINE geo2model_cellindex_real (idom_model, lon_geo, lat_geo, i_model, k_model)
    ...
    i_model = REAL(geo2cell_index1d_glob (cindex_grid(list_domains_for_radar(idom_model)), p_patch(idom_model), &
         is_inner_domain, lon_geo, lat_geo), kind=dp)
    k_model = 1.0_dp
  ...
  SUBROUTINE geo2model_cellindex_int (idom_model, lon_geo, lat_geo, i_model, k_model)
    ...
    i_model = geo2cell_index1d_glob (cindex_grid(list_domains_for_radar(idom_model)), p_patch(idom_model), &
         is_inner_domain, lon_geo, lat_geo)
    k_model = 1
  ...
  FUNCTION is_inside_loc_domain (idom_model, lon_geo, lat_geo) RESULT (is_inside)
    ...
    is_inside = ( geo2cell_index1d (cindex_grid(list_domains_for_radar(idom_model)), p_patch(idom_model), &
                                                            is_inner_domain, lon_geo, lat_geo) > -HUGE(1) )
  ...
  SUBROUTINE get_rotlatlon_domain_for_superobing (idom_model,pollon_s,pollat_s,polgam_s,startlon_s,startlat_s,endlon_s,endlat_s)
    ...
    !  for superobing. Use full extend of current ICON domain:

    idom_r = list_domains_for_radar(idom_model)

    IF (my_radar_id == 0) THEN
      pollon_s = MODULO(dom_center_lon(idom_r), 360.0_dp) - 180.0_dp
      pollat_s = 90.0_dp - dom_center_lat(idom_r)
      polgam_s = 0.0_dp
      ! Bounding box (ie min/max of lat/lon of model domain in geocoords has been
      !  determined in get_domaincenter_global. A rectangular domain in geocoords becomes
      !  pillow-shaped with curved boundary along the geocoord parallels. Hence, we
      !  determine the bounding box of domain in rotlatlon from the geocoord corner
      !  points AND mid-points along the parallels.
      ! We need to derive the rotlatlon coords for these 4+2 points and get min/max of
      !  their rotlatlon lat/lon. This rotlatlon domain extends slighty unequally in N-S
      !  from the dom_center.
      clon_min = HUGE(1.0_dp)
      clon_max = -HUGE(1.0_dp)
      clat_min = HUGE(1.0_dp)
      clat_max = -HUGE(1.0_dp)
      ! BLC
      CALL geo2rotll_coord (dom_minimu_lon(idom_r),dom_minimu_lat(idom_r),&
                            pollon_s, pollat_s, polgam_s,&
                            rlon, rlat)
      clon_min = MIN(clon_min, rlon)
      clon_max = MAX(clon_max, rlon)
      clat_min = MIN(clat_min, rlat)
      clat_max = MAX(clat_max, rlat)
      ! TLC
      CALL geo2rotll_coord (dom_minimu_lon(idom_r),dom_maximu_lat(idom_r),&
                            pollon_s, pollat_s, polgam_s,&
                            rlon, rlat)
      clon_min = MIN(clon_min, rlon)
      clon_max = MAX(clon_max, rlon)
      ...
      clat_max = MAX(clat_max, rlat)
      ! TRC
      CALL geo2rotll_coord (dom_maximu_lon(idom_r),dom_maximu_lat(idom_r),&
                            pollon_s, pollat_s, polgam_s,&
                            rlon, rlat)
      clon_min = MIN(clon_min, rlon)
      clon_max = MAX(clon_max, rlon)
      ...
      clat_max = MAX(clat_max, rlat)
      ! BRC
      CALL geo2rotll_coord (dom_maximu_lon(idom_r),dom_minimu_lat(idom_r),&
                            pollon_s, pollat_s, polgam_s,&
                            rlon, rlat)
      clon_min = MIN(clon_min, rlon)
      clon_max = MAX(clon_max, rlon)
      ...
      clat_max = MAX(clat_max, rlat)
      ! CSB
      CALL geo2rotll_coord (dom_center_lon(idom_r),dom_minimu_lat(idom_r),&
                            pollon_s, pollat_s, polgam_s,&
                            rlon, rlat)
      clon_min = MIN(clon_min, rlon)
      clon_max = MAX(clon_max, rlon)
      ...
      clat_max = MAX(clat_max, rlat)
      ! CNB
      CALL geo2rotll_coord (dom_center_lon(idom_r),dom_maximu_lat(idom_r),&
                            pollon_s, pollat_s, polgam_s,&
                            rlon, rlat)
      clon_min = MIN(clon_min, rlon)
      clon_max = MAX(clon_max, rlon)
      ...
      cart_width_deg = supob_cart_resolution/r_earth_dp*raddeg
  ...
  ! JM250628:
  ! This is the old, radar station based routine for determining the superobbing domain extend.
  ! For now, keep for reference.
  SUBROUTINE get_rotlatlon_domain_for_superobing_from_radsta (idom_model,pollon_s,pollat_s,polgam_s,startlon_s,startlat_s,endlon_s,endlat_s)
  ...
  END SUBROUTINE get_rotlatlon_domain_for_superobing_from_radsta
  ...
  SUBROUTINE interp2D_model2geo_horiz_scalar (idom_model, field3D_model, k_int, lon_geo, lat_geo, dat2D_geo, found)
    ...
    CALL geo2cell_index2d ( cindex_grid(list_domains_for_radar(idom_model)), p_patch(idom_model), &
         is_inner_domain, lon_geo, lat_geo, idx, blk )
    found = ( idx > -HUGE(1) )
