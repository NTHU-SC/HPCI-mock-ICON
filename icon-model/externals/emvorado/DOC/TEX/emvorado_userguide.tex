...
%%%   2) pdflatexuli -n -s -b emvorado_userguide.tex 3
...
\machemireinpdftrue    %%% Fuer lauf mit pdflatexuli  -n -s -b artif_docu.tex 3
...
\newcommand{\mussgleich}{\ensuremath{\overset{!}{=}}\xspace}% nur math-mode!
...
\quetsche{24.9.2024}   & \quetsche{Ulrich Blahak}  &
         \spvierformat{New namelist switch \paramform{labort_if_problems_gribout} in \srcform{/RADARSIM_PARAMS/}.\par
                       Inserted into Table~\ref{tab:radarsimparams}.} \\\hline
\quetsche{19.12.2024}   & \quetsche{Jana Mendrok}  &
         \spvierformat{Split namelist parameter \paramform{supob_ave_width} into separate parameters for reflectivity and radial wind. Updated in Table~\ref{tab:radarsimparams}.} \\\hline
\quetsche{20.12.2024}   & \quetsche{Jana Mendrok}  &
         \spvierformat{New namelist parameter \paramform{rain2mom_mu_incloud} in \srcform{/RADARSIM_PARAMS/}. Inserted into Table~\ref{tab:radarsimparams}.} \\\hline
\quetsche{21.1.2025}   & \quetsche{Ulrich Blahak}  &
         \spvierformat{Update names of F90 source files, because in the last year some really big files have been split into several smaller modules.} \\\hline
\quetsche{28.1.2025}   & \quetsche{Ulrich Blahak}  &
         \spvierformat{New namelist variable \paramform{fof_file_pattern} to define the file name pattern for feedback (fof) files.} \\\hline
\quetsche{14.4.2025}   & \quetsche{Ulrich Blahak}  &
         \spvierformat{New namelist parameters \paramform{itype_Dlim_sgh}, \paramform{Dlim_drysnow}, \paramform{Dlim_meltsnow}, \paramform{Dlim_meltgraupel} and \paramform{Dlim_melthail} in \srcform{/RADARSIM_PARAMS/} to limit excessive radar signals coming from the large tail of (some) hydrometeor PSDs. Inserted into Table~\ref{tab:radarsimparams}.} \\\hline
\quetsche{14.7.2025}   & \quetsche{Ulrich Blahak}  &
         \spvierformat{Extend \paramform{rs_meta($i$)\%station_name} length from 3 to 5-characters to allow for the 5-character OPERA convention.} \\\hline
\quetsche{14.8.2025}   & \quetsche{Ulrich Blahak}  &
         \spvierformat{New namelist parameter \paramform{Dlim_rain} for the \paramform{itype_Dlim_sgh} option. Inserted into Table~\ref{tab:radarsimparams}.} \\\hline
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\quetsche{18.9.2019}   & \quetsche{Ulrich Blahak}  &
...
radar bins (polar coordinates range, azimuth, elevation) with an option for explicit ray tracing, optional iterative computation
...
range, azimuth and elevation), compute the radial wind as described in \citeP{zeng2016a} and
...
3D volume scan arrays (range, azimuth, elevation). At this point, additional computations
...
radially outwards from each radar station for all discrete azimuth angles of the radar scan strategy. For the ray tracing,
the complete data of one azimuthal slice have to be present on one processor, which is achieved in a special MPI communication. Moreover,
...
\item \srcform{itype_refl = 5}: Similar to \paramform{itype_refl=1} but replacing Mie by T-matrix calculations assuming oblate spheroids following \citeP{ryzhkov2001,ryzhkov2011}. EMA's and melting model are the same as for Mie.
  ...
  Further necessary assumptions on size-dependend particle asymmetry and hydrometeor-dependend canting angle distributions are largely taken from \citeP{ryzhkov2011} but subject to future changes.
...
\subsection{Enhancement of traditional grid point output of unattenuated $Z_h$}
...
\srcform{DBZ} (\srcform{yvarml}, \srcform{yvarpl}, \srcform{yvarzl}), \srcform{DBZ_850} (\srcform{yvarml}), \srcform{DBZ_CMAX} (\srcform{yvarml}), \srcform{DBZ_CTMAX} (\srcform{yvarml}), \srcform{ECHOTOP} and \srcform{ECHOTOP_in_M} ( both in \srcform{yvarml}).
...
A similar mechanism for grid point reflectivity is also implemented for ICON. It can be activated by setting respective parameters in ICON's \srcform{/synradar_nml/} namelist and concerns the same output variables as for COSMO, namely \srcform{dbz}, \srcform{dbz_850}, \srcform{dbz_cmax}, \srcform{dbz_ctmax}, \srcform{echotop} and \srcform{echotop_in_m}.

The heart of the new namelist parameter set in both models is an instance of the type \srcform{t_dbzcalc_params} (\srcform{radar_data.f90}) as described below in Section~\ref{sec:nml:dbzmeta}. More informations are given in the COSMO Users's Guide and in ICON's own namelist documentation.
...
asymmetric data distribution in space, etc.) are thus taken into account, enabling a fair
...
concerns the ``true'' EMVORADO volume scan simulation compartment (steps~1 and~2 of the reflectivity-, dual polarization- and radial wind operators)
and is given in the namelist \srcform{/RADARSIM_PARAMS/} in file \srcform{INPUT_RADARSIM} (COSMO) respectively \srcform{NAMELIST_EMVORADO} (ICON). Associated with this
...
independent of EMVORADO. For COSMO and ICON, the computation of $Z_h$ for this model output optionally may use the same
fortran procedures than step~1 of EMVORADO (e.g., the more accurate Mie- or T-matrix scattering options).
Again, this output is independent of step~1 of EMVORADO and can be done also if EMVORADO
itself (steps~1 and~2) are switched off.
...
\multicolumn{2}{|l|}{\quetschee{loutdbz}} & \quetsche{L (1)} &
          \spvierformat{
            ...
            \textbf{If} \srcform{lvoldata_output=.TRUE.}, a number of reflectivity- and polarization output variables becomes available for output as volume data (cf.~Tab.~\ref{tab:datasetnames}), feedback files or radar composites.\par
...
\multicolumn{2}{|l|}{\quetschee{lsmooth}} & \quetsche{L (1)} &
          \spvierformat{Master switch to take into account that radar returns are a beam function weighted
            ...
            of an azimuthally scanning radar with axisymmetric beam pattern (\citeNPK{blahak2008b}) is applied for weihting.\par
            ...
            more important than azimuthal smoothing. Notable effects are expected mainly for radial wind. Another aspect is the
...
\multicolumn{2}{|l|}{\quetschee{lmds_z}} & \quetsche{L (1)} &
          \spvierformat{Take into account the limited sensitivity of the radar receiver for the simulated
	    reflectivity. If .TRUE. simulated reflectivities below a range dependent threshold are set to
	    -99.99 dBZ (``correct zero'').
            The threshold is defined by the inverse square-dependence of the returned energy
            on range and a reference minimal detectable signal at a reference range (\citeNPK{zeng2016a}).
	    Both parameters are part of the radar metadata list for each station
	    (\srcform{rs_meta($i$)\%mds_Z0} and \srcform{rs_meta($i$)\%mds_r0})
	    and meaningful defaults for each station are defined in the code, depending on the country
	    and the WMO station ID (if \srcform{lreadmeta_from_netcdf=.TRUE.}).\par
	    Furthermore, if .TRUE. and simulated reflectivity in the bin is below the range dependent
	    threshold, simulated observable polarimetric parameters (zdr, rhv, kdp, ldr) are set to
	    value -999.99 (``missing'').
          } & \spvierformat{\quetsche{.FALSE.} } \\\hline%%%
%
%
\multicolumn{2}{|l|}{\quetschee{lmds_vr}} & \quetsche{L (1)} &
          \spvierformat{Take into account the limited sensitivity of the radar receiver for the simulated
	    radial wind. If .TRUE. radial winds at bins with simulated reflectivities below a range
	    dependent threshold are set to -999.99 m/s (``missing'').}
          & \spvierformat{\quetsche{.FALSE.}} \\\hline%%%
%
%
...
%
  & \quetsche{voldata_ostream($n$)\%file_pattern} & \quetsche{C (\paramform{cmaxlen})} &
           \spvierformat{File pattern for $n$-th output stream for file formats \srcform{'cdfin'}, \srcform{'cdfin-mulmom'}, \srcform{'grib2'}, \srcform{'grib2-mulmom'}, \srcform{'ascii'}, \srcform{'ascii-gzip'}, \srcform{'f90-binary'}.\par
            ...
            \begin{tabitemize}
              ...
              \item \paramform{<tmodelini_mm>} (optional): same, but in format YYYYMMDDhhmm without seconds
              \item \paramform{<tstart>} (only cdfin- and grib2-formats): absolute date and time of start of time window of file content in format YYYYMMDDhhmmss
              \item \paramform{<tstart_mm>}: same, but in format YYYYMMDDhhmm without seconds
              \item \paramform{<tend>} (only cdfin- and grib2-formats): absolute date and time of end of time window of file content in format YYYYMMDDhhmmss
              \item \paramform{<tend_mm>}: same, but in format YYYYMMDDhhmm without seconds
              \item \paramform{<tact>}: absolute date and time of actual model time in format YYYYMMDDhhmmss
              \item \paramform{<tact_mm>}: same, but in format YYYYMMDDhhmm without seconds
              \item \paramform{<tvvzstart>} (only cdfin- and grib2-formats): forecast lead time of start of time window of file content in format DDhhmmss
              \item \paramform{<tvvzend>} (only cdfin- and grib2-formats): forecast lead time of end of time window of file content in format DDhhmmss
              \item \paramform{<tvvzact>}: forecast lead time of actual model time in format DDhhmmss
              \item \paramform{<scantype>} (mandatory): identifier for the scantype. It will be \paramform{rs_meta(i)\%scanname}, e.g. \paramform{'PPI0800'} or \paramform{'PRECIP'}
                by default, but can be adjusted by the parameters \paramform{voldata_ostream($n$)\%pat_scantype_volscan} and \paramform{voldata_ostream($n$)\%pat_scantype_precipscan}.
                For example, if the files should later be used as OSSE nature run input, the keywords \paramform{'volscan'} or \paramform{'precipscan'} are needed for them, respectively.
              \item \paramform{<dom>}: identifier for the model domain, will result in e.g. \paramform{'01'}.
            \end{tabitemize}
            ...
            \item[-] \paramform{<varname>} is mandatory for \srcform{'cdfin'}, \srcform{'grib2'} and \srcform{'ascii*'} and optional for  \srcform{'cdfin-mulmom'}, \srcform{'grib2-mulmom'}.
            \item[-] \paramform{<dom>} is optional, but may become mandatory if there is more than one radar-active model domain and if the output of at least two domains/ output streams goes into the same output directory.
            \end{tabitemize}
            (continued in next row)
        } & \spvierformat{\quetsche{(see next row!)}}  \\\hline
%
%
  & \quetsche{voldata_ostream($n$)\%file_pattern} & \quetsche{C (\paramform{cmaxlen})} &
         \spvierformat{
            EMVORADO will throw an appropriate error message about missing mandatory keys.\\[0.5em]
            Time strings for ascii- and f90-binary contain also seconds, i.e., YYYYMMDDhhmmss.\\[0.5em]
            \textbf{The default patterns for cdfin and grib2 are}
            \paramform{'cdfin_<varname>_<stationid>_<tstart_mm>_<tend_mm>_<scantype>'}
            \paramform{'grib2_<varname>_<stationid>_<tstart_mm>_<tend_mm>_<scantype>'}\\[0.5em]
            \textbf{The default patterns for ascii and f90-binary are}
            \paramform{'<varname>_<stationid>_<scantype>_<tmodelini>_<tvvzact>_<levtype>.dat}\\
            (file extension for f90-binary is \paramform{.bin})
         } & \spvierformat{\quetsche{(see to the left)}}  \\\hline
%
  & \quetsche{voldata_ostream($n$)\%pat_scantype_volscan} & \quetsche{C (12)} &
         \spvierformat{Alternative identifier for volume scans for the placeholder \paramform{<scantype>} in \paramform{voldata_ostream($n$)\%file_pattern}. If empty, \paramform{<scantype>} will resolve to \paramform{rs_meta(i)\%scanname}, e.g. \paramform{'PPI0800'}.
         } & \spvierformat{\quetsche{' '}}  \\\hline
%
  & \quetsche{voldata_ostream($n$)\%pat_scantype_precipscan} & \quetsche{C (12)} &
         \spvierformat{Alternative identifier for DWD's precipitation scans for the placeholder \paramform{<scantype>} in \paramform{voldata_ostream($n$)\%file_pattern}. If empty, \paramform{<scantype>} will resolve to \paramform{rs_meta(i)\%scanname}, which is \paramform{'PRECIP'} in this case.
         } & \spvierformat{\quetsche{' '}} \\\hline
%
...
%
  & \quetsche{lequal_azi_alldatasets} & \quetsche{L (1)} &
          \spvierformat{Flag to indicate that observation files for the same station and observation time(s) but
            ...
            azimuths and elevations. Normally this should be the case, therefore its default is .TRUE. and a series of
            cross-checks among files for different radar moments are bypassed to save time.\par
            If .FALSE., a thorough check for equal azimuths and elevations is performed, and in case of error the
...
%
  & \quetsche{lfdbk_output} & \quetsche{L (1)} & \spvierformat{Trigger output of NetCDF feedback (fof) files. Only possible for \paramform{lreadmeta_from_netcdf=.TRUE.}. The files will be stored under \paramform{ydirradarout} or optionally the subdirectory \paramform{ysubdirfof} herein.} & \spvierformat{\quetsche{.FALSE}}  \\\hline
%
...
%
  & \quetsche{fof_file_pattern} & \quetsche{C (\paramform{cmaxlen})} &
          \spvierformat{File pattern for feedback (fof) output files. This enables a flexible definition of fof file names as a mix of arbitrary text parts and the following placeholders:\par
            \begin{tabitemize}
            \item \paramform{<stationid>}:  will result in e.g. \paramform{'id-010908'}
            \item \paramform{<scantype>}: identifier for the scantype. If not specified, it will be \paramform{rs_meta(i)\%scanname}, e.g. \paramform{'PPI0800'}.
            \item \paramform{<tmodelini>}: absolute date and time of model start in format YYYYMMDDhhmmss
            \item \paramform{<dom>}: identifier of the model domain, will result in e.g. \paramform{'01'}.
            \end{tabitemize}
            \paramform{<stationid>} and \paramform{<scantype>} are mandatory.\\[0.5em]
            \paramform{<dom>} is optional, but may become mandatory if there is more than one radar-active model domain and if the output of at least two domains goes into the same output directory.\\[0.5em]
            EMVORADO will throw an appropriate error message about missing mandatory keys.\\[0.5em]
            \textbf{The default pattern is}\par \paramform{\textbf{'fof_radar_<stationid>_<scantype>_<tmodelini>.nc'.}}
...
%
  & \quetsche{itype_supobing} & \quetsche{I (1)} & \spvierformat{Type of computation of super-observations 
                  ...
                  \item[1 =] weighted averaging over a symmetric range-azimuth-sector around a superobservation reference point within each PPI (range-azimuth-plane).
...
%
  & \quetsche{thin_step_azi} & \quetsche{I (1)} & \spvierformat{If no superobing: azimuthal step width (array index space) of data thinning for NetCDF feedback files.} & \spvierformat{\quetsche{1}}  \\\hline
%
...
%
  & \quetsche{supob_ave_width_vr} & \quetsche{R (1)} & \spvierformat{Width of averaging area for superobing for $v_r$ [m].} & \spvierformat{\quetsche{$\sqrt{2}\times$ supob_cart_resolution_d}}  \\\hline
%
  & \quetsche{supob_ave_width_z} & \quetsche{R (1)} & \spvierformat{Width of averaging area for superobing for $Z$ [m].} & \spvierformat{\quetsche{$\sqrt{2}\times$ supob_cart_resolution_d}}  \\\hline
%
  & \quetsche{supob_minrange_vr} & \quetsche{R (1)} & \spvierformat{Min. range of superobing points for $v_r$ [m].} & \spvierformat{\quetsche{0.75~$\times$ supob_ave_width_vr_d}}  \\\hline
%
  & \quetsche{supob_minrange_z} & \quetsche{R (1)} & \spvierformat{Min. range of superobing points for $Z$ [m].} & \spvierformat{\quetsche{0.75~$\times$ supob_ave_width_z_d}}  \\\hline
%
  & \quetsche{supob_azi_maxsector_vr} & \quetsche{R (1)} & \spvierformat{Maximal azimuth sector (symmetrical to its center) for v_r superobing [\grad].} & \spvierformat{\quetsche{40.0}}  \\\hline
%
...
%
  & \quetsche{composite_file_pattern} & \quetsche{C (\paramform{cmaxlen})} &
          \spvierformat{File pattern for composite output files. This enables a flexible definition of output file names as a mix of arbitrary text parts and the following placeholders:\par
            \begin{tabitemize}
              \item \paramform{<varname>} (optional): parameter identifier, will result in \paramform{'dbzcmp'}
              ...
              \item \paramform{<dom>}: identifier of the model domain, will result in e.g. \paramform{'01'}.
            \end{tabitemize}
            One of \paramform{<tact>} and \paramform{<tvvzact>} is mandatory, the respective other is then optional.\\[0.5em]
            \paramform{<dom>} is optional, but may become mandatory if there is more than one radar-active model domain and if the output of at least two domains goes into the same output directory.\\[0.5em]
            EMVORADO will throw an appropriate error message about missing mandatory keys.\\[0.5em]
            \textbf{The default pattern is}\par \paramform{\textbf{'<varname>_<simobs>_<tmodelini>_<tact>.grb2'.}}
         } & \spvierformat{\quetsche{(see left)}}  \\\hline
%
%
...
%
  & \quetsche{composite_file_pattern_bub} & \quetsche{C (\paramform{cmaxlen})} &
          \spvierformat{File pattern for bubble generator composite output files. This enables a flexible definition of output file names as a mix of arbitrary text parts and the following placeholders:\par
            \begin{tabitemize}
              \item \paramform{<varname>} (optional): parameter identifier, will result in \paramform{'dbzcmpbub'}
              \item \paramform{<simobs>} (optional): \paramform{'sim'} or \paramform{'obs'}, to be able to discriminate files with simulated and observed composites
              \item \paramform{<tmodelini>} (optional): absolute date and time of model start in format YYYYMMDDhhmmss
              \item \paramform{<tact>}: absolute model forecast date and time in format YYYYMMDDhhmmss
              \item \paramform{<tvvzact>}: relative forecast date and time in format DDhhmmss 
...
%
  & \quetsche{nsmoothpoints_for_comp_bub_glob} & \quetsche{I (1)} & \spvierformat{For \srcform{ldo_bubbles=.TRUE.}: width of symmetric 2D binomial smoother in grid points. Not yet in the namelist, up to now hardcoded in \srcform{radar_namelist_read.f90}} & \spvierformat{\quetsche{9}}  \\\hline
%
...
%
  & \quetsche{ready_file_pattern} & \quetsche{C (\paramform{cmaxlen})} &
          \spvierformat{For \paramform{lwrite_ready=.TRUE.}:\par
            ...
            \end{tabitemize}
            One of \paramform{<tact>} and \paramform{<tvvzact>} is mandatory, the respective other is then optional.\\[0.5em]
            \paramform{<dom>} is optional, but may become mandatory if there is more than one radar-active model domain and if the ready files of at least two domains go into the same output directory.\\[0.5em]
            \textbf{The default pattern is}\par \paramform{\textbf{'READY_EMVORADO_<tact>'.}}
         } & \spvierformat{\quetsche{(see left)}}  \\\hline
%
%
\multicolumn{2}{|l|}{\quetschee{rain2mom_mu_incloud}} & \quetsche{R (1)} &
	  \spvierformat{Experimental \textbf{(do not use unless you know what you are doing!)}.\par
	    Value to overwrite with the shape parameter $\mu$ of rain drop size distribution (ie exponent of drop diameter $D$), which otherwise is provided by the host model. For \paramform{rain2mom_mu_incloud}$<=$-1.0, host model value is kept. Only applied with 2-moment microphysics scheme.
          } & \spvierformat{\quetsche{-999.9}}  \\\hline
%
%
\multicolumn{2}{|l|}{\quetschee{itype_Dlim_sgh}} & \quetsche{I (1)} &
\spvierformat{Options to limit excessive radar signals coming from the large tail of (some) hydrometeor PSDs: when integrating radar moments over the PSD, the mass of particles larger than a certain diameter $D_c$ (separate namelist parameters for hydrometeors, see below) is redistributed to
  \begin{itemize}
  \item[option 1] particles of exact size $D_c$
  \item[option 2] to all particles with $D<D_c$ by scaling up the PSD there
  \end{itemize}
  before integrating over the PSD. If set to 0 (default), no such redistribution is applied.\\[0.5em]
  Let $Z_{m}$ be any radar moment for a hydrometeor type, normally defined as
  \begin{equation*}
    Z_{m}\sim\ \int\limits_{0}^{\infty}\sigma(D)N(D)dD\mpunkt
  \end{equation*}
  For \textbf{option 1}: re-distributing the mass $x=a_{geo}D^{b_geo}$ in the PSD above $D_c$ to particles of exact size $D_c$ means
  \begin{equation*}
    \int\limits_{D_c}^{\infty}x(D)N(D)dD \mussgleich\ \int\limits_{D_c}^{\infty}x(D_c)N^{\ast}(D)dD\ =\ x(D_c)\int\limits_{D_c}^{\infty}N^{\ast}(D)dD
  \end{equation*}
  which leads to the scaled PSD $N^{\ast}(D)=\frac{x(D)}{x(D_c)}N(D)=\left(\frac{D}{D_c}\right)^{b_{geo}}N(D)$ and the modified $Z^{\ast}_{mv}$
  \begin{align*}
    Z^{\ast}_{m}\sim &\ \int\limits_{0}^{D_c}\sigma(D)N(D)dD\ +\ \sigma(D_c)\int\limits_{D_c}^{\infty}\left(\frac{D}{D_c}\right)^{b_{geo}}N(D)dD\ \\= &\ \int\limits_{0}^{\infty}\sigma(\min[D,D_c])\left(\frac{\max[D,D_c]}{D_c}\right)^{b_{geo}}N(D)dD\mpunkt
  \end{align*}
  For \textbf{option 2}: re-distributing the mass to $D<D_c$ by way of a scaling factor $g$ means
  \begin{equation*}
    \int\limits_{0}^{D_c}x(D)gN(D)dD \mussgleich\ \int\limits_{0}^{D_c}N(D)dD+\int\limits_{D_c}^{\infty}N(D)dD=m_{inner}+m_{tail}=m_{total}
  \end{equation*}
  which leads to 
  \begin{equation*}
    (g-1)m_{inner}=m_{tail}\quad\Longrightarrow\quad g=\frac{m_{tail}}{m_{inner}}+1=\frac{m_{total}}{m_{total}-m_{tail}}\mpunkt
  \end{equation*}
          } & \spvierformat{\quetsche{0}}  \\\hline
%
  & \quetsche{Dlim_rain} & \quetsche{R (1)} &
  \spvierformat{For \paramform{itype_Dlim_sgh>0}: limiting diameter [m] for rain. Does not have any effect if set to a value $>D_{max,r}$ (upper integration limit from \srcform{radar_data_mie.f90}). The default is such a large value.}
          & \spvierformat{\quetsche{999.0}}  \\\hline
%
  & \quetsche{Dlim_drysnow} & \quetsche{R (1)} &
  \spvierformat{For \paramform{itype_Dlim_sgh>0}: limiting diameter [m] for dry snow. Does not have any effect if set to a value $>D_{max,s}$ (upper integration limit from \srcform{radar_data_mie.f90}). The default is such a large value.}
          & \spvierformat{\quetsche{999.0}}  \\\hline
%
  & \quetsche{Dlim_meltsnow} & \quetsche{R (1)} &
  \spvierformat{For \paramform{itype_Dlim_sgh>0}: limiting diameter [m] for melting snow. Does not have any effect if set to a value $>D_{max,s}$ (upper integration limit from \srcform{radar_data_mie.f90}). The default is such a large value.}
          & \spvierformat{\quetsche{999.0}}  \\\hline
%
  & \quetsche{Dlim_meltgraupel} & \quetsche{R (1)} &
  \spvierformat{For \paramform{itype_Dlim_sgh>0}: limiting diameter [m] for melting graupel. Does not have any effect if set to a value $>D_{max,g}$ (upper integration limit from \srcform{radar_data_mie.f90}). The default is such a large value.}
          & \spvierformat{\quetsche{999.0}}  \\\hline
%
  & \quetsche{Dlim_melthail} & \quetsche{R (1)} &
  \spvierformat{For \paramform{itype_Dlim_sgh>0}: limiting diameter [m] for melting hail. Does not have any effect if set to a value $>D_{max,h}$ (upper integration limit from \srcform{radar_data_mie.f90}). The default is such a large value.}
          & \spvierformat{\quetsche{999.0}}  \\\hline
%
%
%%%%%
...
%
%
\multicolumn{2}{|l|}{\quetschee{\%station_name}} & \quetsche{C (5)} &
          \spvierformat{5-character station akronym to allow for station names following the OPERA convention, where the first two characters represent the country and 3 characters are an abbreviation of the station name.\\Note that station names are in principle arbitrary and do not have to follow this convention.} & \spvierformat{\quetsche{'XXXXX'} } \\\hline%%%
%
%
...
\multicolumn{2}{|l|}{\quetschee{\%smth_interv_fact}} & \quetsche{R (1)} &
          \spvierformat{Factor to determine the azimuthal and elevational integration range
...
%  & \quetsche{\texttt{\%ARmodel}}    
  ...
  &  &  & \spvierformat{
  \begin{tabitemize}
  \item[\texttt{'Poly'} =] First order polynomial in particle size (in terms of maximum diameter $D$\ [m]) with a lower bound, i.e. aspect ration $\textrm{AR} =$ \mbox{$max(\texttt{c0} + \texttt{c1}\cdot D,\texttt{ARmin})$}.
...
COSMO/ICON output fields \srcform{DBZ} (COSMO: \srcform{yvarml}, \srcform{yvarpl}, \srcform{yvarzl} in the \srcform{/GRIBOUT/} namelist(s)), \srcform{DBZ_850} (COSMO: \srcform{yvarml}), \srcform{DBZ_CMAX} (COSMO: \srcform{yvarml}), \srcform{DBZ_CTMAX} (COSMO: \srcform{yvarml}), \srcform{ECHOTOP} and \srcform{ECHOTOP_in_M} (COSMO: \srcform{yvarml})
, there is a new instance of the derived type \srcform{t_dbzcalc_params} in the model's namelist(s). For COSMO, there is an instance called \srcform{dbz} in each output namelist (\srcform{/GRIBOUT/}) and for ICON there is a separate namelist \srcform{synradar_nml} with a single instance called \srcform{synradar_meta}. This derived type is exactly the same
...
Just replace the prefix \srcform{dbz_meta($i$)} by \srcform{dbz} respectively \srcform{synradar_meta} when you put it into the respective COSMO / ICON namelist(s).
...
are part of namelist \srcform{IOCTL} (see the COSMO User's Guide \citeNPK{schaettler2018}). For ICON, these are in \srcform{synradar_nml}.
...
Note that in COSMO, each \srcform{GRIBOUT} namelist has its own independent reflectivity configuration \srcform{dbz} (it is possible to output several reflectivity ``versions'' in one run), while ICON only provides one global configuration \srcform{synradar_meta} in \srcform{synradar_nml} for all reflectivities in all output namelists.
...
Following data formats are supported by choosing the namelist parameter \srcform{voldata_format}:
  \begin{itemize}
  \item[\textbf{'ascii'}:] simple ASCII output of 3D volume scans according to range, azimuth and elevation.
    ...
    \item a second header line with 3 whitespace-separated integers denoting the number of ranges, azimuths and elevations,
...
There is one file per station, per scan strategy and per model forecast, collecting pairs
  ...
  within azimuth-range-boxes around the points of a quasi-kartesian horizontal grid on the PPI-planes.
...
because of some clutter filters to remove ground clutter or ``blanked'' azimuth sectors because of known
...
\item several initialization routines from \srcform{radar_interface.f90} which are called once during model initialization from
  the numerical model.
\item the generic organizational subroutine \srcform{organize_radar()} in module \srcform{radar_organize.f90}. This is the
...
\item \srcform{radar_mie_iface_cosmo_driver.f90}, \srcform{radar_mie_iface_cosmo_1mom.f90}, \srcform{radar_mie_iface_cosmo_2mom.f90} and \srcform{radar_mie_meltdegree.f90}: interface procedures
...
\item \srcform{radar_output_driver.f90} and \srcform{radar_model2rays.f90} contain the generic interface routines for step~2 of the operator, which
  are directly called from \srcform{organize_radar()} in \srcform{radar_organize.f90}.\\
  Down the call tree, these routines call further routines for computing \srcform{superobservations}
  and for output of \srcform{volume data}, \srcform{feedback files} and \srcform{composites} (grib2), which are contained in
  \srcform{radar_output_station.f90}, \srcform{radar_output_station_smth.f90}, \srcform{radar_output_country_obs.f90},
  \srcform{radar_output3d_ascii.f90}, \srcform{radar_output_composites.f90}, \srcform{radar_output_fdbk.f90}, \srcform{radar_output_readyfile.f90} and
  \srcform{radar_output_volscan.f90}.
...
\item There are model-specific procedures (interpolation to/from model grid, time housekeeping, parallelization, etc.)
  ...
  model-related procedures used in \srcform{radar_model2rays.f90} associated with the model grid (interpolation),
...
\item Calls of the generic organizational subroutine \srcform{organize_radar()} from module \srcform{radar_organize.f90}
  from \srcform{lmorg} for the 'init' and 'compute' stages, also described below in Section~\ref{sec:impltop}.
\item Specific calls to interface routines from \srcform{radar_mie_iface_cosmo_driver.f90} and \srcform{radar_mie_meltdegree.f90} or step~1 of the operator described below in Section~\ref{sec:impl1}.
...
from \srcform{radar_mie_iface_cosmo_driver.f90}
...
are contained in \srcform{radar_model2rays.f90} and are called from the top-level
...
\item Add corresponding line to subroutines \srcform{rsm_multitime2onetime} and \srcform{rsm_onetime2multitime} from \srcform{radar_data.f90}.
