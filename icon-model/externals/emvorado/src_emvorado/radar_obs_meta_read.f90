...
! Modules used:
  ...
  USE radar_utilities, ONLY : get_filenames_in_directory, jul2yyyymmdd, split_string, &
       &                      diff_seconds, new_datetime, round_datestr_min, tolower, toupper, ischar
    ...
    IF (my_radar_id == 0) THEN
      ...
      filter_loop: DO ii=1, ninfiles
        ...
        IF (INDEX(TRIM(infilenames(ii)), 'cdfin') > 0 .AND. nwords == 6) THEN
          IF (TRIM(words(6)) == 'volscan' .OR. TRIM(words(6)) == 'precipscan' .OR. &
               INDEX(TRIM(words(6)),'PPI') > 0 .OR. TRIM(words(6)) == 'PRECIP') THEN
    ...
    IF (my_radar_id == 0) THEN
      ...
      check_loop_1: DO ii = 1, nradsta_multi
        ...
        DO i=1, rs_meta_buf_multitimes(ii)%nobs_times_obs
          ...
          checkflagtext(i_qualvrad) = 'lqc_flag=.TRUE., but '
...
!!$    END DO
  ...
  SUBROUTINE get_metadata_from_cdfin ( infilename, fid, rsm, rsm_bg, err )
    ...
    INTEGER                  :: mii, niii, len_report, len_azimuth,len_range,len_ele, &
    ...
    INTEGER                  :: varid_azres  ! azimuthal resolution (021202)
    ...
    CHARACTER (LEN=LEN(rsm%station_name)) :: yssosn
    CHARACTER (LEN=80)         :: yzerrmsg
    ...
    status = nf90_inquire_dimension(ncid, AziDim, len = len_azimuth) ! Number of azimuth
    ...
    IF (status == NF90_NOERR) THEN
      ...
      ! hack for German cdfin obs files where only the national 3-char station name is contained:
      IF (.NOT.ischar(yssosn(4:5))) THEN
        yssosn = c_dwd//yssosn(1:3)
      END IF
    ELSE
      yssosn = 'XXXXX'
    ENDIF
    ...
    IF (yssosn /= 'XXXXX') THEN
      rsm%station_name   = yssosn
    END IF
    rsm%station_id       = station_id
    rsm%naz_ncdf(:,fid)  = len_azimuth
...
! az_start is not necessarily the center of the first output azimuth sampling interval, so do not overtake it!
!   value is taken from the background list!
...
!!$!          the scan, so disregard nsecm and take time-field (= "hhmm00") instead:
  ...
  SUBROUTINE get_metadata_from_h5_dwd ( infilename, fid, rsmo, rsmm_bg, err )
    ...
    CHARACTER(len=cobsflen), ALLOCATABLE   :: infilenames(:)
    INTEGER                                :: i, k, kk, ii, ierr, ierr2, ninfiles, nwords
    ...
    INTEGER             :: ival, year, fidindlist(ndatakind), tmpfid
    ...
    CHARACTER(len=20)   :: quantity, quantity_last, dsetlist(ndatakind), quality1name
    CHARACTER(len=60)   :: cpath
    CHARACTER(len=7)    :: juldate   ! YYYYMMDD
    CHARACTER (LEN=14)  :: cdate, cdate_last
    CHARACTER (len=5)   :: statid_char
    LOGICAL             :: station_found, have_quality_fid, have_quality(ndatakind)
...
!!$ ====== since some date between 2020 and 2024 also quality flags in case of dbzh: ====
!!$ attribute  /dataset1/data1/quality1/what/gain
!!$ attribute  /dataset1/data1/quality1/what/nodata
!!$ attribute  /dataset1/data1/quality1/what/offset
!!$ attribute  /dataset1/data1/quality1/what/quantity
!!$ attribute  /dataset1/data1/quality1/what/undetect
!!$ ==================================================
...
!!$ }
 ...
 !!$ FIXME: ras07-files: here we use urhohv and uphidp, but these are problematic according to M. Werner. Check! Make namelist parameters instead of hardcoding names!
       ! This is a sweeph5onem file with one moment only, so we can check the consistency
        ...
        ! .. Check if quality flags are present in group '/dataset1/data1/quality1':
        IF (lqc_flag) THEN
          quality1name(:) = ' '
          CALL read_attr_odim (file_id, '/dataset1/data1/quality1/what', 'quantity', ierr, cval=quality1name, silent=.TRUE.)
          IF (ierr == 0) THEN
            have_quality_fid = .TRUE.
          ELSE
            have_quality_fid = .FALSE.
          END IF
        END IF
        ...
        probe_loop: DO kk=1, 100
          ...
          IF (ierr == 0) THEN
            ...
            ELSE IF (TRIM(quantity) == TRIM(rsmo%obs_hdf5_varname_rhv)) THEN
              ...
              tmpfid = i_rhv
            END IF
            ! .. Check if quality flags are present in group '/dataset1/dataXX/quality1':
            IF (lqc_flag) THEN
              cpath(:) = ' '
              WRITE (cpath, '("/dataset1/data",i0,"/quality1/what")') kk
              quality1name(:) = ' '
              CALL read_attr_odim (file_id, TRIM(cpath), 'quantity', ierr2, cval=quality1name, silent=.TRUE.)
              IF (ierr2 == 0) THEN
                have_quality(tmpfid) = .TRUE.
              END IF
            END IF
          ELSE
...
!!$      END IF
!!$      az_start_last = dval
!!$      az_start = dval + 0.5_dp*rsmo%az_inc   ! Use the nominal value of az_inc to compute the center of the azimuth bin
...
!!$    rsmo%naz              = naz      ! not reliable from the file, use background val
    ...
    IF (fid < 1) THEN
      ...
      DO i=1, ndatakind
        IF (fids(i) > 0) THEN
          ...
          ! .. If quality flags are desired and present, they are in the same file:
          IF (lqc_flag .AND. have_quality(i)) THEN
            SELECT CASE (i)
            CASE (i_dbzh)
              rsmo%obsfile(:,i_qualdbzh)   = rsmo%obsfile(:,i)
              fids(i_dbzh) = i_dbzh
            CASE (i_vrad)
              rsmo%obsfile(:,i_qualvrad)   = rsmo%obsfile(:,i)
              fids(i_vrad) = i_vrad
            END SELECT
          END IF
        END IF
    ...
    ELSE
      ...
      ! .. If quality flags are desired and present, they are in the same file:
      IF (lqc_flag .AND. have_quality_fid) THEN
        SELECT CASE (fid)
        CASE (i_dbzh)
          rsmo%obsfile(:,i_qualdbzh)   = rsmo%obsfile(:,fid)
        CASE (i_vrad)
          rsmo%obsfile(:,i_qualvrad)   = rsmo%obsfile(:,fid)
        END SELECT
      END IF
    END IF
    ...
    ELSE
      ...
      IF (lqc_flag .AND. have_quality_fid) THEN
        SELECT CASE (fid)
        CASE (i_dbzh)
          CALL put_obstimes_to_rsm_onetime ( rsmo, TRIM(infilename), (/ i_qualdbzh /), ydate_ini_mod, cdate, ierr )
        CASE (i_vrad)
          CALL put_obstimes_to_rsm_onetime ( rsmo, TRIM(infilename), (/ i_qualvrad /), ydate_ini_mod, cdate, ierr )
        END SELECT
      END IF
    END IF
    IF (ierr /= 0) THEN
...
!!$ "a1gate" is always 0 here, but originally it should be the azimuth index of the true start of the PPI scan
!!$ and it is not useful to determine the azimuth start of the data in the file itself.
...
!!$      az_start = REAL(ival, KIND=dp) + 0.5_dp*az_inc
      ...
      ! Nominal first azimuth is always half the azimuth increment for Swiss radars:
...
!      CALL read_attr_odim (file_id, '/'//TRIM(dsetname(j))//'/where', 'rstart', ierr, dval=dval)  ! rstart = ra_inc!
      ...
      ! Nominal first azimuth is chosen as half the azimuth increment.
      ! This is always the best rounded value of the true azimuth of the first ray in the data,
      !  because for Italy, the first true antenna azimuth with a value > 0.0 is stored at the first
      !  index.
      az_start = 0.5_dp*az_inc
...
!      CALL read_attr_odim (file_id, '/'//TRIM(dsetname(j))//'/how', 'range_start', ierr, dval=dval)  ! rstart = ra_inc!
      ...
      ! Nominal first azimuth is chosen as half the azimuth increment.
      ! This is a good esimated value of the true azimuth of the first ray in the data
      az_start = 0.5_dp*az_inc
...
!!$              END IF
            ...
            ! Nominal first azimuth is chosen as half the azimuth increment.
            ! This is always the best rounded value of the true azimuth of the first ray in the data,
            !  because for ODIM-HDF5, the first true antenna azimuth with a value > 0.0 is stored at the first
            !  index.
            az_start = 0.5_dp*az_inc
...
!!$                                        H5I_BADID_F
  ...
  SUBROUTINE get_stationID_from_HDF5_source(srcstring, station_id, country_id, statid_char, ierr)
    ...
    IF (have_wmo) THEN
      ...
      IF (have_nod) THEN
        statid_char = toupper(TRIM(nodword))
      END IF
    ELSE
      ! Some countries do not or not always deliver WMO IDs but only NODs. This can be repaired here:
      IF (have_nod) THEN
        statid_char = toupper(TRIM(nodword))
        SELECT CASE (toupper(nodword(1:2)))
          ! NOTE: we check upper case of country ID because it is not allowed to apply tolower(c_xxx) in below CASE choices!
        CASE (c_arpasim)
