...
! ------------------------------------------------------------------------------
  ...
  SUBROUTINE cdfin_define (fid, rsm, glob_att, ncdf_shortname, ind_ele_outlist, nele_out)
    ...
    status = check_nc( nf90_def_dim(fid, "strlen",      LEN(rsm%station_name),   strlen_dimid), 'nf90_def_dim strlen')  ! String length dimension
...
!         '   scaled value and scale factor: ', scaled_val, scale_factor
  ...
  SUBROUTINE write_grib_to_file ( grib_outfilename,  &
    ...
    ! Open grib file:
    CALL codes_open_file(ioutfile, grib_outfilename, cmode, ilocerr)
    ...
    IF(grib%sec2%setLocalDefinition == GRIB_TRUE) THEN
      ...
      !!$ UB: temporary hack for 'localStationName', because up to now there are only 4 chars allocated in eccodes:
      CALL codes_set(igribclone,'localStationName',grib%sec2%localStationName(3:5)//' ',ilocerr)
      !!$ UB: after upgrade to 5 chars in eccodes template local.78.single_radar_scan.def:
      !!$ CALL codes_set(igribclone,'localStationName',grib%sec2%localStationName(1:5),ilocerr)
