...
!!$ - height above radar station in feedback instead of height MSL? However, station height is also given in the feedback file header ...
  ...
  USE radar_data, ONLY : &
       miss_threshold, miss_thresh_rhv, miss_value, missval_int, &
       zero_value, shield_value, &
       Z_crit_radar, dBZ_crit_radar, &
       shield_low_threshold, shield_up_threshold, shield_low_thresh_rhv, &
 ...
 !------------------------------------------------------------------------------
  ...
  USE radar_data_namelist, ONLY : &
       ...
       lfall, lonline, lreadmeta_from_netcdf, &
       ldealiase_vr_obs, &
       lmds_z, lmds_vr, &
       lfill_vr_backgroundwind, &
       ldo_composite, nel_composite, &
       ldo_bubbles, &
  ...
  USE radar_data_io, ONLY : radgeomoutputunit, radwindoutputunit, radrefloutputunit, &
       &                    zdroutputunit, rhvoutputunit, ldroutputunit, &
       &                    kdpoutputunit, ahoutputunit, adpoutputunit
