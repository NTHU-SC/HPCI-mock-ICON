...
!!$ Changes 30.4.2018:
!!$ - rs_meta%az_start is now hardcoded the center of the sampling interval of the first nominal output azimuth,
...
!!$   The real radar azimuths are mapped to these nominal azimuths via nearest neighbour, without any interpolation.
...
!!$
!!$   The azimuth-dependent elevations for each DWD station are stored in the new .incf-file
...
!    rs_meta(1)%station_name        = c_dwd//'KAR'
...
!================================================================================
 ...
 !------------------------------------------------------------------------------
  ...
  USE radar_data,               ONLY :       &
       ...
       mpi_comp_meta_typ, mpi_voldata_ostream_typ, &
  ...
  USE radar_data_namelist, ONLY :  &
       ...
       ldebug_radsim, &
       loutradwind, loutdbz, loutpolstd, loutpolall, lextdbz, &
...
CONTAINS
  ...
  SUBROUTINE init_grid_info ( idom_in )
    ...
    ! Initialize number of boundlines in auxiliary azimuth slice
    ! grid if necessary:
    IF (lsmooth .AND. lonline) THEN
      ! >= 1 line of overlap at the lower and upper azimuth index ends for the
      ! auxiliary azimuth-slices-grid (must result in more space than needed
...
!!$ Need modifications for RHI mode!
    ...
    IF (lonline) THEN
      ...
      DO ista = 1, nradsta
        ...
        ! .. The azimuth slice grid's horizontal grid distance is
        ...
        ! (extra azimuth slices at lower and upper azimuth range for lsmooth = .TRUE. .AND. lonline = .true.
...
IF (action == 'init') THEN
    ...
    IF (lcompute_pe_fwo) THEN
      CALL get_model_hydrometeors (itlrad_qx, idom_in, unused_value)
...
!!$===============================================================
    ...
    IF (ANY(lcalc)) THEN
      ...
      ! .. Link the needed Model variables:
      IF (lcompute_pe_fwo) THEN
        CALL get_model_hydrometeors (itlrad_qx, idom_in, rain2mom_mu_incloud)
        CALL get_model_variables (itlrad_dyn, itlrad_qx, idom_in)
  ...
  SUBROUTINE init_radar (idom_in)
    ...
    ! Initialize the parameters for the azimuth slice grid for each radar (if needed):
