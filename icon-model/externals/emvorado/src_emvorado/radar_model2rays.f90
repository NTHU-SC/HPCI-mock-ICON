...
!$omp end parallel do
  ...
  SUBROUTINE calc_geometry_grid
    ...
    !              the radar geometry is also constant in time and this subroutine
    !              is only called in the first call of the operator.
    !              A halo region of nbl_az at the azimuthal field margins is
...
!!$
  ...
  SUBROUTINE calc_geometry_onlinenew(lcalc)
    ...
    DO ista = 1,nradsta
      ...
      ! HERE NBL_AZ=0, SINCE NO HORIZONTAL INTERPOLATION WITHIN THE AZIMUTH-SLICE-DATA!
...
!!$        azimuth_vec(iae)   = azarr_idx(iaz) + smthpoint / rs_meta(ista)%az_inc
          ...
          ! azimuth of the smoothing point, depending on el_sta:
...
!!$                     smthpoint
          ...
          DO iae = 1, nae
            ...
            IF ( flag(iae,ira-1) > 0 .AND. &
              ...
              ! Store 5D hash index, take azimuth index without nbl_az:
              ...
              ! azimuth slice in the auxiliary grid, including nbl_az
...
!NEC$ ivdep
            DO irp = 1, nrp2
              ...
              ! (Indices relative to azimuth-slice-grid, i.e., shifted by nbl_az)
...
!!$      CALL dbz_to_linear(zh_radar)
  ...
  SUBROUTINE distribute_onlineinfos_reals(rvector_loc,ipos_loc,size_loc, &
    ...
    !
    ! Method:      mpi_alltoall and mpi_alltoallv, based on the azimuth distribution from para_range_all().
    !
    ...
    INTEGER, INTENT(IN) :: size_loc,                         & ! size of vector
                           ...
                           npes,                             & ! number of PEs
                           naz_nbl,                          & ! number of azimuths in total azimuth-slice-grid (incl. 2*nbl_az)
...
!CDIR NODEP,VOVERTAKE,VOB
        DO igrd = 1, size_loc
          ...
          np = ipos_loc(igrd)
          ! determine indices in azimuth for current grid point in total azimuth-slice-grid
...
!CDIR NODEP,VOVERTAKE,VOB
  ...
  SUBROUTINE distribute_onlineinfos_integers(ivector_loc,ipos_loc,size_loc,iaz_start, &
    ...
    !              sends them back to processors according to the azimuths of data  FOR ONE RADAR STATION!
    !
    ! Method:      mpi_alltoall and mpi_alltoallv, based on the azimuth distribution from para_range().
    !
    ! Remarks:     DEPRECATED! is less efficient than the newer equivalent routines
...
!CDIR NODEP,VOVERTAKE,VOB
  ...
  SUBROUTINE distr_onlinf_all_reals_all2allv (lcalc,rvector_loc,ipos_loc,nsta, &
    ...
    !
    ! Description: This subroutine collects the data for a variable (in type real) from all processors and
    !              sends them back to processors according to the combined azimuth vector of all radar stations.
    !
    ! Method:      mpi_alltoall and mpi_alltoallv, based on the azimuth distribution from para_range_all().
    !
    ...
    INTEGER, INTENT(IN) :: nsta,                         & ! number of radar stations
                           icomm,                        & ! MPI-communicator
                           npes,                         & ! number of PEs
                           naz_nbl(nsta),                & ! number of azimuths in total azimuth-slice-grid (incl. 2*nbl_az)
...
!$omp end parallel do
  ...
  SUBROUTINE distribute_onlineinfos_all_reals(lcalc,rvector_loc,ipos_loc,nsta, &
    ...
    !              based on the azimuth distribution from para_range_all().
    !
    ! Remarks:    - is more efficient than distribute_onlineinfos_reals()
...
!$omp end parallel do
  ...
  SUBROUTINE grd2geo(latsta,lonsta,naz,nal,az,al,lat,lon)
    ...
    !              azimuth and arc distance at mean sea level
    !
    !------------------------------------------------------------------------------
    !
    ! Subroutine / Function arguments
  ...
  SUBROUTINE online2geo(rsm,al,hl,el_loc,lat,lon)
    ...
    !
    ! Method:      Compute geographic coordinates from given radar site,
    !              azimuth and arc distance at mean sea level. For blocked parts of
  ...
  SUBROUTINE calc_vert_weight_online(np,alt,hfl,azarr_idx,naz_loc,istart_az,iend_az,nal,nhl,iaz,ial,weight,kout,wk)
    ...
    INTEGER, INTENT (IN) :: &
         ...
         azarr_idx(naz_loc), &                    ! azimuth index range of this processor in the aux. grid
         iaz(np), &
         ial(np)
...
!!$ UB>> ???      IF (alt(irp) < hfl_low .OR. alt(irp) < 0.0 .OR. hfl_low < 0.0 .OR. hfl_up(irp,kbot) < 0.0) THEN
  ...
  SUBROUTINE calc_vert_weight_onsmth(np,alt,hfl,istart_az,iend_az,nal,nhl,iaz,ial,wa,wl,kout,wk)
    ...
    DO irp = 1, np
      ...
      ! Select only azimuth-slice-grid intervals which are fully in the model domain:
...
!!$ UB>>      IF (alt(irp) < hfl_low .or. alt(irp) < 0.0 .or. hfl_low < 0.0 .OR. hfl_up(irp,kbot) < 0.0) THEN
  ...
  SUBROUTINE para_range(n1,n2,nprocs,nbl_az,istart,iend)
    ...
    !------------------------------------------------------------------------------
    ! Description: This subroutine calculates the range of azimuths of a single station
    !              for each processor
    !              assuming a parallelization model where the azimuths of the
    !              station are evenly divided among the PEs, so that each PE
    !              gets an equal share of azimuths of the station.
    !
    !              If there are no azimuths on a certain PE,
    !              istart and iend will be returned as -1. This happens when
    !              the number of PEs is larger than the number of azimuths.
    !
    !------------------------------------------------------------------------------
    !
    ! Subroutine / Function arguments
    ...
    DO irank = 0, MIN(nprocs,ntot)-1
      ...
      ! UB>> Append 2*nbl_az overlap at the upper bound of the azimuth index range:
      !      (however, physically nbl_az azimuth intervals are append both the lower and upper bound)
  ...
  SUBROUTINE para_range_all(n1,n2,nsta,nprocs,nbl_az,istart,iend)
    ...
    !------------------------------------------------------------------------------
    ! Description: This subroutine calculates the range of azimuths for each processor
    !              assuming a parallelization strategy where the azimuths of all
    !              stations are combined into one long azimuth vector, which is then divided
    !              evenly among the PEs.
    !
    !              If there are no azimuths of a certain station on a certain PE,
    !              istart and iend will be returned as -1. This happens when
    !              the number of PEs is larger than the total number of azimuths
    !              of all stations.
    !
    !------------------------------------------------------------------------------
    !
    ...
    ! 1) Divide the combined azimuth vector of all stations evenly among the PEs:
    ...
    !   Offset of the index range for each station in the combined azimuth vector:
    offs(0) = 0
    DO ista = 1, nsta
      offs(ista) = offs(ista-1) + n2(ista) - n1(ista) + 1
    END DO

    !   now hop through the combined azimuth vector from one station boundary (offs)
