...
!    vt_const%am = 3.5d0  ! 3.5 is in the appendix of Zeng et al. (2016)
  ...
  SUBROUTINE vtradar_normalize( &
      vt_local, z_local, rho, rho_0, ilow, iup, jlow, jup, ku, ko, &
      vt_radar)

    IMPLICIT NONE

    INTEGER, INTENT(IN) :: ilow, iup, jlow, jup, ku, ko
    REAL(KIND=dp), INTENT(IN) :: rho_0
    REAL(KIND=wp), DIMENSION(:,:,:), INTENT(IN) :: rho
    REAL(KIND=dp), DIMENSION(:,:,:), INTENT(IN) :: vt_local, z_local

    REAL(KIND=dp), DIMENSION(:,:,:), INTENT(out) :: vt_radar
...
!$OMP END PARALLEL DO
  ...
  END SUBROUTINE vtradar_normalize
