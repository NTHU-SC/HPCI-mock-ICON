...
! Modules used:
  ...
  ! Country codes in station names (fisrt 2 chars), taken from the hdf5 5-char station identifier.
  !  (The next 3 chars will be set to the previous 3-char station identifiers in radar_obs_meta_list.f90)
  CHARACTER(len=2), PARAMETER :: c_dwd         = 'DE'
  CHARACTER(len=2), PARAMETER :: c_meteoswiss  = 'CH'
  CHARACTER(len=2), PARAMETER :: c_arpasim     = 'IT'
  CHARACTER(len=2), PARAMETER :: c_belgium     = 'BE'
  CHARACTER(len=2), PARAMETER :: c_denmark     = 'DK'
  CHARACTER(len=2), PARAMETER :: c_france      = 'FR'
  CHARACTER(len=2), PARAMETER :: c_poland      = 'PL'
  CHARACTER(len=2), PARAMETER :: c_czech       = 'CZ'
  CHARACTER(len=2), PARAMETER :: c_netherlands = 'NL'
  CHARACTER(len=2), PARAMETER :: c_slovakia    = 'SK'
...
!======================================================================================
  ...
  TYPE radar_meta_type
    ...
    REAL    (KIND=wpfwo)      :: az_start       ! start azimuth of first az bin rel. to true north  (deg)
    ...
    REAL    (KIND=wpfwo)      :: dalpha         ! azimuthal averaging interval for calculation of one
    ...
    REAL    (KIND=wpfwo)      :: smth_interv_fact ! Factor to determine the azimuthal and elevational integration range
    ...
    CHARACTER (LEN=5)         :: station_name       ! short station name
  ...
  TYPE radar_data_type
    ...
    INTEGER             , POINTER :: ind_intp(:,:) => NULL() ! grid indices for each observation (first dimension)
                                                             ...
                                                             !        from radial, azimuthal and elevation index
  ...
  TYPE cart_data_type
     ...
     REAL    (KIND=wpfwo)          :: width_vr     ! half total averaging width for radial wind [m]
     REAL    (KIND=wpfwo)          :: width_deg_vr ! half total averaging width for radial wind [deg]
     REAL    (KIND=wpfwo)          :: width_z      ! half total averaging width for reflectivity [m]
     REAL    (KIND=wpfwo)          :: width_deg_z  ! half total averaging width for reflectivity [deg]
     ...
     INTEGER                    :: ni_tot_vr       ! total number in i-direction for radial wind
     INTEGER                    :: nj_tot_vr       ! total number in j-direction for radial wind
     INTEGER                    :: ni_tot_z        ! total number in i-direction for reflectivity
     INTEGER                    :: nj_tot_z        ! total number in j-direction for reflectivity
     ...
     INTEGER                    :: naz_average
     INTEGER                    :: nra_average_vr  ! half number of range bins for averaging for radial wind
     INTEGER                    :: nra_average_z   ! half number of range bins for averaging for reflectivity
  ...
  TYPE radar_grid_type
    ...
    REAL    (KIND=wpfwo)   , POINTER :: vt_azgrd(:) => NULL()          ! model aequivalents of fall speed on local PE after collection of entire azimuth slices
...
!======================================================================================
  ...
  ! array of auxiliary azimuth grid data (local PE)
  ...
  ! overlap for the auxiliary azimuth-slice grid at the lower and upper azimuth ends
  INTEGER                     :: nbl_az
