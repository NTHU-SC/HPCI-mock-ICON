...
!!$ - height above radar station in feedback instead of height MSL? However, station height is also given in the feedback file header ...
  ...
  USE radar_data_namelist, ONLY :  &
       ...
       supob_ave_width_vr, supob_ave_width_z, supob_minrange_vr, supob_minrange_z, &
       supob_vrw, supob_rfl, &
...
!================================================================================
  ...
  SUBROUTINE init_cart_info (idom_in)
    ...
    DO ista = 1, nradsta
      ...
      cart_data(ista)%width_vr     = 0.5*supob_ave_width_vr                  ! half constant averaging width for radial wind [m]
      cart_data(ista)%width_deg_vr = cart_data(ista)%width_vr*(0.025/2800.0) ! half constant averaging width for radial wind [deg]
      cart_data(ista)%width_z      = 0.5*supob_ave_width_z                   ! half constant averaging width for reflectivity [m]
      cart_data(ista)%width_deg_z  = cart_data(ista)%width_z*(0.025/2800.0)  ! half constant averaging width for reflectivity [deg]

      cart_data(ista)%nra_average_vr = CEILING(cart_data(ista)%width_vr/rs_meta(ista)%ra_inc) ! half number of range bins for averaging for radial wind [-]
      cart_data(ista)%nra_average_z  = CEILING(cart_data(ista)%width_z/rs_meta(ista)%ra_inc)  ! half number of range bins for averaging for reflectivity [-]
      ...
      cart_data(ista)%ni_tot_vr = CEILING(((cart_data(ista)%endlon - cart_data(ista)%startlon) - &
                                          2.0_dp*cart_data(ista)%width_deg_vr) / cart_data(ista)%dlon)
      cart_data(ista)%nj_tot_vr = CEILING(((cart_data(ista)%endlat - cart_data(ista)%startlat) - &
                                          2.0_dp*cart_data(ista)%width_deg_vr) / cart_data(ista)%dlat)
      cart_data(ista)%ni_tot_z  = CEILING(((cart_data(ista)%endlon - cart_data(ista)%startlon) - &
                                          2.0_dp*cart_data(ista)%width_deg_z) / cart_data(ista)%dlon)
      cart_data(ista)%nj_tot_z  = CEILING(((cart_data(ista)%endlat - cart_data(ista)%startlat) - &
                                          2.0_dp*cart_data(ista)%width_deg_z) / cart_data(ista)%dlat)
