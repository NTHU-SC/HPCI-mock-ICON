...
! Modules used:
  ...
  USE radar_data_namelist, ONLY : composite_meta_type
...
!!$            lat = phirot2phi(latrot,lonrot,comp_meta%pollat,comp_meta%pollon,comp_meta%polgam)
            ...
            ! compute azimuth, arc length s and radar range from radar station
            !  to model point:
...
!$omp end parallel do
  ...
  SUBROUTINE collect_smooth_composite ( idom_model, compdata_tot, comp_meta, lsmooth_composite, &
    ...
    INTEGER, INTENT(in):: &
         ...
         nsmoothpoints_for_comp, &  ! width of symmetric 2D binomial smoother in grid points
