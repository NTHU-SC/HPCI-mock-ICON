...
MODULE radar_output_fdbk
  ...
  USE radar_interface, ONLY : abort_run, get_datetime_ini
...
!!$ Caution: data fields in order (azi, range, ele), not (range, azi, ele)!
  ...
  !   which is determined by the number of nominal ranges, azimuths and elevations of the
...
!!$ below !!!
  ...
  INTEGER , INTENT (IN)  :: &
       ...
       naz                 ,& ! number of azimuth bins in rdata (regular 3D data set)
  ...
  REAL    (KIND=dp)    , INTENT (IN)  :: &
       ...
       azivec    (naz      ) ,& ! Vector of regular azimuths
...
!!$  CHARACTER (LEN=22)  :: zyakdat2  ! actual date, form:   wd   dd.mm.yy  hh UTC
  ...
  CHARACTER (len=cmaxlen) :: filenamefof  ! path and filename of the fof-file to produce
...
!  imax_rays = rs_meta(ista)%nobs_times_fdbk * nele * thin_naz * tot_num_observables
  ...
  CALL fof_create_filename (ista, fof_file_pattern, time_mod, filenamefof)

  IF (ysubdirfof(1:1) == '/') THEN
    filenamefof    = TRIM(ysubdirfof)//TRIM(filenamefof)
  ELSE
    filenamefof    = TRIM(ydirradarout)//TRIM(ysubdirfof)//TRIM(filenamefof)
  END IF

  INQUIRE (file=TRIM(filenamefof), exist=lfdbkfile_exist)

  !-------------------------------------------------------------------------------
  ! .. Create fof file if necessary:

  IF (.NOT. lfdbkfile_exist) THEN
    ...
    IF (ldebug_radsim) WRITE(*,'(i3,2x,a)') my_radar_id, 'Creating fdbk file: '//TRIM(filenamefof)

    IF ( LEN_TRIM(filenamefof) > LEN(fb(ista,idom)% nc% path) ) THEN
      WRITE( yerr ,'("ERROR '//TRIM(yzroutine)//': filename of NetCDF file too long! len = ",i6", maxlen = ",i6)' )&
           LEN_TRIM(filenamefof), LEN(fb(ista,idom)% nc% path)
      jerr = 35
      RETURN
    END IF
    ...
    !    ===========
         TRIM(filenamefof),& ! path and filename
    ...
    n_veri          = nfcast
    IF (ldebug_radsim) WRITE  (*,'(A)') 'Created fdbk file: '//TRIM(filenamefof)

    CALL close_fdbk (fb(ista,idom))

  ENDIF   ! .not. lfdbkfile_exist


  ! .. (Re-)Open the existing feedback file and retrieve global attributes:
  IF (ldebug_radsim) WRITE  (*,'(A)') '(Re)Open fdbk file: '//TRIM(filenamefof)

  ! .. Open for read access to read the meta data structure:
  CALL open_fdbk_read ( fb(ista,idom), TRIM(filenamefof) )
  ...
  CALL open_fdbk_write ( fb(ista,idom), TRIM(filenamefof) )


  IF (ldebug_radsim) WRITE  (*,'(A)') 'Feedback file opened: '//TRIM(filenamefof)
...
!!$  DO irep=1, nw_rep
!!$
!!$    rep_header(irep)% statid(6:len(rep_header(irep)%statid))  = ' '
...
!!$    rep_header(irep)% statid(4:4)   = rs_meta(ista)%station_name(4:4)
!!$    rep_header(irep)% statid(5:5)   = rs_meta(ista)%station_name(5:5)
!!$
!!$  END DO
...
!CDIR VOVERTAKE,VOB3
    DO kk = 1, kkv(irep)
      ...
      IF (ivarno == VN_RADVEL .AND. rs_meta(ista)%ext_nyq(k,itime) < rs_meta(ista)%vnyq_min_for_vr_active_fdbk) THEN
        ...
        rep_body(kk,irep)% check    = 1
      END IF
!!$ UB FIXME: rquali_obs currently comes from the raw data files and is valid only at the nearest radar bin to the
!!$      center of the superobing region. This is not the correct information. We need the fraction of
!!$      valid data points within the superobing region with respect to all data points in the region!
...
CALL ftrace_region_begin(TRIM(yzroutine)//'_writenetcdf')
  ...
  IF (nw_rep > 0) THEN
    ...
    CALL write_report_radar_1 ( fb(ista,idom), rep_header(1:nw_rep), rep_body(:,1:nw_rep), &
                            ...
                            yzstatid=rs_meta(ista)%station_name)
    !   =================
  ...
  IF ( lastpr .AND. ldebug_radsim ) THEN
    WRITE (*,'(A,/,T10,A)')     '    NetCDF FEEDOBS FILE created: ' , TRIM(filenamefof)
...
END SUBROUTINE obs_write_cdf_feedobs
  ...
  !=========================================================================
  !
  ! Procedure for creating output file names for fof files
  ! based on a pattern given in the file_pattern on input
  !
  !=========================================================================
  
  SUBROUTINE fof_create_filename (ista, file_pattern, time_mod, ofilename)

    INTEGER         , INTENT(in)  :: ista  ! station nr. in the rs_meta(ista) - meta data structure
    ...
    CHARACTER(len=*), PARAMETER :: yzroutine = 'emvorado::fof_create_filename'
    CHARACTER(len=14) :: model_starttime
    CHARACTER(len=2)  :: domain_string
    CHARACTER(len=cmaxlen) :: fpat
    CHARACTER(len=20) :: key, cstation_id
    INTEGER           :: ku, ko, pos
    LOGICAL           :: have_id, have_scantype
    ...
    cstation_id(:) = ' '
    WRITE (cstation_id, '("id-",i6.6)') rs_meta(ista)%station_id
    ...
    IF (LEN_TRIM(file_pattern) <= 0) THEN
      ...
      ofilename(:) = ' '
      WRITE(ofilename, '("fof_radar",3("_",a),".nc")') cstation_id, rs_meta(ista)%scanname, TRIM(model_starttime)
    ...
    ELSE
      ...
      DO
        ...
        CASE ('<scantype>')
          have_scantype = .TRUE.
          CALL replace_substr_with_value (fpat, ku, ko, TRIM(rs_meta(ista)%scanname))
        CASE ('<tmodelini>')
      ...
      DO
        ...
        CALL abort_run (my_radar_id, 39074, &
             'ERROR '//TRIM(yzroutine)//': fof_file_pattern "'//TRIM(file_pattern)// &
             '" contains unknown key '//TRIM(key), &
             TRIM(yzroutine)//', input parameter ''fof_file_pattern''')
      END DO

      ! .. if still some remaining single '<' or '>' characters are in fpat, there is an error:
      IF (INDEX(fpat,'<') > 0 .OR. INDEX(fpat,'>') > 0) THEN
        CALL abort_run (my_radar_id, 39075, &
             'ERROR '//TRIM(yzroutine)//': fof_file_pattern "'//TRIM(file_pattern)// &
      ...
      IF (.NOT.have_id) THEN
        CALL abort_run (my_radar_id, 39077, &
             'ERROR '//TRIM(yzroutine)//': fof_file_pattern "'//TRIM(file_pattern)// &
      ...
      IF (.NOT.have_scantype) THEN
        CALL abort_run (my_radar_id, 39078, &
             'ERROR '//TRIM(yzroutine)//': fof_file_pattern "'//TRIM(file_pattern)// &
  ...
  END SUBROUTINE fof_create_filename
