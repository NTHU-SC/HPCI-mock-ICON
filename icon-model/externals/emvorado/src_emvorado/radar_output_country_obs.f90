...
!!$ - height above radar station in feedback instead of height MSL? However, station height is also given in the feedback file header ...
  ...
  USE radar_output_utils, ONLY : control_output
...
CONTAINS
  ...
  SUBROUTINE output_radar_country_obs ( ista, time_mod, itime, vr_mod, &
    ...
    !              according to range, azimuth and elevation indices,
    !              data(range,azi,ele),
    ...
    zlradwind_ok   = rs_meta(ista)%lobs_avail(itime,i_vrad) .AND. loutradwind
    zlzradar_avail = rs_meta(ista)%lobs_avail(itime,i_dbzh)
    ...
    IF (lfdbk_output .AND. it_is_time_for_feedback .AND. loutradwind .AND. rs_meta(ista)%lvrad_to_fdbk) THEN
      ...
      DO i=1, rs_meta(ista)%nobs_times_fdbk
        ...
        IF (k > missthr_int) THEN
          IF (rs_meta(ista)%obs_startrec(k,i_vrad) > missthr_int) THEN
    ...
    IF (lfdbk_output .AND. it_is_time_for_feedback .AND. loutdbz .AND. rs_meta(ista)%ldbzh_to_fdbk) THEN
      ...
      DO i=1, rs_meta(ista)%nobs_times_fdbk
        k = get_obstime_ind_of_modtime ( rs_meta(ista)%obs_times_fdbk(i) , &
                                         rs_meta(ista)%obs_times(1:rs_meta(ista)%nobs_times) )
        IF (k > missthr_int) THEN
          IF (rs_meta(ista)%obs_startrec(k,i_dbzh) > missthr_int) THEN
            tot_num_observables_fdbk = tot_num_observables_fdbk + 1
            EXIT
          END IF
        END IF
...
!$omp end parallel do
      ...
      !   to sorted azimuth, range and elevation:
...
!$omp end parallel do
      ...
      IF (lqc_flag .AND. rs_meta(ista)%lobs_avail(itime,i_qualdbzh)) THEN
...
!$omp end parallel do
      ...
      IF (lqc_flag .AND. rs_meta(ista)%lobs_avail(itime,i_qualvrad)) THEN
...
!$omp end parallel do
    ...
    IF ( zlradwind_ok ) THEN
      ...
      IF (itype_supobing > 0) THEN
        ...
        CALL output3d_ascii_radar(itime, &
             rs_meta(ista)%naz, rs_meta(ista)%nra, rs_meta(ista)%nel, &
             vrobs_supob, 'vrsupobs', &
             'Observed radial velocity, no aliasing', 'm/s', 'polar', &
             rmeta=rs_meta(ista), dbzmeta=dbz_meta(ista), is_aliased=.NOT.ldealiase_vr_obs)
          
        CALL output3d_ascii_radar(itime, &
             rs_meta(ista)%naz, rs_meta(ista)%nra, rs_meta(ista)%nel, &
             vrmod_supob, 'vrsupsim', &
    ...
    IF ( zlzradar_ok ) THEN
      ...
      IF (itype_supobing > 0) THEN
        ...
        CALL output3d_ascii_radar(itime, &
             rs_meta(ista)%naz, rs_meta(ista)%nra, rs_meta(ista)%nel, &
             zrobs_supob, 'zrsupobs', &
             'Observed radar reflectivity', 'dBZ', 'polar', &
             rmeta=rs_meta(ista), dbzmeta=dbz_meta(ista), is_aliased=.TRUE.)

        CALL output3d_ascii_radar(itime, &
             rs_meta(ista)%naz, rs_meta(ista)%nra, rs_meta(ista)%nel, &
             zrmod_supob, 'zrsupsim', &
...
!$omp end parallel do
      ...
      IF (zlzdrradar_ok) THEN
        CALL output3d_ascii_radar(itime, rs_meta(ista)%naz, rs_meta(ista)%nra, rs_meta(ista)%nel, &
             zdrpolar_obs, 'zdrobs', 'Observed differential radar reflectivity', 'dB', 'polar', &
             rmeta=rs_meta(ista), dbzmeta=dbz_meta(ista), is_aliased=.TRUE.)
        IF (ldebug_radsim) THEN
      ...
      IF (zlkdpradar_ok) THEN
        CALL output3d_ascii_radar(itime, rs_meta(ista)%naz, rs_meta(ista)%nra, rs_meta(ista)%nel, &
             kdppolar_obs, 'kdpobs', 'Observed spec. differential phase shift', 'deg/km', 'polar', &
             rmeta=rs_meta(ista), dbzmeta=dbz_meta(ista), is_aliased=.TRUE.)
        IF (ldebug_radsim) THEN
      ...
      IF (zlphidpradar_ok) THEN
        CALL output3d_ascii_radar(itime, rs_meta(ista)%naz, rs_meta(ista)%nra, rs_meta(ista)%nel, &
             phidppolar_obs, 'phidpobs', 'Observed differential propagation phase', 'deg', 'polar', &
             rmeta=rs_meta(ista), dbzmeta=dbz_meta(ista), is_aliased=.TRUE.)
      END IF
      IF (zlrhvradar_ok) THEN
        CALL output3d_ascii_radar(itime, rs_meta(ista)%naz, rs_meta(ista)%nra, rs_meta(ista)%nel, &
             rhvpolar_obs, 'rhvobs', 'Observed H/V correllation coeff.', '-', 'polar', &
             rmeta=rs_meta(ista), dbzmeta=dbz_meta(ista), is_aliased=.TRUE.)
        IF (ldebug_radsim) THEN
...
!$omp threadprivate(count_obs, count_mod, indloc)
    ...
    REAL(KIND=dp), ALLOCATABLE, SAVE :: &
         ...
         rdata_obs_varsum(:) ,&
         rdata_mod_varsum(:)
!$omp threadprivate(w_obs,w_mod,rdata_obs_wsum,rdata_obserr_wsum,rdata_mod_wsum)
!$omp threadprivate(rdata_obs_sum,rdata_mod_sum,rdata_obs_varsum,rdata_mod_varsum)
    ...
    IF (TRIM(cvarname) == 'vr') THEN
      ni_tot = cart_data(ista)%ni_tot_vr
      nj_tot = cart_data(ista)%nj_tot_vr
      ncp    = ni_tot * nj_tot
      width     = cart_data(ista)%width_vr
      width_deg = cart_data(ista)%width_deg_vr
    ELSE IF (TRIM(cvarname) == 'z') THEN
      ni_tot = cart_data(ista)%ni_tot_z
      nj_tot = cart_data(ista)%nj_tot_z
      ncp    = ni_tot * nj_tot
      width     = cart_data(ista)%width_z
      width_deg = cart_data(ista)%width_deg_z
    END IF
...
!CDIR NODEP
        DO iaz = 1,  rs_meta(ista)%naz
          ...
          i = NINT((rlon_r - cart_data(ista)%startlon-width_deg) / cart_data(ista)%dlon) + 1
          j = NINT((rlat_r - cart_data(ista)%startlat-width_deg) / cart_data(ista)%dlat) + 1

          IF (i >= 1 .AND. i <= ni_tot .AND. j >= 1 .AND. j <= nj_tot) THEN
            ...
            clon_r = cart_data(ista)%startlon + width_deg + (i-1)*cart_data(ista)%dlon
            clat_r = cart_data(ista)%startlat + width_deg + (j-1)*cart_data(ista)%dlat
            ...
            IF (dis_tmp < cart_data(ista)%dis(iel,ip)) THEN
              ...
              cart_data(ista)%aw(iel,ip) = CEILING ( &
                   ATAN(width/(ira*rs_meta(ista)%ra_inc))*raddeg / &
                   rs_meta(ista)%az_inc )
            END IF
      ...
      IF (TRIM(cvarname) == 'vr') THEN
        ...
        DO iaz_average = -naz_average, naz_average, 1
          DO ira_average = -cart_data(ista)%nra_average_vr, cart_data(ista)%nra_average_vr, 1
            DO ip = 1, np
              ...
              IF  (ABS(iaz_average) <= aw_loc          .AND. &
                ...
                w_tmp =  (REAL((1.05*aw_loc)**2 - iaz_average**2)   &
                        ...
                        *(REAL((1.05*cart_data(ista)%nra_average_vr)**2 - ira_average**2) &
                        / REAL((1.05*cart_data(ista)%nra_average_vr)**2 + ira_average**2))
      ...
      ELSE IF (TRIM(cvarname) == 'z') THEN
        ...
        DO iaz_average = -naz_average, naz_average, 1
          DO ira_average = -cart_data(ista)%nra_average_z, cart_data(ista)%nra_average_z, 1
            DO ip = 1, np
...
!!$ UB: increased radius influence by 5 % to avoid 0 weight for the outermost points:
                w_tmp = ( REAL((1.05*aw_loc)**2 - iaz_average**2)   &
                        / REAL((1.05*aw_loc)**2 + iaz_average**2))  &
                        *(REAL((1.05*cart_data(ista)%nra_average_z)**2 - ira_average**2) &
                        / REAL((1.05*cart_data(ista)%nra_average_z)**2 + ira_average**2))
...
!!$                    z_tmp = 10.0_dp**(0.1_dp* rdata_obs(iaz_sub,ira_sub,iel))
      ...
      DO ip = 1, np
        ...
        i = MOD((cart_data(ista)%cartdata_ind(iel,ip)-1),ni_tot) + 1
        j = (cart_data(ista)%cartdata_ind(iel,ip)-1)/ni_tot  + 1
...
!CDIR NODEP
        DO iaz = 1,  rs_meta(ista)%naz
          ...
          IF (i < 1 .OR. i > ni_tot .OR. j < 1 .OR. j > nj_tot) THEN
           CYCLE
          END IF
      ...
      IF (TRIM(cvarname) == 'vr') THEN
        ...
        nct = (2*cart_data(ista)%aw_max+1)*(2*cart_data(ista)%nra_average_vr+1)
...
!!$ UB: this can be made more efficient, like in averagesuperobing_vec()!
      ...
      ELSE IF (TRIM(cvarname) == 'z') THEN
        ...
        naz_average = MAXVAL(cart_data(ista)%aw(iel,:))
        !  maximum number of raw radar bins for one superobing point:
        nct = (2*naz_average+1)*(2*cart_data(ista)%nra_average_z+1)
