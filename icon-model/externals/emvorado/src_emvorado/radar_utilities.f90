...
!    geographic azimuth azi (degrees) relative to the point with geographic longitude
...
!   This routine computes the geographic longitude and latitude of a point
!    at great circle distance s (taken at MSL, not at radar height!) and
!    geographic azimuth azi relative to the point with geographic longitude
!    lon_radar and latitude lat_radar.
...
!==============================================================================
  ...
  !=========================================================================
  !
  ! .. Effective twoway beam function of an azimuthally scanning radar
  !    after the parameterization of Blahak (2008), JAOTECH.
  !
  ! RESULT:  Value of the beam function for a specific sub-ray at
  !          azimuth al and elevation el relative to the radar coordinate system.
  !
  ! INPUTS:  ista    : index of radar station
  !          az0     : azimuth of beam center    [degrees]
  !          el0     : elevation of beam center [degrees]
  !          az      : azimuth of the respective sub-ray [degrees]
  ...
  ! The smoothing points are computed in a way that the azimuth and elevation
  ...
  ! the beamwidth and azimuthal averaging interval of the radar.
  !
...
!   a 2D binomial filter with nlength symmetric filter weights nfilt times.
...
!===============================================================================
  ...
  ! query if all chars in c are upper case letters:
  FUNCTION isupper (c) RESULT (l)
    ...
    DO i = 1, LEN_TRIM(c)
      icode = IACHAR(c(i:i))
      l = l .AND. (icode >= uacode .AND. icode <= uzcode)
    END DO

  END FUNCTION isupper

  ! query if all chars in c are lower case letters:
  FUNCTION islower (c) RESULT (l)

    IMPLICIT NONE

    CHARACTER(len=*) :: c
    LOGICAL          :: l
    INTEGER :: i, lacode, lzcode, icode 
    ...
    DO i = 1, LEN_TRIM(c)
      icode = IACHAR(c(i:i))
      l = l .AND. (icode >= lacode .AND. icode <= lzcode)
    END DO

  END FUNCTION islower

  ! query if all chars in c are letters (lower or upper case):
  FUNCTION ischar (c) RESULT (l)

    IMPLICIT NONE

    CHARACTER(len=*) :: c
    LOGICAL          :: l
    INTEGER :: i, lacode, lzcode, uacode, uzcode, icode 
    ...
    DO i = 1, LEN_TRIM(c)
      icode = IACHAR(c(i:i))
      l = l .AND. ((icode >= lacode .AND. icode <= lzcode) .OR. (icode >= uacode .AND. icode <= uzcode))
    END DO
