...
! Modules used:
  ...
  USE radar_data, ONLY : &
       ...
       dbz_meta, mpi_comp_meta_typ, mpi_voldata_ostream_typ, &
  ...
  USE radar_data_namelist, ONLY:       &
       ...
       lfdbk_output, fof_file_pattern, lwrite_ready, ready_file_pattern, &
  ...
  !
       ...
       supob_ave_width, supob_ave_width_vr, supob_ave_width_z, supob_minrange, supob_minrange_vr, supob_minrange_z, &
       supob_vrw, supob_rfl, &
       ...
       composite_meta_type, comp_meta, comp_meta_bub, &
       ...
       ldo_bubbles, ldo_bubbles_manual, ydirlistfile, lcomposite_output_bub, &
       composite_file_pattern_bub, comp_grib2_packingtype, &
...
CONTAINS
  ...
  !
               ...
               lequal_azi_alldatasets_d,& ! indicate if all azimuths in the different files for z, vr, qz, qv are the same
    ...
    CHARACTER(len=cmaxlen)      :: ydirradarin_d, ydirlistfile_d, ydirradarout_d,         &
         ...
         &                         ydir_ready_write_d, ready_file_pattern_d, fof_file_pattern_d
    ...
    INTEGER :: &
         ...
         itype_obserr_vr_d, &  ! option for observation error for (superob'ed or original) radial wind (fdbk%e_o):
                               !   = 0: constant observation error (=baseval_obserr_vr)
                               !   = 1: e_o is a linear ramp as function of (superob'ed or original) reflectivity
                               !   = 2: superob'ed e_o + e_o-weighted superob'ed radial wind. For this,
         ...
         itype_Dlim_sgh_d, &  ! option for limiting the size of dry/melting snow and melting graupel/hail
                              ! based on the diameters Dlim_drysnow, Dlim_meltsnow, Dlim_meltgraupel and Dlim_melthail
    ...
    REAL (KIND=dp)  :: &
         supob_azi_maxsector_vr_d , &  ! maximal azimuth sector (symmetrical to its center) for v_r superobing [deg]
         supob_cart_resolution_d  , &  ! resolution of the cartesian grid for superobing [m]
         supob_ave_width_d        , &  ! width of averaging area for superobing [m], base value for all quantities
         supob_ave_width_vr_d     , &  ! width of averaging area for superobing for vr [m], takes priority if set
         supob_ave_width_z_d      , &  ! width of averaging area for superobing for z  [m], takes priority if set
         supob_minrange_d         , &  ! min. range of superobing points [m], base value for all quantities
         supob_minrange_vr_d      , &  ! min. range of superobing points for vr [m], takes priority if set
         supob_minrange_z_d       , &  ! min. range of superobing points for z  [m], takes priority if set
         ...
         rain2mom_mu_incloud_d    , &  ! value to update with the PSD shape parameter mu of rain (in D-space! = exponent to D)
         Dlim_rain_d              , &  ! Limiting raindrop diameter: mass of larger rain DSD part is re-distributed to smaller particles
         Dlim_drysnow_d           , &  ! Limiting dry snow diameter: mass of larger snow PSD part is re-distributed to smaller particles
         Dlim_meltsnow_d          , &  ! Limiting wet snow diameter: mass of larger snow PSD part is re-distributed to smaller melting particles
         Dlim_meltgraupel_d       , &  ! Limiting wet graupel diameter: mass of larger graupel PSD part is re-distributed to smaller melting particles
         Dlim_melthail_d          , &  ! Limiting wet hail diameter: mass of larger graupel PSD part is re-distributed to smaller melting particles
    ...
    REAL    (KIND=dp)               :: rlon_r, rlat_r, hsurf_r, wi, wj, &
                                       rlon_min, rlon_max, rlat_min, rlat_max, tmptime, Dlim_min
    LOGICAL                         :: is_inside
    ...
    CHARACTER(len=*), PARAMETER       :: format_path_vec   = '(T8,A,i2.2,A,T40,A,/,T40,A,T70,A)'
    ...
    CHARACTER(len=*), PARAMETER       :: format_real_f6    = '(T8,A,T40,f12.6,T55,f12.6,T70,A)'
    ...
    NAMELIST /RADARSIM_PARAMS/ dom, ldebug_radsim, loutradwind, &
                               ...
                               fof_file_pattern, lwrite_ready, ready_file_pattern, &
  !.. Define in public, but only apply behind AUXOUT_OFFLINE (and __COSMO__) compiler directives
  !.. This avoids trouble in case there are accidential leftover switch settings in runscripts
                               lmodfield_output, loutnwp, loutvolaux, &
  !
                               ...
                               thin_step_azi, thin_step_range, thin_step_ele, ind_ele_fdbk_glob, ind_ele_voldata_glob, &
                               supob_azi_maxsector_vr, supob_cart_resolution, &
                               supob_ave_width, supob_ave_width_vr, supob_ave_width_z, &
                               supob_minrange, supob_minrange_vr, supob_minrange_z, &
                               supob_vrw, supob_rfl, &
                               ngpsm_h_glob, ngpsm_v_glob, lmds_z, lmds_vr, &
                               ...
                               obs_times_voldata_glob, dt_obs_voldata_glob, ra_inc_coarse_glob, &
                               rain2mom_mu_incloud, itype_Dlim_sgh, Dlim_rain, Dlim_drysnow, &
                               Dlim_meltsnow, Dlim_meltgraupel, Dlim_melthail, &
    ...
    ldo_composite_d = .FALSE.
      ...
      composite_file_pattern_d(:) = '<varname>_<simobs>_<tmodelini>_<tact>.grb2'
      nel_composite_d                    = 2
      ...
      lsmooth_composite_glob = .FALSE.    ! If composite is smoothed by binomial filter
      nsmoothpoints_for_comp_glob = 9     ! width of symmetric 2D binomial smoother in grid points
      nfilt_for_comp_glob = 1             ! number of consecutive applications of the smoother
    ...
    ldo_bubbles_d   = .FALSE.
      ...
      composite_file_pattern_bub_d(:) = '<varname>_<simobs>_<tmodelini>_<tact>.grb2'
      ...
      nsmoothpoints_for_comp_bub_glob = 9     ! width of symmetric 2D binomial smoother in grid points
    ...
    lfdbk_output_d  = .FALSE.
      fof_file_pattern_d(:) = ' '
      fof_file_pattern_d    = 'fof_radar_<stationid>_<scantype>_<tmodelini>.nc'
    lwrite_ready_d  = .FALSE.
      ready_file_pattern_d(:) = ' '
      ready_file_pattern_d    = 'READY_EMVORADO_<tact>'
  !.. Define in public, but only apply behind AUXOUT_OFFLINE (and __COSMO__) compiler directives
  !.. This avoids trouble in case there are accidential leftover switch settings in runscripts
  ...
  !
    ...
    supob_azi_maxsector_vr_d = 40.0_dp                                ! maximal azimuth sector (symmetrical to its center) for v_r superobing [deg]
    supob_cart_resolution_d  = 20000.0_dp                             ! resolution of the cartesian grid for superobing [m]
    supob_ave_width_d        = SQRT(2.0_dp) * supob_cart_resolution_d ! width of averaging area for superobing [m] for all quantities
    supob_ave_width_vr_d     = miss_value                             ! width of averaging area for superobing for vr[m], takes priority if set
    supob_ave_width_z_d      = miss_value                             ! width of averaging area for superobing for z [m], takes priority if set
    supob_minrange_d         = 0.75_dp*supob_ave_width_d              ! min. range of superobing points [m] for all quantities
    supob_minrange_vr_d      = miss_value                             ! min. range of superobing points for vr [m], takes priority if set
    supob_minrange_z_d       = miss_value                             ! min. range of superobing points for z  [m], takes priority if set
    ...
    itype_obserr_vr_d        = 0  ! option for observation error for radial wind:
                                  !   = 0: constant observation error (=baseval_obserr_vr)
                                  !   = 1: e_o is a linear ramp as function of superob'ed reflectivity (see sketch below)
                                  !   = 2: superob'ed e_o + e_o-weighted superob'ed radial wind. For this,
                                  !        e_o is computed from reflectivity before superobing (same linear ramp as for option 1) and is used:
                                  !        a) as second weight (inverse e_o) additionally to spatial Cressman-weight in superobbing of radial wind
                                  !        b) for superobbing of e_o itself with the same weighting
    ...
    rain2mom_mu_incloud_d = unused_value
    itype_Dlim_sgh_d  = 0  ! Option to choose how to implement the size limitation of melting snow and graupel: 
                           ! 0 = no limitation
                           ! 1 = D>Dlim --> Dlim
                           ! 2 = mass(D>Dlim) --> re-DISTRIBUTE to N(D<=Dlim)
    Dlim_rain_d          = 999.0_dp  ! [m] - large neutral value > Dmax_r
    Dlim_drysnow_d       = 999.0_dp  ! [m] - large neutral value > Dmax_s
    Dlim_meltsnow_d      = 999.0_dp  ! [m] - large neutral value > Dmax_s
    Dlim_meltgraupel_d   = 999.0_dp  ! [m] - large neutral value > Dmax_g
    Dlim_melthail_d      = 999.0_dp  ! [m] - large neutral value > Dmax_h
    ...
    IF (my_radar_id == 0) THEN
      ...
      supob_ave_width_vr     = supob_ave_width_vr_d
      supob_ave_width_z      = supob_ave_width_z_d
      supob_minrange         = supob_minrange_d
  ...
  !
      ...
      fof_file_pattern = fof_file_pattern_d
      ...
      rain2mom_mu_incloud    = rain2mom_mu_incloud_d
      itype_Dlim_sgh         = itype_Dlim_sgh_d
      Dlim_rain              = Dlim_rain_d
      Dlim_drysnow           = Dlim_drysnow_d
      Dlim_meltsnow          = Dlim_meltsnow_d
      Dlim_meltgraupel       = Dlim_meltgraupel_d
      Dlim_melthail          = Dlim_melthail_d
      ...
      !-------------------------------------------------------------------------------
      ! .. Check limiting diameter of melting snow and graupel:

      IF (.NOT. ANY(itype_Dlim_sgh == (/0, 1, 2/))) THEN
        errstring(:) = ' '
        WRITE (errstring,'(a,i10)') 'Wrong setting of itype_Dlim_sgh in '//TRIM(radarnmlfile)//'.' // &
             ' Must be 0, 1, or 2, but is ', itype_Dlim_sgh
        CALL abort_run (my_radar_id, 9977, &
             'ERROR: problem in input_radarnamelist(): '//TRIM(ADJUSTL(errstring)), &
             'radar_namelist_read.f90, input_radarnamelist()')
      END IF

      Dlim_min = Dmin_r + 0.05_dp*(Dmax_r-Dmin_r)
      IF (Dlim_rain < Dlim_min) THEN
        errstring(:) = ' '
        WRITE (errstring,'(a,es10.3,a)') 'Wrong value Dlim_rain in '//TRIM(radarnmlfile)//'.' // &
             ' Must be >= ', Dlim_min, ' m !'
        CALL abort_run (my_radar_id, 9978, &
             'ERROR: problem in input_radarnamelist(): '//TRIM(ADJUSTL(errstring)), &
             'radar_namelist_read.f90, input_radarnamelist()')
      END IF
      
      Dlim_min = Dmin_s + 0.05_dp*(Dmax_s-Dmin_s)
      IF (Dlim_drysnow < Dlim_min) THEN
        errstring(:) = ' '
        WRITE (errstring,'(a,es10.3,a)') 'Wrong value Dlim_drysnow in '//TRIM(radarnmlfile)//'.' // &
             ' Must be >= ', Dlim_min, ' m !'
        CALL abort_run (my_radar_id, 9978, &
             'ERROR: problem in input_radarnamelist(): '//TRIM(ADJUSTL(errstring)), &
             'radar_namelist_read.f90, input_radarnamelist()')
      ...
      IF (Dlim_meltsnow < Dlim_min) THEN
        errstring(:) = ' '
        WRITE (errstring,'(a,es10.3,a)') 'Wrong value Dlim_meltsnow in '//TRIM(radarnmlfile)//'.' // &
             ' Must be >= ', Dlim_min, ' m !'
        CALL abort_run (my_radar_id, 9978, &
             'ERROR: problem in input_radarnamelist(): '//TRIM(ADJUSTL(errstring)), &
             'radar_namelist_read.f90, input_radarnamelist()')
      ...
      Dlim_min = Dmin_g + 0.05_dp*(Dmax_g-Dmin_g)
      IF (Dlim_meltgraupel < Dlim_min) THEN
        errstring(:) = ' '
        WRITE (errstring,'(a,es10.3,a)') 'Wrong value Dlim_meltgraupel in '//TRIM(radarnmlfile)//'.' // &
             ' Must be >= ', Dlim_min, ' m !'
        CALL abort_run (my_radar_id, 9979, &
             'ERROR: problem in input_radarnamelist(): '//TRIM(ADJUSTL(errstring)), &
             'radar_namelist_read.f90, input_radarnamelist()')
      END IF
      
      Dlim_min = Dmin_h + 0.05_dp*(Dmax_h-Dmin_h)
      IF (Dlim_melthail < Dlim_min) THEN
        errstring(:) = ' '
        WRITE (errstring,'(a,es10.3,a)') 'Wrong value Dlim_melthail in '//TRIM(radarnmlfile)//'.' // &
             ' Must be >= ', Dlim_min, ' m !'
        CALL abort_run (my_radar_id, 9979, &
             'ERROR: problem in input_radarnamelist(): '//TRIM(ADJUSTL(errstring)), &
             'radar_namelist_read.f90, input_radarnamelist()')
      ...
      !-------------------------------------------------------------------------------
      ! .. Check components for voldata_outstream's:

      DO i=1, noutstreams_max
        
        !--------------------------------------------------------------------------------
        ! .. Enforce full list of all possible fields, if 'all' is given in the namelist:
        ...
        !--------------------------------------------------------------------------------
        ! .. If no output file name patterns are given, set patterns depending on the
        !    output format:
        
        IF (LEN_TRIM(voldata_ostream(i)%file_pattern) <= 0 .AND. ANY( len_TRIM( voldata_ostream(i)%output_list(:)) > 0 )) THEN
          
          SELECT CASE (TRIM(voldata_ostream(i)%FORMAT))
          CASE ('cdfin','cdfin-mulmom')
            voldata_ostream(i)%file_pattern = 'cdfin_<varname>_<stationid>_<tstart_mm>_<tend_mm>_<scantype>'          
          CASE ('grib2','grib2-mulmom')
             voldata_ostream(i)%file_pattern = 'grib2_<varname>_<stationid>_<tstart_mm>_<tend_mm>_<scantype>'          
          CASE ('ascii','ascii-gzip')
            voldata_ostream(i)%file_pattern = '<varname>_<stationid>_<scantype>_<tmodelini>_<tvvzact>_<levtype>.dat'
          CASE ('f90-binary')
            voldata_ostream(i)%file_pattern = '<varname>_<stationid>_<scantype>_<tmodelini>_<tvvzact>_<levtype>.bin'
          CASE default
            errstring(:) = ' '
            WRITE (errstring,'(a,i2.2,a,a)') 'Wrong voldata_ostream(',i,')%format in '//TRIM(radarnmlfile)//'.' // &
                 ' Must be ''cdfin'', ''cdfin-mulmom'', ''grib2'', ''grib2-mulmom'', '// &
                 '''ascii'', ''ascii-gzip'', or ''f90-binary'', but is ''', &
                 voldata_ostream(i)%FORMAT, ''' !'
            CALL abort_run (my_radar_id, 10983, &
                 'ERROR: problem in input_radarnamelist(): '//TRIM(ADJUSTL(errstring)), &
                 'radar_namelist_read.f90, input_radarnamelist()')
          END SELECT
      ...
      !-------------------------------------------------------------------------------
      ! .. Check namelist parameters for superobservations for fof-files:

      IF (supob_ave_width_vr < miss_threshold) THEN
        supob_ave_width_vr = supob_ave_width
      END IF
      IF (supob_ave_width_z < miss_threshold) THEN
        supob_ave_width_z = supob_ave_width
      END IF
      
      IF (supob_minrange_vr < miss_threshold) THEN
        supob_minrange_vr = supob_minrange
      END IF
      IF (supob_minrange_z < miss_threshold) THEN
        supob_minrange_z = supob_minrange
      END IF
    ...
    IF (num_radar > 1) THEN
      ...
      IF (my_radar_id == 0) THEN
        ...
        nibuf = nibuf + 1 ;   intbuf(nibuf)  = itype_Dlim_sgh
        ...
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = supob_ave_width_vr
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = supob_ave_width_z
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = supob_minrange
        ...
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = rain2mom_mu_incloud
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = Dlim_rain
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = Dlim_drysnow
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = Dlim_meltsnow
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = Dlim_meltgraupel
        nrbuf = nrbuf + 1 ;   realbuf(nrbuf)  = Dlim_melthail
        ...
        nlbuf = nlbuf + 1 ;   logbuf(nlbuf)  = labort_if_problems_gribout
  ...
  !
      ...
      IF (my_radar_id /= 0) THEN
        ...
        nibuf = nibuf + 1 ;   itype_Dlim_sgh         = intbuf(nibuf)
        ...
        nrbuf = nrbuf + 1 ;   supob_ave_width_vr     = realbuf(nrbuf)
        nrbuf = nrbuf + 1 ;   supob_ave_width_z      = realbuf(nrbuf)
        nrbuf = nrbuf + 1 ;   supob_minrange         = realbuf(nrbuf)
        ...
        nrbuf = nrbuf + 1 ;   rain2mom_mu_incloud    = realbuf(nrbuf)
        nrbuf = nrbuf + 1 ;   Dlim_rain              = realbuf(nrbuf)
        nrbuf = nrbuf + 1 ;   Dlim_drysnow           = realbuf(nrbuf)
        nrbuf = nrbuf + 1 ;   Dlim_meltsnow          = realbuf(nrbuf)
        nrbuf = nrbuf + 1 ;   Dlim_meltgraupel       = realbuf(nrbuf)
        nrbuf = nrbuf + 1 ;   Dlim_melthail          = realbuf(nrbuf)
        ...
        nlbuf = nlbuf + 1 ;   labort_if_problems_gribout =logbuf(nlbuf)
  ...
  !
      ...
      CALL distribute_path_radar ( fof_file_pattern , icomm_radar )
...
!!$             - This would enable to choose different radars/scan strategies for different composites.
      ...
      CASE ( i_fakeobs )
        ...
        IF (my_radar_id == 0) THEN
          ...
          DO ista=1, nradsta
            IF (ANY(rs_meta_ncdf(ista)%obs_times >= miss_threshold)) THEN
              rs_meta_ncdf(ista)%obs_times_obs = rs_meta_ncdf(ista)%obs_times
            ELSE
              CALL check_and_fix_dtobs (rs_meta_ncdf(ista)%dt_obs, errstring, ierr)
              CALL create_obstimes_from_dtobs (rs_meta_ncdf(ista)%dt_obs, rs_meta_ncdf(ista)%nobs_times, &
                                               rs_meta_ncdf(ista)%obs_times, errstring, ierr)
              rs_meta_ncdf(ista)%obs_times_obs  = rs_meta_ncdf(ista)%obs_times
              rs_meta_ncdf(ista)%nobs_times_obs = rs_meta_ncdf(ista)%nobs_times
            END IF
            rs_meta_ncdf(ista)%obsfile(:,i_vrad)(:) = ' '
            rs_meta_ncdf(ista)%obsfile(:,i_vrad)    = 'fake_obs'  ! needed to allow fof file output
            rs_meta_ncdf(ista)%obsfile(:,i_dbzh)(:) = ' '
            rs_meta_ncdf(ista)%obsfile(:,i_dbzh)    = 'fake_obs'
          END DO
        END IF
...
!#ifdef AUXOUT_OFFLINE
      ...
      WRITE(*,'(a,T35,"=",es12.5)')  '  ra_inc_coarse_glob', ra_inc_coarse_glob
      WRITE(*,'(a,T35,"=",es12.5)')  '  rain2mom_mu_incloud', rain2mom_mu_incloud
      WRITE(*,'(a,T35,"=",i8)')      '  itype_Dlim_sgh', itype_Dlim_sgh
      WRITE(*,'(a,T45,"=",es12.5)')  '  Dlim_rain', Dlim_rain
      WRITE(*,'(a,T45,"=",es12.5)')  '  Dlim_drysnow', Dlim_drysnow
      WRITE(*,'(a,T45,"=",es12.5)')  '  Dlim_meltsnow', Dlim_meltsnow
      WRITE(*,'(a,T45,"=",es12.5)')  '  Dlim_meltgraupel', Dlim_meltgraupel
      WRITE(*,'(a,T45,"=",es12.5)')  '  Dlim_melthail', Dlim_melthail
      ...
      WRITE(*,'(a,T35,"=",es12.4)') '  supob_ave_width_vr', supob_ave_width_vr
      WRITE(*,'(a,T35,"=",es12.4)') '  supob_ave_width_z', supob_ave_width_z
      WRITE(*,'(a,T35,"=",es12.4)') '  supob_minrange', supob_minrange
    ...
    IF ( my_radar_id == 0 ) THEN
      ...
      DO ii=1, noutstreams_max
        ...
        ELSE
          DO  i=1, noutput_fields_max
            ...
            END IF
          END DO
          IF (LEN_TRIM(voldata_ostream(ii)%output_subdir) > 0) THEN
            WRITE (ctrlfile_unit, TRIM(format_path_vec))                     &
                 'voldata_ostream(',ii,')%output_list', TRIM(voldata_ostream(ii)%output_subdir), '-', 'C*300'
          ELSE
            WRITE (ctrlfile_unit, TRIM(format_path_vec))                     &
                 'voldata_ostream(',ii,')%output_list', '-', '-', 'C*300'
          END IF
          ...
          END IF
          IF (LEN_TRIM(voldata_ostream(ii)%pat_scantype_volscan) > 0) THEN
            WRITE (ctrlfile_unit, '(T2,A,i2.2,A,T50,A,T65,A,T80,A)')         &
                 'voldata_ostream(',ii,')%pat_scantype_volscan', voldata_ostream(ii)%pat_scantype_volscan, '-', 'C*12'
          ELSE
            WRITE (ctrlfile_unit, '(T2,A,i2.2,A,T50,A,T65,A,T80,A)')         &
                 'voldata_ostream(',ii,')%pat_scantype_volscan', '-', '-', 'C*300'
          END IF
          IF (LEN_TRIM(voldata_ostream(ii)%pat_scantype_volscan) > 0) THEN
            WRITE (ctrlfile_unit, '(T2,A,i2.2,A,T50,A,T65,A,T80,A)')         &
                 'voldata_ostream(',ii,')%pat_scantype_precipscan', voldata_ostream(ii)%pat_scantype_precipscan, '-', 'C*12'
          ELSE
            WRITE (ctrlfile_unit, '(T2,A,i2.2,A,T50,A,T65,A,T80,A)')         &
                 'voldata_ostream(',ii,')%pat_scantype_precipscan', '-', '-', 'C*12'
          END IF
      ...
      END IF
      IF (LEN_TRIM(fof_file_pattern) > 0) THEN
        WRITE (ctrlfile_unit, TRIM(format_path))                        &
             'fof_file_pattern', TRIM(fof_file_pattern), '-', 'C*300'
      ELSE
        WRITE (ctrlfile_unit, TRIM(format_path))                        &
             'fof_file_pattern', '-', '-', 'C*300'
      END IF
      WRITE (ctrlfile_unit, TRIM(format_logical))                       &
      ...
      WRITE (ctrlfile_unit, TRIM(format_real))                       &
           'supob_ave_width_vr', supob_ave_width_vr, supob_ave_width_vr_d, ' R '
      WRITE (ctrlfile_unit, TRIM(format_real))                       &
           'supob_ave_width_z', supob_ave_width_z, supob_ave_width_z_d, ' R '
      WRITE (ctrlfile_unit, TRIM(format_real))                       &
           'supob_minrange', supob_minrange, supob_minrange_d, ' R '
      WRITE (ctrlfile_unit, TRIM(format_real))                       &
      ...
      WRITE (ctrlfile_unit, TRIM(format_real_f))                       &
           'rain2mom_mu_incloud', rain2mom_mu_incloud, rain2mom_mu_incloud_d, ' R '
      WRITE (ctrlfile_unit, TRIM(format_int))                       &
           'itype_Dlim_sgh', itype_Dlim_sgh, itype_Dlim_sgh_d, ' I '
      WRITE (ctrlfile_unit, TRIM(format_real_f6))                       &
           'Dlim_rain', Dlim_rain, Dlim_rain_d, ' R '
      WRITE (ctrlfile_unit, TRIM(format_real_f6))                       &
           'Dlim_drysnow', Dlim_drysnow, Dlim_drysnow_d, ' R '
      WRITE (ctrlfile_unit, TRIM(format_real_f6))                       &
           'Dlim_meltsnow', Dlim_meltsnow, Dlim_meltsnow_d, ' R '
      WRITE (ctrlfile_unit, TRIM(format_real_f6))                       &
           'Dlim_meltgraupel', Dlim_meltgraupel, Dlim_meltgraupel_d, ' R '
      WRITE (ctrlfile_unit, TRIM(format_real_f6))                       &
           'Dlim_melthail', Dlim_melthail, Dlim_melthail_d, ' R '
  ...
  SUBROUTINE ctrl_output_rsmeta_nuspec_fwo(funit, ista, rs_m, rs_m_d, rsname)
    ...
    WRITE (funit, TRIM(format_char))                     &
         TRIM(cname), 'station_name',  rs_m%station_name, rs_m_d%station_name, ' C*5 '
    WRITE (funit, TRIM(format_char))                     &
