...
! Contact information: jana.mendrok (at) dwd.de
!                      ulrich.blahak (at) dwd.de 
!
...
! Modules used:
  ...
  USE radar_data, ONLY :  &
       ...
       c_dwd, c_meteoswiss, c_arpasim, c_belgium, c_denmark, c_france, c_poland, c_czech, c_netherlands, c_slovakia, &       
       c_format_cdfin, &
  ...
  USE radar_data_namelist, ONLY : ldebug_radsim, &
       ...
       lequal_azi_alldatasets, labort_if_problems_obsfiles

  USE radar_interface, ONLY : get_obstime_ind_of_currtime, abort_run
  ...
  INTERFACE qcrad_flag_eval_dwd
    MODULE PROCEDURE            &
         qcrad_flag_eval_dwd_3D,    &
         qcrad_flag_eval_dwd_1D
  END INTERFACE qcrad_flag_eval_dwd
...
CONTAINS
  ...
  SUBROUTINE read_field_obs_opera_h5(ista, idata, itime, hdf5_shortname, &
    ...
    !    available azimuths and elevations for different data sets
    ...
    DO o = 1, rs_meta(ista)%nel        ! Loop over Reports (one elevation per record)
      DO m = 1, rs_meta(ista)%naz      ! Loop over Azimuth
  ...
  SUBROUTINE read_field_obs_dwd_h5(ista, idata, itime, hdf5_shortname, &
    ...
    !
    ! Method: The HDF5 files for one or all variable(s) and one PPI scan
    !         are opened within this routine.
    !         "datap" has to be a pointer to a 1D data vector in
    !         the rs_data(ista) structure, which points to the desired
    !         input field. "ncdf_shortname" denotes the NetCDF shortname
    ...
    !         "ldata_avail" is a flag indicating failure or success of reading.
    !         "missing_data" is the file-internal missing value for the data.
    !         The reader is capable of also reading the quality flags (group "quality1") associated
    !         to each dataset in the hdf5 files, if present.
    !
    ! IS CALLED FROM "READ_OBS_RAD".
    !
    ! Caveat: At the moment, this subroutine has some side-effects through
    ...
    INTEGER, INTENT(IN)    :: &
         ista     , & ! Station nr.
         idata    , & ! Flag for the kind of data (i_vrad, iqualvrad, i_dbzh, i_qualdbzh, ...)
         itime        ! time index, from 1 ... rs_meta(ista)%nobs_times
    ...
    ! .. If quality flags are to be read, append 'quality1' to the vardataset name:
    SELECT CASE (idata)
    CASE (i_qualvrad, i_qualdbzh)
      vardataset = TRIM(vardataset) // '/quality1'
    END SELECT
  ...
  SUBROUTINE read_field_obs_dwd(ista, idata, itime, ncdf_shortname, &
    ...
    IF (.NOT.lequal_azi_alldatasets .OR. MAXVAL(rs_data(ista)%check) == -99) THEN
      ...
      !    that azimuths are exactly equal for all datasets.
      ...
      ! .. Get azimuth- and elevation- indices for sorting of the radar data into a regular
      !    3D data set with nominal range, azimuth and elevation:

      ! .. Azimuth: assign azimuth index by rounding to the next regular azimuth value
      ! .. Elevation: assign index from next regular elevation in rs_meta(ista)%el_arr(:)
      ! .. mabaz0 is the end of the azimuth interval in sweep direction, so, in the following
      !    subroutine, add/subtract half of az_inc (dep. on antenna rotation direction) to
      !    center it on the center of the azimuth interval
      ...
      !    which the indices for range, azimuth and elevation can be restored:
      ...
      DO o = start_rec,end_rec           ! Loop over Reports (one elevation per record)
        DO m = 1,rs_meta(ista)%naz_ncdf(itime,idata)  ! Loop over Azimuth
!CDIR NODEP
          DO n = 1,rs_meta(ista)%nra_obs     ! Loop over Range
            nobs = nobs + 1
...
!!$ UB, Alberto: set nobs only if there is a valid data file:
    ...
    ELSE
      ...
      !    azimuths and elevations of this data set with %check:

      ! .. Some azimuth- and/or elevation values in different data type files might deviate
      !    by 0.01 degree (rounding error???), so we need a little tolerance
      !    in elevation- and azimuth checking:

      tol = nrep*1  ! tolerance in the sum of azimuths/elevations = nrep * 0.01 (nrep*1 because of factor 100 below!)
  ...
  SUBROUTINE read_field_obs_kit_h5(ista, idata, itime, hdf5_shortname, &
    ...
    !    - azimuths are not stored in startazA list, but along other informations
  ...
  SUBROUTINE get_dataset_odim_h5 (yzroutine, infilename, file_id, hdf5_shortname, &
    ...
    INTEGER,               INTENT(inout) :: nazis(:)         ! Vector of number of azimuths for each elevation, will be filled when reading the data
    LOGICAL,               INTENT(in)    :: lcheck_azi_sorting ! Whether to check the azimuth sorting and if necessary, shift the start azi to North, based on attribute "startazA"
    LOGICAL,               INTENT(in)    :: lwarn_on_missing_startazA ! Whether or not to report if "startazA" is missing in the file and azimuth cannot be checked/shifted. In this case, we have to trust the azimuth sorting in the file to be referenced to North.
    ...
    IF (eleind >= missthr_int) THEN
      ...
      IF (rsm%icountry == i_dwd .AND. rsm%station_name == c_dwd//'BON') THEN
        ! Workaround for problematic files from KIT-Cube which may contain different numbers of azimuths
        ! for the same elevations of different variables, and/or different numbers of azimuths for different
        ! elevations. Mostly this is one more azimuth (361) than nominal (360). As this happens
        ...
        ELSE IF (ival > rsm%naz) THEN
          ! We tolerate only 1 azimuth more than nominal:
    ...
    IF (ldata_avail) THEN
      ...
      IF (idata /= i_qualvrad .AND. idata /= i_qualdbzh) THEN
      ...
      ELSE
        ! .. quality flags do not contain any voids, so we set dummy values:
        undetected = miss_value
        nodata = miss_value
      END IF
      ...
      IF (ALLOCATED(iarr2d)) THEN
        ...
        IF (rsm%icountry == i_dwd .AND. rsm%station_name == c_dwd//'BON') THEN
          IF (naz > nazis(eleind)) THEN
            ...
            ! If necessary, cut out the superfluous azimuth:
            ...
            ELSE
              ...
              IF (a1gate < 1 .OR. a1gate > naz) THEN
                ...
                ! no data will be read below, because nazis(eleind) /= naz
              ELSE
                WRITE (*,'(a,i0,a)') 'WARNING '//TRIM(yzroutine)//': deleting superfluous azimuth with index=', a1gate-1, &
                     ' from '//TRIM(cpath)//' in '//TRIM(infilename)
                ! eliminate the superfluous azimuth at the true end of the PPI-scan. This is the index right before a1gate:
                IF (a1gate > 1) THEN
          ...
          IF (lcheck_azi_sorting) THEN
            ...
            IF (ALLOCATED(startaza)) THEN
              ...
              IF (ABS(ABS(dazi)-rsm%az_inc) <= 0.02_dp) THEN
                ...
                !  in ascending azimuth direction:
...
!!$                  END IF
                ...
                END DO
                IF (n_noazi > 0) THEN
                  WRITE (*,*) 'WARNING '//TRIM(yzroutine)//': Some azimuths missing in '// &
                       TRIM(infilename)
                  DO n=1, rsm%naz
                    IF (azichecklist(n) == -1) THEN
                      WRITE (*,'(a,i0,a,f0.2)') ' |--> Missing azimuth no. ',n, ' for elevation ', rsm%el_arr(eleind)
                    END IF
                  END DO
                END IF
              ...
              ELSE
                ...
                WRITE (*,*) 'WARNING '//TRIM(yzroutine)//': Discard file, wrong azimuth increment, '// &
                     TRIM(infilename)
                ldata_avail = .FALSE.
            ...
            ELSE
              ...
              ! We have to trust the azimuth sorting, namely that the first azimuth is north and that the following are sorted
              !  in ascending azimuth direction:
              DO nn=1, naz
                azindlist(nn) = nn
              END DO
          ...
          ELSE
            ...
            ! We trust the azimuth sorting, namely that the first azimuth is north and that the following are sorted
            !  in ascending azimuth direction:
            DO nn=1, naz
              azindlist(nn) = nn
            END DO
          ...
          IF (ldata_avail) THEN
            ...
            CASE default
              ...
              WRITE (*,*) 'WARNING '//TRIM(yzroutine)//': Discard file, wrong idata for hdf5_shortname '''// &
                   TRIM(hdf5_shortname)//''' when reading the 2D data from '//TRIM(infilename)
              ldata_avail = .FALSE.
        ...
        ELSE
          ...
          WRITE (*,*) 'WARNING '//TRIM(yzroutine)//': Discard file, wrong range or azimuth dimension '// &
               'when reading the 2D data for hdf5_shortname '''// &
                   TRIM(hdf5_shortname)//''' from '//TRIM(infilename)
          ldata_avail = .FALSE.
      ...
      ELSE
        ...
        WRITE (*,*) 'WARNING '//TRIM(yzroutine)//': Failed reading the 2D data for hdf5_shortname '''// &
                   TRIM(hdf5_shortname)//''' from '//TRIM(infilename)
        ldata_avail = .FALSE.
...
!!$    }
    ...
    IF (eleind >= missthr_int) THEN
      ...
      ! Workaround for problematic files from KIT which may contain different numbers of azimuths
      ! for the same elevations of different variables, and/or different numbers of azimuths for different
      ! elevations. Mostly this is one more azimuth (361) than nominal (360). As this happens
      ! very often and these data are not used operationally, those files have not been discarded right away,
      ! but in the data reader we use a1gate to separate out the superfluous datum.
      ...
      IF (ALLOCATED(iarr2d)) THEN
        ...
        IF (naz > nazis(eleind)) THEN
          ...
          IF (ndiff > 1) THEN
            ...
            ! no data will be read below, because nazis(eleind) /= naz
          ELSE
            WRITE (*,'(a,i0,a)') 'WARNING '//TRIM(yzroutine)//': deleting all azimuths with index > ', nazis(eleind), &
                 ' from '//TRIM(cpath)//' in '//TRIM(infilename)
            ! eliminate the superfluous azimuth at the true end of the PPI-scan:
            iarr2d(:,naz) = miss_value
            naz = nazis(eleind)
          END IF
        END IF
...
!!$                  END IF
              ...
              ELSE
                ...
                WRITE (*,*) 'WARNING '//TRIM(yzroutine)//': Discard ', TRIM(cpath),', wrong azimuth increment, '// &
                     TRIM(infilename)
                ldata_avail = .FALSE.
  ...
  SUBROUTINE check_and_complete_dataset ( yzroutine, rsd, rsm, itime, idata, nel, nobs_max, &
    ...
    IF (.NOT.lequal_azi_alldatasets .OR. MAXVAL(rsd%check) == -99) THEN
      ...
      DO o = 1, rsm%nel        ! Loop over elevations
        DO m = 1, rsm%naz      ! Loop over Azimuth
    ...
    ELSE
      ...
      tol = nel*1  ! tolerance in the sum of azimuths/elevations = nel * 0.01 (nel*1 because of factor 100 below!)
  ...
  SUBROUTINE read_obs_rad ( ista, time_mod, action )
    ...
    !         - round first azimuth value to nearest multiple of dazi
    !
    ! MISSING: Interpolation to regular azimuth grid!
    !
    !------------------------------------------------------------------------------
    !
    ! Subroutine / Function arguments
      ...
      ELSE
        ...
        IF ( rs_meta(ista)%icountry == i_dwd .AND. &
          ...
          IF (loutradwind) THEN
            ...
            IF (rs_meta(ista)%lobs_avail(irep,i_vrad)) THEN
              ! .. Apply quality flags:
              IF (lqc_flag) THEN
                !  0 = ok or noecho, 1 = nodata/filtered:
                CALL read_field_obs_dwd_h5 (ista,i_qualvrad,irep, &
                     'QUAL_VRAD',rs_data(ista)%radwind_obs_q, rs_data(ista)%ind_intp_obs_qv, &
                     rs_meta(ista)%lobs_avail(irep,i_qualvrad))
                IF (.NOT.rs_meta(ista)%lobs_avail(irep,i_qualvrad)) THEN
                  ! .. sometimes there is only quality files for i_dbzh. In this case,
                  ! .. try and use dbzh flags instead and store them in the field for vrad flags:
                  CALL read_field_obs_dwd_h5 (ista,i_qualdbzh,irep, &
                       'QUAL_DBHZ_FOR_VRAD',rs_data(ista)%radwind_obs_q, rs_data(ista)%ind_intp_obs_qv, &
                       rs_meta(ista)%lobs_avail(irep,i_qualvrad))
                END IF
                IF ( rs_meta(ista)%lobs_avail(irep,i_qualvrad) ) THEN
                  IF (TRIM(rs_meta(ista)%obsfile_format(irep)) == TRIM(c_format_h5_native_mmss)) THEN
                    ! .. These are data from the radar sites, not from POLARA QC. Qualtiy flags have a different meaning there:
                    !  0 = ok, 1 = nodata/filtered, 2 = noecho. noecho and ok should be both 0, so fix that:
                    WHERE (rs_data(ista)%radwind_obs_q == 2.0_dp)
                      rs_data(ista)%radwind_obs_q = 0.0_dp
                    END WHERE
                  END IF
                  ! This assumes that 0 = correct and /= 0 should be set to miss_value:
                  CALL qcrad_flag_eval_dwd (ista, 'vr', rs_data(ista)%radwind_obs_q, rs_data(ista)%radwind_obs)
                ELSE
                  errstring(:) = ' '
                  WRITE (errstring,'(a,i6.6,a)') 'ERROR '//TRIM(yzroutine)// &
                       ': no hdf5 quality flags for station ', &
                       rs_meta(ista)%station_id, ' at time '//rs_meta(ista)%obs_cdate(irep)//&
                       '! Neither for VRAD nor for DBZH! Quality flags for VRAD cannot be applied!'
                  IF (labort_if_problems_obsfiles) THEN
                    CALL abort_run (my_radar_id, 20189, TRIM(ADJUSTL(errstring)), &
                         'radar_obs_data_read.f90, '//TRIM(yzroutine))
                  ELSE
                    WRITE (*,'(a)') TRIM(errstring)
                  END IF
                END IF
              END IF
            END IF
          ...
          IF (loutdbz .OR. (loutradwind .AND. itype_obserr_vr > 0)) THEN
            ...
            IF ( rs_meta(ista)%lobs_avail(irep,i_dbzh) ) THEN

              IF (lqc_flag) THEN
                CALL read_field_obs_dwd_h5 (ista,i_qualdbzh,irep, &
                     'QUAL_DBZH',rs_data(ista)%zh_radar_obs_q, rs_data(ista)%ind_intp_obs_qz, &
                     rs_meta(ista)%lobs_avail(irep,i_qualdbzh))
                IF (rs_meta(ista)%lobs_avail(irep,i_qualdbzh)) THEN
                  IF (TRIM(rs_meta(ista)%obsfile_format(irep)) == TRIM(c_format_h5_native_mmss)) THEN
                    ! .. These are data from the radar sites, not from POLARA QC. Qualtiy flags have a different meaning there:
                    !  0 = ok, 1 = nodata/filtered, 2 = noecho. noecho and ok should be both 0, so fix that:
                    WHERE (rs_data(ista)%zh_radar_obs_q == 2.0_dp)
                      rs_data(ista)%zh_radar_obs_q = 0.0_dp
                    END WHERE
                  END IF
                  ! This assumes that 0 = correct and /= 0 should be set to miss_value:
                  CALL qcrad_flag_eval_dwd (ista, 'z', rs_data(ista)%zh_radar_obs_q, rs_data(ista)%zh_radar_obs)
                ELSE
                  errstring(:) = ' '
                  WRITE (errstring,'(a,i6.6,a)') 'ERROR '//TRIM(yzroutine)// &
                       ': no hdf5 quality flags for DBZH for station ', &
                       rs_meta(ista)%station_id, ' at time '//rs_meta(ista)%obs_cdate(irep)//&
                       '! Quality flags for DBZH cannot be applied!'
                  IF (labort_if_problems_obsfiles) THEN
                    CALL abort_run (my_radar_id, 20190, TRIM(ADJUSTL(errstring)), &
                         'radar_obs_data_read.f90, '//TRIM(yzroutine))
                  ELSE
                    WRITE (*,'(a)') TRIM(errstring)
                  END IF
        ...
        IF ( TRIM(rs_meta(ista)%obsfile_format(irep)) == TRIM(c_format_cdfin) .AND. &
          ...
          !                "cdfin_z_id-010908_20130728220000_20130728225500_PPI0080" or
          !                "cdfin_z_id-010908_20130728220000_20130728225500_PRECIP"
          ...
          IF (loutradwind) THEN
            ...
            IF (rs_meta(ista)%lobs_avail(irep,i_vrad)) THEN
              ! .. Mask out missing values from the cdfin file. This can only happen beyond the max range of an elevation:
              ...
              IF (lqc_flag) THEN
                ...
                ELSE
                  errstring(:) = ' '
                  WRITE (errstring,'(a,i6.6,a)') 'ERROR '//TRIM(yzroutine)// &
                       ': no cdfin quality flag files for station ', &
                       rs_meta(ista)%station_id, ' at time '//rs_meta(ista)%obs_cdate(irep)//&
                       '! Neither for VRAD nor for DBZH! Quality flags for VRAD cannot be applied!'
                  IF (labort_if_problems_obsfiles) THEN
                    CALL abort_run (my_radar_id, 20089, TRIM(ADJUSTL(errstring)), &
                         'radar_obs_data_read.f90, '//TRIM(yzroutine))
                  ELSE
                    WRITE (*,'(a)') TRIM(errstring)
                  END IF
          ...
          IF (loutdbz .OR. (loutradwind .AND. itype_obserr_vr > 0)) THEN
            ...
            IF ( rs_meta(ista)%lobs_avail(irep,i_dbzh) ) THEN
              ...
              ELSE
                ...
                !   We assign missing value -999.99 to the "_fill_values" and "correct 0's"
                !   to the -32:
                WHERE (rs_data(ista)%zh_radar_obs == missing_z)
              ...
              IF (lqc_flag) THEN
                ...
                ELSE
                  errstring(:) = ' '
                  WRITE (errstring,'(a,i6.6,a)') 'ERROR '//TRIM(yzroutine)// &
                       ': no cdfin quality flag file for DBZH for station ', &
                       rs_meta(ista)%station_id, ' at time '//rs_meta(ista)%obs_cdate(irep)//&
                       '! Quality flags for DBZH cannot be applied!'
                  IF (labort_if_problems_obsfiles) THEN
                    CALL abort_run (my_radar_id, 20090, TRIM(ADJUSTL(errstring)), &
                         'radar_obs_data_read.f90, '//TRIM(yzroutine))
                  ELSE
                    WRITE (*,'(a)') TRIM(errstring)
                  END IF
...
!!$            rs_meta(ista)%lobs_avail(irep,i_qualdbzh) = rs_meta(ista)%lobs_avail(irep,i_dbzh)
        ...
        ELSE
          ...
          ! .. Optional range coarsening (applies qc-flags before coarsening and
          !    re-defines the data vectors and rs_data%nobs_obs):
          !    NOTE: quality flags only really implemented for dbzh and vradh.
          !          Need a good concept for polar. moments and need to implement it!
  ...
  SUBROUTINE get_posind_rounded_fullrev (info_in_case_of_error, az_volscan, el_volscan, naz_ncdf, nele_ncdf,  &
    ...
    !------------------------------------------------------------------------------
    !
    ! Description: For azimuth- and elevation-data of a DWD volume scan,
    !              determine the usable data sector (constant elevation,
    !              continuous valid azimuth range). Then, compute the
    !              "regular" azimuth- and elevation indices valid for
    !              approximating the "true" ones by a regular azimuth- grid
    !              which spans one full antenna revolution (360 degrees)
    !              with a constant increment of "az_inc".
    !
    ! Method:
    !
    !   Round "middle" azimuth value of the sweeps to nearest multiple of az_inc and successively
    !   increment/decrement the azimuth index by 1 from neighbour to neighbour.
    !   The direction of antenna rotation is correctly taken into account.
    !
    !   We take into account the fact that the azimuth values given in
    !   the radar data files denote the end of the azimuthal pulse averaging interval
    ...
    !
    !   As a result, the subroutine returns regular azimuth indices i in a nearest-neighbour
    !   sense for each ray of the sweep.
    !   From i, the nominal azimuth value can be computed by:
    !
    !   az(i) = az_start + (i-1)*az_inc   ,  i = 1 ... naz (full antenna revolution)
    !
    !      where    naz = NINT(360 / az_inc)
    !
    !   and az_start denotes the center of the first azimuth pulse averaging interval (sampling interval).
    !
    ! Outputs:
    !
    !   - naz : number of nominal azimuths for full antenna revolution
    !   - azind(1:naz_ncdf, 1:nele_ncdf): nominal azimuth index (equals "i" above)
    !                                     for each radar bin in each elevation sweep
    !   - eleind(1:nele_ncdf): nominal elevation index for each elevation
    !
    !   - "missing" azimuths and elevations in the data file are flagged by azind(i) = missval_int
    !
    !
    ! POSSIBLE IMPROVEMENT: Interpolation to regular azimuth grid!
    !
    !------------------------------------------------------------------------------
    !
    ! Subroutine / Function arguments
    ...
    REAL (KIND=dp), INTENT(in) :: az_volscan(naz_ncdf,nele_ncdf) ! array of azimuths for each ray of a volume scan     [deg]
    REAL (KIND=dp), INTENT(in) :: el_volscan(naz_ncdf,nele_ncdf) ! array of elevations for each ray of a volume scan  [deg]
    REAL (KIND=dp), INTENT(in) ::  az_start, az_inc              ! nominal azimuth start and increment [deg]
    REAL (KIND=dp), INTENT(in) ::  el_arr(nele)                  ! nominal elevation vector           [deg]
    REAL (KIND=dp), INTENT(in) ::  missing_az                    ! missing value in azimuth array      [deg]

    ! .. INTENT(OUT):
    INTEGER, INTENT(out) ::  naz                 ! number of nominal azimuths for full antenna revolution
    INTEGER, INTENT(out) ::  azind(naz_ncdf,nele_ncdf) ! nominal azimuth index of each ray
    ...
    !.. number of azimuths for full antenna revolution:
    ...
    !.. Determine the regular azimuths and their nominal indices for each elevation
    !   along with the nominal elevation index of the elevation:
    eleloop: DO o = 1, nele_ncdf

      !.. Clip the azimuth range to its non-missing range (search from the back):
      DO i = naz_ncdf, 1, -1
      ...
      !.. Antenna rotation direction +1 or -1, determined from the first few azimuths:
      ...
      !.. The azimuth values in the radar files denote the start of the azimuth averaging interval
      !   in antenna rotation direction. To ensure that the below determined
      !   nearest regular azimuths represent the center of the averaging intervals,
      !   cheat here by shifting the "regular" az_start accordingly by 0.5*az_inc:
      az_start_mod = az_start + rotsign*az_inc*0.5

      !.. Determine mean elevation of sweep from mid section of azimuth interval
...
!CDIR NODEP
      ...
      !.. Find the azimuth part with constant elevation by searching from both sides
      !   of the azimuth sector towards its center. This assumes that only
      !   the margins of the azimuthal sweep deviate:
      ...
      IF (azmin_el == missval_int .OR. azmax_el == missval_int) THEN
        WRITE (*,'(a,2i5,1x,f0.4,1x,a)') 'WARNING '//TRIM(yzroutine)//': a sweep from a NetCDF file' // &
             ' does not have azimuths at constant elevation!  naz, eleind, elemean, data_info = ', znaz_ncdf, o, elemean, &
             TRIM(info_in_case_of_error)
        CYCLE eleloop
      END IF

      !.. number of azimuths with constant elevation:
      znaz = azmax_el - azmin_el + 1

      IF ( znaz < 20 ) THEN
        WRITE (*,'(a,3i5,1x,a)') 'WARNING '//TRIM(yzroutine)//': a sweep from a NetCDF file' // &
             ' has less than 20 azimuths at constant elevation! ',azmin_el,azmax_el,o, TRIM(info_in_case_of_error)
      ELSE IF ( znaz < naz ) THEN
        WRITE (*,'(a,3i5,1x,a)') 'WARNING '//TRIM(yzroutine)//': a sweep from a NetCDF file' // &
             ' does not cover one complete antenna revolution! ',azmin_el,azmax_el,o, TRIM(info_in_case_of_error)
      END IF

      !.. Rounding of the nominal azimuth index of the middle regular azimuth
      !   and incrementing the indices for the next azimuths, starting from the
      !   first regular azimuth. This assumes that
      !   the azimuths of the sweep are continuous. The direction of
      !   antenna rotation is correctly taken into account, so that
      !   the nominal azimuth index is increasing with increasing azimuth:
      ...
      !.. Check azimuths against nominal values, taking into account a rounding shift of
      !   the azimuth determined from the mid point of the sweep:
...
!CDIR NODEP
      ...
      IF ( MAXVAL(azinomdiff(1:MIN(znaz,naz))) > 0.15*ABS(az_inc) ) THEN
        WRITE (*,'(a,i5,1x,a)') 'WARNING '//TRIM(yzroutine)//': not all azimuths from NetCDF file' // &
  ...
  SUBROUTINE range_coarsening ( ista, irep, ierr )
    ...
    INTEGER                     :: iobs, i, ii, k, kk, m, mm, n, nn, o, nra, nra_obs, naz, nel, nobs_max, nobs_max_obs, &
                                   nobs_obs(ndatakind), n_aggr, n_aggr_w
    ...
    IF (n_aggr > 1) THEN
      ...
      IF (MOD(n_aggr,2) == 0) THEN
        ...
        ! We need one point more in the work arrays, because both border points contribute to the summation with half of the other weights,
        !  because the borders of the coarse range bins coincide with the centers
        !  of the original range bins:
        n_aggr_w         = n_aggr + 1
        ALLOCATE (w_aggr(n_aggr_w))
        w_aggr(:)        = 1.0_dp
        w_aggr(1)        = 0.5_dp
        w_aggr(n_aggr+1) = 0.5_dp
        ira_offset = n_aggr / 2
      ELSE
        n_aggr_w         = n_aggr
      ...
      ALLOCATE(var_aggr(nobs_max, n_aggr_w))
      ALLOCATE(var_aggr_wgt(nobs_max, n_aggr_w))
      ...
      IF (ASSOCIATED(rs_data(ista)%radwind_obs_q)) THEN
        ! .. If at least one original pixel is good (q=0) within the coarse pixel,
        !     we assign a good quality to the coarse pixel.
        ...
        ! .. Compute aggregated values:
        CALL aggr_var_quali ( var_aggr(:,:), var_sum(:) )       ! Computes var_sum(:) from var_aggr(:,:)
        ! .. Re-define storage vector in rs_data(ista):
...
!!$ for debug output to check aggregation indices below:
!!$ DEBUG:       INTEGER :: intaggr(SIZE(varaggr,dim=1),SIZE(varaggr,dim=2))
...
!!$ DEBUG: intaggr(tmpind,i) = m
          IF (MOD(n_aggr,2) == 0 .AND. i == 1 .AND. mm > 1) THEN
            ...
            varaggr(tmpind, n_aggr_w) = zloc
!!$ DEBUG: intaggr(tmpind,n_aggr_w) = m
          END IF
        END IF
        
      END DO

!!$ DEBUG: DO iobs=1, nobs_max
!!$ DEBUG:   CALL ind2sub3D (iobs, naz, nra, n, mm, o)
!!$ DEBUG:   WRITE (*,'(a,i3)', advance='NO') 'ULI ', mm
!!$ DEBUG:   DO i=1,n_aggr_w-1
!!$ DEBUG:     WRITE (*,'(i5)', advance='NO') intaggr(iobs,i)
!!$ DEBUG:   END DO
!!$ DEBUG:   WRITE (*,'(i5)') intaggr(iobs,n_aggr_w)
!!$ DEBUG: END DO
...
!!$        ic = n_aggr/2+1
!!$      END IF
      ic = n_aggr/2+1  ! This is correct for all n_aggr!
      varmax(:) = missing_obs
    ...
    SUBROUTINE aggr_var_quali ( varaggr, varq )

      REAL(kind=dp), INTENT(in)  :: varaggr(:,:)
      REAL(kind=dp), INTENT(out) :: varq(:)
      INTEGER :: iobs, i

      ! This is for quality flags, assumed 0=good, 1=bad, missing_obs=outside range of the actual elevation
      varq(:) = missing_obs
      DO i = 1, n_aggr_w
        DO iobs = 1, nobs_max
          ! We assign a good value 0.0 if at least one of the n_aggr_w pixels is good.
          ! If all pixels are outside, missing_obs will be kept.
          ! If no pixel is good, but at least one pixel is inside and "bad", we set "bad" value 1.0.
          IF ( varaggr(iobs,i) > miss_threshold) THEN
            IF ( ABS(varaggr(iobs,i)) < 1e-20_dp ) THEN
              varq(iobs) = 0.0_dp
            ELSE IF (ABS(varq(iobs)) >= 1e-20_dp) THEN
              varq(iobs) = 1.0_dp
            END IF
          END IF
        END DO
      END DO
    END SUBROUTINE aggr_var_quali
  ...
  !==============================================================================
  !
  ! Apply the quality flags in array rdata_obs_q on the data in array rdata_obs.
  ! Currently only implemented for vrad and dbzh!
  !
  ! 3D and 1D versions of the module procedure qcrad_flag_eval_dwd
  !
  !==============================================================================

  SUBROUTINE qcrad_flag_eval_dwd_3d(ista, cvarname, rdata_obs_q, rdata_obs)

    CHARACTER (LEN=*), PARAMETER :: yzroutine = 'qcrad_flag_eval_dwd_3d'
    CHARACTER (LEN= * )      , INTENT (IN)  :: &
         cvarname               ! Tag for type of output field: 'vr' or 'z'
    INTEGER, INTENT(IN) :: ista         ! index of radar station
    REAL(KIND=dp), INTENT(IN)   , DIMENSION(:,:,:) :: rdata_obs_q
    REAL(KIND=dp), INTENT(INOUT), DIMENSION(:,:,:) :: rdata_obs
...
!$omp parallel do private(i,j,k)
      DO k=lb(3), ub(3)
        DO j=lb(2), ub(2)
          DO i=lb(1), ub(1)
            IF (ABS(rdata_obs_q(i,j,k)) > eps) THEN
...
!$omp end parallel do
  ...
  END SUBROUTINE qcrad_flag_eval_dwd_3d

  SUBROUTINE qcrad_flag_eval_dwd_1d(ista, cvarname, rdata_obs_q, rdata_obs)

    CHARACTER (LEN=*), PARAMETER :: yzroutine = 'qcrad_flag_eval_dwd_1d'
    CHARACTER (LEN= * )      , INTENT (IN)  :: &
         cvarname               ! Tag for type of output field: 'vr' or 'z'
    INTEGER, INTENT(IN) :: ista         ! index of radar station
    REAL(KIND=dp), INTENT(IN)   , DIMENSION(:) :: rdata_obs_q
...
!$omp parallel do private(i)
      DO i=lb(1), ub(1)
        IF (ABS(rdata_obs_q(i)) > eps) THEN
          rdata_obs(i) = miss_value
        END IF
      END DO
!$omp end parallel do
...
!$omp end parallel do
  ...
  END SUBROUTINE qcrad_flag_eval_dwd_1d
