...
!
  ...
  USE radar_output_utils,  ONLY : get_next_key_from_pos, &
...
CONTAINS
  ...
  SUBROUTINE output3d_ascii_radar_loop(itime, ni, nj, nk, zfield, &
    ...
    CHARACTER (len=cmaxlen) :: zydirradarout
    INTEGER            :: ierr
...
!        CHARACTER(kind=c_char) :: gzError
    ...
    cfieldname(:) = ' '
    cfieldname    = TRIM(fieldname)

#ifndef AUXOUT_OFFLINE
    !   Compare with list elements:
    ...
    IF (volstream%output_subdir(1:1) == '/') THEN
      ...
      zydirradarout = TRIM(volstream%output_subdir)//'/'
    ELSE
      ! Otherwise, it is a subdir to ydirradarout, which has already a '/' at the end::
      IF (LEN_TRIM(volstream%output_subdir) == 0) THEN
        zydirradarout = TRIM(ydirradarout)
      ELSE
        zydirradarout = TRIM(ydirradarout)//TRIM(volstream%output_subdir)//'/'
      END IF
    END IF
...
!!$                               "cdfin_z_id-010908_201307282200_201307282255_precipscan"
      ...
      CALL ostream_create_filename_cg (volstream, rmeta, time_mod, cfieldname, ausdateiname, zacttime, yforrange)
      ausdateiname = TRIM(zydirradarout)//TRIM(ausdateiname)
...
!!$                               "grib2_id-010908_201307282200_201307282255_precipscan"
    ...
    CASE ('ascii','ascii-gzip')
      ...
      CALL ostream_create_filename_ascii (volstream%file_pattern, rmeta, time_mod, cfieldname, ylevtyp, ausdateiname, &
                                          '.dat', zacttime, yforrange)
      ausdateiname = TRIM(zydirradarout)//TRIM(ausdateiname)

    CASE ('f90-binary')

      CALL ostream_create_filename_ascii (volstream%file_pattern, rmeta, time_mod, cfieldname, ylevtyp, ausdateiname, &
                                          '.bin', zacttime, yforrange)
      ausdateiname = TRIM(zydirradarout)//TRIM(ausdateiname)
...
!!$  written inbetween the single output records. This would make it totally compatible with C-style binary files.
      ...
      OPEN(unit=funit, file=TRIM(ausdateiname), status='replace', &
           form='unformatted', iostat=ios)
      IF (ios /= 0) THEN
        CALL abort_run (my_radar_id, 10072, &
             'ERROR: problem in '//TRIM(yzroutine)//': error opening '//TRIM(ausdateiname), &
             TRIM(yzroutine)//', opening output file')
      ENDIF
      ...
      ELSE
        dimen = ACHAR(3)
        WRITE (funit) dimen
        WRITE (funit) nj, ni, nk_out  ! nj=range, ni=azimuth, nk_out=elevation
      END IF
    ...
    CASE ('ascii')
      ...
      WRITE (*,'(A)') TRIM(ausdateiname)
      OPEN(unit=funit, file=TRIM(ausdateiname), status='replace', iostat=ios)
      IF (ios /= 0) THEN
        CALL abort_run (my_radar_id, 10072, &
             'ERROR: problem in '//TRIM(yzroutine)//': error opening '//TRIM(ausdateiname), &
             TRIM(yzroutine)//', opening output file')
...
! compile with -DWITH_ZLIB -lz
    CASE ('ascii-gzip')
      ...
      IF (ldebug_radsim) THEN
        WRITE (*,'(A)') TRIM(ausdateiname)//'.gz'
      END IF

      funit_gz = gzopen(TRIM(ausdateiname)//'.gz'//c_null_char, TRIM(mode_gz)//c_null_char)
      IF (.NOT.C_ASSOCIATED(funit_gz)) THEN
        CALL abort_run (my_radar_id, 10072, &
             'ERROR: problem in '//TRIM(yzroutine)//': error opening '//TRIM(ausdateiname)//'.gz', &
      ...
      IF (gzierror /= 0) THEN
        ...
        CALL abort_run (my_radar_id, 10072, &
             'ERROR: problem in '//TRIM(yzroutine)//': error writing '//TRIM(ausdateiname)//'.gz', &
...
!!$      USE, INTRINSIC :: iso_c_binding, ONLY: c_ptr,c_f_pointer,c_char,c_null_char
  ...
  !=========================================================================
  !
  ! Procedures for creating output file names depending on the file format
  ! and patterns given in the t_voldata_ostream type
  !
  !=========================================================================

  SUBROUTINE ostream_create_filename_cg (volstream, rmeta, time_mod, cfieldname, ofilename, opt_timerange, opt_vvzrange)

    ! .. for cdfin and grib2 files
    ...
    IF (LEN_TRIM(volstream%file_pattern) <= 0) THEN
      ...
      IF (is_singlemom) THEN
        WRITE (ofilename, '(a,4("_",a))') TRIM(formatprefix), TRIM(cfieldname), cstation_id,  &
      ...
      ELSE
        ...
        IF (is_geometry_parameter(TRIM(cfieldname))) THEN
          WRITE (ofilename, '(a,4("_",a))') TRIM(formatprefix), 'allgeom', cstation_id,  &
               content_starttime(1:12), content_endtime(1:12)            
        ELSE IF (INDEX(TRIM(cfieldname),'obs') > 0) THEN
          WRITE (ofilename, '(a,4("_",a))') TRIM(formatprefix), 'allobs', cstation_id,  &
               content_starttime(1:12), content_endtime(1:12)
        ELSE
          WRITE (ofilename, '(a,4("_",a))') TRIM(formatprefix), 'allsim', cstation_id,  &
               content_starttime(1:12), content_endtime(1:12)
        END IF
      END IF
      ...
      CASE ('PRECIP')
        IF (len_TRIM(volstream%pat_scantype_precipscan) > 0) THEN
          scantypesuffix = volstream%pat_scantype_precipscan
        ELSE
          scantypesuffix = TRIM(rmeta%scanname)
        END IF
      CASE default
        IF (len_TRIM(volstream%pat_scantype_volscan) > 0) THEN
          scantypesuffix = volstream%pat_scantype_volscan
        ELSE
          scantypesuffix = TRIM(rmeta%scanname)
        END IF
      END SELECT
    ...
    ELSE
      ...
      IF (is_singletime) THEN
        ...
        DO
          ...
          CASE ('<tmodelini>')
            ! optional:
            CALL replace_substr_with_value (fpat, ku, ko, model_starttime(1:14))
          CASE ('<tmodelini_mm>')
            ! optional:
            CALL replace_substr_with_value (fpat, ku, ko, model_starttime(1:12))
          CASE ('<tact>','<tstart>')
            have_time = .TRUE.
            CALL replace_substr_with_value (fpat, ku, ko, content_starttime(1:14))
          CASE ('<tact_mm>','<tstart_mm>')
            have_time = .TRUE.
            CALL replace_substr_with_value (fpat, ku, ko, content_starttime(1:12))
          CASE ('<tend>')
            have_time = .TRUE.
            CALL replace_substr_with_value (fpat, ku, ko, content_endtime(1:14))
          CASE ('<tend_mm>')
            have_time = .TRUE.
            CALL replace_substr_with_value (fpat, ku, ko, content_endtime(1:12))
      ...
      ELSE
        IF (TRIM(formatprefix) == 'grib2' .OR. TRIM(formatprefix) == 'cdfin') THEN
      ...
      DO
        ...
        CASE ('<scantype>')
          ...
          CASE ('PRECIP')
            ...
            ELSE
              CALL replace_substr_with_value (fpat, ku, ko, TRIM(rmeta%scanname))
            END IF
          CASE default
            IF (len_TRIM(volstream%pat_scantype_volscan) > 0) THEN
        ...
        CASE ('<dom>')
          ! optional:
          CALL replace_substr_with_value (fpat, ku, ko, TRIM(domain_string))
        END SELECT
      END DO
  ...
  END SUBROUTINE ostream_create_filename_cg

  SUBROUTINE ostream_create_filename_ascii (file_pattern, rmeta, time_mod, cfieldname, clevtype, ofilename, &
                                            opt_ext_def, opt_acttime, opt_vvz)

    ! .. for ascii and f90-binary files
    
    CHARACTER(len=*), INTENT(in)         :: file_pattern ! parameter name
    TYPE(radar_meta_type), INTENT(in)    :: rmeta
    REAL(KIND=dp)   , INTENT(in)         :: time_mod     ! time in seconds since model start
    CHARACTER(len=*), INTENT(in)         :: cfieldname   ! parameter name
    CHARACTER(len=*), INTENT(in)         :: clevtype     ! name for type of level
    CHARACTER(len=*), INTENT(out)        :: ofilename    ! output file name
    CHARACTER(len=*), INTENT(out), OPTIONAL :: opt_acttime  ! actual time string for usage
                                                            !  in the calling PROGRAM ("YYYYMMDDhhmmss")
    CHARACTER(len=*), INTENT(out), OPTIONAL :: opt_vvz      ! relative time string for usage
                                                            !  in the calling PROGRAM ("ddhhmmss")
    CHARACTER(len=*), INTENT(in),  OPTIONAL :: opt_ext_def  ! optional default file name extension

    CHARACTER(len=*), PARAMETER :: yzroutine = 'emvorado::ostream_create_filename_ascii'
    ...
    IF (PRESENT(opt_acttime)) THEN
      opt_acttime(:) = ' '
      opt_acttime(:) = model_validtime
    END IF
    IF (PRESENT(opt_vvz)) THEN
      opt_vvz(:) = ' '
      opt_vvz    = ddhhmmss_validtime
    END IF
    
    IF (LEN_TRIM(file_pattern) <= 0) THEN

      !--------------------------------------------------------------------------
      ! Set the default file name pattern without extension:

      ofilename = TRIM(cfieldname)//'_'//TRIM(cstation_id)//'_'//TRIM(rmeta%scanname)//'_'// &
           TRIM(model_starttime)//'_'//TRIM(ddhhmmss_validtime)//'_'//TRIM(clevtype)
      IF (PRESENT(opt_ext_def)) THEN
        IF (len_TRIM(opt_ext_def) > 0) ofilename = TRIM(ofilename)//TRIM(opt_ext_def)
      END IF
    ...
    ELSE
      ...
      DO
        ...
        CASE ('<levtype>')
          ! optional key:
          CALL replace_substr_with_value (fpat, ku, ko, TRIM(clevtype))
        CASE ('<dom>')
          ! optional:
          CALL replace_substr_with_value (fpat, ku, ko, TRIM(domain_string))
        END SELECT
      ...
      IF (.NOT.have_id) THEN
        CALL abort_run (my_radar_id, 29077, &
             'ERROR '//TRIM(yzroutine)//': volstream%file_pattern "'//TRIM(file_pattern)// &
             '" does not contain the mandatory key <stationid>', &
             TRIM(yzroutine)//', input parameter ''volstream%file_pattern''')
      END IF
      ...
      IF (.NOT.have_varname) THEN
        CALL abort_run (my_radar_id, 29079, &
             'ERROR '//TRIM(yzroutine)//': volstream%file_pattern "'//TRIM(file_pattern)// &
             '" does not contain the mandatory key <varname>', &
             TRIM(yzroutine)//', input parameter ''volstream%file_pattern''')
      END IF
      ...
      IF (.NOT.have_scantype) THEN
        CALL abort_run (my_radar_id, 29078, &
             'ERROR '//TRIM(yzroutine)//': volstream%file_pattern "'//TRIM(file_pattern)// &
             '" does not contain the mandatory key <scantype>', &
             TRIM(yzroutine)//', input parameter ''volstream%file_pattern''')
      END IF
  ...
  END SUBROUTINE ostream_create_filename_ascii
