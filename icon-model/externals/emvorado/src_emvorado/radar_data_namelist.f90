...
! Modules used:
  ...
  USE radar_data, ONLY :   &
       ...
       list_domains_for_radar, list_domains_for_model
  ...
  TYPE t_voldata_ostream
    SEQUENCE
    ! .. Format of the volume data output:  'ascii', 'ascii-gzip', 'f90-binary', 'cdfin', 'cdfin-mulmom', 'grib2', 'grib2-mulmom'
    ...
    ! .. Filename pattern for output (all formats).
    !
    !     May consist of the following:
    !
    !     - Any valid characters for filenames
    !
    !     - For grib2- and cdfin formats:
    ...
    !       - <tmodelini_mm> : will result in YYYYMMDDhhmm  (absolute datetime of model start)
    ...
    !       - <tstart_mm>    : will result in YYYYMMDDhhmm  (absolute datetime of start of time window of file content)
    !       - <tend_mm>      : will result in YYYYMMDDhhmm  (absolute datetime of end of time window of file content)
    !       - <tact_mm>      : will result in YYYYMMDDhhmm  (absolute datetime of actual model time)
    ...
    !       - <scantype>     : will result in e.g. 'PPI0080' by default, but can be defined by pat_scantype_XXX below
    !       - <dom>          : will result in 'XX'  (domain index of the hosting model domain) (optional)
    !
    !     - For ascii- and f90-binary formats:
    !       - <stationid>    : will result in e.g. 'id-123456'
    !       - <varname>      : will result in cfieldname
    ...
    !       - <scantype>     : will result in e.g. 'PPI0080' by default, but can be defined by pat_scantype_XXX below
    !       - <levtype>      : will result in name of level type (as specified when calling the routine) (optional)
    !       - <dom>          : will result in 'XX'  (domain index of the hosting model domain) (optional)
    !
    !    Some patterns are mandatory, depending on the file format. If missing, there will be appropriate error messages.
    !    The defaults for the filename patterns are definded radar_namelist_read.f90.
  ...
  ! .. File pattern for ready-files containing the above (optional and/or mandatory) <keys>
  !
  !       - <tmodelini>    : will result in YYYYMMDDhhmmss  (absolute datetime of model start) (optional)
  !       - <tact>         : will result in YYYYMMDDhhmmss  (absolute datetime of actual model time) \
  !       - <tvvzact>      : will result in DDhhmmss  (forecast lead time of actual model time)      / (one of both is mandatory)
  ...
  CHARACTER (len=cmaxlen)    :: fof_file_pattern    ! file pattern containing (optional and mandatory) <keys>
  ! .. List of valid <keys> for fof filename patterns:
  !
  !       - <stationid>    : will result in e.g. 'id-123456'
  !       - <scantype>     : will result in e.g. 'PPI0080' by default, but can be defined by pat_scantype_XXX below
  !       - <tmodelini>    : will result in YYYYMMDDhhmmss  (absolute datetime of model start) (optional)
  ...
  ! .. Namelist parameter to configure rotated lat/lon grid for general
  !    radar composites and special radar composite for bubble generator.
  TYPE(composite_meta_type)  :: comp_meta, comp_meta_bub
  ...
  ! .. List of valid <keys> for this filename pattern:
  !
  !       - <varname>      : will result in 'dbzcmp' or 'dbzcmpbub', depending if the composite is for the bubble generator or not (optional)
  ...
  INTEGER                        :: supob_nrb
  REAL (KIND=wpfwo)              :: supob_azi_maxsector_vr  ! maximal azimuth sector (symmetrical to its center) for v_r superobing [deg]
  REAL (KIND=wpfwo)              :: supob_cart_resolution   ! resolution of the cartesian grid for superobing [m]
  REAL (KIND=wpfwo)              :: supob_ave_width         ! width of averaging area for superobing, base value for all quantities [m]
  REAL (KIND=wpfwo)              :: supob_ave_width_vr      ! width of averaging area for superobing for vr [m], takes priority if set
  REAL (KIND=wpfwo)              :: supob_ave_width_z       ! width of averaging area for superobing for z [m], takes priority if set
  REAL (KIND=wpfwo)              :: supob_minrange          ! min. range of superobing points [m], base value for all quantities
  REAL (KIND=wpfwo)              :: supob_minrange_vr       ! min. range of superobing points for vr [m], takes priority if set
  REAL (KIND=wpfwo)              :: supob_minrange_z        ! min. range of superobing points for z  [m], takes priority if set
  ...
  INTEGER                        :: itype_obserr_vr         ! Mode to specify observation errors for (superob'ed or original) radial wind in feedback files (fdbk%e_o):
                                                            !   = 0: constant observation error (=baseval_obserr_vr)
                                                            !   = 1: e_o is a linear ramp as function of (superob'ed or original) reflectivity (see sketch below)
  ...
  INTEGER                    ::   eleind_for_composite_bub_glob,   &  ! elevation index to be used for compositing for warm bubble generator (comp_dbzsim_bub, comp_dbzobs_bub)
                                  nsmoothpoints_for_comp_bub_glob, &  ! width of symmetric 2D binomial smoother in grid points
  ...
  INTEGER                    ::   eleindlist_for_composite_glob(1:nel_composite_max), &    ! elevation index list to be used for compositing (comp_dbzsim, comp_dbzobs)
                                  ...
                                  nsmoothpoints_for_comp_glob, &  ! width of symmetric 2D binomial smoother in grid points
  ...
  ! Shape parameter mu (ie exponent to D) of rain PSD (in D-space) for rain coexisting with liquid cloud.
  !  Replaces the mu in particle-type variable rain, which is passed down from the host model (or set in
  !  wolkenkonstanten.f90 in case of emvorado-offline) and converted from m- into D-space by init_2mom_types.
  !  Default is "unused_value"; any rain2mom_mu_incloud.le.-1 skips the replacement and keeps the original
  !  value from the host model.
  !  (only effective if itype_gscp>200, ie for 2mom scheme)
  REAL    (KIND=wpfwo)             :: rain2mom_mu_incloud

  ! Limiting wet graupel diameter: mass of larger graupel PSD part is re-distributed to smaller melting particles
  ! in different ways:
  !   Option to choose how to implement the size limitation of melting snow and graupel:
  INTEGER :: itype_Dlim_sgh   ! 0 = no limitation
                              ! 1 = D>Dlim --> Dlim
                              ! 2 = mass(D>Dlim) --> re-DISTRIBUTE to N(D<=Dlim)
  REAL    (KIND=wpfwo) :: Dlim_rain
  REAL    (KIND=wpfwo) :: Dlim_drysnow
  REAL    (KIND=wpfwo) :: Dlim_meltsnow
  REAL    (KIND=wpfwo) :: Dlim_meltgraupel
  REAL    (KIND=wpfwo) :: Dlim_melthail
  ...
  !
    ...
    TYPE(composite_meta_type)  :: comp_meta_bub
    ...
    CHARACTER(LEN=cmaxlen)     :: ready_file_pattern
    CHARACTER(LEN=cmaxlen)     :: fof_file_pattern
    LOGICAL                    :: ldo_bubbles
    ...
    REAL (KIND=wpfwo)          :: supob_ave_width_vr
    REAL (KIND=wpfwo)          :: supob_ave_width_z
...
CONTAINS
  ...
  SUBROUTINE crosscheck_domains_radar_nml ( )
    ...
    INTEGER :: i, j, ii, jj
    LOGICAL :: lneed_diff_odirs, ldom_present, liii, ljjj
    CHARACTER (LEN=cmaxlen) :: zyoutdiri, zyoutdirj
    ...
    !----------------------------------------------------------------------------------------------
    ! check composite output file patterns:

    DO i=1, ndoms_radar
      DO j=i, ndoms_radar
        IF (i /= j) THEN

          lneed_diff_odirs = .FALSE.

          IF ( glob_nml_container(i)%ldo_composite .AND. glob_nml_container(i)%lcomposite_output .AND. &
               glob_nml_container(j)%ldo_composite .AND. glob_nml_container(j)%lcomposite_output ) THEN
            ldom_present = INDEX(glob_nml_container(i)%composite_file_pattern, '<dom>') > 0 .OR. &
                           INDEX(glob_nml_container(j)%composite_file_pattern, '<dom>') > 0 
            IF ( TRIM(glob_nml_container(i)%composite_file_pattern) == &
                 TRIM(glob_nml_container(j)%composite_file_pattern) .AND. &
                 .NOT. ldom_present ) THEN
              lneed_diff_odirs = .TRUE.
            END IF
          END IF
            
          zyoutdiri(:) = ' '
          IF (glob_nml_container(i)%ysubdircomp(1:1) == '/') THEN
            zyoutdiri = TRIM(glob_nml_container(i)%ysubdircomp)
          ELSE
            zyoutdiri = TRIM(glob_nml_container(i)%ydirradarout)//TRIM(glob_nml_container(i)%ysubdircomp)
          END IF
          
          zyoutdirj(:) = ' '
          IF (glob_nml_container(j)%ysubdircomp(1:1) == '/') THEN
            zyoutdirj = TRIM(glob_nml_container(j)%ysubdircomp)
          ELSE
            zyoutdirj = TRIM(glob_nml_container(j)%ydirradarout)//TRIM(glob_nml_container(j)%ysubdircomp)
          END IF
          
          ! .. It is forbidden that EMVORADO writes his composite output for different domains into the same output directory:
          IF (lneed_diff_odirs .AND. TRIM(zyoutdiri) == TRIM(zyoutdirj) ) THEN
            WRITE (*,'(80("="),/,5(a,/),80("="))') 'ERROR crosscheck_domains_radar_nml: ' // &
                 'same composite output directory (ydirradarout/ysubdircomp) ', &
                 TRIM(zyoutdiri), &
                 'chosen for different domains, but <dom> key not contained in composite_file_pattern''s: ', &
                 TRIM(glob_nml_container(i)%composite_file_pattern), &
                 'Either add <dom> keys or choose different ydirradarout/ysubdircomp''s!'
            STOP
          END IF

          lneed_diff_odirs = .FALSE.

          IF ( glob_nml_container(i)%ldo_bubbles .AND. glob_nml_container(i)%lcomposite_output_bub .AND. &
               glob_nml_container(j)%ldo_bubbles .AND. glob_nml_container(j)%lcomposite_output_bub ) THEN
            ldom_present = INDEX(glob_nml_container(i)%composite_file_pattern_bub, '<dom>') > 0 .OR. &
                           INDEX(glob_nml_container(j)%composite_file_pattern_bub, '<dom>') > 0 
            IF ( TRIM(glob_nml_container(i)%composite_file_pattern_bub) == &
                 TRIM(glob_nml_container(j)%composite_file_pattern_bub) .AND. &
                 .NOT. ldom_present ) THEN
              lneed_diff_odirs = .TRUE.
            END IF
          END IF
          ...
          IF (lneed_diff_odirs .AND. TRIM(zyoutdiri) == TRIM(zyoutdirj) ) THEN
            WRITE (*,'(80("="),/,5(a,/),80("="))') 'ERROR crosscheck_domains_radar_nml: ' // &
                 'same composite_bub output directory (ydirradarout/ysubdircomp) ', &
                 TRIM(zyoutdiri), &
                 'chosen for different domains, but <dom> key not contained in composite_file_pattern_bub''s: ', &
                 TRIM(glob_nml_container(i)%composite_file_pattern_bub), &
                 'Either add <dom> keys or choose different ydirradarout/ysubdircomp''s!'
            STOP
          END IF
    ...
    DO i=1, ndoms_radar
      DO j=i, ndoms_radar
        IF (i /= j) THEN
          ...
          IF ( glob_nml_container(i)%lfdbk_output .AND. glob_nml_container(j)%lfdbk_output ) THEN
            ldom_present = INDEX(glob_nml_container(i)%fof_file_pattern, '<dom>') > 0 .OR. &
                           INDEX(glob_nml_container(j)%fof_file_pattern, '<dom>') > 0 
            IF ( TRIM(glob_nml_container(i)%fof_file_pattern) == TRIM(glob_nml_container(j)%fof_file_pattern) .AND. &
                 .NOT. ldom_present ) THEN
              lneed_diff_odirs = .TRUE.
            END IF
          END IF
          ...
          zyoutdiri(:) = ' '
          IF (glob_nml_container(i)%ysubdirfof(1:1) == '/') THEN
            zyoutdiri = TRIM(glob_nml_container(i)%ysubdirfof)
          ELSE
            zyoutdiri = TRIM(glob_nml_container(i)%ydirradarout)//TRIM(glob_nml_container(i)%ysubdirfof)
          END IF
          
          zyoutdirj(:) = ' '
          IF (glob_nml_container(j)%ysubdirfof(1:1) == '/') THEN
            zyoutdirj = TRIM(glob_nml_container(j)%ysubdirfof)
          ELSE
            zyoutdirj = TRIM(glob_nml_container(j)%ydirradarout)//TRIM(glob_nml_container(j)%ysubdirfof)
          END IF
          
           ! .. It is forbidden that EMVORADO writes his fdbk file output for different domains into the same output directory:
          IF (lneed_diff_odirs .AND. TRIM(zyoutdiri) == TRIM(zyoutdirj) ) THEN
            WRITE (*,'(80("="),/,5(a,/),80("="))') 'ERROR crosscheck_domains_radar_nml: ' // &
                 'same fdbk (fof) output directory (ydirradarout/ysubdirfof) ', &
                 TRIM(zyoutdiri), &
                 'chosen for different domains, but <dom> key not contained in fof_file_pattern''s: ', &
                 TRIM(glob_nml_container(i)%fof_file_pattern), &
                 'Either add <dom> keys or choose different ydirradarout/ysubdirfof''s!'
            STOP
          END IF
    ...
    DO i=1, ndoms_radar
      DO j=i, ndoms_radar
        IF (i /= j) THEN
          ...
          IF ( glob_nml_container(i)%lwrite_ready .AND. glob_nml_container(j)%lwrite_ready ) THEN
            ldom_present = INDEX(glob_nml_container(i)%ready_file_pattern, '<dom>') > 0 .OR. &
                           INDEX(glob_nml_container(j)%ready_file_pattern, '<dom>') > 0 
            IF ( TRIM(glob_nml_container(i)%ready_file_pattern) == TRIM(glob_nml_container(j)%ready_file_pattern) .AND. &
                 .NOT. ldom_present ) THEN
              lneed_diff_odirs = .TRUE.
            END IF
          END IF
            
           ! .. It is forbidden that EMVORADO writes his ready file output for different domains into the same output directory:
          IF (lneed_diff_odirs .AND. TRIM(glob_nml_container(i)%ydir_ready_write) == &
                                     TRIM(glob_nml_container(j)%ydir_ready_write) ) THEN
            WRITE (*,'(80("="),/,5(a,/),80("="))') 'ERROR crosscheck_domains_radar_nml:'//&
                 'same ready file output directory (ydir_ready_write) ', &
                 TRIM(glob_nml_container(i)%ydir_ready_write), &
                 'chosen for different domains, but <dom> key not contained in ready_file_pattern''s: ', &
                  TRIM(glob_nml_container(i)%ready_file_pattern), &
                 'Either add <dom> keys or choose different ydir_ready_write''s!'
            STOP
          END IF
    ...
    !----------------------------------------------------------------------------------------------
    ! check voldata file patterns: only if the format is cdfin or grib2, a check is needed.
    !   The ASCII and f90-bin formats already have the <dom> key in their filenames (not settable
    !   via the voldata_ostream%output_subdir)
      ...
      DO j=i, ndoms_radar
        IF (i /= j) THEN
          ...
          DO ii=1, noutstreams_max
            DO jj=ii, noutstreams_max

              liii = ANY( LEN_TRIM(glob_nml_container(i)%voldata_ostream(ii)%output_list(:)) > 0 )
              ljjj = ANY( LEN_TRIM(glob_nml_container(j)%voldata_ostream(jj)%output_list(:)) > 0 )

              lneed_diff_odirs = .FALSE.

              IF (liii .AND. ljjj) THEN
                ldom_present = INDEX(glob_nml_container(i)%voldata_ostream(ii)%file_pattern, '<dom>') > 0 .OR. &
                               INDEX(glob_nml_container(j)%voldata_ostream(jj)%file_pattern, '<dom>') > 0 
                IF ( TRIM(glob_nml_container(i)%voldata_ostream(ii)%file_pattern) == &
                     TRIM(glob_nml_container(j)%voldata_ostream(jj)%file_pattern) .AND. &
                     .NOT. ldom_present ) THEN
                  IF ( .NOT.( TRIM(glob_nml_container(i)%voldata_ostream(ii)%FORMAT) == 'ascii' .AND. &
                              TRIM(glob_nml_container(j)%voldata_ostream(jj)%FORMAT) == 'ascii-gzip') .AND. &
                       .NOT.( TRIM(glob_nml_container(i)%voldata_ostream(ii)%FORMAT) == 'ascii-gzip' .AND. &
                              TRIM(glob_nml_container(j)%voldata_ostream(jj)%FORMAT) == 'ascii') )  THEN
                    lneed_diff_odirs = .TRUE.
                  END IF
                END IF
              END IF

              IF (lneed_diff_odirs) THEN
                
                zyoutdiri(:) = ' '
                IF (glob_nml_container(i)%voldata_ostream(ii)%output_subdir(1:1) == '/') THEN
                  ! If volstream%output_subdir starts with a /, it is interpreted as an absolute path name:
                  zyoutdiri = TRIM(glob_nml_container(i)%voldata_ostream(ii)%output_subdir)//'/'
                ELSE
                  ! Otherwise, it is a subdir to ydirradarout, which has already a '/' at the end::
                  IF (LEN_TRIM(glob_nml_container(i)%voldata_ostream(ii)%output_subdir) == 0) THEN
                    zyoutdiri = TRIM(glob_nml_container(i)%ydirradarout)
                  ELSE
                    zyoutdiri = TRIM(glob_nml_container(i)%ydirradarout)//&
                         TRIM(glob_nml_container(i)%voldata_ostream(ii)%output_subdir)//'/'
                  END IF
                END IF

                zyoutdirj(:) = ' '
                IF (glob_nml_container(j)%voldata_ostream(jj)%output_subdir(1:1) == '/') THEN
                  ! If volstream%output_subdir starts with a /, it is interpreted as an absolute path name:
                  zyoutdirj = TRIM(glob_nml_container(j)%voldata_ostream(jj)%output_subdir)//'/'
                ELSE
                  ! Otherwise, it is a subdir to ydirradarout, which has already a '/' at the end::
                  IF (LEN_TRIM(glob_nml_container(j)%voldata_ostream(jj)%output_subdir) == 0) THEN
                    zyoutdirj = TRIM(glob_nml_container(j)%ydirradarout)
                  ELSE
                    zyoutdirj = TRIM(glob_nml_container(j)%ydirradarout)//&
                         TRIM(glob_nml_container(j)%voldata_ostream(jj)%output_subdir)//'/'
                  END IF
                END IF

                IF (TRIM(zyoutdiri) == TRIM(zyoutdirj) ) THEN
                  WRITE (*,'(80("="),/,2(a,/),2(a,i2),3(a,/),80("="))') 'ERROR crosscheck_domains_radar_nml: ' // &
                       'same voldata output directory ', &
                       TRIM(zyoutdiri), &
                       'chosen for different output streams on domains ', list_domains_for_model(i), &
                       ' and ', list_domains_for_model(j), &
                       ', but <dom> key not contained in these voldata_ostream%file_pattern''s: ', &
                       TRIM(glob_nml_container(i)%voldata_ostream(ii)%file_pattern), &
                       'Either add <dom> keys or choose different ydirradarout/output_subdir''s!'
                  STOP
                END IF
  ...
  !
    ...
    glob_nml%comp_meta                        = comp_meta
    glob_nml%comp_meta_bub                    = comp_meta_bub
    ...
    glob_nml%fof_file_pattern                 = fof_file_pattern
    ...
    glob_nml%labort_if_problems_gribout       = labort_if_problems_gribout
    ...
    glob_nml%supob_ave_width_vr               = supob_ave_width_vr
    glob_nml%supob_ave_width_z                = supob_ave_width_z
  ...
  !
    ...
    comp_meta                           =   glob_nml%comp_meta
    comp_meta_bub                       =   glob_nml%comp_meta_bub
    ...
    fof_file_pattern                    =   glob_nml%fof_file_pattern
    ...
    labort_if_problems_gribout          =   glob_nml%labort_if_problems_gribout
    ...
    supob_ave_width_vr                  =   glob_nml%supob_ave_width_vr
    supob_ave_width_z                   =   glob_nml%supob_ave_width_z
