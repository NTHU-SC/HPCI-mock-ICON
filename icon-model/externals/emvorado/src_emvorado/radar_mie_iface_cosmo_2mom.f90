...
!===============================================================================
  ...
  SUBROUTINE radar_mie_2mom_vec(myproc, lambda_radar, itype_gscp_fwo, &
       ...
       itype_Dlim_sgh, Dlim_rain, Dlim_drysnow, Dlim_meltsnow, Dlim_meltgraupel, Dlim_melthail )
    ...
    INTEGER, INTENT(in), OPTIONAL               :: itype_Dlim_sgh ! Option to choose how to implement the size limitation of dry/melting snow and melting graupel/hail: 
                                                                  ! 0 = no limitation
                                                                  ! 1 = D>Dlim --> Dlim
                                                                  ! 2 = mass(D>Dlim) --> re-DISTRIBUTE to N(D<=Dlim)
    REAL(KIND=dp), INTENT(IN), OPTIONAL         :: Dlim_rain         ! Limiting raindrop diameter: mass of larger DSD part is re-distributed to smaller particles
    REAL(KIND=dp), INTENT(IN), OPTIONAL         :: Dlim_drysnow      ! Limiting dry snow diameter: mass of larger PSD part is re-distributed to smaller particles
    REAL(KIND=dp), INTENT(IN), OPTIONAL         :: Dlim_meltsnow     ! Limiting wet snow diameter: mass of larger PSD part is re-distributed to smaller melting particles
    REAL(KIND=dp), INTENT(IN), OPTIONAL         :: Dlim_meltgraupel  ! Limiting wet graupel diameter: mass of larger PSD part is re-distributed to smaller melting particles
    REAL(KIND=dp), INTENT(IN), OPTIONAL         :: Dlim_melthail  ! Limiting wet hail diameter: mass of larger PSD part is re-distributed to smaller melting particles
    ...
    INTEGER :: i, j, k, ku, ko, ni, nj, nk, iDlim_sgh
    ...
    REAL(KIND=dp) :: zh, ah, &
                     zv, rrhv, irhv, kdp, adp, zvh, Dlim_r, Dlim_ds, Dlim_ms, Dlim_mg, Dlim_mh 
    ...
    CHARACTER(len=500)         :: chydroconfig, chydroconfig_Dlim, ctableconfig, chydroconfig_pMPrain
    CHARACTER(len=3000)        :: hashtext, cbaseconfig, cbaseconfig_gh_0C
    ...
    IF (.NOT.PRESENT(itype_Dlim_sgh)) THEN
      iDlim_sgh = 0
    ELSE
      iDlim_sgh = itype_Dlim_sgh
    END IF
    
    IF (.NOT.PRESENT(Dlim_rain)) THEN
      Dlim_r = 999.0_dp  ! [m], neutral value > Dmax_r
    ELSE
      Dlim_r = Dlim_rain
    END IF
    
    IF (.NOT.PRESENT(Dlim_drysnow)) THEN
      Dlim_ds = 999.0_dp  ! [m], neutral value > Dmax_s
    ELSE
      Dlim_ds = Dlim_drysnow
    END IF
    
    IF (.NOT.PRESENT(Dlim_meltsnow)) THEN
      Dlim_ms = 999.0_dp  ! [m], neutral value > Dmax_s
    ELSE
      Dlim_ms = Dlim_meltsnow
    END IF
    
    IF (.NOT.PRESENT(Dlim_meltgraupel)) THEN
      Dlim_mg = 999.0_dp  ! [m], neutral value > Dmax_g
    ELSE
      Dlim_mg = Dlim_meltgraupel
    END IF
    
    IF (.NOT.PRESENT(Dlim_melthail)) THEN
      Dlim_mh = 999.0_dp  ! [m], neutral value > Dmax_h
    ELSE
      Dlim_mh = Dlim_melthail
    END IF
...
!$omp end parallel
    ...
    IF (llookup) THEN
      ...
      !    dimension info. these are hydrometeor class specific, hence prepared within each
      ...
      IF (ldo_qr) THEN
        ...
        chydroconfig_Dlim(:) = ' '
        IF (PRESENT(Dlim_rain) .AND. iDlim_sgh > 0 .AND. Dlim_r <= Dmax_r) THEN
          WRITE (chydroconfig_Dlim, '(A,i3,es9.2)') &
               TRIM(chydroconfig), iDlim_sgh, Dlim_rain
        ELSE
          chydroconfig_Dlim = TRIM(chydroconfig)
        END IF
        ...
        CALL hash_radar_lookup ( &
             ctableconfig, chydroconfig_Dlim, cbaseconfig, &
        ...
        IF (ilkup_rain < 0) THEN
          ...
          look_Z_rain(nlkup_rain)%chydroconfig       = TRIM(chydroconfig_Dlim)
          ...
          look_Z_rain_muD(nlkup_rain)%chydroconfig   = TRIM(chydroconfig_Dlim)
        ...
        IF (.NOT. look_Z_rain(ilkup_rain)%is_initialized) THEN
          CALL zradar_rain_2mom_lookupcreate(&
               ...
               ray_const%Z_fac,Dmin_r,Dmax_r,Dlim_r,iDlim_sgh,rain,&
               impipar_lookupgen, pe_start, pe_end,&
               linterp_mode_dualpol,&
      ...
      IF (ldo_qi .AND. .NOT.ldo_qi_rayleigh) THEN
        ...
        IF (.NOT. look_Z_meltice(ilkup_meltice)%is_initialized) THEN
          ...
          CALL zradar_meltice_2mom_lookupcreate( &
               ...
               ray_const%Z_fac,Dmin_i,Dmax_i,ice,rain,&
               mixingrulestring_i,matrixstring_i,inclusionstring_i,&
               hoststring_i,hostmatrixstring_i,hostinclusionstring_i,&
               impipar_lookupgen,pe_start, pe_end,&
               linterp_mode_dualpol,&
      ...
      IF (ldo_qs) THEN
        ...
        chydroconfig_Dlim(:) = ' '
        IF (PRESENT(Dlim_drysnow) .AND. iDlim_sgh > 0 .AND. Dlim_ds <= Dmax_s) THEN
          WRITE (chydroconfig_Dlim, '(A,i3,es9.2)') &
               TRIM(chydroconfig), iDlim_sgh, Dlim_drysnow
        ELSE
          chydroconfig_Dlim = TRIM(chydroconfig)
        END IF
        ...
        IF (ilkup_snow < 0) THEN
          ...
          look_Z_snow(nlkup_snow)%chydroconfig    = TRIM(chydroconfig_Dlim)
        ...
        IF (.NOT. look_Z_snow(ilkup_snow)%is_initialized) THEN
          ...
          CALL zradar_snow_2mom_lookupcreate(&
               ...
               ray_const%Z_fac,Dmin_s,Dmax_s,Dlim_ds,iDlim_sgh,snow,DBLE(Tmeltbegin_s),&
               mixingrulestring_s_shell_dry,matrixstring_s_shell_dry,inclusionstring_s_shell_dry,&
               mixingrulestring_s_core_dry,matrixstring_s_core_dry,inclusionstring_s_core_dry,&
               impipar_lookupgen,pe_start,pe_end,&
               linterp_mode_dualpol,&
        ...
        ! melt* tables also depend on rain polMP properties, hence include them
        !   in the melt* hashes in addition to the ice ones
        IF (PRESENT(Dlim_meltsnow) .AND. iDlim_sgh > 0 .AND. Dlim_ms <= Dmax_s) THEN
          ! this IF branch triggers new LUTs only if the Dlim settings have an effect. So existing LUTs without the effect can still be used.
          WRITE (chydroconfig, '(2A,i3,es9.2)') &
               TRIM(chydroconfig), TRIM(chydroconfig_pMPrain), iDlim_sgh, Dlim_meltsnow
        ELSE
          WRITE (chydroconfig, '(2A)') &
               TRIM(chydroconfig), TRIM(chydroconfig_pMPrain)
        END IF
        ...
        IF (.NOT. look_Z_meltsnow(ilkup_meltsnow)%is_initialized) THEN
          ...
          CALL zradar_meltsnow_2mom_lookupcreate(&
               ...
               ray_const%Z_fac,Dmin_s,Dmax_s,Dlim_ms,iDlim_sgh,snow,rain,&
               mixingrulestring_s_shell,matrixstring_s_shell,inclusionstring_s_shell,&
               mixingrulestring_s_core,matrixstring_s_core,inclusionstring_s_core,&
               hoststring_s_shell,hostmatrixstring_s_shell,hostinclusionstring_s_shell,&
               hoststring_s_core,hostmatrixstring_s_core,hostinclusionstring_s_core,&
      ...
      IF (ldo_qg) THEN
        ...
        !   in the melt* hashes in addition to the ice ones
        chydroconfig_Dlim(:) = ' '
        IF (PRESENT(Dlim_meltgraupel) .AND. iDlim_sgh > 0 .AND. Dlim_mg <= Dmax_g) THEN
          ! this IF branch triggers new LUTs only if the Dlim settings have an effect. So existing LUTs without the effect can still be used.
          WRITE (chydroconfig_Dlim, '(2A,i3,es9.2)') &
               TRIM(chydroconfig), TRIM(chydroconfig_pMPrain), iDlim_sgh, Dlim_meltgraupel
        ELSE
          WRITE (chydroconfig_Dlim, '(2A)') &
               TRIM(chydroconfig), TRIM(chydroconfig_pMPrain)
        END IF
        ...
        IF (ilkup_meltgrau < 0) THEN
          ...
          look_Z_meltgraupel(nlkup_meltgrau)%chydroconfig     = TRIM(chydroconfig_Dlim)
        ...
        IF (.NOT. look_Z_meltgraupel(ilkup_meltgrau)%is_initialized) THEN
          ...
          CALL zradar_meltgraupel_2mom_lookupcreate(&
               ...
               ray_const%Z_fac,Dmin_g,Dmax_g,Dlim_mg,iDlim_sgh,graupel,rain,&
               mixingrulestring_g,matrixstring_g,inclusionstring_g, &
               mixingrulestring_g_shell,matrixstring_g_shell,inclusionstring_g_shell,&
               hoststring_g,hostmatrixstring_g,hostinclusionstring_g,&
               impipar_lookupgen,pe_start,pe_end,&
        ...
        IF (ldynamic_wetgrowth_gh .AND. Tmeltbegin_g < T0C_fwo) THEN
          ...
          CALL hash_radar_lookup ( &
               ctableconfig, chydroconfig_Dlim, cbaseconfig_gh_0C, &
          ...
          IF (ilkup_meltgrau_0C < 0) THEN
            ...
            look_Z_meltgraupel_0C(nlkup_meltgrau_0C)%chydroconfig     = TRIM(chydroconfig_Dlim)
      ...
      IF (ldo_qh) THEN
        ...
        !   in the melt* hashes in addition to the ice ones
        chydroconfig_Dlim(:) = ' '
        IF (PRESENT(Dlim_melthail) .AND. iDlim_sgh > 0 .AND. Dlim_mh <= Dmax_h) THEN
          ! this IF branch triggers new LUTs only if the Dlim settings have an effect. So existing LUTs without the effect can still be used.
          WRITE (chydroconfig_Dlim, '(2A,i3,es9.2)') &
               TRIM(chydroconfig), TRIM(chydroconfig_pMPrain), iDlim_sgh, Dlim_melthail
        ELSE
          WRITE (chydroconfig_Dlim, '(2A)') &
               TRIM(chydroconfig), TRIM(chydroconfig_pMPrain)
        END IF
        ...
        IF (ilkup_melthail < 0) THEN
          ...
          look_Z_melthail(nlkup_melthail)%chydroconfig     = TRIM(chydroconfig_Dlim)
        ...
        IF (.NOT. look_Z_melthail(ilkup_melthail)%is_initialized) THEN
          ...
          CALL zradar_melthail_2mom_lookupcreate(&
               ...
               ray_const%Z_fac,Dmin_h,Dmax_h,Dlim_mh,iDlim_sgh,hail,rain,&
               mixingrulestring_h,matrixstring_h,inclusionstring_h,&
               impipar_lookupgen,pe_start,pe_end,&
               linterp_mode_dualpol,&
               ydir_lookup_read,ydir_lookup_write,unit_lookup,hashtext)
        ...
        IF (ldynamic_wetgrowth_gh .AND. Tmeltbegin_h < T0C_fwo) THEN
          ...
          IF (ilkup_melthail_0C < 0) THEN
            ...
            look_Z_melthail_0C(nlkup_melthail_0C)%chydroconfig     = TRIM(chydroconfig_Dlim)
            ...
            IF (ldo_qr) THEN
              ...
              IF (q_r >= q_crit_radar%liquid) THEN
                ...
                CALL zradar_rain_mie_vec(mgd,m_w(i,j,k),lambda_radar_3digits,&
                     ...
                     ray_const%Z_fac,rain,Dmin_r,Dmax_r,Dlim_r,iDlim_sgh,&
              ...
              IF (q_s >= q_crit_radar%frozen) THEN
                ...
                IF (T_a > Tmeltbegin_s) THEN
                  CALL zradar_wetsnow_mie_vec(mgd,T_a,m_i(i,j,k),m_w(i,j,k),&
                       ...
                       ray_const%Z_fac,snow,rain,Dmin_s,Dmax_s,Dlim_ms,iDlim_sgh,&
                       zh,ah,zv,rrhv,irhv,kdp,adp,zvh,&
                       mixingrulestring_s_shell, matrixstring_s_shell, inclusionstring_s_shell,&
                       mixingrulestring_s_core, matrixstring_s_core, inclusionstring_s_core,&
                       hoststring_s_shell, hostmatrixstring_s_shell, hostinclusionstring_s_shell,&
                ...
                ELSE
                  CALL zradar_snow_mie_vec(mgd,m_i(i,j,k),&
                       ...
                       ray_const%Z_fac,snow,Dmin_s,Dmax_s,Dlim_ds,iDlim_sgh,&
                       zh,ah,zv,rrhv,irhv,kdp,adp,zvh,&
                ...
                IF (T_a > Tmeltbegin_g_loc) THEN
                  ...
                  CASE(1)
                    CALL zradar_wetgr_mie_vec(mgd,T_a,m_i(i,j,k),m_w(i,j,k),&
                         ...
                         ray_const%Z_fac,graupel,rain,Dmin_g,Dmax_g,Dlim_mg,iDlim_sgh,&
                         zh,ah,zv,rrhv,irhv,kdp,adp,zvh,&
                         mixingrulestring_g, matrixstring_g, inclusionstring_g,&
                         hoststring_g, hostmatrixstring_g, hostinclusionstring_g,&
              ...
              IF (q_h >= q_crit_radar%frozen) THEN
                ...
                IF (T_a > Tmeltbegin_h_loc) THEN
                  ...
                  CALL zradar_wethail_mie_vec(mgd,T_a,m_i(i,j,k),m_w(i,j,k),&
                       ...
                       ray_const%Z_fac,hail,rain,Dmin_h,Dmax_h,Dlim_mh,iDlim_sgh,&
                       zh,ah,zv,rrhv,irhv,kdp,adp,zvh,&
                       mixingrulestring_h, matrixstring_h, inclusionstring_h,&
...
!!$ qgl is prognostic, this computation can be done.
    ...
    CALL vtradar_normalize(vt_local, z_local, rho, rho_0, &
                           ilow, iup, jlow, jup, ku, ko, vt_radar)
