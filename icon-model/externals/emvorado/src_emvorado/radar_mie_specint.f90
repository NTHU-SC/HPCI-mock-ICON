!NEC$ options "-finline-max-depth=4 -finline-max-function-size=5000"
...
!==========================================================================================
  ...
  SUBROUTINE zradar_rain_mie_vec(mgd,m_w,lambda,&
                                 ...
                                 Zfac,parti,Dmin,Dmax,Dlim,itype_Dlim,&
    ...
    INTEGER, INTENT(in)            :: itype_Dlim
    REAL(KIND=dp), INTENT(in)      :: lambda, Zfac, Dmin, Dmax, Dlim
    COMPLEX(kind=dp)               :: m_w
    ...
    REAL(KIND=dp)      :: dD, &
                          D_w(n_stuetz+1), D_w_in(n_stuetz+1), &
                          itw(n_stuetz+1), itw_x(n_stuetz+1), &
                          aspectratio(n_stuetz+1), sig, m_tot, m_tail
...
!      ENDIF
    ...
    IF (luse_tmatrix) THEN
      aspectratio = aspectratio_singlephase(D_w,n_stuetz+1,'rain',parti,pMP,sig)
...
!        aspectratio = aspectratio_singlephase(D_w,n_stuetz+1,'rain',parti,pMP)
!        CALL calc_orient_singlephase(n_stuetz+1,pMP%sig,arain)
!!      ENDIF
...
!    m_wv = m_w
    ...
    IF (.NOT.lmode_lgen .OR. lambda_last /= lambda .OR. m_w_last /= m_w) THEN
      ...
      IF (luse_tmatrix) THEN
        CALL SPHEROID_SCATTER_TMAT_VEC(MAX(D_w_in,Deps),m_wv,DBLE(m_air),aspectratio,lambda,&
                                       fa,fb,fa0,fb0,n_stuetz+1)
      ELSE
        CALL SPHERE_SCATTER_BH_VEC(MAX(D_w_in,Deps),m_wv,DBLE(m_air),lambda,&
                                   fa,fb,fa0,fb0,n_stuetz+1)
      ENDIF
    ...
    ELSE
      ...
      ! .. Otherwise, overtake the saved scattering amplitudes from the last computation:
      fa  = fa_last 
      fb  = fb_last 
      fa0 = fa0_last
      fb0 = fb0_last
    ...
    SELECT CASE (itype_Dlim)
    CASE (1)
      itw = itw * (MAX(D_w,Dlim)/Dlim)**parti%b_geo
    CASE (2)
      IF (Dlim < D_w(n_stuetz+1)) THEN
        itw_x = itw * parti%a_geo*D_w**parti%b_geo ! compute mass in the size bins
        m_tot = SUM(itw_x)                         ! total mass
        m_tail = SUM(itw_x, mask=(D_w>=Dlim))      ! mass in the tail D_w>Dlim
        WHERE (D_w >= Dlim) 
          itw = 0.0_dp
        ELSEWHERE
          itw = m_tot / MAX(m_tot-m_tail, 1e-20_dp) * itw
        END WHERE
      ENDIF
    END SELECT
...
!      ENDIF
    ...
    IF (luse_tmatrix) THEN
      aspectratio = aspectratio_singlephase(D_i,n_stuetz+1,'ice',parti,pMP,sig)
      IF (.NOT. ldo_nonsphere) aspectratio = 1d0
      CALL calc_orient_singlephase(n_stuetz+1,sig,aice)
    ELSE
      aspectratio = 1d0
  ...
  SUBROUTINE zradar_snow_mie_vec(&
    ...
    INTEGER, INTENT(in)            :: itype_Dlim
    COMPLEX(kind=dp), INTENT(in)   :: m_i
    REAL(KIND=dp), INTENT(in)      :: lambda, Zfac, Rratio, Dmin, Dmax, Dlim
    ...
    COMPLEX(kind=dp)   :: m_iv(n_stuetz+1), &
                          ...
                          fa0(n_stuetz+1), fb0(n_stuetz+1)
    REAL(KIND=dp)      :: dD, &
                          D_s(n_stuetz+1), D_s_in(n_stuetz+1), &
                          itw(n_stuetz+1), itw_x(n_stuetz+1), &
                          aspectratio(n_stuetz+1), sig, m_tot, m_tail
...
!      ENDIF
    ...
    IF (luse_tmatrix) THEN
      aspectratio = aspectratio_singlephase(D_s,n_stuetz+1,'snow',parti,pMP,sig)
      IF (.NOT. ldo_nonsphere) aspectratio = 1d0
      CALL calc_orient_singlephase(n_stuetz+1,sig,asnow)
    ELSE
      aspectratio = 1d0
    ...
    IF (.NOT.lmode_lgen .OR. lambda_last /= lambda .OR. m_i_last /= m_i .OR. &
      ...
      CALL MIE_DRYSNOW_TWOSPH_VEC(&
           D_s_in,parti%a_geo,parti%b_geo,&
    ...
    SELECT CASE (itype_Dlim)
    CASE (1)
      itw = itw * (MAX(D_s,Dlim)/Dlim)**parti%b_geo
    CASE (2)
      IF (Dlim < D_s(n_stuetz+1)) THEN
        itw_x = itw * parti%a_geo*D_s**parti%b_geo ! compute mass in the size bins
        m_tot = SUM(itw_x)                         ! total mass
        m_tail = SUM(itw_x, mask=(D_s>=Dlim))      ! mass in the tail D_s>Dlim
        WHERE (D_s >= Dlim) 
          itw = 0.0_dp
        ELSEWHERE
          itw = m_tot / MAX(m_tot-m_tail, 1e-20_dp) * itw
...
!      ENDIF
    ...
    IF (luse_tmatrix) THEN
      aspectratio = aspectratio_singlephase(D_g,n_stuetz+1,'graupel',parti,pMP,sig)
      IF (.NOT. ldo_nonsphere) aspectratio = 1d0
      CALL calc_orient_singlephase(n_stuetz+1,sig,agraupel)
    ELSE
      aspectratio = 1d0
...
!      ENDIF
    ...
    IF (luse_tmatrix) THEN
      aspectratio = aspectratio_singlephase(D_g,n_stuetz+1,'hail',parti,pMP,sig)
      IF (.NOT. ldo_nonsphere) aspectratio = 1d0
      CALL calc_orient_singlephase(n_stuetz+1,sig,ahail)
    ELSE
      aspectratio = 1d0
  ...
  SUBROUTINE zradar_wetice_mie_vec(&
    ...
    IF (luse_tmatrix) THEN
      ardry = aspectratio_singlephase(D_i,n_stuetz+1,'ice',parti,pMP,sig)
  ...
  SUBROUTINE zradar_wetsnow_mie_vec(&
      ...
      luse_tmatrix,ldo_nonsphere,pMP,pMPr,&
      Zfac,parti,rain,Dmin,Dmax,Dlim,itype_Dlim,&
      zh,ah,zv,rrhv,irhv,kdp,adp,zvh,&
      mixingrulestring_shell,matrixstring_shell,inclusionstring_shell,&
      mixingrulestring_core,matrixstring_core,inclusionstring_core,&
    ...
    TYPE(particle), INTENT(in)     :: parti, rain
    TYPE(t_mgd_params), INTENT(in) :: mgd
    INTEGER, INTENT(in)            :: itype_Dref_fmelt, itype_Dlim
    LOGICAL, INTENT(in)            :: luse_tmatrix, ldo_nonsphere
    TYPE(t_polMP), INTENT(in)      :: pMP, pMPr
    COMPLEX(kind=dp), INTENT(in)   :: m_i, m_w
    REAL(KIND=dp), INTENT(in)      :: T_a, lambda, Rratio, Dmin, Dmax, Dlim, Zfac, &
                                      Tmeltbegin, meltdegTmin,Tmax
    CHARACTER(len=*), INTENT(in)   :: mixingrulestring_core, matrixstring_core, inclusionstring_core,&
                                      mixingrulestring_shell, matrixstring_shell, inclusionstring_shell,&
                                      hoststring_shell, hostmatrixstring_shell, hostinclusionstring_shell,&
    ...
    REAL(KIND=dp)            :: dD, meltratio_outside, &
                                ...
                                D_s(n_stuetz+1), D_s_in(n_stuetz+1), &
                                itw(n_stuetz+1), itw_x(n_stuetz+1), fmelt(n_stuetz+1), &
                                aspectratio(n_stuetz+1), sig, sigr, &
                                awetsnow(7,n_stuetz+1), m_tot, m_tail
    ...
    CASE (1)
      ! Old method with Dref=fct(PSD mean size):
      CALL degree_of_melting_xxx(mgd,T_a,D_s_in,n_stuetz+1,Dexpo_type1,Dref_max,parti,&
                                 Tmeltbegin,meltdegTmin,Tmin_f,Tmax,fmelt)
    CASE (2)
      ! New method with fixed Dref = 1e-6 m instead of Dref from PSD, to save computing time below:
      CALL degree_of_melting_xxx_Dref(T_a,D_s_in,n_stuetz+1,Dexpo_type2,Dref_type2,&
                                      Tmeltbegin,meltdegTmin,Tmin_f,Tmax,fmelt)
    END SELECT
    
    IF (luse_tmatrix) THEN
      ardry = aspectratio_singlephase(D_s_in,n_stuetz+1,'snow',parti,pMP,sig)

      D_r = ((parti%a_geo * D_s_in**parti%b_geo)/rain%a_geo)**(1d0/rain%b_geo)
      arrain = aspectratio_singlephase(D_r,n_stuetz+1,'rain',rain,pMPr,sigr)
    ...
    IF ( .NOT.lmode_lgen .OR. itype_Dref_fmelt /= 2 .OR. lambda_last /= lambda .OR. &
      ...
      CALL MIE_WETSNOW_TWOSPH_VEC(&
           D_s_in,parti%a_geo,parti%b_geo,fmelt,meltratio_outside,m_wv,m_iv,&
  ...
  SUBROUTINE zradar_wetgr_mie_vec(&
    ...
    TYPE(t_polMP), INTENT(in)      :: pMP, pMPr
    COMPLEX(kind=dp), INTENT(in)   :: m_i, m_w
    REAL(KIND=dp), INTENT(in)      :: T_a,lambda, Zfac, Dmin, Dmax, Dlim, &
                                      Tmeltbegin, meltdegTmin,Tmax
    CHARACTER(len=*), INTENT(in)   :: mixingrulestring, matrixstring, inclusionstring,&
                                      hoststring, hostmatrixstring, hostinclusionstring
    LOGICAL, INTENT(in), OPTIONAL  :: llookupgen_mode
    ...
    COMPLEX(kind=dp)         :: m_iv(n_stuetz+1), m_wv(n_stuetz+1), &
                                ...
                                fa0(n_stuetz+1), fb0(n_stuetz+1)
    REAL(KIND=dp)            :: dD, meltratio_outside, &
                                D_r(n_stuetz+1), D_g(n_stuetz+1), D_g_in(n_stuetz+1), &
                                itw(n_stuetz+1), itw_x(n_stuetz+1), fmelt(n_stuetz+1), &
                                aspectratio(n_stuetz+1), sig, sigr, m_tot, m_tail, &
    ...
    CASE (1)
      ! Old method with Dref=fct(PSD mean size):
      CALL degree_of_melting_xxx(mgd,T_a,D_g_in,n_stuetz+1,Dexpo_type1,Dref_max,parti,&
                                 Tmeltbegin,meltdegTmin,Tmin_f,Tmax,fmelt)
    CASE (2)
      ! New method with fixed Dref = 5e-4 m instead of Dref from PSD, to save computing time below:
      CALL degree_of_melting_xxx_Dref(T_a,D_g_in,n_stuetz+1,Dexpo_type2,Dref_type2,&
                                      Tmeltbegin,meltdegTmin,Tmin_f,Tmax,fmelt)
    END SELECT

    IF (luse_tmatrix ) THEN
      ardry = aspectratio_singlephase(D_g_in,n_stuetz+1,'graupel',parti,pMP,sig)

      D_r = ((parti%a_geo * D_g_in**parti%b_geo)/rain%a_geo)**(1d0/rain%b_geo)
      arrain = aspectratio_singlephase(D_r,n_stuetz+1,'rain',rain,pMPr,sigr)
    ...
    IF ( .NOT.lmode_lgen .OR. itype_Dref_fmelt /= 2 .OR. lambda_last /= lambda .OR. &
      ...
      ! MIE_MEAN_WETGR_VEC(), MIE_WETSNOW_TWOSPH_VEC()
      CALL MIE_SOAK_WETGR_VEC(&
           D_g_in,parti%a_geo,parti%b_geo,fmelt,meltratio_outside,&
           m_wv,m_iv,aspectratio,ardry,lambda,&
           fa,fb,fa0,fb0,&
           mixingrulestring,matrixstring,inclusionstring,&
           hoststring,hostmatrixstring,hostinclusionstring,&
    ...
    SELECT CASE (itype_Dlim)
    CASE (1)
      itw = itw * (MAX(D_g,Dlim)/Dlim)**parti%b_geo
    CASE (2)
      IF (Dlim < D_g(n_stuetz+1)) THEN
        itw_x = itw * parti%a_geo*D_g**parti%b_geo ! compute mass in the size bins
        m_tot = SUM(itw_x)                         ! total mass
        m_tail = SUM(itw_x, mask=(D_g>=Dlim))      ! mass in the tail D_g>Dlim
        WHERE (D_g >= Dlim) 
          itw = 0.0_dp
        ELSEWHERE
          itw = m_tot / MAX(m_tot-m_tail, 1e-20_dp) * itw
  ...
  SUBROUTINE zradar_wetgr_twosph_mie_vec(&
    ...
    IF ( .NOT.lmode_lgen .OR. itype_Dref_fmelt /= 2 .OR. lambda_last /= lambda .OR. &
      ...
      CALL MIE_SOAK_TWOSPH_WETGR_BH_VEC(&
           D_g_in,parti%a_geo,parti%b_geo,fmelt,&
           m_wv,m_iv,aspectratio,ardry,lambda,&
           fa,fb,fa0,fb0,&
           mixingrulestring_iceair, matrixstring_iceair, inclusionstring_iceair, &
           mixingrulestring_icewater, matrixstring_icewater, inclusionstring_icewater, &
  ...
  SUBROUTINE zradar_wethail_mie_vec(&
    ...
    IF (luse_tmatrix) THEN
      ardry = aspectratio_singlephase(D_g_in,n_stuetz+1,'hail',parti,pMP,sig)
    ...
    IF ( .NOT.lmode_lgen .OR. itype_Dref_fmelt /= 2 .OR. lambda_last /= lambda .OR. &
      ...
      CALL MIE_SPONGY_WETHAIL_BH_VEC(&
           D_g_in,parti%a_geo,parti%b_geo,fmelt,fwater_shell,&
    ...
    CASE (2)
      IF (Dlim < D_g(n_stuetz+1)) THEN
        itw_x = itw * parti%a_geo*D_g**parti%b_geo ! compute mass in the size bins
        m_tot = SUM(itw_x)                         ! total mass
        m_tail = SUM(itw_x, mask=(D_g>=Dlim))      ! mass in the tail D_s>Dlim
        WHERE (D_g >= Dlim) 
          itw = 0.0_dp
        ELSEWHERE
          itw = m_tot / MAX(m_tot-m_tail, 1e-20_dp) * itw
...
!                ( a(7,:) * real(fb0(:)-fa0(:)) ) )
  ...
  SUBROUTINE zradar_rain_1mom_lookupcreate (   &
       ...
       Zfac,Dmin,Dmax,Dlim,itype_Dlim,rain,&
    ...
    LOGICAL, INTENT(in)                   :: luse_tmatrix, ldo_nonsphere
    TYPE(t_polMP), INTENT(in)             :: pMP
    INTEGER, INTENT(in)                   :: unitnr, itype_Dlim
    REAL(KIND=dp), INTENT(in)             :: lambda_radar, Zfac, Dmin, Dmax, Dlim
          ...
          DO i=1, look_Z_rain%nqi
            ...
            DO kk=1, 3
              ...
              CALL zradar_rain_mie_vec(mgd, m_w(j), lambda_radar,              &
                                       luse_tmatrix, ldo_nonsphere, pMP,       &
                                       Zfac, rain, Dmin, Dmax, Dlim, itype_Dlim, &
                                       zh, ah, zv, rrhv, irhv, kdp, adp, zvh , &
  ...
  SUBROUTINE zradar_snow_1mom_lookupcreate(&
       ...
       lambda_radar,luse_tmatrix,ldo_nonsphere,pMP,&
       Zfac,Dmin,Dmax,Dlim,itype_Dlim,snow,Tmeltbegin,&
       mixingrulestring_shell,matrixstring_shell,inclusionstring_shell,&
       mixingrulestring_core,matrixstring_core,inclusionstring_core,&
       impipar_lookupgen,pe_start,pe_end,&
       linterp_mode_dualpol,&
    ...
    LOGICAL, INTENT(in)                   :: luse_tmatrix, ldo_nonsphere
    TYPE(t_polMP), INTENT(in)             :: pMP
    INTEGER, INTENT(in)                   :: isnow_n0temp, unitnr, itype_Dlim
    REAL(KIND=dp), INTENT(in)             :: Tmeltbegin, lambda_radar, Zfac, Dmin, Dmax, Dlim
    INTEGER, INTENT(in)                   :: impipar_lookupgen, pe_start, pe_end
    LOGICAL, INTENT(in)                   :: linterp_mode_dualpol
          ...
          DO i=1, look_Z_snow%nqi
            ...
            DO kk=1, 3
              ...
              CALL zradar_snow_mie_vec(mgd, m_i(j), 0.5d0, lambda_radar, &
                                       luse_tmatrix, ldo_nonsphere, pMP, &
                                       Zfac, snow, Dmin, Dmax, Dlim, itype_Dlim, &
                                       zh, ah, zv, rrhv, irhv, kdp, adp, zvh, &
                                       mixingrulestring_shell, matrixstring_shell, inclusionstring_shell, &
                                       mixingrulestring_core, matrixstring_core, inclusionstring_core, &
                                       llookupgen_mode=.TRUE.)
  ...
  SUBROUTINE zradar_meltice_1mom_lookupcreate(&
    ...
    TYPE(t_tabledef), INTENT(in)          :: tableprops
    TYPE(particle), INTENT(in)            :: ice, rain
    LOGICAL, INTENT(in)                   :: luse_tmatrix, ldo_nonsphere
    TYPE(t_polMP), INTENT(in)             :: pMP, pMPr
  ...
  SUBROUTINE zradar_meltsnow_1mom_lookupcreate(&
       ...
       Tmeltbegin,meltdegTmin,Tmax_min,Tmax_max,&
       lambda_radar,luse_tmatrix,ldo_nonsphere,pMP,pMPr,&
       Zfac,Dmin,Dmax,Dlim,itype_Dlim,snow,rain,&
       mixingrulestring_shell,matrixstring_shell,inclusionstring_shell,&
       mixingrulestring_core,matrixstring_core,inclusionstring_core,&
       hoststring_shell,hostmatrixstring_shell,hostinclusionstring_shell,&
       hoststring_core,hostmatrixstring_core,hostinclusionstring_core,&
    ...
    TYPE(t_tabledef), INTENT(in)          :: tableprops
    TYPE(particle), INTENT(in)            :: snow, rain
    LOGICAL, INTENT(in)                   :: luse_tmatrix, ldo_nonsphere
    TYPE(t_polMP), INTENT(in)             :: pMP, pMPr
    INTEGER, INTENT(in)                   :: isnow_n0temp, itype_Dref_fmelt, unitnr, itype_Dlim
    REAL(KIND=dp),  INTENT(in)            :: Tmeltbegin, meltdegTmin, Tmax_min, Tmax_max, &
                                             lambda_radar, Zfac, &
                                             Dmin, Dmax, Dlim
    INTEGER, INTENT(in)                   :: impipar_lookupgen, pe_start, pe_end
            ...
            DO i=1, look_Z_meltsnow%nqi
              ...
              DO kk=1, 3
                ...
                CALL zradar_wetsnow_mie_vec(mgd, look_Z_meltsnow%T_a(j),&
                     ...
                     luse_tmatrix, ldo_nonsphere, pMP, pMPr, &
                     Zfac, snow, rain, Dmin, Dmax, Dlim, itype_Dlim, &
                     zh, ah, zv, rrhv, irhv, kdp, adp, zvh, &
                     mixingrulestring_shell, matrixstring_shell, inclusionstring_shell, &
                     mixingrulestring_core, matrixstring_core, inclusionstring_core, &
                     hoststring_shell, hostmatrixstring_shell, hostinclusionstring_shell, &
  ...
  SUBROUTINE zradar_meltgraupel_1mom_lookupcreate(&
       ...
       Tmeltbegin,meltdegTmin,Tmax_min,Tmax_max,&
       lambda_radar,luse_tmatrix,ldo_nonsphere,pMP,pMPr,&
       Zfac,Dmin,Dmax,Dlim,itype_Dlim,graupel,rain,&
       mixingrulestring,matrixstring,inclusionstring,&
       mixingrulestring_shell,matrixstring_shell,inclusionstring_shell,&
       hoststring,hostmatrixstring,hostinclusionstring,&
       impipar_lookupgen,pe_start,pe_end,&
    ...
    TYPE(t_tabledef), INTENT(in)          :: tableprops
    TYPE(particle), INTENT(in)            :: graupel, rain
    LOGICAL, INTENT(in)                   :: luse_tmatrix, ldo_nonsphere
    TYPE(t_polMP), INTENT(in)             :: pMP, pMPr
    INTEGER, INTENT(in)                   :: igraupel_type, itype_Dref_fmelt, unitnr, itype_Dlim
    REAL(KIND=dp), INTENT(in)             :: Tmeltbegin, meltdegTmin, Tmax_min, Tmax_max, &
                                             lambda_radar, Zfac, &
                                             Dmin, Dmax, Dlim
    INTEGER, INTENT(in)                   :: impipar_lookupgen, pe_start, pe_end
              ...
              DO kk=1, 3
                ...
                CASE(1)
                  CALL zradar_wetgr_mie_vec(mgd,look_Z_meltgraupel%T_a(j),&
                                            ...
                                            itype_Dref_fmelt,Tmeltbegin,meltdegTmin,&
                                            lambda_radar,luse_tmatrix,ldo_nonsphere,pMP,pMPr,&
                                            Zfac,graupel,rain,Dmin,Dmax,Dlim,itype_Dlim,&
                                            zh,ah,zv,rrhv,irhv,kdp,adp,zvh,&
                                            mixingrulestring,matrixstring,inclusionstring,&
                                            hoststring,hostmatrixstring,hostinclusionstring,&
  ...
  SUBROUTINE zradar_meltice_2mom_lookupcreate(&
    ...
    CLASS(particle), INTENT(in)           :: ice, rain
    INTEGER, INTENT(in)                   :: impipar_lookupgen, pe_start, pe_end
    LOGICAL, INTENT(in)                   :: linterp_mode_dualpol
    CHARACTER(len=*), INTENT(in)          :: mixingrulestring, matrixstring, inclusionstring, &
                                             hoststring, hostmatrixstring, hostinclusionstring, &
  ...
  SUBROUTINE zradar_meltsnow_2mom_lookupcreate(&
    ...
    LOGICAL, INTENT(in)                   :: luse_tmatrix, ldo_nonsphere
    TYPE(t_polMP), INTENT(in)             :: pMP, pMPr
    INTEGER, INTENT(in)                   :: itype_Dref_fmelt, unitnr, itype_Dlim
    REAL(KIND=dp), INTENT(in)             :: Tmeltbegin, meltdegTmin, Tmax_min, Tmax_max, &
                                             lambda_radar, Zfac, Dmin, Dmax, Dlim
    CLASS(particle), INTENT(in)           :: snow, rain
    CHARACTER(len=*), INTENT(in)          :: mixingrulestring_shell, matrixstring_shell, inclusionstring_shell, &
                                             mixingrulestring_core, matrixstring_core, inclusionstring_core, &
                                               hoststring_shell, hostmatrixstring_shell, hostinclusionstring_shell, &
                                           hoststring_core, hostmatrixstring_core, hostinclusionstring_core
  ...
  SUBROUTINE zradar_meltgraupel_2mom_lookupcreate(&
    ...
    REAL(KIND=dp), INTENT(in)             :: Tmeltbegin, meltdegTmin, Tmax_min, Tmax_max, &
                                             lambda_radar, Zfac, Dmin, Dmax, Dlim
    CLASS(particle), INTENT(in)           :: graupel,rain
    INTEGER, INTENT(in)                   :: impipar_lookupgen, pe_start, pe_end
    LOGICAL, INTENT(in)                   :: linterp_mode_dualpol
    CHARACTER(len=*), INTENT(in)          :: mixingrulestring, matrixstring, inclusionstring, &
                                             mixingrulestring_shell, matrixstring_shell, inclusionstring_shell, &
...
!!$    END IF
  ...
  SUBROUTINE zradar_melthail_2mom_lookupcreate(&
       ...
       Tmeltbegin,meltdegTmin,Tmax_min,Tmax_max,&
       lambda_radar,luse_tmatrix,ldo_nonsphere,pMP,pMPr,&
       Zfac,Dmin,Dmax,Dlim,itype_Dlim,hail,rain,&
       mixingrulestring,matrixstring,inclusionstring,&
       impipar_lookupgen, pe_start, pe_end,    &
       linterp_mode_dualpol,                   &
       savepath_read,savepath_write,unitnr,hashtext)
    ...
    REAL(KIND=dp), INTENT(in)             :: Tmeltbegin, meltdegTmin, Tmax_min, Tmax_max, &
                                             lambda_radar, Zfac, &
                                             Dmin, Dmax, Dlim
    CLASS(particle), INTENT(in)           :: hail, rain
    INTEGER, INTENT(in)                   :: impipar_lookupgen, pe_start, pe_end
    LOGICAL, INTENT(in)                   :: linterp_mode_dualpol
    CHARACTER(len=*), INTENT(in)          :: mixingrulestring, matrixstring, inclusionstring, &
                                             savepath_read, savepath_write, hashtext
            ...
            DO i=1, look_Z_melthail%nqi
              ...
              DO kk=1, 3
                ...
                CALL zradar_wethail_mie_vec(mgd,look_Z_melthail%T_a(j),&
                     ...
                     itype_Dref_fmelt,Tmeltbegin,meltdegTmin,&
                     lambda_radar,luse_tmatrix,ldo_nonsphere,pMP,pMPr,&
                     Zfac,hail,rain,Dmin,Dmax,Dlim,itype_Dlim,&
                     zh,ah,zv,rrhv,irhv,kdp,adp,zvh,&
                     mixingrulestring,matrixstring,inclusionstring,&
