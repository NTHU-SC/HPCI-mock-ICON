...
! Emission Routines
  ...
  PUBLIC  :: art_emission_interface, art_sh_fire_interface
  ...
  IF (lart) THEN
    ...
    IF (art_config(jg)%lart_aerosol .OR. art_config(jg)%lart_chem) THEN
      ...
      IF (art_config(jg)%lart_chem) THEN
        IF (p_art_oem_data%configure%emis_tracer <= 0 .AND. p_art_oem_data%configure%bg_tracer <= 0 .AND. p_art_oem_data%configure%vprm_tracer  <= 0) THEN
          CALL finish('mo_art_emission_interface:art_emission_interface',  &
      ...
      ierror = 0
      IF (oem_config%emis_tracer>0 .OR. oem_config%bg_tracer>0) THEN
        ...
        SELECT CASE(art_config(jg)%iart_fire)
          ...
          CASE(1) ! for both cases: 'biomass_buning' and 'soot'
            ...
            CALL art_emission_biomBurn_prepare(                                               &
              ...
              &          p_art_data(jg)%ext%biomBurn_prop%flux_co(:,jb),                      &
              &          p_art_data(jg)%ext%biomBurn_prop%flux_co2(:,jb),                     &
              &          p_art_data(jg)%ext%biomBurn_prop%flux_h2o(:,jb),                     &
              &          p_art_data(jg)%ext%biomBurn_prop%flux_cr(:,jb),                      &
              &          p_art_data(jg)%ext%biomBurn_prop%frp(:,jb),                          &
              &          p_art_data(jg)%ext%biomBurn_prop%flux_sh(:,jb),                      &
              &          jb, istart, iend, current_date,art_atmo%lon(:,jb) )
          CASE default
            CALL finish('mo_art_emission_interface:art_emission_interface', &
                    ...
                    SELECT CASE(this%name)
                      ...
                      CASE('biomass_burning')
                        ...
                        CALL art_emission_biomBurn(                                               &
                          ...
                          &          dtime, art_config(jg)%rart_qv_fire)
                        CALL this%calc_number_from_mass(emiss_rateM, emiss_rate0, istart, iend, 1,  &
                          &                             nlev)
                        CALL this%distribute_emissions(emiss_rateM, emiss_rate0, tracer(:,:,:,:),   &
                          &                            art_atmo%rho(:,:,jb), dtime, istart, iend, 1,&
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE art_sh_fire_interface(p_patch, prm_diag)
  !! Calculate the sensible heat emission of wildfires
  !! Lisa Muth, KIT (2025-07-28)
  TYPE(t_patch), TARGET, INTENT(IN)  :: &
    &  p_patch          !< Patch on which computation is performes
  TYPE(t_nwp_phy_diag), INTENT(inout):: &
    &  prm_diag         !< atm phys vars
  INTEGER                   :: &
    &  jb, jc, jg,    & !< patch id and loop counters
...
! --- Initialize local variables
  ...
  IF (art_config(jg)%rart_shfl_fire > 0.0_wp) THEN
! --- Get the loop indizes
    DO jb = art_atmo%i_startblk, art_atmo%i_endblk
      CALL art_get_indices_c(jg, jb, istart, iend)

      DO jc = istart, iend
        prm_diag%shfl_s(jc,jb) = prm_diag%shfl_s(jc,jb) - p_art_data(jg)%ext%biomBurn_prop%flux_sh(jc,jb)
      ENDDO !jc
    ENDDO !jb
  ENDIF !shfl_fire
