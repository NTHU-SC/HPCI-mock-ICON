...
MODULE mo_art_init_interface
  ...
  !OEM
  USE mo_art_oem_init,                  ONLY: art_oem_init_restart_field

  USE mo_art_oem_ensemble,              ONLY: art_oem_add_bg_ens
  ...
  CHARACTER(len=*), PARAMETER :: routine = 'mo_art_init_interface'
...
#ifdef _OPENACC
      CALL message(TRIM(routine), 'ART (art_calc_ntracer_and_names): Only OEM and VPRM are currently supported on GPU')
#endif
...
SUBROUTINE art_init_atmo_tracers_nwp(jg, mtime_current, p_nh_state, ext_data, &
  ...
  CHARACTER(len=*), PARAMETER :: routine = 'art_init_atmo_tracers_nwp'
  ...
  IF (lart) THEN
    ...
    IF (art_config(jg)%lart_chem) THEN
      CALL art_oem_init_restart_field(jg,tracer,ierror,yerrmsg)
      IF (ierror /= 0) THEN
        CALL finish (TRIM(routine), 'CALL of art_oem_init_restart_field')
      ENDIF
      IF ((oem_config%l_boundary_scaling) .AND. (.NOT. isRestart())) THEN
        CALL art_oem_add_bg_ens(jg,tracer,ierror,yerrmsg)
        IF (ierror /= 0) THEN
          CALL finish (TRIM(routine), 'CALL of art_oem_add_bg_ens')
        ENDIF
      ENDIF
    ENDIF
