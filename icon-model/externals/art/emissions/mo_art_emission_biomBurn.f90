...
SUBROUTINE art_emission_biomBurn_prepare(lu_frac_crop_irrig,lu_frac_crop_rain,lu_frac_crop_mos,  &
  ...
  &                                      dc_emis_res, flux_co, flux_co2, flux_h2o, flux_cr, frp, &
  &                                      flux_sh, jb, istart, iend, current_date, lon)
!>
...
!<
  ...
  REAL(wp),INTENT(INOUT)  :: &
    ...
    &  flux_h2o(:), flux_sh(:)      !< moistuure and sensible heat flux from fire
  ...
  DO jc = istart, iend
    ...
    IF (ls_time < 1._wp) ls_time =  0.1_wp
    ! local solar time, sun is at the zenith at ls_time=12.0

    ! Calculate moisture emission
    ! combustion moisture: H2O/(CO + CO2) = 0.75 -> Pamer et al., 2008
    ! fuel moisture (dead fuel (30%) with 14.6% moisture & live fuel (70%) with 101.5%
    flux_h2o(jc) = 0.75_wp * (flux_co(jc) + flux_co2(jc)) + 0.72585_wp *  flux_cr(jc)


    ! Calculate sensible heat release from fire using FRP and diurnal cycle
    !   - Assumed radiative fraction (FRP) is 10% of total heat released
    !   - Assumed convective fraction is 55% of total energy
    !   - Applied diurnal cycle function to modulate heat release over time
    flux_sh(jc) = frp(jc) * dc_emis_res(jc,jb,CEILING(ls_time)) * 24._wp  * 5.5_wp
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE art_emission_biomBurn(t,p,u,v,rho,qv,z_mc,z_ifc,lon,area,dz,current_date,  &
  &                              dc_hflux_min_res,dc_hflux_max_res,dc_burnt_area_res, &
  &                              dc_emis_res,flux_bc,flux_h2o,emiss_rate,nlev,nlevp1, &
  &                              jb,istart,iend,dtime,rart_qv_fire)
...
!<
  ...
  REAL(wp),INTENT(IN)       :: &
    ...
    &  flux_h2o(:),              & !< wildfire moisture flux, proportional to FRP (kg/(m2 s)
    &  rart_qv_fire                !< consider moisture release by fire if > 0.0, consieders enhancement factors  
  ...
  DO jc = istart, iend
    ...
    IF ( flux_bc(jc) > 0.005e-9_wp .and. dc_emis_res(jc,jb,13) > 0._wp ) THEN
      ...
      ! Consider moisture release
      IF (rart_qv_fire > 0.0_wp) THEN
        CALL art_calc_wgt(ztop,zbot,nlev,jc,z_ifc(:,:),emiss_rate(:,:),                     &
             &               dc_emis_res(jc,jb,CEILING(ls_time)),flux_h2o(jc),area(jc),dz(:,:) )
      ! qv in kg m-3, emiss_rate in mug m-3 s-1
        qv(jc,:) = qv(jc,:) + emiss_rate(jc,:)*dtime/rho(jc,:)*1e-9_wp*rart_qv_fire
      ENDIF


      CALL art_calc_wgt(ztop,zbot,nlev,jc,z_ifc(:,:),emiss_rate(:,:),                     &
        &               dc_emis_res(jc,jb,CEILING(ls_time)),flux_bc(jc),area(jc),dz(:,:) )
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE art_emission_biomBurn_bbplume(t,p,u,v,rho,qv,z_mc,z_ifc,lon,area,dz,current_date,  &
  &                              dc_hflux_min_res,dc_hflux_max_res,dc_burnt_area_res, &
  &                              dc_emis_res,flux_bc,flux_h2o,emiss_rate,nlev,nlevp1, &
  &                              jb,istart,iend,dtime,rart_qv_fire)
!>
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE art_calc_wgt( ztop,zbot,nlev,jc,z_ifc,emiss_rate,dc_emis_res,flux_bc,area,dz )
...
!<
  ...
  IF ( dc_emis_res > 0._wp ) THEN
    ...
    DO jk=1,nlev
      ...
      emiss_rate(jc,jk) = flux_bc * wgt(jk) * dc_emis_res / dz(jc,jk) * 1.e9_wp * 24._wp
      ! in (mug m-3 s-1)
    ENDDO
