...
MODULE mo_art_oem_init
  ...
  PUBLIC :: art_oem_init, art_oem_init_and_read_gridded_emissions, &
    ...
    &       art_oem_boundary_scaling, &
    &       art_oem_init_restart_field, &
  ...
  ! Constant variable
  INTEGER,  PARAMETER :: ncat_max = 400
...
SUBROUTINE art_oem_init(jg,p_prog_list,ierror,yerrmsg)
  ...
  CHARACTER(:), ALLOCATABLE :: oem_type_string, oem_cat_string, oem_vp_string, &
     &                         oem_tp_string, oem_ftype_string, oem_restart_string, &
     &                         oem_bges_string, bg_ens_string, oem_vprm_bges_flag

  CHARACTER(LEN=20), DIMENSION(:), ALLOCATABLE :: ycatl_temp, yvpl_temp, ytpl_temp, ens_name

  CHARACTER(LEN=20) :: tracer_name

  INTEGER :: n_cat, iemis, ivprm, ic, n, ens_nr, ierr, iv, irs, oem_itscale, ierr_diag, iemis_diag, ncid, & 
     &       dimid, varid, n_lamda_cat, ierr_bgens, ierr_vprm_bgens, bg_ens_tracer_idx
...
!------------------------------------------------------------------------------
  ...
  oem_config%restart_tracer = 0
  oem_config%l_boundary_scaling = .FALSE.
  oem_config%l_restarted = .FALSE.
  oem_config%vprm_tracer = 0
  oem_config%emis_diag_index = 0
  bg_ens_tracer_idx = 0

  l_emis_ensemble = .FALSE.
    ...
    IF (info_dyn%tracer%lis_tracer) THEN
      SELECT TYPE(meta => info_dyn%tracer)
        CLASS IS(t_chem_meta)
          ...
          ! First, count all OEM-tracers with "restart"
          CALL meta%opt_meta%get("oem_restart",oem_restart_string,ierr)
          IF (ierr==0) THEN
            IF (oem_restart_string=="file") THEN
              oem_config%restart_tracer = oem_config%restart_tracer + 1
            ENDIF
          ENDIF
          ...
          IF (ANY(oem_type_string == ["emis", "vprm", "ens "]) ) THEN
            ! Count OEM-tracers for which we want to output emissions
            CALL meta%opt_meta%get("output_emis",oem_config%emis_diag,ierr_diag)
            IF (ierr_diag==0) THEN
              IF (oem_config%emis_diag==1) THEN
                oem_config%emis_diag_index = oem_config%emis_diag_index +1
              ENDIF  
            ENDIF
          ENDIF 
          ...
          SELECT CASE(oem_type_string)
            ...
            CASE("ens")
              ...
              ! Optional: An ensemble member with disturbed background concentrations
              ! => To an emis-tracer with oem_bg_ens=XY the concentration of tracer XY is added at the beginning (and is scaled with the lambdas)
              CALL meta%opt_meta%get("oem_bg_ens",oem_bges_string,ierr_bgens)
              CALL meta%opt_meta%get("oem_vprm_bg_ens",oem_vprm_bges_flag,ierr_vprm_bgens)

              IF (ierr_bgens==0) THEN
                oem_config%l_boundary_scaling = .TRUE.
                bg_ens_tracer_idx = bg_ens_tracer_idx + 1
              ENDIF
  ...
  IF (oem_config%ens_tracer>0) THEN
    ! First, allocate the table where the indices and numbers are stored
    ALLOCATE(oem_ensemble%ens_table(2,oem_config%ens_tracer))
    ALLOCATE(oem_ensemble%ens_name(oem_config%ens_tracer))
    ALLOCATE(oem_ensemble%bg_ens_name(oem_config%ens_tracer))
    ALLOCATE(oem_ensemble%vprm_bg_ens_name(2,oem_config%ens_tracer))
    ALLOCATE(oem_ensemble%vprm_bg_ens(oem_config%ens_tracer))
    oem_ensemble%ens_name = ''
    oem_ensemble%vprm_bg_ens_name = ''
  ENDIF
            ...
            CASE("ens")
              ...
              DO n=1,oem_config%ens_tracer
                IF (oem_ensemble%ens_name(n)=='') THEN
                  ...
                  IF (ierr_bgens==0) THEN
                    oem_ensemble%bg_ens_name(n) = oem_bges_string
                    bg_ens_string = oem_bges_string
                  ENDIF
                  IF (ierr_vprm_bgens==0) THEN 
                    oem_ensemble%vprm_bg_ens(n) = .TRUE.
                    READ(UNIT=oem_vprm_bges_flag, fmt=*) oem_ensemble%vprm_bg_ens_name(:, n)
                    oem_ensemble%vprm_bg_ens_name(1, n) = TRIM(oem_ensemble%vprm_bg_ens_name(1, n))
                    oem_ensemble%vprm_bg_ens_name(2, n) = TRIM(oem_ensemble%vprm_bg_ens_name(2, n))
                  ELSE
                    oem_ensemble%vprm_bg_ens(n) = .FALSE.
                  ENDIF
                  EXIT
                ENDIF
              ENDDO !n=1,oem_config%ens_tracer
          END SELECT !oem_type_string=="emis"/"vprm"/"ens"
      END SELECT !SELECT TYPE(meta => info_dyn%tracer)
    ENDIF !info_dyn%tracer%lis_tracer
  END DO
...
!------------------------------------------------------------------------------
  ...
  IF (bg_ens_tracer_idx>0) THEN
    ALLOCATE(oem_config%bg_idx(1))
    oem_config%bg_idx = 0
    ALLOCATE(oem_config%bg_name(1)) 
    oem_config%bg_name = ''
  ENDIF

  IF (oem_config%restart_tracer>0) THEN
    ALLOCATE(oem_config%restart_idx(oem_config%restart_tracer)); oem_config%restart_idx = 0
    ALLOCATE(oem_config%restart_name(oem_config%restart_tracer)); oem_config%restart_name = ''
  ENDIF
  IF (oem_config%vprm_tracer>0) THEN
    ...
    ALLOCATE(oem_data%newflux_vprm(8))
    oem_data%newflux_vprm = 0.0_wp
  ENDIF

  IF (oem_config%ens_tracer>0) THEN
    ! Open the NetCDF file with ens. regions and try to get indicies of lambda categories
    ierror = nf90_open(ens_reg_nc, nf90_nowrite, ncid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    
    ! Get number of lambda categories
    ierror = nf90_inq_dimid(ncid, 'cat', dimid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF

    ierror = nf90_inquire_dimension(ncid, dimid, len=n_lamda_cat)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF

    ALLOCATE(oem_data%lambda_categories_ids(n_lamda_cat))

    ierror = nf90_inq_varid(ncid, 'Lambda_indicies', varid )
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    ierror = nf90_get_var(ncid, varid, &
       &     oem_data%lambda_categories_ids(:), start = (/1/), count=(/n_lamda_cat/))
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
  ...
  IF (oem_config%emis_diag_index>0) THEN
    ALLOCATE(oem_config%emis_diag_out(oem_config%emis_diag_index)); oem_config%emis_diag_out = 0
  ELSE
    ALLOCATE(oem_config%emis_diag_out(1)); oem_config%emis_diag_out = 0
  ENDIF  
    ...
    IF (info_dyn%tracer%lis_tracer) THEN
      SELECT TYPE(meta => info_dyn%tracer)
        CLASS IS(t_chem_meta)
          ...
          ! Read name and index of "restart"-tracer
          CALL meta%opt_meta%get("oem_restart",oem_restart_string,ierr)
          IF (ierr==0) THEN
            IF (oem_restart_string=="file") THEN
              oem_config%restart_idx(irs) = info%ncontained  ! index in 4D tracer container
              oem_config%restart_name(irs) = TRIM(meta%name)
              irs = irs+1
            ENDIF
          ENDIF

          !Read the name of the BG tracer that we want to perturb
          IF (TRIM(meta%name)==bg_ens_string) THEN
            oem_config%bg_idx(1) = info%ncontained
            oem_config%bg_name(1) = TRIM(meta%name)
          ENDIF
          ...
          IF (ierr==0) THEN
            ...
            IF (oem_type_string=="emis") THEN
              ...
              !Get indicies of OEM-tracers for which we need to output emission values
              ierr_diag = 0         
              CALL meta%opt_meta%get("output_emis",oem_config%emis_diag,ierr_diag)
              IF (ierr_diag==0) THEN
                IF (oem_config%emis_diag==1) THEN 
                  oem_config%emis_diag_out(iemis_diag) = info%ncontained
                  iemis_diag = iemis_diag + 1
                ENDIF
              ENDIF
              ! Check if this emission tracer is an ensemble:
              IF (oem_config%ens_tracer>0) THEN
                IF (ANY(oem_ensemble%ens_name(:)==oem_config%emis_name(iemis))) THEN
                  l_emis_ensemble = .True.
                ENDIF
              ENDIF
              ! Tracer catepories
            ...
            ELSEIF (oem_type_string=="vprm") THEN
              ...
              !Get indices of OEM-VPRM-tracers for which we need to output emission values
              ierr_diag = 0         
              CALL meta%opt_meta%get("output_emis",oem_config%emis_diag,ierr_diag)
              IF (ierr_diag==0) THEN
                IF (oem_config%emis_diag==1) THEN 
...
!------------------------------------------------------------------------------
  ...
  oem_config%l_first_tend_scaled = .TRUE.
  IF (oem_config%l_boundary_scaling) THEN
    oem_config%l_first_tend_scaled = .FALSE.
    CALL art_oem_boundary_scaling(jg, ierror, yerrmsg)
    IF (ierror /= 0) THEN 
      CALL finish (TRIM(routine), yerrmsg)
    ENDIF
    CALL message(routine,'Background scaling by direction initialized')
  ENDIF
...
!==============================================================================
!==============================================================================
!+ Initialize the scaling of the background by direction
!------------------------------------------------------------------------------

  SUBROUTINE art_oem_boundary_scaling(jg, ierror, yerrmsg)

!------------------------------------------------------------------------------
!
! Description:  This subroutine reads in the background scaling factors lambda (for
!   every bg-tracer there are 4 lambdas - for each direction) and distributes them to
!   the boundary grid cells, such that the bg concentrations at the lateral boundaries
!   can be scaled at every latbc time interval (done in mo_art_oem_emission).
!
!-------------------------------------------------------------------------------
    ...
    REAL, DIMENSION(:,:), ALLOCATABLE :: bound_lambdas_3D

    INTEGER, DIMENSION(:), ALLOCATABLE :: global_cell_idx
    INTEGER, DIMENSION(:,:), ALLOCATABLE :: region_mask
    
    INTEGER :: jb, jc, i_startblk, i_endblk, ncid, varid, is, ie, jtracer, dimid, &
      &        nens, nreg, n_bdry_cell, ireg, locidx, &
      &        glbidx, regid

    INTEGER :: bdryidx(1)

    CHARACTER(*), PARAMETER :: routine = "art_oem_boundary_scaling"
...
!------------------------------------------------------------------------------
    ...
    ! Open mask file boundary_regions_nc
    ierror = nf90_open(boundary_regions_nc, NF90_NOWRITE, regid)
    IF (ierror /= nf90_noerr) THEN 
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF

    ! Get number of boundary grid cells
    ierror = nf90_inq_dimid(regid, 'cell', dimid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    ierror = nf90_inquire_dimension(regid, dimid, len=n_bdry_cell)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    ...
    ierror = nf90_inq_dimid(regid, 'reg', dimid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    ierror = nf90_inquire_dimension(regid, dimid, len=nreg)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    ...
    ! Open the file containing the lambdas
    ierror = nf90_open(boundary_lambda_nc, NF90_NOWRITE, ncid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ...
    ierror = nf90_inq_dimid(ncid, 'ens', dimid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    ierror = nf90_inquire_dimension(ncid, dimid, len=nens)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    ...
    ! Allocate variables
    ALLOCATE(region_mask(nreg,p_patch(jg)%n_patch_cells_g)); region_mask=0
    ALLOCATE(global_cell_idx(n_bdry_cell)); global_cell_idx=0
    ALLOCATE(bound_lambdas_3D(nreg, nens)); bound_lambdas_3D = 1.0_wp
    ALLOCATE(oem_data%boundary_lambdas(art_atmo%nproma,art_atmo%nblks,nens)); oem_data%boundary_lambdas = 1.0_wp


    ! Read region mask and global_cell_idx
    ierror = nf90_inq_varid(regid, 'boundaryregion', varid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ierror = nf90_get_var(regid, varid, region_mask(:,1:n_bdry_cell), &
                      start = (/1,1/), count=(/nreg,n_bdry_cell/) )
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ierror = nf90_inq_varid(regid, 'global_cell_idx', varid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ierror = nf90_get_var(regid, varid, global_cell_idx(:), &
                      start = (/1/), count=(/n_bdry_cell/) )
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ...
    IF (ierror /= nf90_noerr) THEN
      ...
      RETURN
    ENDIF
    ierror = nf90_get_var(ncid, varid, bound_lambdas_3D(:,:), &
                      start = (/1,1/), count=(/nreg,nens /) )
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(NF90_strerror(ierror))
      RETURN
    ENDIF
    ...
    ierror = nf90_close(regid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ENDIF
    ! Close the lambda NetCDF file
    ierror = nf90_close(ncid)
    IF (ierror /= nf90_noerr) THEN
      yerrmsg = TRIM(nf90_strerror(ierror))
      RETURN
    ...
    ! Distribute scaling factors to the grid cells according to the mask:
    ! Loop only over boundary cells...
    i_startblk = p_patch(jg)%cells%start_block(1)
    i_endblk   = p_patch(jg)%cells%end_block(min_rlcell)
    DO jb = i_startblk, i_endblk
      ! read indices within this block:
      CALL get_indices_c(p_patch(jg), jb, i_startblk, i_endblk, is, ie, &
        &                1, min_rlcell)
      DO jc = is, ie
        locidx = idx_1d(jc,jb)
        glbidx = p_patch(jg)%cells%decomp_info%glb_index(locidx)
        DO ireg = 1,nreg
          IF (region_mask(ireg,glbidx)==1) THEN
            oem_data%boundary_lambdas(jc,jb,:) = bound_lambdas_3D(ireg,:)
          ENDIF !region_mask(ireg,bdryidx)==1
        ENDDO !ireg=1,nreg
      ENDDO !jc=is,ie
    ENDDO !jb=i_startblk,i_endblk
  ...
  END SUBROUTINE art_oem_boundary_scaling



!==============================================================================
!==============================================================================
!+ Initialize chemical tracer from ICON-output
!------------------------------------------------------------------------------

  SUBROUTINE art_oem_init_restart_field(jg, tracer, ierror, yerrmsg)

!-----------------------------------------------------------------------------
! Description: This subroutine reads the background tracer from the file, 
!              interpolates it vertically using the existing ICON interpolation
!              routines and initializes the background tracer.
!-----------------------------------------------------------------------------
    ...
    REAL(KIND=wp),    DIMENSION(:,:),   ALLOCATABLE :: chem_init_2D

    INTEGER :: nblks_c, nlev, nproma, nt, glbidx, locidx, jc, jb, jk, &
      &        i_startblk, i_endblk, is, ie, trcr_idx, n_glb_cells, ncid, dimid, varid

    CHARACTER(LEN=20) :: trcr_name


    CHARACTER(*), PARAMETER :: routine = "art_oem_init_restart_field"
...
!==============================================================================
    ...
    IF (oem_config%restart_tracer>0) THEN
      ...
      nlev = art_atmo%nlev
      nproma = art_atmo%nproma

      ALLOCATE(oem_data%chem_init_3D(nproma,nlev,nblks_c,oem_config%restart_tracer))
      oem_data%chem_init_3D = 0._wp

      ! open file and read dimensions
      !=============================
      ! read dimensions of file
      ierror = nf90_open(chem_restart_nc, NF90_NOWRITE, ncid)
      IF (ierror /= nf90_noerr) THEN
        yerrmsg = TRIM(nf90_strerror(ierror))
        RETURN
      ENDIF

      ierror = nf90_inq_dimid(ncid, 'ncells', dimid)
      IF (ierror /= nf90_noerr) THEN
        yerrmsg = TRIM(nf90_strerror(ierror))
        RETURN
      ENDIF
      ...
      ! Loop over restart-tracers:
      DO nt=1,oem_config%restart_tracer

        ! get tracer name and idx in ICON-structure
        trcr_name = oem_config%restart_name(nt)

        ALLOCATE(chem_init_2D(n_glb_cells,nlev)); chem_init_2D=0._wp

        ! open file and read chem-fields:
        !=============================
        ierror = nf90_inq_varid(ncid, TRIM(trcr_name), varid)
        IF (ierror /= nf90_noerr) THEN
          yerrmsg = TRIM(nf90_strerror(ierror))
          RETURN
        ENDIF
        ierror = nf90_get_var(ncid, varid, &
          &      chem_init_2D(:,:), start = (/1,1/), count=(/n_glb_cells,nlev/))
        IF (ierror /= nf90_noerr) THEN
          yerrmsg = TRIM(nf90_strerror(ierror))
          RETURN
        ENDIF

        ! distribute values to 2D/3D-field:
        i_startblk = art_atmo%i_startblk
        i_endblk = art_atmo%i_endblk
        DO jb = i_startblk, i_endblk
          CALL art_get_indices_c(jg, jb, is, ie)
          DO jc = is, ie
            ...
            glbidx = p_patch(jg)%cells%decomp_info%glb_index(locidx)
            DO jk=1,nlev
              ! distribute tracer-field
              oem_data%chem_init_3D(jc,jk,jb,nt) = chem_init_2D(glbidx,jk)
            ENDDO !jk = 1,nlev
          ENDDO !jc = is,ie
        ENDDO !jb = startblk,endblk

        DEALLOCATE(chem_init_2D)
      ENDDO
      ...
      ! If rstart_init_time is 0., then add the concentrations to the tracer right here
      IF (restart_init_time==0._wp) THEN

        DO nt=1,oem_config%restart_tracer

          trcr_idx = oem_config%restart_idx(nt)

          DO jb = i_startblk, i_endblk
            CALL art_get_indices_c(jg, jb, is, ie)
            DO jc = is, ie
              tracer(jc,:,jb,trcr_idx) = oem_data%chem_init_3D(jc,:,jb,nt)
            ENDDO !jc = is,ie
          ENDDO !jb = startblk,endblk

        ENDDO
        oem_config%l_restarted = .TRUE.
      ENDIF !restart_init_time==0.

    ENDIF


  END SUBROUTINE art_oem_init_restart_field
...
SUBROUTINE art_oem_init_and_read_vegetation_indices(jg, ierror, yerrmsg)
  ...
  INTEGER :: i, ncid, dimid, n_glb_cells, varid, jc, jb,           &
...
SUBROUTINE art_oem_init_vprm_lu_class_fraction(jg, ext_data, ierror, yerrmsg)
  ...
  INTEGER :: i, jc, jb, i_startblk, i_endblk, is, ie
