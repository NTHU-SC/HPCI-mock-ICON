...
!       2. In the GHGCTL-group, the parameters
!          'ens_reg_nc' (see init_and_read_regional_map for requirements)
!          'ens_lambda_nc' (see init_and_read_lambdas for requirements)
...
! by David Ochsner and adapted for ICON-OEM by Michael Steiner, EMPA
...
IMPLICIT NONE
  ...
  INTEGER, ALLOCATABLE          ::                                  &
    &    reg_map_2D(:)        ! region numbers on 1D-ICON-grid
                              ! DIMS:(ncell)
...
SUBROUTINE art_oem_add_bg_ens(jg, tracer, ierror, yerrmsg)
  !----------------------------------------------------------------------------
  ! Description:
  !   Initialize the ensemble members with the background field.
  !
  !----------------------------------------------------------------------------
  INTEGER, INTENT(in)                :: jg !< patch on which computation is performed
  ...
  ! Local variables
  INTEGER                            ::                                  &
    &   nt, nens, table_nr, bg_idx, bt, jb, i_startblk, i_endblk, jc,    &
    &   is, ie, k, nlev,ens_int_idx

    CHARACTER(*), PARAMETER :: routine="art_oem_add_bg_ens"
  ierror = 0
  yerrmsg = ''

  !-----------------------------------------------------------------------------
  ! Section 1: Loop through the ensemble members and add bg field if necessary
  !-----------------------------------------------------------------------------
  ...
  DO nt = 1,oem_config%emis_tracer
    DO nens = 1,oem_config%ens_tracer
      DO table_nr=1,oem_config%ens_tracer
        IF (oem_ensemble%ens_table(1,table_nr)==nens .AND. oem_ensemble%ens_name(table_nr)==oem_config%emis_name(nt)) THEN
          ...
          ! Get index of BG field
          bg_idx = -1
          DO bt=1,SIZE(oem_config%bg_idx)!oem_config%bg_tracer
            IF (oem_ensemble%bg_ens_name(nens)==oem_config%bg_name(bt)) THEN
              bg_idx = oem_config%bg_idx(bt)
            ENDIF
          ENDDO
          ...
          IF (bg_idx/=-1) THEN
            ...
            DO k = 1,nlev
              tracer(:,k,:,ens_int_idx) = tracer(:,k,:,ens_int_idx) &
              & + tracer(:,k,:,bg_idx) * oem_data%boundary_lambdas(:,:,nens)
            ENDDO
...
END SUBROUTINE art_oem_add_bg_ens
...
SUBROUTINE art_oem_init_lambdas(filepath, ierror, yerrmsg)
  ...
  ierror = nf90_get_var(ncid, varid, oem_data%lambda_mat(:,:,:,:),                  &
    &    start=(/1,1,1,1/), count=(/ntra, ncat, nreg, nensembles/))! , map=(/nparam, 1/))
  IF (ierror /= nf90_noerr) THEN
    yerrmsg = nf90_strerror(ierror)
    RETURN
  ENDIF
