...
!!-------------------------------------------------------------------------
!!
SUBROUTINE art_read_biomBurndata(jg,p_biomBurn_prop, input_folder, current_date)
!<
...
!>
  ...
  CALL art_create_filenames(jg, TRIM(input_folder), IEXT_BCF, biomBurndataset, .TRUE., TRIM(sdatetime(1:10)) )
  ...
  CALL message(TRIM(routine)//':art_read_biomBurndata',                               &
    &          'Reading flux of organic carbon as wildfire locations: '//TRIM(vname))
  CALL art_read(jg, cdi_artdataset_id, cdi_param, TRIM(vname),         &
    &           p_biomBurn_prop%flux_bc)

  vname = 'frpfire'
  CALL message(TRIM(routine)//':art_read_biomBurndata',                               &
    &          'Reading fire radiative power at wildfire locations: '//TRIM(vname))
  CALL art_read(jg, cdi_artdataset_id, cdi_param, TRIM(vname),         &
    &           p_biomBurn_prop%frp)

  vname = 'co2fire'
  CALL message(TRIM(routine)//':art_read_biomBurndata',                               &
    &          'Reading flux of carbon dioxide at wildfire locations: '//TRIM(vname))
  CALL art_read(jg, cdi_artdataset_id, cdi_param, TRIM(vname),         &
    &           p_biomBurn_prop%flux_co2)

  vname = 'crfire'
  CALL message(TRIM(routine)//':art_read_biomBurndata', &
    &          'Reading flux of burned fuel load at wildfire locations: '//TRIM(vname))
  CALL art_read(jg, cdi_artdataset_id, cdi_param, TRIM(vname),         &
    &           p_biomBurn_prop%flux_cr)

  vname = 'cofire'
  CALL message(TRIM(routine)//':art_read_biomBurndata',                               &
    &          'Reading flux of carbon monoxide at wildfire locations: '//TRIM(vname))
  CALL art_read(jg, cdi_artdataset_id, cdi_param, TRIM(vname),         &
    &           p_biomBurn_prop%flux_co)
  CALL art_close_cdi(cdi_artdataset_id, cdi_param)
