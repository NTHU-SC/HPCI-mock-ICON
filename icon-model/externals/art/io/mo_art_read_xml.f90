...
! Local variables
  ...
  INTEGER                                  :: &
    ...
    &  ierr_read                              !< IOSTAT for internal read
  CHARACTER(LEN=IART_XMLTAGLEN)            :: &
    &  trimmed_cval                             !< Temporary storage for trimmed strings
  CHARACTER, POINTER                       :: &
  ...
  IF (nelements>0) THEN
    ...
    DO je = 1, nelements
      ...
      IF (element_name(1:8) == "#comment" .OR. TRIM(element_name) == "source_lifetime") CYCLE
      ...
      IF (TRIM(ADJUSTL(cval))=='real') THEN
        ! Get the raw text of the element first.
        retval = tixiGetTextElement(tixi_file%handle, TRIM(x_path_meta), cval_tixi)
        IF (retval /= SUCCESS) THEN
          CALL finish(TRIM(routine)//':art_read_elements_xml', &
            &       'Could not read text for REAL element '//TRIM(ADJUSTL(element_name))//' of ' &
            &       //TRIM(parent_name)//'.')
        END IF
        WRITE(cval_elem,*) cval_tixi
        trimmed_cval = TRIM(ADJUSTL(cval_elem))
        ! Use Fortran's internal read to convert the text to a real number.
        ! IOSTAT will be non-zero if the string is not a valid real (e.g., "???").
        READ(trimmed_cval, *, IOSTAT=ierr_read) rval_tixi
        IF (ierr_read /= 0) THEN
          CALL finish(TRIM(routine)//':art_read_elements_xml', 'Content "'//trimmed_cval//'" of element ' &
            &       //TRIM(ADJUSTL(element_name))//' in '//TRIM(parent_name)//' is not a valid real number.')
        END IF
      ...
      ELSEIF (TRIM(ADJUSTL(cval))=='int') THEN
        ...
        IF (retval /= SUCCESS) THEN
          CALL finish(TRIM(routine)//':art_read_elements_xml', &
            &       'Could not read text for INTEGER element '//TRIM(ADJUSTL(element_name))//' of ' &
            &       //TRIM(parent_name)//'.')
        END IF
        WRITE(cval_elem,*) cval_tixi
        trimmed_cval = TRIM(ADJUSTL(cval_elem))
        ! Use Fortran's internal read to convert the text to a integer number.
        ! IOSTAT will be non-zero if the string is not a valid integer (e.g., "???").
        READ(trimmed_cval, *, IOSTAT=ierr_read) ival_tixi
        IF (ierr_read /= 0) THEN
          CALL finish(TRIM(routine)//':art_read_elements_xml', 'Content "'//trimmed_cval//'" of element ' &
            &       //TRIM(ADJUSTL(element_name))//' in '//TRIM(parent_name)//' is not a valid integer number.')
        END IF
      ...
      ELSEIF (TRIM(ADJUSTL(cval))=='char') THEN
        ...
        IF (retval /= SUCCESS) THEN
          CALL finish(TRIM(routine)//':art_read_elements_xml', &
    &         'Could not read CHAR value for element '//TRIM(ADJUSTL(element_name))//' of ' &
    &         //TRIM(parent_name)//'.')
        END IF
        WRITE(cval_elem,*) cval_tixi
