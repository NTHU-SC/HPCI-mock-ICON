...
! ART
  ...
  USE mo_art_string_tools,              ONLY: key_value_storage_as_string, get_number_of_digits, print_integer
...
! Local variables
  ...
  WRITE (message_text,'(A31,I7)') 'ART: Number of active sources: ',nsources
  ...
  IF ( iart_radioact == 1 .AND. TRIM(radioactfile) /= '') THEN
    ...
    IF (lexist) THEN
      ...
      WRITE (message_text,'(A47,I7)') 'ART: Number of additional radioactive sources: 9 x ',nsources_radio
      CALL message (TRIM(routine)//':art_emiss_init_pntSrc', message_text)
      p_art_data%pntSrc%nsources = nsources + (nsources_radio * 9) ! each source is active for 9 tracer
    ELSE
      WRITE (message_text,'(A47,I7)') 'ART: WARNING: cart_radioact_file = '//TRIM(radioactfile)//' not found.'
      CALL message (TRIM(routine)//':art_emiss_init_pntSrc', message_text)
    ENDIF
  ENDIF
  ...
  DO jsource = 1, nsources
    ...
    jsource_str = print_integer(jsource, get_number_of_digits(jsource))
