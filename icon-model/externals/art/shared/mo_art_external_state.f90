...
!>
  ...
  !!          ENDIF !iart_volcano
    ...
    IF (artconf%iart_fire == 1) THEN
      ...
      ! Read external data
      CALL art_read_biomBurndata(jg ,art_extdata%biomBurn_prop, TRIM(artconf%cart_input_folder),time_config%tc_current_date)

      ! Add enhancement factor to fire sensible heat release 
      art_extdata%biomBurn_prop%frp = art_extdata%biomBurn_prop%frp * artconf%rart_shfl_fire
    ENDIF !iart_fire
...
#ifdef _OPENACC
      CALL message(TRIM(routine), 'ART (art_external_init): Only OEM and VPRM are currently supported on GPU')
#endif
