...
struct experiment_configuration {
  ...
  {// GEN_MASK
   ...
   {.write_weights = 0,
    .write_vtk = 0,
    .component_names = (char const*[]){"dummy"},
    .num_components = 1,
    .interpolations =
      (enum interp_stack_type[]){INTERP_STACK_CONSERV_DESTAREA,
                                 INTERP_STACK_CONSERV_FRACAREA,
...
int main (int argc, char *argv[]) {
  ...
  */
  ...
  int * cell_core_mask = NULL;
  if (strcmp(comp_name, "dummy"))
  ...
  YAC_VTK_FILE *vtk_file = NULL;
  if (exp_config.write_vtk) {
  ...
  while ((opt = getopt(argc, argv, "e:")) != -1) {
    ...
    switch (opt) {
      ...
      case 'e':
        ...
        else if (!strcmp(optarg, "DUMMY")) *experiment = DUMMY;
