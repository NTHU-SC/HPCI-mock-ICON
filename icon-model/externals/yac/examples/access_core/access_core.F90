...
!> \example access_core.F90
!! Example demonstrating the use for the Fortran interface for the YAC-core.
...
PROGRAM access_core
  ...
  ! location of source/target field
  INTEGER(kind=c_int) :: src_point_location, tgt_point_location

  ! information on duplicated cells
  TYPE(c_ptr) :: tgt_duplicated_point_idx
  TYPE(c_ptr) :: tgt_orig_point_global_id
  INTEGER(kind=c_size_t) :: tgt_nbr_duplicated_points

  INTEGER(kind=c_int), PARAMETER :: &
    on_existing_weight_file = INT(YAC_WEIGHT_FILE_OVERWRITE, c_int)
  ...
  tgt_grid = &
    read_basic_grid( &
      ...
      tgt_point_location, tgt_duplicated_point_idx, tgt_orig_point_global_id, &
      tgt_nbr_duplicated_points)
  ...
  interp_grid =                                                &
    yac_interp_grid_new_c(                                     &
      ...
      (/src_point_location/), (/0_c_size_t/), (/-1_c_size_t/), &
      tgt_point_location, 0_c_size_t, -1_c_size_t)
  ...
  CALL yac_interp_stack_config_add_nnn_c( &
    interp_stack_config, YAC_INTERP_NNN_AVG, INT(4, c_size_t), &
    YAC_INTERP_NNN_MAX_SEARCH_DISTANCE_DEFAULT_F, &
  ...
  CALL yac_duplicate_stencils_c( &
    interp_weights, tgt_grid, tgt_orig_point_global_id, &
    tgt_duplicated_point_idx, tgt_nbr_duplicated_points, &
    tgt_point_location)

  CALL yac_free_c(tgt_orig_point_global_id)
  CALL yac_free_c(tgt_duplicated_point_idx)
  ...
  CALL yac_interp_weights_write_to_file_c( interp_weights, &
    TRIM("weights_" // src_grid_name // "_" // tgt_grid_name // ".nc") // c_null_char,  &
    TRIM(src_grid_name) // c_null_char,                    &
    TRIM(tgt_grid_name) // c_null_char,                    &
    get_global_grid_size(src_grid, src_point_location),    &
    get_global_grid_size(tgt_grid, tgt_point_location),    &
    on_existing_weight_file)
...
CONTAINS
  ...
  FUNCTION read_basic_grid( &
    ...
    grid_name, use_ll, point_location, duplicated_point_idx, &
    orig_point_global_id, nbr_duplicated_points)
    ...
    INTEGER(kind=c_int), INTENT(OUT)   :: point_location
    TYPE(c_ptr), OPTIONAL, INTENT(OUT) :: duplicated_point_idx
    TYPE(c_ptr), OPTIONAL, INTENT(OUT) :: orig_point_global_id
    INTEGER(kind=c_size_t), OPTIONAL, INTENT(OUT) :: nbr_duplicated_points

    TYPE(c_ptr) :: read_basic_grid

    INTEGER(kind=c_size_t) :: coordinates_idx
    TYPE(c_ptr) :: duplicated_point_idx_
    TYPE(c_ptr) :: orig_point_global_id_
    INTEGER(kind=c_size_t) :: nbr_duplicated_points_
...
!AP DO WE NEED A REGULAR CONSTRUCTOR (e.g. for bggd) ?
    ...
    duplicated_point_idx_ = c_null_ptr
    orig_point_global_id_ = c_null_ptr
    nbr_duplicated_points_ = 0

    ! read in grid data from SCRIP formated files
    if (PRESENT(duplicated_point_idx) .AND. PRESENT(orig_point_global_id)) THEN
      read_basic_grid = &
        yac_read_scrip_generic_basic_grid_parallel_c( &
          ...
          duplicated_point_idx_, orig_point_global_id_, &
          nbr_duplicated_points_, point_location)
    ELSE
      read_basic_grid = &
        yac_read_scrip_generic_basic_grid_parallel_c( &
          TRIM(grid_filename) // c_null_char, &
          ...
          INT(0, c_int), TRIM(grid_name) // c_null_char, &
          INT(MERGE(1,0, use_ll), c_int), coordinates_idx, &
          c_null_ptr, c_null_ptr, nbr_duplicated_points_, point_location)
    END IF

    IF (PRESENT(duplicated_point_idx)) &
      duplicated_point_idx = duplicated_point_idx_
    IF (PRESENT(orig_point_global_id)) &
      orig_point_global_id = orig_point_global_id_
    IF (PRESENT(nbr_duplicated_points)) &
      nbr_duplicated_points = nbr_duplicated_points_
  ...
  FUNCTION get_global_grid_size(grid, location)

    USE, INTRINSIC :: iso_c_binding
    USE yac_core

    TYPE(c_ptr), INTENT(IN) :: grid
    INTEGER(kind=c_int), INTENT(IN) :: location
    ...
    local_grid_size(1) = &
        INT(yac_basic_grid_get_data_size_c(grid, location))
    CALL MPI_Allreduce( &
