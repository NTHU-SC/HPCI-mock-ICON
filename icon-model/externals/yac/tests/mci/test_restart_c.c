...
static void utest_generate_icon_grid(
  int comm_rank, int comm_size, char const * grid_dir);
static void utest_generate_cube_grid(int comm_rank, int comm_size);

static void utest_run_model_config(
  ...
  if (point_id == INT_MAX) {
    if (is_icon) utest_generate_icon_grid(local_rank, local_size, grid_dir);
    else         utest_generate_cube_grid(local_rank, local_size);
  }
