...
int main (void) {
  ...
  { // three component, defined on disjoint sets of processes
    ...
    if (rank < 6) utest_check_comm(instance, (size_t[]){0, 1}, 2, 6);

    // comm between comp_1 and comp_3
    if ((rank < 3) || (rank >= 6))
      utest_check_comm(instance, (size_t[]){0, 2}, 2, 6);

    // comm between comp_2 and comp_3
    if (rank >= 3) utest_check_comm(instance, (size_t[]){1, 2}, 2, 6);

    // comm between comp_1, comp_2, and comp_3
    utest_check_comm(instance, (size_t[]){0, 1, 2}, 3, 9);
  ...
  { // two components sharing all processes
    ...
    utest_check_comm(instance, (size_t[]){0, 1}, 2, 9);
  ...
  { // three components one is defined on all processes, the other two are
    ...
    // comm between comp_1 and comp_3
    utest_check_comm(instance, (size_t[]){0, 2}, 2, 9);

    // comm between comp_2 and comp_3
    if ((rank < 3) || (rank >= 6))
      utest_check_comm(instance, (size_t[]){1, 2}, 2, 6);
  ...
  { // three components, one having its one processes, the other two sharing
    ...
    // comm between comp_1 and comp_2
    if (rank < 8) utest_check_comm(instance, (size_t[]){0, 1}, 2, 8);
  ...
  { // two components and a some processes that do not define any YAC instance
    ...
    if ((rank % 3) == 2) {
      ...
      MPI_Comm yac_comm;
      char const * yac_groupname = yac_cget_mpi_handshake_group_name();
      ...
      // comm between comp_1 and comp_2
      utest_check_comm(instance, (size_t[]){0, 1}, 2, 6);
