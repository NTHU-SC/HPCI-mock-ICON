...
void utest_defs(const char* comp_name){
...
void * utest_put_field_loop(void* ptr_field_id_){
  int info = 0, ierror;
  double buffer[9];
...
void utest_compA(){
  ...
  pthread_create( &thread1, NULL, &utest_put_field_loop, (void*) &field_id1);
  pthread_create( &thread2, NULL, &utest_put_field_loop, (void*) &field_id2);

  pthread_join( thread1, NULL);
  pthread_join( thread2, NULL);
}

void * utest_get_field_loop(void* ptr_field_id_){
  int info = 0, ierror;
  double buffer[9];
  int* ptr_field_id = ptr_field_id_;
  for(int i = 0; i<60; ++i){
...
void utest_compB(){
  utest_defs("compB");
  pthread_t thread1, thread2;
  pthread_create( &thread1, NULL, &utest_get_field_loop, (void*) &field_id1);
  pthread_create( &thread2, NULL, &utest_get_field_loop, (void*) &field_id2);
