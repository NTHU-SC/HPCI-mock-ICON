...
typedef int (*func_check_tgt_ptr)(
  double value, int idx, int collection_idx, int with_field_mask,
  func_check_src_ptr utest_check_src);

static int utest_check_src_no_mask(
  double value, int idx, int collection_idx, int with_field_mask);
static int utest_check_src_even_mask(
  double value, int idx, int collection_idx, int with_field_mask);
static int utest_check_src_odd_mask(
  double value, int idx, int collection_idx, int with_field_mask);
static int utest_check_tgt_no_mask(
  double value, int idx, int collection_idx, int with_field_mask,
  func_check_src_ptr utest_check_src);
static int utest_check_tgt_even_mask(
  double value, int idx, int collection_idx, int with_field_mask,
  func_check_src_ptr utest_check_src);
static int utest_check_tgt_odd_mask(
  double value, int idx, int collection_idx, int with_field_mask,
  func_check_src_ptr utest_check_src);

func_check_src_ptr utest_check_src[3] = {
  utest_check_src_even_mask,
  utest_check_src_odd_mask,
  utest_check_src_no_mask};
func_check_tgt_ptr utest_check_tgt[3] = {
  utest_check_tgt_even_mask,
  utest_check_tgt_odd_mask,
  utest_check_tgt_no_mask};
...
int main(int argc, char** argv) {
  ...
  yac_cdef_grid_reg2d(
    ...
    (double[]){0*YAC_RAD,1*YAC_RAD,2*YAC_RAD},
    (double[]){0*YAC_RAD,1*YAC_RAD,2*YAC_RAD}, &grid_id);
  ...
  yac_cdef_points_reg2d(
    ...
    (double[]){0*YAC_RAD,1*YAC_RAD,2*YAC_RAD},
    (double[]){0*YAC_RAD,1*YAC_RAD,2*YAC_RAD}, &point_id);
  ...
  for (int with_field_mask = 0; with_field_mask < 2; ++with_field_mask) {
    for (int source_mask_type = 0; source_mask_type < 3; ++source_mask_type) {
      for (int target_mask_type = 0; target_mask_type < 3; ++target_mask_type) {
        ...
        yac_cget_ext_couple_config_tgt_mask_name(ext_couple_config, &tgt_mask_name);
        ...
        for (int with_field_mask = 0; with_field_mask < 2; ++with_field_mask) {
          for (int source_mask_type = 0; source_mask_type < 3; ++source_mask_type) {
            for (int target_mask_type = 0; target_mask_type < 3; ++target_mask_type) {
              ...
              for (int j = 0; j < COLLECTION_SIZE; ++j)
                for (int k = 0; k < NUM_POINTS; ++k)
                  if (utest_check_tgt[target_mask_type](
                        recv_field[j][k], k, j, with_field_mask,
                        utest_check_src[source_mask_type]))
...
static int utest_check_tgt_even_mask(
  ...
  return
    (idx&1)?
      (value != -1.0):
      utest_check_src(value, idx, collection_idx, with_field_mask);
}
...
static int utest_check_tgt_odd_mask(
  ...
  return
    (idx&1)?
      utest_check_src(value, idx, collection_idx, with_field_mask):(value != -1.0);
}
