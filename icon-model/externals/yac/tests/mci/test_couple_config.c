...
static void utest_check_couple_config(
  struct yac_couple_config * couple_config);
static void utest_check_couple_config_no_delete(
  struct yac_couple_config * couple_config);
static struct yac_couple_config *
  utest_generate_couple_config_from_YAML_parallel(
    char const * config_filename, int parse_flags);
static struct yac_couple_config * utest_generate_couple_config_from_YAML(
  char const * config_filename, int parse_flags);
static void utest_write_couple_config_to_YAML(
  struct yac_couple_config * couple_config, char const * config_filename,
  int emit_flags);
static int utest_compare_string(char const * a, char const * b);
...
int main(int argc, char** argv) {
  ...
  utest_check_couple_config_no_delete(couple_config);
  ...
  // check the emitting of coupling configurations (on rank zero only)
  {
    ...
    for (int i = 0; i < NUM_FORMATS; ++i) {
      ...
      if (rank == 0) {
        utest_write_couple_config_to_YAML(
          couple_config, config_filename, formats[i].emit_flags);

        utest_check_couple_config(
          utest_generate_couple_config_from_YAML(
  ...
  utest_check_couple_config(couple_config);
  ...
  { // reproduces a bug in routine dist_merge
    ...
    yac_couple_config_def_couple(
      ...
      interp_stack_config, 0, 0, NULL,
      YAC_WEIGHT_FILE_ON_EXISTING_DEFAULT_VALUE, 0, 1.0, 0.0, 0,
      NULL, NULL, NULL, 0);
...
static void utest_check_couple_config_no_delete(
  ...
  {
    struct yac_interp_spmap_config * spmap_default_config =
      yac_interp_spmap_config_new(
        0.1 * YAC_RAD, 0.2 * YAC_RAD, YAC_INTERP_SPMAP_DIST, NULL);
    struct {
      struct yac_point_selection * src_point_selection;
      struct yac_interp_spmap_config * spmap_config;
    } spmap_overwrite_config_data[] = {
      {.src_point_selection = // amazon_delta
          yac_point_selection_bnd_circle_new(
            -50.0 * YAC_RAD, 0.0 * YAC_RAD, 4.0 * YAC_RAD),
       .spmap_config =
          yac_interp_spmap_config_new(
            0.3 * YAC_RAD,
      ...
      {.src_point_selection = // north_pole
          yac_point_selection_bnd_circle_new(
            0.0 * YAC_RAD, 90.0 * YAC_RAD, 2.0 * YAC_RAD),
       .spmap_config =
          yac_interp_spmap_config_new(
            ...
            YAC_INTERP_SPMAP_WEIGHTED_DEFAULT,
            YAC_INTERP_SPMAP_SCALE_CONFIG_DEFAULT)},
      {.src_point_selection = // south_pole
          yac_point_selection_bnd_circle_new(
            180.0 * YAC_RAD, -90.0 * YAC_RAD, 2.0 * YAC_RAD),
       .spmap_config =
          yac_interp_spmap_config_new(
            0.5 * YAC_RAD,
            0.6 * YAC_RAD,
            ...
            YAC_INTERP_SPMAP_SCALE_CONFIG_DEFAULT)}};
    enum {
      NUM_SPMAP_OVERWRITE_CONFIGS =
        sizeof(spmap_overwrite_config_data)/
        sizeof(spmap_overwrite_config_data[0])};
    struct yac_spmap_overwrite_config ** spmap_overwrite_configs =
      xmalloc(
        (NUM_SPMAP_OVERWRITE_CONFIGS + 1) * sizeof(*spmap_overwrite_configs));
    for (size_t i = 0; i < NUM_SPMAP_OVERWRITE_CONFIGS; ++i)
      spmap_overwrite_configs[i] =
        yac_spmap_overwrite_config_new(
          spmap_overwrite_config_data[i].src_point_selection,
          spmap_overwrite_config_data[i].spmap_config);
    spmap_overwrite_configs[NUM_SPMAP_OVERWRITE_CONFIGS] = NULL;
    DEF_INTERP_STACK(0,
      ADD_INTERP(nnn,
      ...
      ADD_INTERP(
        user_file, "weights.nc", YAC_INTERP_FILE_ON_MISSING_FILE_DEFAULT,
        YAC_INTERP_FILE_ON_SUCCESS_DEFAULT)
      ADD_INTERP(spmap_ext,
        spmap_default_config, spmap_overwrite_configs)
    ...
    yac_spmap_overwrite_configs_delete(spmap_overwrite_configs);
    yac_interp_spmap_config_delete(spmap_default_config);
  }
  ...
  DEF_INTERP_STACK(3,
    ...
    ADD_INTERP(spmap,
      ...
      2.0, YAC_INTERP_SPMAP_FILENAME_DEFAULT,
      YAC_INTERP_SPMAP_VARNAME_DEFAULT, YAC_INTERP_SPMAP_MIN_GLOBAL_ID_DEFAULT)
  ...
  DEF_INTERP_STACK(5,
    ...
    ADD_INTERP(user_callback, "compute_weights")
    ADD_INTERP(
      user_file, "weights.nc", YAC_INTERP_FILE_MISSING_CONT,
  ...
  if (utest_compare_string(start_datetime, ref_start_datetime))
    PUT_ERR("ERROR in yac_couple_config_get_start_datetime\n");
  free(start_datetime);

  char * end_datetime =
    yac_couple_config_get_end_datetime(couple_config);
  if (utest_compare_string(end_datetime, ref_end_datetime))
  ...
  for (size_t i = 0; i < REF_NUM_COUPLES; ++i) {
    ...
    for (size_t j = 0; (j < REF_NUM_COUPLES) && (couple_idx == SIZE_MAX); ++j) {
      ...
      if ((!utest_compare_string(couple_comp_names[0], ref_comp_names[0]) &&
           !utest_compare_string(couple_comp_names[1], ref_comp_names[1])) ||
          (!utest_compare_string(couple_comp_names[0], ref_comp_names[1]) &&
           !utest_compare_string(couple_comp_names[1], ref_comp_names[0])))
        couple_idx = j;
    }
  ...
  for (size_t i = 0; i < REF_NUM_FIELD_COUPLES; ++i) {
    ...
    if (yac_couple_config_get_weight_file_on_existing(
          couple_config, couple_idx, field_couple_idx) !=
        ref_field_couple_data[i].weight_file_on_existing)
      PUT_ERR("ERROR in yac_couple_config_get_weight_file_on_existing");
    ...
    if (yac_couple_config_get_use_raw_exchange(
          couple_config, couple_idx, field_couple_idx) !=
        ref_field_couple_data[i].use_raw_exchange)
      PUT_ERR("ERROR in yac_couple_config_get_use_raw_exchange");

    if (utest_compare_string("multi_grid_field", tgt_field_name)) {
      ...
      if (utest_compare_string(src_comp_name, src_comp_name_) ||
          utest_compare_string(src_grid_name, src_grid_name_) ||
          utest_compare_string(src_field_name, src_field_name_))
...
static struct yac_couple_config *
  ...
  if (rank == 0) {
    ...
    yac_couple_config_def_couple(
      ...
      interp_stack, 0, 0, NULL, YAC_WEIGHT_FILE_ON_EXISTING_DEFAULT_VALUE,
      0, 9.0/5.0, 32.0, 2, (char const *[]){"src_mask1", "src_mask2"},
      "tgt_mask", "irecv_isend", 1);
  ...
  if (rank == 1)
    yac_couple_config_set_missing_definition_is_fatal(couple_config, 1);
