...
static void utest_run_comp_a(int with_field_mask, int exchange_type);
static void utest_run_comp_b(int with_field_mask, int exchange_type);
  ...
  for (int exchange_type = 0; exchange_type < 2; ++exchange_type) {
    for (int with_field_mask = 0; with_field_mask < 2; ++with_field_mask) {
      switch(rank) {
        ...
        case(1):
          utest_run_comp_a(with_field_mask, exchange_type);
          break;
        case(2):
          utest_run_comp_b(with_field_mask, exchange_type);
          break;
        default:
