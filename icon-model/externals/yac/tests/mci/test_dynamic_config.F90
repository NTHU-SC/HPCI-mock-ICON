...
PROGRAM main
  ...
  INTEGER, PARAMETER :: n_cells(2) = (/32, 16/)
  INTEGER, PARAMETER :: n_corners(2) = (/33, 17/)
...
CONTAINS
  ...
  SUBROUTINE comp1_main()
    ...
    CALL yac_fset_config_output_file( &
      ...
      YAC_CONFIG_OUTPUT_SYNC_LOC_ENDDEF, include_definitions=.FALSE.)
  ...
  SUBROUTINE comp2_main()
    IMPLICIT NONE
    INTEGER :: yac_id, avg_interp, i, field_in(12)
    ...
    field_in(4) = gen_field_rbf(yac_id)
    field_in(5) = gen_field_conserv(yac_id)
    field_in(6) = gen_field_spmap_yac(yac_id)
    field_in(7) = gen_field_spmap_file(yac_id)
    field_in(8) = gen_field_spmap_ext(yac_id)
    field_in(9) = gen_field_hcsbb(yac_id)
    field_in(10) = gen_field_user_file(yac_id)
    field_in(11) = gen_field_fixed(yac_id)
    field_in(12) = gen_field_creep(yac_id)
  ...
  FUNCTION gen_field_rbf(yac_id)
    INTEGER, INTENT(IN) :: yac_id
    INTEGER :: gen_field_rbf
    INTEGER :: interp_stack
    CALL yac_fget_interp_stack_config(interp_stack)
    CALL yac_fadd_interp_stack_config_rbf(interp_stack)
    gen_field_rbf = gen_field(yac_id, "rbf", interp_stack)
  END FUNCTION gen_field_rbf
  ...
  FUNCTION gen_field_spmap_yac(yac_id)
    INTEGER, INTENT(IN) :: yac_id
    INTEGER :: gen_field_spmap_yac
    INTEGER :: interp_stack
    CALL yac_fget_interp_stack_config(interp_stack)
    CALL yac_fadd_interp_stack_config_spmap( &
      interp_stack, 0.0d0, 0.0d0, YAC_SPMAP_AVG, YAC_SPMAP_NONE, &
      1.0d0, "" // c_null_char, "" // c_null_char, 0, &
      1.0d0, "" // c_null_char, "" // c_null_char, 0)
    gen_field_spmap_yac = gen_field(yac_id, "spmap_yac", interp_stack)
  END FUNCTION gen_field_spmap_yac

  FUNCTION gen_field_spmap_file(yac_id)
    INTEGER, INTENT(IN) :: yac_id
    INTEGER :: gen_field_spmap_file
    INTEGER :: interp_stack
    CALL yac_fget_interp_stack_config(interp_stack)
    CALL yac_fadd_interp_stack_config_spmap( &
      interp_stack, 0.0d0, 0.0d0, YAC_SPMAP_AVG, YAC_SPMAP_NONE, &
      0.0d0, "area.nc" // c_null_char, "src_cell_area" // c_null_char, 0, &
      0.0d0, "area.nc" // c_null_char, "tgt_cell_area" // c_null_char, 0)
    gen_field_spmap_file = gen_field(yac_id, "spmap_file", interp_stack)
  END FUNCTION gen_field_spmap_file

  FUNCTION gen_field_spmap_ext(yac_id)
    INTEGER, INTENT(IN) :: yac_id
    INTEGER :: gen_field_spmap_ext
    INTEGER :: interp_stack
    INTEGER :: spmap_overwrite_config_ids(2)
    CALL yac_fget_spmap_overwrite_config_bnd_circle ( &
      0.0d0, 0.0d0, 1.0d0, spmap_overwrite_config_ids(1), &
      spread_distance = 0.5d0)
    CALL yac_fget_spmap_overwrite_config_bnd_circle ( &
      0.0d0, 1.5d0, 1.0d0, spmap_overwrite_config_ids(2), &
      max_search_distance = 0.6d0, weight_type = YAC_SPMAP_DIST)
    CALL yac_fget_interp_stack_config(interp_stack)
    CALL yac_fadd_interp_stack_config_spmap( &
      interp_stack, 0.0d0, 0.0d0, YAC_SPMAP_AVG, YAC_SPMAP_NONE, &
      1.0d0, "" // c_null_char, "" // c_null_char, 0, &
      1.0d0, "" // c_null_char, "" // c_null_char, 0, &
      spmap_overwrite_config_ids = spmap_overwrite_config_ids)
    gen_field_spmap_ext = gen_field(yac_id, "spmap_ext", interp_stack)
    CALL yac_ffree_spmap_overwrite_config( &
      spmap_overwrite_config_ids(2))
    CALL yac_ffree_spmap_overwrite_config( &
      spmap_overwrite_config_ids(1))
  END FUNCTION gen_field_spmap_ext
