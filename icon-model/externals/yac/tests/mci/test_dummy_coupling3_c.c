...
static struct intra_comp_yac_info utest_setup_intra_comp_yac(
  MPI_Comm comp_comm, char const * comp_name,
  int base_point_id, int regional_point_id, char const * config_dir);

static struct comp_pair_yac_info utest_setup_comp_pair_yac(
  char const * comp_pair_names[2], int point_ids[2], int nbr_cells[2],
  int local_comp_indices[2], char const * config_dir);

static struct grid_data utest_generate_icon_grid_data(
  MPI_Comm comm, char const * grid_dir);
static struct grid_data utest_generate_cube_grid_data(MPI_Comm comm);
static struct grid_data utest_generate_mpiom_grid_data(
  MPI_Comm comm, char const * grid_dir);
static void utest_grid_data_free(struct grid_data grid_data);

static void utest_def_default_fields(
...
int main(int argc, char** argv) {
    ...
    for (int comp_idx = 0; comp_idx < num_local_comps; ++comp_idx) {
      switch(local_comp_names[comp_idx][5]) {
        case('a'): {
          ...
          utest_generate_icon_grid_data(default_comp_comms[comp_idx], argv[2]);
          break;
        }
        case('b'): {
          comp_grid_data[comp_idx] =
            utest_generate_cube_grid_data(default_comp_comms[comp_idx]);
          break;
        }
        case('c'): {
          comp_grid_data[comp_idx] =
            utest_generate_mpiom_grid_data(default_comp_comms[comp_idx], argv[3]);
          break;
        }
        default:
          exit(EXIT_FAILURE);
  ...
  if (!is_dummy) {
    ...
    for (int comp_idx = 0; comp_idx < num_local_comps; ++comp_idx) {
      ...
      utest_grid_data_free(comp_grid_data[comp_idx]);
    }
