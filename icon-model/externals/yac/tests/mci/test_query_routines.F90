...
PROGRAM test_query_routines
  ...
  DOUBLE PRECISION, PARAMETER :: YAC_RAD = 0.017453292519943295769d0 ! M_PI / 180
  ...
  ELSE
    ...
    IF (rank_type > 1) THEN
      ...
      CALL yac_fdef_grid( &
        grid_name, [2, 2], [0,0], [0.,180.]*YAC_RAD, &
        [-45.,45.]*YAC_RAD, grid_id)

      CALL yac_fdef_points( &
        grid_id, [1,1], YAC_LOCATION_CELL, &
        [90.]*YAC_RAD, [0.]*YAC_RAD, point_id)
      ...
      IF (rank_type > 2) THEN
        ...
        IF (rank_type > 3) THEN
          ...
          !----------------------------------------
          CALL test(yac_fget_field_is_defined(comp_name, grid_name, field_name))
          CALL test(.NOT. yac_fget_field_is_defined(comp_name, grid_name, "undefined_field"))
          CALL test(yac_fget_field_is_defined(yac_id, comp_name, grid_name, field_name))
          CALL test(.NOT. yac_fget_field_is_defined(yac_id, comp_name, grid_name, "undefined_field"))
