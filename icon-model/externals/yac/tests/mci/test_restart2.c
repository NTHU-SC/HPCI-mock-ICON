...
static void utest_run_setup(char local_comp_id, char comp_groups[2][2]);
...
int main (void) {
  ...
  utest_run_setup(local_comp_id, (char[2][2]){{'A','B'},{'C','D'}});

  // couple A-C and B-D (each in their own world comm)
  utest_run_setup(local_comp_id, (char[2][2]){{'A','C'},{'B','D'}});

  // couple A-D and B-C (each in their own world comm)
  utest_run_setup(local_comp_id, (char[2][2]){{'A','D'},{'B','C'}});
