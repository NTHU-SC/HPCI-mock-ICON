...
int main(void) {
  ...
  { // cell containing zero length edge
    double icon_cell_coords[2][4] = {{0,120,240,240}, {85,85,85,85}};
    enum yac_edge_type icon_cell_edge_types[4] =
      {YAC_GREAT_CIRCLE_EDGE, YAC_GREAT_CIRCLE_EDGE,
       YAC_GREAT_CIRCLE_EDGE, YAC_GREAT_CIRCLE_EDGE};
    struct yac_grid_cell cells[] = {
      generate_cell_deg(
        icon_cell_coords[0], icon_cell_coords[1], icon_cell_edge_types, 4)};

    yac_write_grid_cells_to_file(
      cells, sizeof(cells) / sizeof(cells[0]), "test_grid2vtk_5", 5);

    if (!yac_file_exists("test_grid2vtk_5.vtk")) {
      PUT_ERR("ERROR in yac_write_grid_cells_to_file");
    } else {
      unlink("test_grid2vtk_5.vtk");
    }
