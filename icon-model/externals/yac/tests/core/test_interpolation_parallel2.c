...
int main(int argc, char *argv[]) {
  ...
  if (comp_flag) utest_submain_1(split_comm, reorder_type);
  else           utest_submain_2(split_comm, reorder_type);
...
static void utest_submain_2(
  ...
  { // test interpolation using a fractional mask
    ...
    interpolations[0] =
      yac_interp_weights_get_interpolation(
        weights, reorder_type, 1, frac_mask_value, 1.0, 0.0, NULL, 1, 1);
    interpolations[1] =
