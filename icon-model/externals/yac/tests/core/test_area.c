...
static int utest_compare_area(double a, double b);
static void utest_test_area(struct yac_grid_cell cell, double ref_area,
...
int main (void) {
...

  {
    ...
    */
    ...
    utest_test_area(Quad, ref_area, yac_pole_area);
    utest_test_area(Quad, ref_area, yac_huiliers_area);
...

  {
    ...
    for (size_t i = 0; i < sizeof(dx) / sizeof(dx[0]); ++i) {
      ...
      if (utest_compare_area(base_area, partial_area))
  ...
  { // cell with two edges (one great circle and one circle of latitude)
    ...
    for (int y = -90; y <= 90; y += 5) {
      ...
      utest_test_area(cell, ref_area, yac_pole_area);
      utest_test_area(cell, ref_area, yac_huiliers_area);
  ...
  { // concave cell
    ...
    if (utest_compare_area(partial_area,
    ...
    for (int i = 0; i < 2; ++i) {
      utest_test_area(cells[i], ref_area, yac_pole_area);
      utest_test_area(cells[i], ref_area, yac_huiliers_area);
    }
...

  {
    ...
    {
      struct yac_grid_cell ref_cell =
        generate_cell_deg(
          (double[]){0.0,1.0,1.0, 0.0},
          (double[]){0.0,0.0,1.0, 1.0}, gc_edges, 4);
      ref_area = yac_huiliers_area_info(ref_cell, ref_barycenter, 1.0);
      yac_free_grid_cell(&ref_cell);
      normalise_vector(ref_barycenter);
    }

    enum {NUM_CORNERS = 6};
    double coordinates_x[NUM_CORNERS] = {0.0, 1.0, 1.0, 0.0, 1.0, 0.0};
    double coordinates_y[NUM_CORNERS] = {0.0, 0.0, 1.0, 0.0, 1.0, 1.0};
    ...
    for (size_t i = 0; i < NUM_CORNERS; ++i) {
      ...
      for (size_t j = 0; j < NUM_CORNERS; ++j) {
        temp_coordinates_x[(i+j)%NUM_CORNERS] = coordinates_x[j];
        temp_coordinates_y[(i+j)%NUM_CORNERS] = coordinates_y[j];
      }
      double area = 0.0, barycenter[3] = {0.0, 0.0, 0.0};
      struct yac_grid_cell cell =
        generate_cell_deg(
          temp_coordinates_x, temp_coordinates_y, gc_edges, NUM_CORNERS);
      area = yac_huiliers_area_info(cell, barycenter, 1.0);
      yac_free_grid_cell(&cell);

      normalise_vector(barycenter);
      if (fabs(area - ref_area) > YAC_AREA_TOL)
        PUT_ERR("ERROR in yac_huiliers_area_info (area)");

      if (get_vector_angle(barycenter, ref_barycenter) > yac_angle_tol)
        PUT_ERR("ERROR in yac_huiliers_area_info (barycenter)");

      // check for NANs
      if ((barycenter[0] != barycenter[0]) ||
          (barycenter[1] != barycenter[1]) ||
          (barycenter[2] != barycenter[2]))
        PUT_ERR("ERROR in yac_huiliers_area_info (barycenter NAN)");
    }
  }

  { // check influence of lat circle edges on computation of the barycenter
    // of a cell

    for (int pole = -1; pole <= 1; pole += 2) {
      for (int cell_order_a = 0; cell_order_a < 2; ++cell_order_a) {
        for (int cell_order_b = 0; cell_order_b < 2; ++cell_order_b) {

          enum yac_edge_type ll_edges[3] =
            {YAC_LON_CIRCLE_EDGE, YAC_LON_CIRCLE_EDGE, YAC_LAT_CIRCLE_EDGE};
          double lon[2][3] = {{0.0,0.0,20.0}, {20.0, 20.0, 0.0}};
          struct yac_grid_cell gc_triangle =
            generate_cell_deg(
              lon[cell_order_a],
              (double[]){copysign(88.0,(double)pole),
                        copysign(90.0,(double)pole),
                        copysign(88.0,(double)pole)}, gc_edges, 3);
          struct yac_grid_cell ll_triangle =
            generate_cell_deg(
              lon[cell_order_b],
              (double[]){copysign(88.0,(double)pole),
                        copysign(90.0,(double)pole),
                        copysign(88.0,(double)pole)}, ll_edges, 3);

          double gc_barycenter[3] = {0.0, 0.0, 0.0};
          double gc_area =
            yac_huiliers_area_info(gc_triangle, gc_barycenter, 1.0);
          double ll_barycenter[3] = {0.0, 0.0, 0.0};
          double ll_area =
            yac_huiliers_area_info(ll_triangle, ll_barycenter, 1.0);

          if (gc_area < 0.0) {
            gc_area = -gc_area;
            gc_barycenter[0] = -gc_barycenter[0];
            gc_barycenter[1] = -gc_barycenter[1];
            gc_barycenter[2] = -gc_barycenter[2];
          }
          normalise_vector(gc_barycenter);

          if (ll_area < 0.0) {
            ll_area = -ll_area;
            ll_barycenter[0] = -ll_barycenter[0];
            ll_barycenter[1] = -ll_barycenter[1];
            ll_barycenter[2] = -ll_barycenter[2];
          }
          normalise_vector(ll_barycenter);

          // the triangle with the lat circle edge should be bigger
          if ((ll_area - gc_area) < YAC_AREA_TOL)
            PUT_ERR("ERROR in yac_huiliers_area_info (area)");

          // the barycenter of the triangle with the gc edges should be closer
          // to the pole
          if (((gc_barycenter[2] - ll_barycenter[2]) * (double)pole) < 1e-9)
            PUT_ERR("ERROR in yac_huiliers_area_info (barycenter)");
        }
      }
    }
...
static void utest_test_area(struct yac_grid_cell cell, double ref_area,
  ...
  for (int order = -1; order <= 1; order += 2) {
    for (int start = 0; start < (int)(cell.num_corners); ++start) {
      ...
      if (utest_compare_area(area, ref_area)) PUT_ERR("ERROR: wrong area\n");
    }
  }
}
