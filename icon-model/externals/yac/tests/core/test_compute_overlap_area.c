 ...
 *  \test
 * Test for \ref yac_compute_overlap_areas routine.
 */
...
static void utest_check_overlap(
  double * lon_a, double * lat_a, enum yac_edge_type * edges_a, int count_a,
  double * lon_b, double * lat_b, enum yac_edge_type * edges_b, int count_b,
  double ref_area, double const * ref_barycenter);
static void utest_check_overlap_polygons(
...
int main (void) {
  ...
  30.0 (lat) x                  x  30.0
    ...
    */
    ...
    utest_check_overlap(
      ...
      (double[]){32.0, 32.0, 30.0, 31.0, 30.0}, gc_edges, 5,
      (double[]){10.5, 11.0, 10.0},
      (double[]){31.5, 29.5, 29.5}, gc_edges, 3,
...

  {
    ...
    */
    ...
    utest_check_overlap(
      (double[]){-5,  5, 5, -5}, (double[]){-5, -5, 5,  5}, gc_edges, 4,
      (double[]){-4, 4, 0}, (double[]){-4, -4, 6}, gc_edges, 3,
      ref_area, ref_barycenter);
  }

  {
    ...
    */
    ...
    utest_check_overlap(
      (double[]){0,0,90,180,270}, (double[]){90,88,88,88,88}, gc_edges, 5,
      (double[]){0,90,180,270}, (double[]){89,89,89,89}, gc_edges, 4,
      ref_area, ref_barycenter);
  }

  {
    ...
    */
    ...
    utest_check_overlap(
      ...
      (double[]){3,3,-3,-3,3,3,-2,-2}, gc_edges, 8,
      (double[]){1,1,-1,-1},
...

  {
    ...
    */
    ...
    utest_check_overlap(
      ...
      (double[]){3,3,-3,-3,3,3,2,2,-2,-2,2,2}, gc_edges, 12,
      (double[]){1,1,-1,-1}, (double[]){1,-1,-1,1}, gc_edges, 4,
      ref_area, ref_barycenter);
  }

  {
    ...
    */
    ...
    utest_check_overlap(
      ...
      (double[]){88,88,88,88,87,87,87,87}, gc_edges, 8,
      (double[]){90,180,90},
      (double[]){90,87.5,87.5}, gc_edges, 3,
      ref_area, ref_barycenter);
  }
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      utest_check_overlap(
        ...
        (double[]){89,89,89,89,89,89,89,89,89,
                   86,86,86,86,86,86,86,86,86}, gc_edges, 18,
        (double[]){0+tri_rotation,90+tri_rotation,180+tri_rotation},
        (double[]){87.5,87.5,87.5}, gc_edges, 3,
        ref_area, ref_barycenter);
    }
  }
...

  {
    ...
    */
    ...
    utest_check_overlap(
      ...
      (double[]){-5.0,-5.0,0.0,0.0}, latlon_edges, 4,
      (double[]){-5.0,5.0,0.0},
      (double[]){0.0,0.0,5.0}, gc_edges, 3,
      ref_area, ref_barycenter);
  }
  ...
  { // test reproducing an issue found in toy_multi example
    ...
    utest_check_overlap_polygons(cells, ref_overlap_areas, ref_overlap_barycenters);
  ...
  { // testing issue found with FESOM grid
    ...
    */
    ...
    utest_check_overlap(
      ...
      (double[]){3.0,3.0,-3.0,-3.0}, gc_edges, 4,
      ref_area, ref_barycenter);
  }
  ...
  { // test an issue found in OYAC
    // (when the intersection between two cells is a polygon with two corners,
    //  where one edge is gc while the other is lat, an assert was raised)

  /*  pole triangle with lat edge
                      x      90
                     / \
                    /   \
                   /     \
      ...
      square with gc edges very close to the triangle
   ...
   */
    ...
    double lat_edge_middle[3];
    double gc_edge_middle[3];
    double overlap_intersections[2][3];

    LLtoXYZ_deg(0.0, 85.0, lat_edge_middle);
    if (!intersect(YAC_GREAT_CIRCLE_EDGE, -5.0, 84.99, 5.0, 84.99,
                   YAC_LON_CIRCLE_EDGE, 0.0, 90.0, 0.0, 80.0,
                   gc_edge_middle) ||
        !intersect(YAC_GREAT_CIRCLE_EDGE, -5.0, 84.99, 5.0, 84.99,
                   YAC_LAT_CIRCLE_EDGE, -10.0, 85.0, 0.0, 85.0,
                   overlap_intersections[0]) ||
        !intersect(YAC_GREAT_CIRCLE_EDGE, -5.0, 84.99, 5.0, 84.99,
                   YAC_LAT_CIRCLE_EDGE, 0.0, 85.0, 10.0, 85.0,
                   overlap_intersections[1]))
      return EXIT_FAILURE;

    struct yac_grid_cell ref_cell_overlap =
        {.coordinates_xyz = overlap_intersections,
         .edge_type =
           (enum yac_edge_type[2]){YAC_GREAT_CIRCLE_EDGE, YAC_LAT_CIRCLE_EDGE},
         .num_corners = 2,
         .array_size = 2};
    double ref_area = yac_huiliers_area(ref_cell_overlap);

    // rough estimate of barycenter of the intersection between the two cells
    double ref_barycenter[3] = {lat_edge_middle[0] + gc_edge_middle[0],
                                lat_edge_middle[1] + gc_edge_middle[1],
                                lat_edge_middle[2] + gc_edge_middle[2]};
    normalise_vector(ref_barycenter);

    utest_check_overlap(
      (double[]){-5.0,5.0,5.0,-5.0},
      (double[]){90.0,90.0,85.0,85.0}, latlon_edges, 4,
      (double[]){-5.0,5.0,5.0,-5.0},
      (double[]){84.99,84.99,80.0,80.0}, gc_edges, 4,
      ref_area, ref_barycenter);
  }

  // cleanup
  yac_compute_overlap_buf_free();
    ...
    for (int order_b = -1; order_b <=1; order_b += 2) {
      for (int start_idx_a = 0; start_idx_a < count_a; ++start_idx_a) {
        for (int start_idx_b = 0; start_idx_b < count_b; ++start_idx_b) {
          ...
          enum yac_edge_type curr_edges_a[count_a];
          enum yac_edge_type curr_edges_b[count_b];

          for (int i = 0; i < count_a; ++i) {
            int idx = (start_idx_a + order_a * i + count_a) % count_a;
            curr_lon_a[i] = lon_a[idx];
            curr_lat_a[i] = lat_a[idx];
            idx = (idx - (order_a < 0) + count_a) % count_a;
            curr_edges_a[i] = edges_a[idx];
          }
          for (int i = 0; i < count_b; ++i) {
            int idx = (start_idx_b + order_b * i + count_b) % count_b;
            curr_lon_b[i] = lon_b[idx];
            curr_lat_b[i] = lat_b[idx];
            idx = (idx - (order_b < 0) + count_b) % count_b;
            curr_edges_b[i] = edges_b[idx];
          }

          struct yac_grid_cell Polygons[2] = {
            generate_cell_deg(curr_lon_a, curr_lat_a, curr_edges_a, count_a),
            generate_cell_deg(curr_lon_b, curr_lat_b, curr_edges_b, count_b)};

          utest_check_overlap_polygons(Polygons, ref_area, ref_barycenter);
