...
static void utest_generate_input_weights();

static void utest_generate_ref_weights();

static void utest_submain_src(
  MPI_Comm comp_comm, enum yac_interp_weights_reorder_type reorder_type);

static void utest_submain_tgt(
  MPI_Comm comp_comm, enum yac_interp_weights_reorder_type reorder_type);
...
int main(int argc, char *argv[]) {
  ...
  utest_generate_input_weights();
  utest_generate_ref_weights();
  ...
  if (comp_flag) utest_submain_src(split_comm, reorder_type);
  else           utest_submain_tgt(split_comm, reorder_type);
  ...
  { // test interpolation
    ...
    struct interp_method * method_stack[] =
      {yac_interp_method_file_new(
         weight_file_in, YAC_INTERP_FILE_ON_MISSING_FILE_DEFAULT,
