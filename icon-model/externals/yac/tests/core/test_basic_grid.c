...
static void utest_check_basic_grid(
...
static void utest_check_interp_field(
...
int main (void) {
  ...
  { // test yac_basic_grid_unstruct_edge_new
    ...
    double x_vertices[] = {0.0,0.1,0.0,0.1,0.0,0.1};
    double y_vertices[] = {0.0,0.0,0.1,0.1,0.2,0.2};
    int cell_to_edge[] = {0,2,3,1, 3,5,6,4};
  ...
  { // test yac_basic_grid_reg_2d_new
    ...
    size_t nbr_vertices[2] = {2,3};
    int cyclic[2] = {0,0};
    double lon_vertices[] = {-0.5*YAC_RAD,0.5*YAC_RAD};
    double lat_vertices[] = {-0.5*YAC_RAD,0.0*YAC_RAD,0.5*YAC_RAD};
    double north_pole_lon = 180.0*YAC_RAD;
    double north_pole_lat = 45.0*YAC_RAD;

    struct yac_basic_grid * grid =
      yac_basic_grid_reg_2d_rot_new(
        grid_name, nbr_vertices, cyclic, lon_vertices, lat_vertices,
  ...
  { // test yac_basic_grid_reg_2d_deg_new
    ...
    size_t nbr_vertices[2] = {2,3};
    int cyclic[2] = {0,0};
    double lon_vertices[] = {-0.5,0.5};
    double lat_vertices[] = {-0.5,0.0,0.5};
    double north_pole_lon = 180.0;
    double north_pole_lat = 45.0;
  ...
  { // check for bug that occured in edge-based unstruct grid definition

    char const * grid_name = "unstruct_edge_bug";
    enum {NBR_EDGES = 3};
    size_t nbr_vertices = NBR_EDGES;
    size_t nbr_cells = 1;
    size_t nbr_edges = NBR_EDGES;
    int num_edges_per_cell[] = {NBR_EDGES};
    double x_vertices[] = {-0.1,0.0,0.1};
    double y_vertices[] = {0.0,0.0,0.1};
    int cell_to_edge[][NBR_EDGES] =
      {{0,1,2}, {0,2,1}, {1,0,2}, {1,2,0}, {2,0,1}, {2,1,0}};
    int edge_to_vertex[NBR_EDGES][2] = {{0,1}, {1,2}, {2,0}};
    int ref_edge_to_vertex[] = {0,1, 1,2, 2,0};

    for (size_t i = 0; i < sizeof(cell_to_edge) / sizeof(cell_to_edge[0]);
         ++i) {

      int edge_vertex_ordering[NBR_EDGES];
      for (edge_vertex_ordering[0] = 0; edge_vertex_ordering[0] < 2;
           ++edge_vertex_ordering[0]) {
        for (edge_vertex_ordering[1] = 0; edge_vertex_ordering[1] < 2;
             ++edge_vertex_ordering[1]) {
          for (edge_vertex_ordering[2] = 0; edge_vertex_ordering[2] < 2;
               ++edge_vertex_ordering[2]) {

            struct yac_basic_grid_data ref_grid_data =
              yac_generate_basic_grid_data_unstruct_edge(
                nbr_vertices, nbr_cells, nbr_edges, num_edges_per_cell,
                x_vertices, y_vertices, cell_to_edge[i], ref_edge_to_vertex);

            int temp_edge_to_vertex[NBR_EDGES][2];

            for (size_t j = 0; j < NBR_EDGES; ++j) {
              temp_edge_to_vertex[j][edge_vertex_ordering[j]] =
                edge_to_vertex[j][0];
              temp_edge_to_vertex[j][edge_vertex_ordering[j]^1] =
                edge_to_vertex[j][1];
            }
            ...
            struct yac_basic_grid * grid =
              yac_basic_grid_unstruct_edge_new(
                grid_name, nbr_vertices, nbr_cells, nbr_edges,
                num_edges_per_cell, x_vertices, y_vertices,
                cell_to_edge[i], &temp_edge_to_vertex[0][0]);

            int const * ref_core_masks[NUM_LOCACTIONS] = {NULL, NULL, NULL};
            size_t const ref_grid_data_sizes[NUM_LOCACTIONS] = {1, 3, 3};
