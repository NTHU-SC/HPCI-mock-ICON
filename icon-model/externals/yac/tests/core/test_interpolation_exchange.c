...
static void utest_check_exchange(
...
#ifdef NVHPC_BUG
    utest_check_exchange(exchanges[0], is_source, is_target, optional_put_get);
    utest_check_exchange(exchanges[1], is_source, is_target, optional_put_get);
#else
...
static void utest_check_exchange(
  ...
  if (!yac_interpolation_exchange_put_test(exchange, "utest_check_exchange"))
    PUT_ERR("ERROR in yac_interpolation_exchange_execute");
  if (!yac_interpolation_exchange_get_test(exchange, "utest_check_exchange"))
    PUT_ERR("ERROR in yac_interpolation_exchange_execute");
  if (yac_interpolation_exchange_status(exchange, "utest_check_exchange") !=
      YAC_INTERP_EXCH_IDLE)
    PUT_ERR("ERROR in yac_interpolation_exchange_status");

  // this should not block
  yac_interpolation_exchange_wait(exchange, "utest_check_exchange");

  // simple exchange
  recv_data_[0] = -1.0, recv_data_[1] = -1.0;
  yac_interpolation_exchange_execute(
    exchange, &send_data, &recv_data, "utest_check_exchange");
  if (is_target)
  ...
  if (!optional_put_get || is_source)
    yac_interpolation_exchange_execute_put(
      exchange, &send_data, "utest_check_exchange");
  status = yac_interpolation_exchange_status(exchange, "utest_check_exchange");
  if (is_target) {
    if (is_source) {
  ...
  if (!optional_put_get || is_target)
    yac_interpolation_exchange_execute_get(
      exchange, &recv_data, "utest_check_exchange");
  if (is_target && ((recv_data_[0] != 0.0) || (recv_data_[1] != 1.0)))
    PUT_ERR("ERROR in yac_interpolation_exchange_execute_get");
  if (is_source) yac_interpolation_exchange_wait(exchange, "utest_check_exchange");
  if (yac_interpolation_exchange_status(exchange, "utest_check_exchange") !=
      YAC_INTERP_EXCH_IDLE)
    PUT_ERR("ERROR in yac_interpolation_exchange_status");
  ...
  if (!optional_put_get || is_source) {
    ...
    if (!is_target)
      ...
      while (!yac_interpolation_exchange_put_test(exchange, "utest_check_exchange"));
  }
  recv_data_[0] = -1.0, recv_data_[1] = -1.0;
  if (!optional_put_get || is_target)
    yac_interpolation_exchange_execute_get(
  ...
  for (int put_get_order = 0; put_get_order < 2; ++put_get_order) {
    ...
    // this should return at some point
    while (!(yac_interpolation_exchange_put_test(exchange, "utest_check_exchange") &&
             yac_interpolation_exchange_get_test(exchange, "utest_check_exchange")));
    if (is_target && ((recv_data_[0] != 0.0) || (recv_data_[1] != 1.0)))
      PUT_ERR("ERROR in yac_interpolation_exchange_execute_get");
  }
