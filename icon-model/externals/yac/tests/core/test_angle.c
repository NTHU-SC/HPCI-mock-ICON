...
static void utest_generate_rand_angle(
  double * angle, struct sin_cos_angle * sin_cos_angle);

static struct sin_cos_angle utest_compute_sin_cos_angle(double angle);
static int utest_compare_double(double a, double b);
static int utest_compare_size_t(size_t a, size_t b);
static void utest_check_angle_sum_sub(
  struct sin_cos_angle a, struct sin_cos_angle b,
  double ref_sum, double ref_sub);
static void utest_check_compare_angles(double dble_angle_a, double dble_angle_b,
...
int main (void) {
  ...
  { // test routine compute_angle
    for (size_t i = 0; i < sizeof(angles) / sizeof(angles[0]); ++i)
      if (fabs(compute_angle(utest_compute_sin_cos_angle(angles[i])) - angles[i]) >
          yac_angle_tol)
        PUT_ERR("error in compute_angle");

    for (size_t i = 0; i < NUM_TESTS; ++i) {
      double temp_angle;
      utest_generate_rand_angle(&temp_angle, &sin_cos_angle);
  ...
  { // test routine compare_angles and sin_cos_angle_to_dble
    for (size_t i = 0; i < sizeof(angles)/sizeof(angles[0]); ++i) {
      for (size_t j = 0; j < sizeof(angles)/sizeof(angles[0]); ++j) {
        struct sin_cos_angle temp_sin_cos_angles[2] =
          {utest_compute_sin_cos_angle(angles[i]), utest_compute_sin_cos_angle(angles[j])};
        utest_check_compare_angles(
    ...
    for (size_t i = 0; i < NUM_TESTS; ++i) {
      ...
      for (size_t j = 0; j < 2; ++j)
        utest_generate_rand_angle(&(temp_angles[j]), &(temp_sin_cos_angles[j]));
      for (size_t j = 0; j < 2; ++j)
        for (size_t k = 0; k < 2; ++k)
          utest_check_compare_angles(temp_angles[j], temp_angles[k],
    ...
    for (size_t i = 0; i < sizeof(sin_cos_angles)/sizeof(sin_cos_angles[0]); ++i) {
      for (size_t j = 0; j < sizeof(sin_cos_angles)/sizeof(sin_cos_angles[0]); ++j) {
        ...
        int compare_ij = utest_compare_size_t(i, j);
        if ((compare_ij != compare_angles(sin_cos_angles[i], sin_cos_angles[j])) ||
            (compare_ij != utest_compare_double(temp[0], temp[1])))
  ...
  { // test routine sum_angles, sub_angles
    for (size_t i = 0; i < sizeof(angles)/sizeof(angles[0]); ++i) {
      for (size_t j = 0; j < sizeof(angles)/sizeof(angles[0]); ++j) {
        struct sin_cos_angle temp_sin_cos_angles[2] =
        {utest_compute_sin_cos_angle(angles[i]), utest_compute_sin_cos_angle(angles[j])};
        utest_check_angle_sum_sub(temp_sin_cos_angles[0], temp_sin_cos_angles[1],
  ...
  { // test routine half_angle
    for (size_t i = 0; i < sizeof(angles)/sizeof(angles[0]); ++i)
      if (fabs(angles[i] * 0.5 -
               compute_angle(half_angle(utest_compute_sin_cos_angle(angles[i])))) >
          yac_angle_tol)
        PUT_ERR("error in half_angle");

    for (size_t i = 0; i < NUM_TESTS; ++i) {
      utest_generate_rand_angle(&angle, &sin_cos_angle);
      if (fabs(angle * 0.5 -
  ...
  { // test routine quarter_angle
    for (size_t i = 0; i < sizeof(angles)/sizeof(angles[0]); ++i)
      if (fabs(angles[i] * 0.25 -
               compute_angle(quarter_angle(utest_compute_sin_cos_angle(angles[i])))) >
          yac_angle_tol)
...
static void utest_generate_rand_angle(
  ...
  *sin_cos_angle = utest_compute_sin_cos_angle(*angle);
}
...
static void utest_check_compare_angles(double dble_angle_a, double dble_angle_b,
  ...
  int ret = utest_compare_double(dble_angle_a, dble_angle_b);
  ...
  if (ret != utest_compare_double(temp[0], temp[1]))
