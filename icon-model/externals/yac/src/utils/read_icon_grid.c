...
void yac_read_icon_grid_information_parallel(
  ...
  YAC_ASSERT(
    ((x_cells == NULL) && (y_cells == NULL)) ||
    ((x_cells != NULL) && (y_cells != NULL)),
    "ERROR(yac_read_icon_grid_information_parallel): "
    "arguments x_cells and y_cells have to be either both NULL or "
    "both different from NULL");

  const int with_cell_coords = x_cells != NULL;
  const int with_cell_mask = cell_msk != NULL;
  ...
  if (local_is_io) {
    ...
    read_cell_lon =
      with_cell_coords?
        xmalloc(read_num_local_cells * sizeof(*read_cell_lon)):NULL;
    read_cell_lat =
      with_cell_coords?
        xmalloc(read_num_local_cells * sizeof(*read_cell_lat)):NULL;
    read_cell_mask =
      with_cell_mask?
        xmalloc(read_num_local_cells * sizeof(*read_cell_mask)):NULL;
    ...
    read_cell_lon =
      with_cell_coords?xmalloc(1 * sizeof(*read_cell_lon)):NULL;
    read_cell_lat =
      with_cell_coords?xmalloc(1 * sizeof(*read_cell_lat)):NULL;
    read_cell_mask =
      with_cell_mask?xmalloc(1 * sizeof(*read_cell_mask)):NULL;
  ...
  double * cell_lon =
    with_cell_coords?xmalloc(num_local_cells * sizeof(*cell_lon)):NULL;
  double * cell_lat =
    with_cell_coords?xmalloc(num_local_cells * sizeof(*cell_lat)):NULL;
  int * cell_mask =
    with_cell_mask?xmalloc(num_local_cells * sizeof(*cell_mask)):NULL;
  ...
  }
  {
    ...
    double * send_cell_lon =
      with_cell_coords?
        xmalloc(num_halo_cells_remote * sizeof(*send_cell_lon)):NULL;
    double * send_cell_lat =
      with_cell_coords?
        xmalloc(num_halo_cells_remote * sizeof(*send_cell_lat)):NULL;
    int * send_cell_mask =
      with_cell_mask?
        xmalloc(num_halo_cells_remote * sizeof(*send_cell_mask)):NULL;
  ...
  *y_vertices = all_vertex_lat;
  ...
  if (with_cell_mask) *cell_msk = cell_mask;
}
...
void yac_read_icon_grid_information_parallel_2(
  ...
  YAC_ASSERT(
    ((x_cells == NULL) && (y_cells == NULL)) ||
    ((x_cells != NULL) && (y_cells != NULL)),
    "ERROR(yac_read_icon_grid_information_parallel_2): "
    "arguments x_cells and y_cells have to be either both NULL or "
    "both different from NULL");
  ...
  if (local_is_io) {
    ...
    if (with_cell_coords) {
      ...
      YAC_HANDLE_ERROR(
        nc_get_vara_double(
          ncid, varid, &read_local_start_cell,
          &read_num_local_cells, read_cell_lon));
      convert_to_rad(ncid, varid, read_cell_lon, read_num_local_cells);
      yac_nc_inq_varid(ncid, "clat", &varid);
      YAC_HANDLE_ERROR(
    ...
    read_cell_lon = with_cell_coords?xmalloc(1 * sizeof(*read_cell_lon)):NULL;
    read_cell_lat = with_cell_coords?xmalloc(1 * sizeof(*read_cell_lat)):NULL;
    read_cell_mask = with_cell_mask?xmalloc(1 * sizeof(*read_cell_mask)):NULL;
    read_vertex_lon = xmalloc(1 * sizeof(*read_vertex_lon));
    read_vertex_lat = xmalloc(1 * sizeof(*read_vertex_lat));
    read_dist_vertex_of_cell = xmalloc(1 * sizeof(*read_dist_vertex_of_cell));
  ...
  // redistribute basic cell data (from io decomposition)
  if (with_cell_coords) {
    *x_cells = xmalloc(num_local_cells * sizeof(**x_cells));
    *y_cells = xmalloc(num_local_cells * sizeof(**y_cells));
  }
  if (with_cell_mask) {
    *cell_msk = xmalloc(num_local_cells * sizeof(**cell_msk));
  }
  int * dist_vertex_of_cell =
    xmalloc(num_local_cells * nv * sizeof(*dist_vertex_of_cell));
    ...
    xmalloc(num_local_cells * nv * sizeof(*dist_edge_of_cell));
  {
    ...
    if (with_cell_coords) {
      MPI_Alltoallv(read_cell_lon, send_count, send_displ, MPI_DOUBLE,
                    *x_cells, recv_count, recv_displ, MPI_DOUBLE, comm);
      MPI_Alltoallv(read_cell_lat, send_count, send_displ, MPI_DOUBLE,
                    *y_cells, recv_count, recv_displ, MPI_DOUBLE, comm);
    }
    if (with_cell_mask) {
      MPI_Alltoallv(read_cell_mask, send_count, send_displ, MPI_INT,
                    *cell_msk, recv_count, recv_displ, MPI_INT, comm);
    }
  ...
  size_t num_core_vertices;
  {
    ...
    *vertex_ids = xmalloc(N * sizeof(**vertex_ids));
    ...
    *vertex_ids =
      xrealloc(*vertex_ids, num_core_vertices * sizeof(**vertex_ids));
  ...
  {
    size_t N = num_local_cells * nv;
    *edge_ids = xmalloc(N * sizeof(**edge_ids));
    size_t * permutation = xmalloc(N * sizeof(*permutation));
    *cell_to_edge = xmalloc(N * sizeof(**cell_to_edge));
    for (size_t i = 0; i < N; ++i)
      (*edge_ids)[i] = (yac_int)dist_edge_of_cell[i];
    for (size_t i = 0; i < N; ++i) permutation[i] = i;
    yac_quicksort_index_yac_int_size_t(*edge_ids, N, permutation);
    // remove duplicated core edges and count number of cells per edge
    yac_int prev_edge_id = XT_INT_MAX;
    num_core_edges = 0;
    for (size_t i = 0; i < N; ++i) {
      yac_int curr_edge_id = (*edge_ids)[i];
      if (prev_edge_id != curr_edge_id)
        (*edge_ids)[num_core_edges++] = (prev_edge_id = curr_edge_id);
      (*cell_to_edge)[permutation[i]] = num_core_edges-1;
      permutation[i] /= nv;
    }
    free(permutation);
    *edge_ids =
      xrealloc(*edge_ids, num_core_edges * sizeof(**edge_ids));
    free(dist_edge_of_cell);
  }

  // generate vertex coordinate data
  {
    *x_vertices = xmalloc(num_core_vertices * sizeof(**x_vertices));
    *y_vertices = xmalloc(num_core_vertices * sizeof(**y_vertices));
    int * send_count = xcalloc(comm_size, sizeof(*send_count));
    int * recv_count = xcalloc(comm_size, sizeof(*recv_count));

    for (size_t i = 0; i < num_core_vertices; ++i)
      send_count[((unsigned long)((*vertex_ids)[i]) *
                  (unsigned long)comm_size + (unsigned long)comm_size - 1) /
                 (unsigned long)nvertices]++;
    ...
    MPI_Alltoallv(
      *vertex_ids, send_count, send_displ, yac_int_dt,
    ...
    MPI_Alltoallv(send_vertex_lon, recv_count, recv_displ, MPI_DOUBLE,
                  *x_vertices, send_count, send_displ, MPI_DOUBLE, comm);
    MPI_Alltoallv(send_vertex_lat, recv_count, recv_displ, MPI_DOUBLE,
                  *y_vertices, send_count, send_displ, MPI_DOUBLE, comm);
  ...
  *edge_to_vertex =
    xmalloc(2 * num_core_edges * sizeof(**edge_to_vertex));
  {
    ...
    for (size_t i = 0; i < num_core_edges; ++i)
      send_count[((unsigned long)((*edge_ids)[i]) *
                  (unsigned long)comm_size + (unsigned long)comm_size - 1) /
    ...
    MPI_Alltoallv(
      *edge_ids, send_count, send_displ, yac_int_dt,
    ...
    for (size_t i = 0, j = 0; i < 2 * num_core_edges; ++i) {
      ...
      while ((j < nvertices) && ((*vertex_ids)[j] < curr_vertex)) ++j;
      YAC_ASSERT(
        (j < nvertices) && ((*vertex_ids)[j] == curr_vertex),
  ...
  *cell_ids = xmalloc(num_local_cells * sizeof(**cell_ids));
  for (size_t i = 0; i < num_local_cells; ++i)
  ...
  *edge_type = xmalloc(num_core_edges * sizeof(**edge_type));
  for (size_t i = 0; i < num_core_edges; ++i)
    (*edge_type)[i] = YAC_GREAT_CIRCLE_EDGE;

  *num_cells = num_local_cells;
  *num_vertices = num_core_vertices;
  *num_edges = num_core_edges;
}
...
struct yac_basic_grid_data yac_read_icon_basic_grid_data_parallel(
  ...
  yac_read_icon_grid_information_parallel_2(
    filename, comm, &x_vertices, &y_vertices, &cell_ids, &vertex_ids, &edge_ids,
    &num_cells, &num_vertices, &num_edges, &num_vertices_per_cell,
    &num_cells_per_vertex, &cell_to_vertex, &cell_to_edge, &vertex_to_cell,
    &edge_to_vertex, &edge_type, NULL, NULL, NULL);

  yac_coordinate_pointer vertex_coordinates =
    xmalloc(num_vertices * sizeof(*vertex_coordinates));
  for (size_t i = 0; i < num_vertices; ++i)
    LLtoXYZ(x_vertices[i], y_vertices[i], vertex_coordinates[i]);
  free(x_vertices);
  free(y_vertices);
  ...
  grid_data.core_cell_mask          = generate_simple_core_mask(num_cells);
  grid_data.core_vertex_mask        = generate_simple_core_mask(num_vertices);
  grid_data.core_edge_mask          = generate_simple_core_mask(num_edges);
  ...
  grid_data.cell_to_vertex_offsets  = generate_offsets(num_cells, num_vertices_per_cell);
  ...
  grid_data.vertex_to_cell_offsets  = generate_offsets(num_vertices, num_cells_per_vertex);
...
void yac_read_icon_basic_grid_parallel_2(
  ...
  struct yac_basic_grid ** basic_grid, size_t * cell_coordinate_idx,
  int ** cell_mask) {
  ...
  yac_read_icon_grid_information_parallel_2(
    ...
    &num_cells, &num_vertices, &num_edges, &num_vertices_per_cell,
    &num_cells_per_vertex, &cell_to_vertex, &cell_to_edge, &vertex_to_cell,
    &edge_to_vertex, &edge_type, &x_cells, &y_cells, cell_mask);
  ...
  basic_grid_data.vertex_coordinates      = vertex_coordinates;
  basic_grid_data.cell_ids                = cell_ids;
  basic_grid_data.vertex_ids              = vertex_ids;
  basic_grid_data.edge_ids                = edge_ids;
  basic_grid_data.num_cells               = num_cells;
  basic_grid_data.num_vertices            = num_vertices;
  basic_grid_data.num_edges               = num_edges;
  basic_grid_data.core_cell_mask          = generate_simple_core_mask(num_cells);
  basic_grid_data.core_vertex_mask        = generate_simple_core_mask(num_vertices);
  basic_grid_data.core_edge_mask          = generate_simple_core_mask(num_edges);
  basic_grid_data.num_vertices_per_cell   = num_vertices_per_cell;
  basic_grid_data.num_cells_per_vertex    = num_cells_per_vertex;
  basic_grid_data.cell_to_vertex          = cell_to_vertex;
  basic_grid_data.cell_to_vertex_offsets  = generate_offsets(num_cells, num_vertices_per_cell);
  basic_grid_data.cell_to_edge            = cell_to_edge;
  basic_grid_data.cell_to_edge_offsets    = basic_grid_data.cell_to_vertex_offsets;
  basic_grid_data.vertex_to_cell          = vertex_to_cell;
  basic_grid_data.vertex_to_cell_offsets  = generate_offsets(num_vertices, num_cells_per_vertex);
  basic_grid_data.edge_to_vertex          = (yac_size_t_2_pointer)&(edge_to_vertex[0]);
  basic_grid_data.edge_type               = edge_type;
  basic_grid_data.num_total_cells         = num_cells;
  basic_grid_data.num_total_vertices      = num_vertices;
  basic_grid_data.num_total_edges         = num_edges;

  *basic_grid = yac_basic_grid_new(gridname, basic_grid_data);

  yac_coordinate_pointer cell_coordinates =
    xmalloc(num_cells * sizeof(*cell_coordinates));
  for (size_t i = 0; i < num_cells; ++i)
    LLtoXYZ(x_cells[i], y_cells[i], cell_coordinates[i]);
  *cell_coordinate_idx =
    yac_basic_grid_add_coordinates_nocpy(
      *basic_grid, YAC_LOC_CELL, cell_coordinates);
  free(x_cells);
  free(y_cells);
}
