...
static void yac_read_scrip_basic_grid_information(
  ...
  size_t * nbr_duplicated_cells_, int with_vertices) {
  ...
  if (with_vertices) {
    ...
    int ndims;
    YAC_HANDLE_ERROR(
      nc_inq_var(ncid, lat_var_id, NULL, NULL, &ndims, dim_ids, NULL));
    YAC_ASSERT_F(
      ndims == 2,
      "ERROR(yac_read_scrip_basic_grid_information): "
  ...
  if (with_vertices) {
    ...
    YAC_ASSERT(
      ...
      "ERROR(yac_read_scrip_basic_grid_information): "
      "no pointers for cell coordinates were provided")

    memcpy(cla, *y_cells, num_cells * sizeof(*cla));
    memcpy(clo, *x_cells, num_cells * sizeof(*clo));
  }
  ...
  if (crn_dim_len > 1) {
    ...
    for (size_t i = 0; i < num_cells; ++i) {
      if (cell_mask[i]) {
        if (to_vertices != from_vertices) *to_vertices = *from_vertices;
        ++to_vertices;
        ++total_num_cell_vertices;
...
static void yac_read_part_scrip_basic_grid_information(
  ...
  int io_rank_idx, int num_io_ranks, MPI_Comm comm, int with_vertices) {
  ...
  if ((io_rank_idx >= 0) && (io_rank_idx < num_io_ranks)) {
    ...
    if (with_vertices) {
      ...
      x_vertices = xmalloc(num_cells * sizeof(*x_vertices));
      y_vertices = xmalloc(num_cells * sizeof(*y_vertices));

      YAC_ASSERT(
        (x_cells_ != NULL) && (y_cells_ != NULL),
        "ERROR(yac_read_part_scrip_basic_grid_information): "
        "no pointers for cell coordinates were provided")

      memcpy(x_vertices, x_cells, num_cells * sizeof(*x_vertices));
      memcpy(y_vertices, y_cells, num_cells * sizeof(*y_vertices));
    }
    ...
    for (size_t y = 0, l = 0; y < y_dim_len; ++y)
      for (size_t x = 0; x < x_count_local; ++x)
        for (size_t n = 0; n < crn_dim_len; ++n, ++l)
          reorder_idx[
            x + y * x_count_local + n * x_count_local * y_dim_len] = l;
  ...
  int * to_vertices = cell_to_vertex;
  int * from_vertices = cell_to_vertex;
  if (max_num_vertices_per_cell > 1) {
    for (size_t i = 0; i < num_cells; ++i) {
      size_t curr_num_vertices = 0;
      if (cell_mask[i]) {
        if (max_num_vertices_per_cell > 1) {
...
static void yac_read_scrip_grid_information_(
  ...
  size_t ** orig_cell_idx, size_t * nbr_duplicated_cells, int with_vertices) {
  ...
  yac_read_scrip_basic_grid_information(
    ...
    orig_cell_idx, nbr_duplicated_cells, with_vertices);
  ...
  YAC_ASSERT(
    !with_vertices || (*num_vertices <= INT_MAX),
...
void yac_read_scrip_grid_information(
  ...
  yac_read_scrip_grid_information_(
    grid_filename, mask_filename, grid_name, valid_mask_value,
    num_vertices, num_cells, num_vertices_per_cell, x_vertices, y_vertices,
    x_cells, y_cells, cell_to_vertex, cell_core_mask,
    duplicated_cell_idx, orig_cell_idx, nbr_duplicated_cells, 1);
}

static int get_io_configuration(
  MPI_Comm comm, int * io_rank_idx, int * num_io_ranks) {

  int local_is_io, * io_ranks;
  ...
  *io_rank_idx = 0;
  ...
  YAC_ASSERT(
    ...
    "ERROR(get_io_configuration): unable to determine io_rank_idx");
  free(io_ranks);
...
static void yac_read_part_scrip_grid_information(
  ...
  int ** cell_to_vertex, int ** cell_core_mask, size_t ** duplicated_cell_idx,
  yac_int ** orig_cell_ids, size_t * nbr_duplicated_cells, int with_vertices) {
  ...
  if (comm == MPI_COMM_NULL) {
    ...
    yac_read_scrip_grid_information_(
      ...
      nbr_duplicated_cells, with_vertices);
    ...
    *cell_ids = xmalloc(*num_cells * sizeof(**cell_ids));
    ...
    int io_rank_idx, num_io_ranks;
    get_io_configuration(comm, &io_rank_idx, &num_io_ranks);
    ...
    yac_read_part_scrip_basic_grid_information(
      ...
      io_rank_idx, num_io_ranks, comm, with_vertices);
...
static struct yac_basic_grid_data yac_read_scrip_basic_grid_data_(
  ...
  yac_read_part_scrip_grid_information(
    ...
    orig_cell_global_ids, nbr_duplicated_cells, 1);
...
static struct yac_basic_grid_data yac_read_scrip_cloud_basic_grid_data_(
  char const * grid_filename, char const * mask_filename, MPI_Comm comm,
  char const * grid_name, int valid_mask_value, size_t ** duplicated_vertex_idx,
  yac_int ** orig_vertex_global_ids, size_t * nbr_duplicated_vertices) {

  size_t file_num_cells;

  double * file_x_cells;
  double * file_y_cells;
  yac_int * file_cell_ids;
  int * file_cell_core_mask;

  yac_read_part_scrip_grid_information(
    grid_filename, mask_filename, comm, grid_name, valid_mask_value,
    NULL, &file_num_cells, NULL, NULL, NULL, NULL,
    &file_x_cells, &file_y_cells, &file_cell_ids,
    NULL, &file_cell_core_mask, duplicated_vertex_idx,
    orig_vertex_global_ids, nbr_duplicated_vertices, 0);

  struct yac_basic_grid_data grid_data =
    yac_generate_basic_grid_data_cloud(
      file_num_cells, file_x_cells, file_y_cells);

  free(file_x_cells);
  free(file_y_cells);

  free(grid_data.core_vertex_mask);
  grid_data.core_vertex_mask = file_cell_core_mask;

  free(grid_data.vertex_ids);
  grid_data.vertex_ids = file_cell_ids;
...
struct yac_basic_grid_data yac_read_scrip_cloud_basic_grid_data(
  ...
  return
    yac_read_scrip_cloud_basic_grid_data_(
      grid_filename, mask_filename, MPI_COMM_NULL,
      grid_name, valid_mask_value, NULL, NULL, NULL);
}
...
struct yac_basic_grid * yac_read_scrip_cloud_basic_grid_parallel(
  ...
  struct yac_basic_grid_data basic_grid_data =
    yac_read_scrip_cloud_basic_grid_data_(
      grid_filename, mask_filename, comm, grid_name, valid_mask_value,
      duplicated_vertex_idx, orig_vertex_global_ids, nbr_duplicated_vertices);
  struct yac_basic_grid * basic_grid =
    yac_basic_grid_new(name, basic_grid_data);

  if (vertex_coord_idx != NULL) {

    size_t num_vertices = basic_grid_data.num_vertices;
    yac_coordinate_pointer vertex_coords =
      xmalloc(num_vertices * sizeof(*vertex_coords));
    memcpy(
      vertex_coords, basic_grid_data.vertex_coordinates,
      num_vertices * sizeof(*vertex_coords));
    *vertex_coord_idx =
      yac_basic_grid_add_coordinates_nocpy(
        basic_grid, YAC_LOC_CORNER, vertex_coords);
  }

  return basic_grid;
}

struct yac_basic_grid * yac_read_scrip_cloud_basic_grid_parallel_f2c(
  char const * grid_filename, char const * mask_filename,
  MPI_Fint comm, char const * grid_name, int valid_mask_value,
  char const * name, size_t * vertex_coord_idx,
  size_t ** duplicated_vertex_idx, yac_int ** orig_vertex_global_ids,
  ...
  return
    yac_read_scrip_cloud_basic_grid_parallel(
      grid_filename, mask_filename, MPI_Comm_f2c(comm), grid_name,
      valid_mask_value, name, vertex_coord_idx, duplicated_vertex_idx,
...
struct yac_basic_grid * yac_read_scrip_cloud_basic_grid(
  ...
  return
    yac_read_scrip_cloud_basic_grid_parallel(
      grid_filename, mask_filename, MPI_COMM_NULL, grid_name, valid_mask_value,
      name, vertex_coord_idx, duplicated_vertex_idx, orig_vertex_global_ids,
      nbr_duplicated_vertices);
}

static int check_corner_information(
  char const * grid_filename, MPI_Comm comm, char const * grid_name) {

  int local_is_io, * io_ranks, num_io_ranks;
  yac_get_io_ranks(comm, &local_is_io, &io_ranks, &num_io_ranks);
  int root_rank = io_ranks[0];

  int comm_rank;
  yac_mpi_call(MPI_Comm_rank(comm, &comm_rank), comm);

  int contains_corner_information = 0;

  if (local_is_io && (comm_rank == root_rank)) {
    ...
    // get variable ids
    int cla_var_id, clo_var_id;

    contains_corner_information =
      (NC_NOERR == nc_inq_varid(ncid, cla_var_name, &cla_var_id)) &&
      (NC_NOERR == nc_inq_varid(ncid, clo_var_name, &clo_var_id));
  ...
  yac_mpi_call(
    MPI_Bcast(
      &contains_corner_information, 1, MPI_INT, root_rank, comm), comm);

  return contains_corner_information;
}
...
struct yac_basic_grid * yac_read_scrip_generic_basic_grid_parallel(
  ...
  int contains_corner_information =
    check_corner_information(grid_filename, comm, grid_name);

  if (contains_corner_information) {
    *point_location = YAC_LOC_CELL;
    return
      yac_read_scrip_basic_grid_parallel(
        grid_filename, mask_filename, comm, grid_name,
        valid_mask_value, name, use_ll_edges, point_coord_idx,
        duplicated_point_idx, orig_point_global_ids, nbr_duplicated_points);
  } else {
    *point_location = YAC_LOC_CORNER;
    return
      yac_read_scrip_cloud_basic_grid_parallel(
        grid_filename, mask_filename, comm, grid_name,
        valid_mask_value, name, point_coord_idx, duplicated_point_idx,
        orig_point_global_ids, nbr_duplicated_points);
  }
}

struct yac_basic_grid * yac_read_scrip_generic_basic_grid_parallel_f2c(
  char const * grid_filename, char const * mask_filename,
  MPI_Fint comm, char const * grid_name, int valid_mask_value,
  char const * name, int use_ll_edges, size_t * point_coord_idx,
  size_t ** duplicated_point_idx, yac_int ** orig_point_global_ids,
...
static void remove_duplicated_vertices_parallel(
  ...
  char const * routine = "remove_duplicated_vertices_parallel";
  ...
  yac_alltoallv_p2p(
    ...
    sizeof(*sort_array), point_with_index_dt, comm, routine, __LINE__);
...
static void detect_duplicated_cells_parallel(
  ...
  char const * routine = "detect_duplicated_cells_parallel";
  ...
  yac_alltoallv_yac_int_p2p(
    ...
    dist_cell_ids, recvcounts, rdispls, comm, routine, __LINE__);
  yac_alltoallv_p2p(
    ...
    cell_to_vertex_ids_dt, comm, routine, __LINE__);
  ...
  yac_alltoallv_yac_int_p2p(
    ...
    recv_local_cell_ids, sendcounts, sdispls, comm, routine, __LINE__);
  yac_free_comm_buffers(sendcounts, recvcounts, sdispls, rdispls);
...
struct yac_basic_grid_data yac_read_scrip_cloud_basic_grid_data(
  ...
  char const * grid_name, int valid_mask_value) {
   ...
   UNUSED(valid_mask_value);
   die(
     "ERROR(yac_read_scrip_cloud_basic_grid_data): "
     "YAC is built without the NetCDF support");
...
struct yac_basic_grid * yac_read_scrip_cloud_basic_grid(
  ...
  yac_int ** orig_vertex_global_ids, size_t * nbr_duplicated_vertices) {
   ...
   UNUSED(valid_mask_value);
   UNUSED(name);
   UNUSED(vertex_coord_idx);
   UNUSED(duplicated_vertex_idx);
   UNUSED(orig_vertex_global_ids);
   UNUSED(nbr_duplicated_vertices);
   die(
     "ERROR(yac_read_scrip_cloud_basic_grid): "
     "YAC is built without the NetCDF support");
...
struct yac_basic_grid * yac_read_scrip_cloud_basic_grid_parallel(
  ...
  size_t * nbr_duplicated_vertices) {
   ...
   UNUSED(nbr_duplicated_vertices);
   die(
     "ERROR(yac_read_scrip_cloud_basic_grid_parallel): "
     "YAC is built without the NetCDF support");
...
struct yac_basic_grid * yac_read_scrip_generic_basic_grid_parallel(
  ...
  size_t * nbr_duplicated_points, int * point_location) {
   ...
   UNUSED(name);
   UNUSED(use_ll_edges);
   UNUSED(point_coord_idx);
   UNUSED(duplicated_point_idx);
   UNUSED(orig_point_global_ids);
   UNUSED(nbr_duplicated_points);
   UNUSED(point_location);
   die(
     "ERROR(yac_read_scrip_generic_basic_grid_parallel): "
     "YAC is built without the NetCDF support");
...
struct yac_basic_grid * yac_read_scrip_basic_grid_parallel_f2c(
  ...
  size_t * nbr_duplicated_cells) {
   ...
   UNUSED(nbr_duplicated_cells);
   die(
     "ERROR(yac_read_scrip_basic_grid_parallel_f2c): "
     "YAC is built without the NetCDF support");
   return NULL;
}
...
struct yac_basic_grid * yac_read_scrip_cloud_basic_grid_parallel_f2c(
  ...
  size_t * nbr_duplicated_vertices) {
   ...
   UNUSED(nbr_duplicated_vertices);
   die(
     "ERROR(yac_read_scrip_cloud_basic_grid_parallel_f2c): "
     "YAC is built without the NetCDF support");
   return NULL;
}
...
struct yac_basic_grid * yac_read_scrip_generic_basic_grid_parallel_f2c(
  ...
  size_t * nbr_duplicated_points, int * point_location) {
   ...
   UNUSED(point_location);
   die(
     "ERROR(yac_read_scrip_generic_basic_grid_parallel_f2c): "
     "YAC is built without the NetCDF support");
   return NULL;
}
