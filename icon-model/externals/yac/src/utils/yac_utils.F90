...
module yac_utils
  ...
  interface
    ...
    function yac_read_scrip_generic_basic_grid_parallel_c( &
      grid_filename, mask_filename, comm, grid_name, valid_mask_value, name, &
      use_ll_edges, point_coord_idx, duplicated_point_idx, &
      orig_point_global_id, nbr_duplicated_points, point_location) &
      bind ( c, name='yac_read_scrip_generic_basic_grid_parallel_f2c' )
      ...
      integer(kind=c_size_t)                 :: point_coord_idx
      type(c_ptr)                            :: duplicated_point_idx ! size_t **
      type(c_ptr)                            :: orig_point_global_id ! yac_int **
      integer(kind=c_size_t)                 :: nbr_duplicated_points
      integer(kind=c_int)                    :: point_location

      ! struct yac_basic_grid *
      type(c_ptr) :: yac_read_scrip_generic_basic_grid_parallel_c
    end function yac_read_scrip_generic_basic_grid_parallel_c
  ...
  !-------------------------
  ! Reading SCRIP cloud grid
  !-------------------------

  interface

    function yac_read_scrip_cloud_basic_grid_parallel_c( &
      grid_filename, mask_filename, comm, grid_name, valid_mask_value, name, &
      vertex_coord_idx, duplicated_vertex_idx, &
      orig_vertex_global_id, nbr_duplicated_vertices) &
      bind ( c, name='yac_read_scrip_cloud_basic_grid_parallel_f2c' )
      ...
      type(c_ptr)                            :: duplicated_vertex_idx ! size_t **
      type(c_ptr)                            :: orig_vertex_global_id ! yac_int **
      integer(kind=c_size_t)                 :: nbr_duplicated_vertices

      ! struct yac_basic_grid *
      type(c_ptr) :: yac_read_scrip_cloud_basic_grid_parallel_c
    end function yac_read_scrip_cloud_basic_grid_parallel_c

    function yac_read_scrip_cloud_basic_grid_c( &
      grid_filename, mask_filename, grid_name, valid_mask_value, name, &
      vertex_coord_idx, duplicated_vertex_idx, &
      orig_vertex_global_id, nbr_duplicated_vertices) &
      bind ( c, name='yac_read_scrip_cloud_basic_grid' )
      ...
      ! struct yac_basic_grid *
      type(c_ptr) :: yac_read_scrip_cloud_basic_grid_c
    end function yac_read_scrip_cloud_basic_grid_c
