...
static size_t do_search_callback (struct interp_method * method,
  ...
  char const * routine = "do_search_callback";
  ...
  yac_alltoallv_p2p(
    ...
    sizeof(*request_recv_buffer), request_data_dt, comm, routine, __LINE__);
  ...
  YAC_ASSERT_F(
    ...
    "ERROR(%s): no callback routine defined on source process", routine)
  ...
  yac_alltoallv_yac_int_p2p(
    ...
    global_ids, recvcounts, rdispls, comm, routine, __LINE__);
...
void yac_interp_method_callback_buf_free() {
  ...
  for (size_t i = 0; i < callback_lookup_table_size; ++i)
    free(callback_lookup_table[i].key);
  free(callback_lookup_table);
  callback_lookup_table = NULL;
  callback_lookup_table_array_size = 0;
  callback_lookup_table_size = 0;
}
