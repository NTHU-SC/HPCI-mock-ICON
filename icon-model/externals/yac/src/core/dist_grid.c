...
static void generate_ce_ids(
  struct yac_basic_grid_data * grid_data, int * vertex_ranks,
  int max_num_vertices_per_cell, MPI_Comm comm) {

  char const * routine = "generate_ce_ids";
  ...
  YAC_ASSERT_F(
    ...
    "ERROR(%s): inconsistent global ids", routine)
  ...
  yac_alltoallv_yac_int_p2p(
    ...
    recv_buffer, total_recvcounts, total_rdispls, comm, routine, __LINE__);
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    YAC_ASSERT_F(
      ...
      "ERROR(%s): global_id out of bounds", routine)
...
static void generate_global_ids(
  ...
  YAC_ASSERT_F(
    ...
    "ERROR(generate_global_ids): internal error grid \"%s\" does not "
    "have global vertex ids", yac_basic_grid_get_name(grid));

  // generate global ids and core masks for all cell and edge
  // (if non-existent and required)
  generate_ce_ids(grid_data, vertex_ranks, max_num_vertices_per_cell, comm);
}
...
static void determine_dist_edge_ranks(
  ...
  yac_size_t_2_pointer edge_to_cell =
    generate_edge_to_cell(
      ...
      grid_data->core_cell_mask, num_cells, num_edges);
...
// (dist owners are sorted by global ids)
static void generate_dist_remote_points(
  struct proc_sphere_part_node * proc_sphere_part,
  struct yac_basic_grid * grid, int * vertex_ranks,
  int max_num_vertices_per_cell, MPI_Comm comm,
  struct remote_point_infos ** dist_point_infos, yac_int ** dist_global_ids,
  size_t * dist_count) {

  char const * routine = "generate_dist_remote_points";
  ...
  struct remote_point * unique_ids = NULL;
  ...
  for (int location = 0; location < 3; ++location) {
    ...
    unique_ids = xrealloc(unique_ids, count * sizeof(*unique_ids));
    ...
    struct remote_point_infos * point_infos =
      ((dist_point_infos[location] =
          xmalloc(num_unique_ids * sizeof(*(dist_point_infos[location])))));
    yac_int * global_ids =
      ((dist_global_ids[location] =
          xmalloc(num_unique_ids * sizeof(*(dist_global_ids[location])))));
    dist_count[location] = num_unique_ids;

    // compact received information
    // (each global id is only stored once and can have multiple
    //  original owners)
    for (size_t i = 0, l = 0; i < num_unique_ids; ++i) {
      global_ids[i] = unique_ids[i].global_id;
      int curr_count = unique_ids[i].data.count;
      point_infos[i].count = curr_count;
      if (curr_count == 1) {
        point_infos[i].data.single = temp_buffer[l].data;
        ++l;
      } else {
        point_infos[i].data.multi =
          xmalloc(
            (size_t)curr_count *
            sizeof(*(point_infos[i].data.multi)));
        for (int k = 0; k < curr_count; ++k, ++l) {
          point_infos[i].data.multi[k] = temp_buffer[l].data;
        }
      }
    }
  } // location
...
static void generate_sorted_ids(
  yac_int * global_ids, size_t count,
  yac_int ** sorted_global_ids, size_t ** reorder_idx) {

  *sorted_global_ids = xmalloc(count * sizeof(**sorted_global_ids));
  memcpy(*sorted_global_ids, global_ids, count * sizeof(**sorted_global_ids));
  *reorder_idx = xmalloc(count * sizeof(**reorder_idx));
...
static void redistribute_vertex_data(
  ...
  struct remote_point_infos * dist_vertex_infos, size_t num_vertices,
  MPI_Comm comm, MPI_Datatype dt_coord, int * vertex_ranks,
  yac_coordinate_pointer * vertex_coordinates_, int ** vertex_owner_,
  struct yac_field_data ** vertex_field_data_) {

  struct yac_basic_grid_data * grid_data = yac_basic_grid_get_data(grid);

  // generate a exchange map: user -> YAC decomposition
  yac_xmap xmap =
    yac_xmap_from_point_infos(dist_vertex_infos, num_vertices, comm);

  // generate redistributing objects for vertex data
  Xt_redist redist_vertex_coords = yac_xmap_generate_redist(xmap, dt_coord);
  Xt_redist redist_vertex_int = yac_xmap_generate_redist(xmap, MPI_INT);
...
static void redistribute_edge_data(
  struct yac_basic_grid * grid,
  struct remote_point_infos * dist_edge_infos, size_t num_edges, MPI_Comm comm,
  MPI_Datatype dt_coord, size_t num_vertices, yac_int * sorted_vertex_ids,
  ...
  yac_xmap xmap = yac_xmap_from_point_infos(dist_edge_infos, num_edges, comm);
  ...
  Xt_redist redist_edge_int = yac_xmap_generate_redist(xmap, MPI_INT);
  Xt_redist redist_edge_coords = yac_xmap_generate_redist(xmap, dt_coord);
  Xt_redist redist_edge_2yac_int = yac_xmap_generate_redist(xmap, dt_2yac_int);
...
static void redistribute_cell_data(
  struct yac_basic_grid * grid,
  struct remote_point_infos * dist_cell_infos, size_t num_cells, MPI_Comm comm,
  ...
  // generate a yaxt exchange map user -> YAC decomposition
  yac_xmap xmap = yac_xmap_from_point_infos(dist_cell_infos, num_cells, comm);
  ...
  Xt_redist redist_cell_int = yac_xmap_generate_redist(xmap, MPI_INT);
  Xt_redist redist_cell_yac_ints = yac_xmap_generate_redist(xmap, dt_yac_ints);
  Xt_redist redist_cell_coords = yac_xmap_generate_redist(xmap, dt_coord);
...
static struct yac_dist_grid generate_dist_grid(
  struct proc_sphere_part_node * proc_sphere_part, int * vertex_ranks,
  struct yac_basic_grid * grid, MPI_Comm comm) {
  ...
  // generate global ids for cells/vertices/edges (if they are missing)
  generate_global_ids(
    grid, vertex_ranks, max_num_vertices_per_cell, comm);

  // generate owner information for all cell/vertices/edges that may be
  // assigned to the local process in the YAC internal decomposition
  // (dist owners are sorted by global ids)
  struct remote_point_infos * dist_point_infos[3];
  yac_int * dist_global_ids[3];
  size_t dist_count[3];
  generate_dist_remote_points(
    proc_sphere_part, grid, vertex_ranks, max_num_vertices_per_cell, comm,
    dist_point_infos, dist_global_ids, dist_count);
  size_t num_cells = dist_count[YAC_LOC_CELL];
  size_t num_vertices = dist_count[YAC_LOC_CORNER];
  size_t num_edges = dist_count[YAC_LOC_EDGE];
  yac_int * cell_ids = dist_global_ids[YAC_LOC_CELL];
  yac_int * vertex_ids = dist_global_ids[YAC_LOC_CORNER];
  yac_int * edge_ids = dist_global_ids[YAC_LOC_EDGE];

  // generate sorted ids (the initial data is already sorted, which makes this
  // a simple copy)
  ...
  generate_sorted_ids(
    cell_ids, num_cells, &sorted_cell_ids, &sorted_cell_reorder_idx);
  generate_sorted_ids(
    vertex_ids, num_vertices, &sorted_vertex_ids,
    &sorted_vertex_reorder_idx);
  generate_sorted_ids(
    edge_ids, num_edges, &sorted_edge_ids, &sorted_edge_reorder_idx);
  ...
  redistribute_vertex_data(
    grid, dist_point_infos[YAC_LOC_CORNER], num_vertices,
    comm, dt_coord, vertex_ranks,
  ...
  redistribute_edge_data(
    grid, dist_point_infos[YAC_LOC_EDGE], num_edges, comm, dt_coord,
  ...
  redistribute_cell_data(
    grid, dist_point_infos[YAC_LOC_CELL], num_cells, comm, dt_coord,
  ...
  struct yac_dist_grid dist_grid =
    {.comm = comm,
     ...
     .owners =
       {dist_point_infos[YAC_LOC_CELL],
        dist_point_infos[YAC_LOC_CORNER],
        dist_point_infos[YAC_LOC_EDGE]}};
...
static struct proc_sphere_part_node * generate_dist_grid_decomposition(
  ...
  // redistribute all vertices and build parallel sphere
  // part for vertices
  struct proc_sphere_part_node * proc_sphere_part;
  yac_int **global_vertex_ids[2] =
    {&(grid_data[0]->vertex_ids), &(grid_data[1]->vertex_ids)};
  yac_proc_sphere_part_new(
    (yac_coordinate_pointer[2]){
      grid_data[0]->vertex_coordinates, grid_data[1]->vertex_coordinates},
      (size_t[2]){grid_data[0]->num_vertices, grid_data[1]->num_vertices},
      &proc_sphere_part, global_vertex_ids, vertex_ranks, comm);
...
struct yac_dist_grid_pair * yac_dist_grid_pair_new(
  ...
  char const * routine = "yac_dist_grid_pair_new";

  YAC_ASSERT_F(
    grid_a, "ERROR(%s): NULL is not a valid value for parameter grid_a",
    routine)
  YAC_ASSERT_F(
    grid_b, "ERROR(%s): NULL is not a valid value for parameter grid_b",
    routine)

  YAC_ASSERT_F(
    strcmp(yac_basic_grid_get_name(grid_a), yac_basic_grid_get_name(grid_b)),
    "ERROR(%s): identical grid names", routine)
  ...
  //     is roughly the same
  int * vertex_ranks[2] = {NULL, NULL};
  grid_pair->proc_sphere_part =
    generate_dist_grid_decomposition(
      grid_a, grid_b, (int**[2]){&vertex_ranks[0], &vertex_ranks[1]}, comm);

  // redistribute grid_a and grid_b according to decomposition
  grid_pair->dist_grid[0] =
    generate_dist_grid(
      grid_pair->proc_sphere_part, vertex_ranks[0], grid_a, comm);
  grid_pair->dist_grid[1] =
    generate_dist_grid(
      grid_pair->proc_sphere_part, vertex_ranks[1], grid_b, comm);
...
static void yac_dist_grid_single_remote_point_to_local(
  ...
  yac_alltoallv_uint64_p2p(
    ...
    "yac_dist_grid_single_remote_point_to_local", __LINE__);
...
void yac_dist_grid_pair_do_point_search_(
  ...
  char const * routine = "yac_dist_grid_pair_do_point_search_";
  ...
  yac_alltoallv_p2p(
    ...
    sizeof(*coord_send_buffer), dt_coord, comm, routine, __LINE__);
  ...
  yac_alltoallv_p2p(
    ...
    sizeof(*id_send_buffer), single_remote_point_dt, comm, routine, __LINE__);
...
void yac_dist_grid_pair_do_nnn_search(
  ...
  char const * routine = "yac_dist_grid_pair_do_nnn_search";
  ...
  YAC_ASSERT_F(
    ...
    "ERROR(%s): invalid max_search_distance (%lf)",
    routine, max_search_distance)
  ...
  if (flag) {
    ...
    YAC_ASSERT_F(
      (size_t)global_num_unmasked_count >= n, "ERROR(%s): "
      "insufficient number of unmasked points (available: %zu required: %zu",
      routine, (size_t)global_num_unmasked_count, n)
  ...
  yac_alltoallv_p2p(
    ...
    sizeof(*send_bnd_circles), bnd_circle_dt, comm, routine, __LINE__);
  ...
  yac_point_sphere_part_search_NNN_bnd_circle(
    sphere_part, recv_count, recv_bnd_circles, n, &result_points,
    &result_points_array_size, num_results_points);
...
void yac_dist_grid_pair_do_bnd_circle_search(
  ...
  char const * routine = "yac_dist_grid_pair_do_bnd_circle_search";
  ...
  yac_alltoallv_p2p(
    ...
    sizeof(*send_buffer), bnd_circle_dt, comm, routine, __LINE__);
...
static void yac_dist_grid_get_cell_neighbours(
  ...
  char const * routine = "yac_dist_grid_get_cell_neighbours";
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    for (size_t j = 0, edge_idx = 0; j < curr_num_edges; ++j, ++neigh_idx) {
      ...
      YAC_ASSERT_F(
        ...
        "ERROR(%s): inconsistent cell_to_edge/edge_to_vertex data", routine)
  ...
  { // get the missing neighbours
    ...
    yac_alltoallv_yac_int_p2p(
      ...
      recv_buffer, recvcounts, rdispls, comm, routine, __LINE__);
    ...
    for (size_t i = 0, j = 0; i < recv_count; ++i) {
      ...
      YAC_ASSERT_F(
        ...
        "ERROR(%s): inconsistent cell edge grid data", routine)
    ...
    for (size_t i = 0, prev_reorder_idx = SIZE_MAX; i < send_count; ++i) {
      ...
      if (curr_reorder_idx != prev_reorder_idx){
        ...
        YAC_ASSERT_F(
          prev_global_id == curr_global_id,
          "ERROR(%s): inconsistent cell edge data", routine)
      }
    }
...
void yac_dist_grid_global_to_local(
  ...
  char const * routine = "yac_dist_grid_global_to_local";
  ...
  yac_alltoallv_packed_p2p(
    ...
    recv_buffer, total_recvcounts, rdispls, comm, routine, __LINE__);
  ...
  for (size_t i = 0, j = 0; i < num_requested_ids; ++i) {
    ...
    YAC_ASSERT_F(
      ...
      "ERROR(%s): no matching core point found for global id %zu",
      routine, (size_t)curr_global_id)
...
static void relocate_points(
  ...
  char const * routine = "relocate_points";
...
static void relocate_weights(
  ...
  char const * routine = "relocate_weights";
  ...
  yac_alltoallv_p2p(
    ...
    sizeof(*send_buffer), MPI_DOUBLE, comm, routine, __LINE__);
...
yac_size_t_2_pointer yac_dist_grid_generate_edge_to_cell(
  struct yac_dist_grid_pair * grid_pair, char const * grid_name) {

  struct yac_dist_grid * dist_grid =
    yac_dist_grid_pair_get_dist_grid(grid_pair, grid_name);

  size_t num_cells = dist_grid->total_count[YAC_LOC_CELL];
  if (num_cells == 0) return NULL;

  size_t num_edges = dist_grid->total_count[YAC_LOC_EDGE];
  int * num_edges_per_cell = dist_grid->num_vertices_per_cell;
  size_t * cell_to_edge = dist_grid->cell_to_edge;
  yac_size_t_2_pointer edge_to_vertex = dist_grid->edge_to_vertex;
  yac_size_t_2_pointer edge_to_cell =
    xmalloc(num_edges * sizeof(*edge_to_cell));
  ...
  for (size_t i = 0, offset = 0; i < num_cells; ++i) {
    ...
    int curr_num_edges = num_edges_per_cell[i];
    const_size_t_pointer curr_cell_to_edge = cell_to_edge + offset;
    offset += curr_num_edges;
    ...
    for (int j = 0; j < curr_num_edges; ++j) {
      ...
      // skip if the current edge has a length of zero
      if (edge_to_vertex[curr_edge][0] == edge_to_vertex[curr_edge][1])
        continue;
      ...
      YAC_ASSERT_F(
        *curr_edge_to_cell == SIZE_MAX,
        "ERROR(yac_dist_grid_generate_edge_to_cell): "
        "more than two cells point to a single edge "
        "(does the grid contain degenrated cells (less than 3 corners) "
        "or duplicated cells; "
        "these can be masked out using the core mask)\n"
        "(grid_name: \"%s\" num_cells: %zu cell_idx: %zu: num_cell_edge %d "
        "cell_ids(global): %" XT_INT_FMT ", %" XT_INT_FMT ", %" XT_INT_FMT " "
        "cell_ids(local): %zu, %zu, %zu)",
        grid_name, num_cells, i, curr_num_edges,
        dist_grid->ids[YAC_LOC_CELL][edge_to_cell[curr_edge][0]],
        dist_grid->ids[YAC_LOC_CELL][edge_to_cell[curr_edge][1]],
        dist_grid->ids[YAC_LOC_CELL][i],
        edge_to_cell[curr_edge][0], edge_to_cell[curr_edge][1], i)
      *curr_edge_to_cell = i;
    }
  }
