...
struct yac_interpolation_raw {
  ...
  struct yac_src_field_exchange_data {
    struct yac_src_field_exchange_data_msgs {
...
struct yac_interp_weights * yac_interp_weights_new(
  ...
  yac_mpi_call(MPI_Comm_dup(comm, &weights->comm), comm);
...
static inline int remote_point_is_valid(
  struct remote_point point, int * rank_is_target) {

  const int count = point.data.count;
  struct remote_point_info * point_infos =
    (count == 1)?&(point.data.data.single):point.data.data.multi;

  int is_valid = 0;
  for (int i = 0; i < count; ++i)
    is_valid |= rank_is_target[point_infos[i].rank];

  return is_valid;
}

static inline struct remote_point copy_remote_point_masked(
  struct remote_point point, int * rank_is_target) {

  const int count = point.data.count;
  struct remote_point_info * remote_point_infos =
    (count == 1)?&(point.data.data.single):point.data.data.multi;

  int valid_count = 0;
  for (int i = 0; i < count; ++i) {
    if (rank_is_target[remote_point_infos[i].rank]) valid_count++;
  }

  struct remote_point remote_point_copy =
    {.global_id = point.global_id, .data.count = valid_count};
  struct remote_point_info * remote_point_infos_copy;
  if (valid_count > 1) {
    remote_point_infos_copy =
      xmalloc((size_t)valid_count * sizeof(*remote_point_infos_copy));
    remote_point_copy.data.data.multi = remote_point_infos_copy;
  } else {
    remote_point_infos_copy = &(remote_point_copy.data.data.single);
  }

  for (int i = 0, j = 0; i < count; ++i) {
    if (rank_is_target[remote_point_infos[i].rank]) {
      remote_point_infos_copy[j] = remote_point_infos[i];
      ++j;
    }
  }
  return remote_point_copy;
}
  ...
  const MPI_Aint array_of_displacements[] =
    {(MPI_Aint)(intptr_t)(const void *)&(dummy.value) -
     ...
     (MPI_Aint)(intptr_t)(const void *)&(dummy.orig_pos) -
       (MPI_Aint)(intptr_t)(const void *)&dummy};
  const MPI_Datatype array_of_types[] = {MPI_DOUBLE, YAC_MPI_SIZE_T};
  yac_mpi_call(
...
static void interp_add_fixed(
  void * interp, double fixed_value, size_t count, size_t * tgt_pos) {

  yac_interpolation_add_fixed(
    (struct yac_interpolation*)interp, fixed_value, count, tgt_pos);
}

static void interp_raw_add_fixed(
  void * interp, double fixed_value, size_t count, size_t * tgt_pos) {

  struct yac_interp_weights_data * interp_weights_data =
    &((struct yac_interpolation_raw *)interp)->interp_weights_data;

  interp_weights_data->fixed_values =
    xrealloc(
      interp_weights_data->fixed_values,
      (interp_weights_data->num_fixed_values + 1) *
      sizeof(*interp_weights_data->fixed_values));
  interp_weights_data->fixed_values[interp_weights_data->num_fixed_values] =
    fixed_value;
  ...
  interp_weights_data->num_tgt_per_fixed_value =
    xrealloc(
      interp_weights_data->num_tgt_per_fixed_value,
      (interp_weights_data->num_fixed_values + 1) *
      sizeof(*interp_weights_data->num_tgt_per_fixed_value));
  interp_weights_data->num_tgt_per_fixed_value[
    interp_weights_data->num_fixed_values] = count;

  size_t total_num_fixed_tgt = 0;
  for (size_t i = 0; i < interp_weights_data->num_fixed_values; ++i)
    total_num_fixed_tgt += interp_weights_data->num_tgt_per_fixed_value[i];
  interp_weights_data->tgt_idx_fixed =
    xrealloc(
      interp_weights_data->tgt_idx_fixed,
      (total_num_fixed_tgt + count) *
      sizeof(*interp_weights_data->tgt_idx_fixed));
  memcpy(
    interp_weights_data->tgt_idx_fixed + total_num_fixed_tgt,
    tgt_pos, count * sizeof(*tgt_pos));

  interp_weights_data->num_fixed_values++;
}

static void yac_interp_weights_redist_fixed(
  MPI_Comm comm, size_t count,
  struct interp_weight_stencil * fixed_stencils,
  void * interp, void (*interp_add_fixed)(void*, double, size_t, size_t*),
  int * rank_is_target) {

  //---------------------------------------------------------------------------
  // redistribute fixed stencils to owners of fixed target points
  // (a target point can be owned by multiple processes)
  //---------------------------------------------------------------------------
  ...
  // count the number of fixed stencils that need to be sent to each process
  for (size_t i = 0; i < count; ++i) {
  ...
  // if a process is not a target, but there are stencils available for it,
  // ignore them
  for (int i = 0; i < comm_size; ++i)
    if (!rank_is_target[i]) sendcounts[i] = 0;
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    for (int j = 0; j < curr_count; ++j) {
      int tgt_rank = curr_point_infos[j].rank;
      if (rank_is_target[tgt_rank]) {
        size_t pos = sdispls[tgt_rank + 1]++;
  ...
  // redistribute fixed stencils to owners of respective target points
  yac_alltoallv_p2p(
    send_buffer, sendcounts, sdispls, recv_buffer, recvcounts, rdispls,
    sizeof(*send_buffer), stencil_fixed_dt, comm,
    "yac_interp_weights_redist_fixed", __LINE__);
  ...
  //---------------------------------------------------------------------------
  // store received stencils in interp data structure
  //---------------------------------------------------------------------------
...
// a source point may have multiple owners, this routine returns one of them
static inline struct remote_point_info select_src(
  char const * routine, struct remote_point_infos src, int * rank_is_source) {

  struct remote_point_info src_point_info = {.rank = -1};

  if (src.count == 1) {

    src_point_info = src.data.single;
    ...
    for (int i = 0; i < src.count; ++i) {
      if ((src.data.multi[i].rank < min_rank) &&
          (rank_is_source[src.data.multi[i].rank])) {
    ...
    src_point_info =
      src.data.multi[(min_rank_idx != SIZE_MAX)?min_rank_idx:0];
  }

  YAC_ASSERT_F(
    rank_is_source[src_point_info.rank],
    "ERROR(%s): missing source field data from rank %d",
    routine, src_point_info.rank);

  return src_point_info;
}
 ...
 * @param[in] src_orig_poses positions of source points that have to be
 *                           sent to other processes
 * @param[in] sendcounts     number of source points to be sent to each process
 * @param[in] tgt_stencils   stencils containing local of target points that
 *                           will be overwritten by received source points
 * @param[in] recvcounts     number of source points to be received by each
 *                           process
 * @param[in] comm           MPI communicator for which the redist is to be
 *                           generated
 * @param[in] redist_config  configuration information for the yaxt redist
 *                           (e.g. yaxt exchanger to be used)
 */
static Xt_redist generate_direct_redist(
  size_t * src_orig_poses, size_t * sendcounts,
  struct interp_weight_stencil_direct * tgt_stencils,
  size_t * recvcounts, MPI_Comm comm, Xt_config redist_config) {
  ...
  // generate yaxt send and receive messages
  nsend = 0;
  nrecv = 0;
  for (int i = 0; i < comm_size; ++i) {
    if (recvcounts[i] > 0) {
  ...
  // only processes that have to sent/receive data are included in the redist,
  // the others receive a dummy redist
  if (total_num_msg > 0) {

    // generate MPI communicator containing all ranks taking part in the
    // exchange
    ...
    // translate the ranks in the sent/receive messages to ones in the
    // split comm
    yac_mpi_call(
    ...
    redist =
      xt_redist_single_array_base_custom_new(
        nsend, nrecv, send_msgs, recv_msgs, split_comm, redist_config);
...
/** generates a yaxt redist for a direct transfer of source to target points
 *  for cases with multiple source fields
 * @param[in] src_orig_poses positions of source points that have to be
 *                           sent to other processes
 * @param[in] sendcounts     number of source points to be sent to each process
 * @param[in] tgt_stencils   stencils containing local of target points that
    ...
    for (size_t src_field_idx = 0; src_field_idx < num_src_fields;
      ...
      redists[src_field_idx] =
        xt_redist_single_array_base_custom_new(
          ...
          send_msgs, recv_msgs, split_comm, redist_config);
...
static MPI_Datatype get_direct_stencil_mpi_datatype(MPI_Comm comm) {
  ...
  MPI_Datatype array_of_types[] =
    {yac_get_remote_point_info_mpi_datatype(comm), YAC_MPI_SIZE_T};
  yac_mpi_call(
    MPI_Type_create_struct(2, array_of_blocklengths, array_of_displacements,
...
static void interp_add_direct(
  void * interp, size_t * src_orig_poses, size_t * sendcounts,
  struct interp_weight_stencil_direct * tgt_stencils,
  size_t * recvcounts, MPI_Comm comm, Xt_config redist_config) {
  ...
  Xt_redist redist =
    generate_direct_redist(
      src_orig_poses, sendcounts, tgt_stencils, recvcounts,
      comm, redist_config);

  yac_interpolation_add_direct((struct yac_interpolation*)interp, redist);
...
static void yac_interp_weights_redist_direct(
  ...
  void * interp,
  void (*interp_add_direct)(
    void *, size_t *, size_t *, struct interp_weight_stencil_direct *,
    size_t *, MPI_Comm, Xt_config), Xt_config redist_config,
    int * rank_is_source, int * rank_is_target) {

  char const * routine = "yac_interp_weights_redist_direct";

  //---------------------------------------------------------------------------
  // redistribute direct stencils to owners of direct target points
  // (a target point can be owned by multiple processes)
  //---------------------------------------------------------------------------
  ...
  // count the number of direct stencils that need to be sent to each process
  for (size_t i = 0; i < count; ++i) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    struct remote_point_info src =
      select_src(
        routine, direct_stencils[i].data.direct.src.data, rank_is_source);
    for (int j = 0; j < curr_count; ++j) {
      int tgt_rank = curr_point_infos[j].rank;
      if (rank_is_target[tgt_rank]) {
        size_t pos = sdispls[tgt_rank + 1]++;
  ...
  size_t * orig_pos_buffer =
    xmalloc((send_buffer_size + recv_buffer_size) * sizeof(*orig_pos_buffer));
  size_t * send_orig_pos_buffer = orig_pos_buffer + recv_buffer_size;
  size_t * recv_orig_pos_buffer = orig_pos_buffer;
  ...
  yac_alltoallv_p2p(
    ...
    sizeof(*send_orig_pos_buffer), YAC_MPI_SIZE_T, comm,
    routine, __LINE__);
  ...
  interp_add_direct(
    interp, recv_orig_pos_buffer, recvcounts, recv_stencil_buffer, sendcounts,
    comm, redist_config);
  ...
  const MPI_Aint array_of_displacements[] =
    {(MPI_Aint)(intptr_t)(const void *)&(dummy.src) -
     ...
     (MPI_Aint)(intptr_t)(const void *)&(dummy.orig_pos) -
       (MPI_Aint)(intptr_t)(const void *)&dummy};
  MPI_Datatype array_of_types[] =
    {yac_get_remote_point_info_mpi_datatype(comm), YAC_MPI_SIZE_T, YAC_MPI_SIZE_T};
  yac_mpi_call(
...
static void interp_add_direct_mf(
  void * interp, size_t num_src_fields, size_t * src_orig_poses,
  size_t * sendcounts, struct interp_weight_stencil_direct_mf * tgt_stencils,
  size_t * recvcounts, MPI_Comm comm, Xt_config redist_config) {
  ...
  yac_interpolation_add_direct_mf(
    (struct yac_interpolation*)interp, redists, (size_t)num_src_fields);
...
static void yac_src_field_exchange_data_init(
  struct yac_src_field_exchange_data * src_field_exchange_data) {

  src_field_exchange_data->send.msg = NULL;
  src_field_exchange_data->send.num_msg = 0;
  src_field_exchange_data->recv.msg = NULL;
  src_field_exchange_data->recv.num_msg = 0;
}

// routine ensures that interp_raw can handle up to num_src_fields source fields
static void yac_src_field_exchange_data_realloc(
  struct yac_interpolation_raw * interp_raw, size_t num_src_fields) {

  if (interp_raw->interp_weights_data.num_src_fields < num_src_fields) {

    interp_raw->src_field_exchange_data =
      xrealloc(
        interp_raw->src_field_exchange_data,
        num_src_fields * sizeof(*interp_raw->src_field_exchange_data));
    interp_raw->interp_weights_data.src_field_buffer_size =
      xrealloc(
        interp_raw->interp_weights_data.src_field_buffer_size,
        num_src_fields *
        sizeof(*interp_raw->interp_weights_data.src_field_buffer_size));

    for (size_t i = interp_raw->interp_weights_data.num_src_fields;
         i < num_src_fields; ++i) {
      yac_src_field_exchange_data_init(interp_raw->src_field_exchange_data + i);
      interp_raw->interp_weights_data.src_field_buffer_size[i] = 0;
    }
    interp_raw->interp_weights_data.num_src_fields = num_src_fields;
  }
}

/** stores information on how to compute target points from given source
 * information
 * @param[in,out] interp_weights_data   interpolation weight data
 * @param[in]     num_src_fields        number of source fields
 * @param[in]     tgt_count             number of target points
 * @param[in]     tgt_idx               local ids of target points
 * @param[in]     num_src_per_tgt       number of source points per target point
 *                                      (if NULL, "1" is assumed for all
 *                                       targets)
 * @param[in]     weights               weights (if NULL, "1.0" is assumed for
 *                                      all targets)
 * @param[in]     src_field_idx         source field indices (if NULL, "0" is
 *                                      assumed for all target points)
 * @param[in]     src_idx               indices of source points in
 *                                      source field buffer (assuming that the
 *                                      source field buffer only contains the
 *                                      source points reference in this call)
 * @param[in]     src_field_buffer_size number of source points per source field
 */
static void yac_interp_weights_data_set_wgt_tgt(
  struct yac_interp_weights_data * interp_weights_data,
  size_t num_src_fields, size_t tgt_count, size_t * tgt_idx,
  size_t * num_src_per_tgt, double * weights, size_t * src_field_idx,
  size_t * src_idx, size_t * src_field_buffer_size) {

  // compute total number of weights
  size_t num_weights = 0;
  if (num_src_per_tgt) {
    for (size_t i = 0; i < tgt_count; ++i) num_weights += num_src_per_tgt[i];
  } else {
    num_weights = tgt_count;
  }

  // compute number of weights already stored in interp_weights_data
  size_t weights_offset = 0;
  for (size_t i = 0; i < interp_weights_data->num_wgt_tgt; ++i)
    weights_offset += interp_weights_data->num_src_per_tgt[i];

  // add local target indices to interp_weights_data
  interp_weights_data->wgt_tgt_idx =
    xrealloc(
      interp_weights_data->wgt_tgt_idx,
      (interp_weights_data->num_wgt_tgt + tgt_count) *
      sizeof(*(interp_weights_data->wgt_tgt_idx)));
  memcpy(
    interp_weights_data->wgt_tgt_idx + interp_weights_data->num_wgt_tgt,
    tgt_idx, tgt_count * sizeof(*tgt_idx));

  // add number of source points per target point to interp_weights_data
  interp_weights_data->num_src_per_tgt =
    xrealloc(
      interp_weights_data->num_src_per_tgt,
      (interp_weights_data->num_wgt_tgt + tgt_count) *
      sizeof(*(interp_weights_data->num_src_per_tgt)));
  if (num_src_per_tgt) {
    memcpy(
      interp_weights_data->num_src_per_tgt + interp_weights_data->num_wgt_tgt,
      num_src_per_tgt, tgt_count * sizeof(*num_src_per_tgt));
  } else {
    for (size_t i = 0, j = interp_weights_data->num_wgt_tgt; i < tgt_count;
         ++i, ++j)
      interp_weights_data->num_src_per_tgt[j] = 1;
  }

  // add weights to interp_weights_data
  interp_weights_data->weights =
    xrealloc(
      interp_weights_data->weights, (weights_offset + num_weights) *
      sizeof(*(interp_weights_data->weights)));
  if (weights) {
    memcpy(
      interp_weights_data->weights + weights_offset,
      weights, num_weights * sizeof(*weights));
  } else {
    for (size_t i = 0, j = weights_offset; i < num_weights; ++i, ++j)
      interp_weights_data->weights[j] = 1.0;
  }

  // add source field indices to interp_weights_data
  interp_weights_data->src_field_idx =
    xrealloc(
      interp_weights_data->src_field_idx, (weights_offset + num_weights) *
      sizeof(*(interp_weights_data->src_field_idx)));
  if (src_field_idx) {
    memcpy(
      interp_weights_data->src_field_idx + weights_offset,
      src_field_idx, num_weights * sizeof(*src_field_idx));
  } else {
    for (size_t i = 0, j = weights_offset; i < num_weights;
         ++i, ++j)
      interp_weights_data->src_field_idx[j] = 0;
    // in case no source field indices were provided, we reference the ones in
    // interp_weights_data
    src_field_idx = interp_weights_data->src_field_idx + weights_offset;
  }

  // add source indices to interp_weights_data
  // (since interp_weights_data may already contains source indices, we have to
  //  offset the ones added here by the number of source indices already in
  //  interp_weights_data)
  interp_weights_data->src_idx =
    xrealloc(
      interp_weights_data->src_idx, (weights_offset + num_weights) *
      sizeof(*(interp_weights_data->src_idx)));
  for (size_t i = 0, j = weights_offset; i < num_weights; ++i, ++j)
    interp_weights_data->src_idx[j] =
      src_idx[i] + interp_weights_data->src_field_buffer_size[src_field_idx[i]];

  // update src_field_buffer_size in interp_weights_data
  for (size_t i = 0; i < num_src_fields; ++i)
    interp_weights_data->src_field_buffer_size[i] += src_field_buffer_size[i];

  // update number of weighted target points
  interp_weights_data->num_wgt_tgt += tgt_count;
}

static struct yac_src_field_exchange_data_msg *
  yac_src_field_exchange_data_msgs_get_msg(
    struct yac_src_field_exchange_data_msgs * msgs, int rank) {

  // search for a matching message
  size_t msg_idx = 0;
  for (; msg_idx < msgs->num_msg; ++msg_idx)
    if (msgs->msg[msg_idx].rank == rank) break;

  // allocate entry, if non for the current rank exists
  if (msg_idx == msgs->num_msg) {
    msgs->num_msg++;
    msgs->msg = xrealloc(msgs->msg, msgs->num_msg * sizeof(*msgs->msg));
    msgs->msg[msg_idx] =
      (struct yac_src_field_exchange_data_msg)
        {.rank = rank, .pos = NULL, .count = 0};
  }

  return msgs->msg + msg_idx;
}

/**
 * adds new data to a source field exchange message
 * @param[in,out] msgs   array of existing source field exchange messages
 * @param[in]     rank   rank of communcation partner
 * @param[in]     count  number of positions to be added
 * @param[in]     pos    positions to be communicated
 * @param[in]     offset offset to be added to all positions that are to
 *                       be added
 */
static void yac_src_field_exchange_data_msgs_add(
  struct yac_src_field_exchange_data_msgs * msgs,
  int rank, size_t count, size_t * pos, size_t offset) {

  // search for a message matching the rank (create empty message if it does
  // not yet exist
  struct yac_src_field_exchange_data_msg * msg =
    yac_src_field_exchange_data_msgs_get_msg(msgs, rank);

  // add positions
  msg->pos =
    xrealloc(msg->pos, ((size_t)msg->count + count) * sizeof(*msg->pos));
  for (size_t i = 0; i < count; ++i, ++msg->count)
    msg->pos[msg->count] = pos[i] + offset;
}

/**
 * adds new data to a source field exchange messages
 * @param[in,out] src_field_exchange_data source field exchange messages
 * @param[in]     num_src_fields          number of source fields
 * @param[in]     comm                    MPI communicator
 * @param[in]     send_msg_sizes          array with send message sizes
 *                                        ([comm_size][num_src_fields])
 * @param[in]     send_pos                positions to be sent
 * @param[in]     recv_msg_sizes          array with receive messages sizes
 *                                        ([comm_size][num_src_fields])
 * @param[in]     recv_pos                positions to be received
 * @param[in]     recv_offsets            offsets to be added to receive
 *                                        messages ([num_src_fields])
 * @remark The send data is extracted from the field data provided by the user.
 * @remark The receive data is written to a buffer, which may already contain
 *         data. Hence the receive positions have to be offset by the amount of
 *         data already in the buffer.
 */
static void yac_src_field_exchange_data_add(
  struct yac_src_field_exchange_data * src_field_exchange_data,
  size_t num_src_fields, MPI_Comm comm,
  size_t * send_msg_sizes, size_t * send_pos,
  size_t * recv_msg_sizes, size_t * recv_pos,
  size_t * recv_offsets) {
  ...
  for (int rank = 0; rank < comm_size; ++rank) {
    ...
    for (size_t src_field_idx = 0; src_field_idx < num_src_fields;
         ++src_field_idx, ++msg_idx) {

      struct yac_src_field_exchange_data * curr_src_field_exchange_data =
        src_field_exchange_data + src_field_idx;

      // if data has to be sent to the current rank
      size_t send_msg_size = send_msg_sizes[msg_idx];
      if (send_msg_sizes[msg_idx] > 0) {

        yac_src_field_exchange_data_msgs_add(
          &curr_src_field_exchange_data->send, rank, send_msg_size,
          send_pos, 0);
        send_pos += send_msg_size;
      }

      // if data has to be received from the current rank
      size_t recv_msg_size = recv_msg_sizes[msg_idx];
      if (recv_msg_size > 0) {

        yac_src_field_exchange_data_msgs_add(
          &curr_src_field_exchange_data->recv, rank, recv_msg_size,
          recv_pos, recv_offsets[src_field_idx]);
        recv_pos += recv_msg_size;
      }
    }
  }
}

static void interp_add_direct_raw(
  void * interp, size_t * src_orig_poses, size_t * sendcounts,
  struct interp_weight_stencil_direct * tgt_stencils,
  size_t * recvcounts, MPI_Comm comm, Xt_config redist_config) {

  UNUSED(redist_config);

  struct yac_interpolation_raw * interp_raw =
    (struct yac_interpolation_raw *)interp;

  int comm_size;
  yac_mpi_call(MPI_Comm_size(comm, &comm_size), comm);

  size_t num_src_fields = 1;
  yac_src_field_exchange_data_realloc(interp_raw, num_src_fields);

  size_t total_sendcounts = 0, total_recvcounts = 0;
  for (int i = 0; i < comm_size; ++i) {
    total_sendcounts += sendcounts[i];
  ...
  size_t * send_pos = size_t_buffer;
  size_t * recv_pos = size_t_buffer + total_sendcounts;

  for (size_t i = 0; i < total_sendcounts; ++i)
    send_pos[i] = (size_t)src_orig_poses[i];
  for (size_t i = 0; i < total_recvcounts; ++i)
    recv_pos[i] = i;

  // store source field data that needs to be exchanged
  yac_src_field_exchange_data_add(
    interp_raw->src_field_exchange_data, num_src_fields, comm,
    sendcounts, send_pos, recvcounts, recv_pos,
    interp_raw->interp_weights_data.src_field_buffer_size);

  size_t * tgt_idx = size_t_buffer;
  size_t * src_idx = size_t_buffer + total_recvcounts;

  for (size_t i = 0; i < total_recvcounts; ++i) {
    tgt_idx[i] = (size_t)(tgt_stencils[i].orig_pos);
    src_idx[i] = i;
  }

  // store mapping formation between source points in the source field buffer
  // to the target points
  size_t * num_src_per_tgt = NULL;
  double * weights = NULL;
  size_t * src_field_idx = NULL;
  size_t src_field_buffer_size = total_recvcounts;
  yac_interp_weights_data_set_wgt_tgt(
    &(interp_raw->interp_weights_data), num_src_fields, total_recvcounts,
    tgt_idx, num_src_per_tgt, weights, src_field_idx, src_idx,
    &src_field_buffer_size);

  free(size_t_buffer);
}

static void interp_add_direct_mf_raw(
  void * interp, size_t num_src_fields, size_t * src_orig_poses,
  size_t * sendcounts, struct interp_weight_stencil_direct_mf * tgt_stencils,
  size_t * recvcounts, MPI_Comm comm, Xt_config redist_config) {
  ...
  size_t total_sendcounts = 0, total_recvcounts = 0;
  for (size_t rank = 0, idx = 0; rank < (size_t)comm_size; ++rank) {
    for (size_t src_field_idx = 0; src_field_idx < num_src_fields;
         ++src_field_idx, ++idx) {
      total_sendcounts += sendcounts[idx];
      total_recvcounts += recvcounts[idx];
    }
  }
  ...
  size_t * send_pos = size_t_buffer;
  size_t * recv_pos = size_t_buffer + total_sendcounts;
  size_t src_field_buffer_size[num_src_fields]; // number of source points to
                                                // be received per source field
  memset(
    src_field_buffer_size, 0,
    num_src_fields * sizeof(src_field_buffer_size[0]));

  for (size_t i = 0; i < total_sendcounts; ++i)
    send_pos[i] = (size_t)src_orig_poses[i];
  for (size_t rank = 0, idx = 0, recv_pos_idx = 0; rank < (size_t)comm_size;
       ++rank) {
    for (size_t src_field_idx = 0; src_field_idx < num_src_fields;
         ++src_field_idx, ++idx) {
      if (recvcounts[idx] > 0) {
        for (size_t i = 0; i < recvcounts[idx]; ++i, ++recv_pos_idx)
          recv_pos[recv_pos_idx] = src_field_buffer_size[src_field_idx]++;
      }
    }
  }
  ...
  size_t * tgt_idx = size_t_buffer;
  size_t * src_idx = size_t_buffer + total_recvcounts;
  size_t * src_field_idx = size_t_buffer + 2 * total_recvcounts;
  memset(
    src_field_buffer_size, 0,
    num_src_fields * sizeof(src_field_buffer_size[0]));

  for (size_t rank = 0, idx = 0, k = 0; rank < (size_t)comm_size; ++rank) {
    for (size_t j = 0; j < num_src_fields; ++j, ++idx) {

      if (recvcounts[idx] > 0) {
        for (size_t i = 0; i < recvcounts[idx]; ++i, ++k) {
          tgt_idx[k] = (size_t)(tgt_stencils[k].orig_pos);
          src_idx[k] = src_field_buffer_size[j]++;
          src_field_idx[k] = j;
        }
      }
    }
  ...
  // store mapping formation between source points in the source fields buffer
  // to the target points
  size_t * num_src_per_tgt = NULL;
  double * weights = NULL;
  yac_interp_weights_data_set_wgt_tgt(
...
static void yac_interp_weights_redist_direct_mf(
  ...
  void * interp,
  void (*interp_add_direct_mf)(
    void *, size_t, size_t *, size_t *,
    struct interp_weight_stencil_direct_mf *, size_t *, MPI_Comm, Xt_config),
    Xt_config redist_config, int * rank_is_source, int * rank_is_target) {

  char const * routine = "yac_interp_weights_redist_direct_mf";

  //---------------------------------------------------------------------------
  // redistribute multi field direct stencils to owners of direct target points
  // (a target point can be owned by multiple processes)
  //---------------------------------------------------------------------------
  ...
  for (size_t i = 0; i < count; ++i) {
    size_t src_field_idx = direct_mf_stencils[i].data.direct_mf.field_idx;
  ...
  // count the number of multi field direct stencils that need to be sent to
  // each process
  for (size_t i = 0; i < count; ++i) {
    ...
    for (int j = 0; j < curr_count; ++j)
      sendcounts[
        (size_t)(curr_point_info[j].rank) * num_src_fields + src_field_idx]++;
  }
  ...
  for (int i = 0; i < comm_size; ++i) {
    if (!rank_is_target[i]) {
      for (size_t j = 0; j < num_src_fields; ++j)
        sendcounts[(size_t)i * num_src_fields + j] = 0;
    }
  }
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    struct remote_point_info src =
      select_src(
        routine, direct_mf_stencils[i].data.direct_mf.src.data,
        rank_is_source);
    size_t src_field_idx = direct_mf_stencils[i].data.direct_mf.field_idx;
    for (int j = 0; j < curr_count; ++j) {
      int tgt_rank = curr_point_infos[j].rank;
      if (rank_is_target[tgt_rank]) {
        size_t pos =
          sdispls[(size_t)tgt_rank * num_src_fields +
                  src_field_idx + 1]++;
        send_stencil_buffer[pos].src = src;
  ...
  yac_alltoallv_p2p(
    ...
    "yac_interp_weights_redist_direct_mf", __LINE__);
  ...
  for (size_t i = 0; i < tgt_count; ++i)
    sendcounts[(size_t)(recv_stencil_buffer[i].src.rank) * num_src_fields +
  ...
  for (size_t i = 0; i < tgt_count; ++i)
    send_orig_pos_buffer[
      sdispls[(size_t)(recv_stencil_buffer[i].src.rank) * num_src_fields +
...
static int get_stencil_pack_size_direct_mf(
  ...
  yac_mpi_call(
    MPI_Pack_size(
      1, YAC_MPI_SIZE_T, comm, &pack_size_src_field_idx), comm);
...
static int get_stencil_pack_size_wsum_mf(
  ...
  yac_mpi_call(
    MPI_Pack_size(
      count, YAC_MPI_SIZE_T, comm, &pack_size_field_indices), comm);
...
static int get_stencil_pack_size_sum_mf(
  ...
  yac_mpi_call(
    MPI_Pack_size(
      ...
      YAC_MPI_SIZE_T, comm, &pack_size_field_indices), comm);
...
static void pack_stencil_direct_mf(
  ...
  // field_idx
  size_t temp_field_idx = stencil->data.direct_mf.field_idx;
  yac_mpi_call(
    MPI_Pack(&temp_field_idx, 1, YAC_MPI_SIZE_T,
             buffer, buffer_size, position, comm), comm);
}
...
static void pack_stencil_sum_mf(
  ...
  // field_indices
  size_t * temp_field_indices = xmalloc(count * sizeof(*temp_field_indices));
  for (size_t i = 0; i < count; ++i)
    temp_field_indices[i] = stencil->data.sum_mf.field_indices[i];
  yac_mpi_call(
    MPI_Pack(temp_field_indices, (int)count, YAC_MPI_SIZE_T,
             buffer, buffer_size, position, comm), comm);
...
static void pack_stencil_wsum_mf(
  ...
  size_t * temp_field_indices = xmalloc(count * sizeof(*temp_field_indices));
  for (size_t i = 0; i < count; ++i)
    temp_field_indices[i] = stencil->data.weight_sum_mf.field_indices[i];
  yac_mpi_call(
    MPI_Pack(temp_field_indices, (int)count, YAC_MPI_SIZE_T,
             buffer, buffer_size, position, comm), comm);
  free(temp_field_indices);
...
static void unpack_stencil_direct_mf(
  ...
  // field_idx
  size_t temp_field_idx;
  yac_mpi_call(
    MPI_Unpack(
      buffer, buffer_size, position, &temp_field_idx,
      1, YAC_MPI_SIZE_T, comm), comm);
...
static void unpack_stencil_sum_mf(
  ...
  yac_mpi_call(
    MPI_Unpack(
      ...
      (int)count, YAC_MPI_SIZE_T, comm), comm);
  for (size_t i = 0; i < count; ++i)
...
static struct interp_weight_stencil * exchange_stencils(
  ...
  yac_alltoallv_packed_p2p(
    ...
    recv_buffer, recvcounts, rdispls, comm, "exchange_stencils", __LINE__);
  yac_free_comm_buffers(sendcounts, recvcounts, sdispls, rdispls);
  free(send_buffer);
...
static struct interp_weight_stencil *  yac_interp_weights_get_stencils(
  ...
  char const * routine = "yac_interp_weights_get_stencils";
  ...
  YAC_ASSERT_F(
    count <= INT_MAX, "ERROR(%s): count exceeds INT_MAX", routine);
  ...
  size_t recv_count =
    rdispls[comm_size - 1] + recvcounts[comm_size - 1];
  size_t * size_t_buffer =
    xmalloc((count + recv_count) * sizeof(*size_t_buffer));
  size_t * send_stencil_indices = size_t_buffer;
  size_t * recv_stencil_indices = size_t_buffer + count;
  for (size_t i = 0; i < count; ++i)
    send_stencil_indices[i] = stencil_indices[i];
  yac_alltoallv_size_t_p2p(
    send_stencil_indices, sendcounts, sdispls+1,
    recv_stencil_indices, recvcounts, rdispls, comm, routine, __LINE__);
  ...
  for (size_t i = 0; i < recv_count; ++i) {
    YAC_ASSERT_F(
      ...
      "ERROR(%s): invalid stencil index", routine);
...
static struct interp_weight_stencils_wsum_mf * generate_w_sum_mf_stencils(
  ...
  char const * routine = "generate_w_sum_mf_stencils";
  ...
  for (size_t i = 0; i < count; ++i) {
    if (remote_point_is_valid(stencils[i].tgt, rank_is_target)) {
      YAC_ASSERT_F(
        stencils[i].type == stencil_type,
        "ERROR(%s): wrong stencil type", routine)
  ...
  wsum_stencils->data = xmalloc(valid_count * sizeof(*(wsum_stencils->data)));
  wsum_stencils->count = valid_count;
  ...
  for (size_t i = 0, j = 0, k = 0; i < count; ++i) {
    ...
    if (remote_point_is_valid(curr_stencil->tgt, rank_is_target)) {

      struct interp_weight_stencil_wsum_mf * curr_wsum_stencil =
        wsum_stencils->data + j;
      struct interp_weight_stencil_wsum_mf_weight * curr_links =
        &(temp->buffer[k]);
      size_t curr_stencil_size = curr_stencil->data.weight_sum.srcs->count;
      struct remote_point * curr_srcs =
        curr_stencil->data.weight_sum.srcs->data;
      curr_wsum_stencil->tgt =
        copy_remote_point_masked(curr_stencil->tgt, rank_is_target);
      curr_wsum_stencil->count = curr_stencil_size;
      curr_wsum_stencil->data = curr_links;
      for (size_t l = 0; l < curr_stencil_size; ++l) {
        int curr_count = curr_srcs[l].data.count;
        YAC_ASSERT_F(
          curr_count >= 1, "ERROR(%s): global src id no found", routine)
        curr_links[l].src =
          select_src(routine, curr_srcs[l].data, rank_is_source);

        switch(stencil_type) {
          YAC_UNREACHABLE_DEFAULT_F(
            "ERROR(%s): unsupported stencil type", routine);
          case(SUM):
            curr_links[l].weight = 1.0;
            curr_links[l].src_field_idx = 0;
            break;
          case(WEIGHT_SUM):
            curr_links[l].weight = curr_stencil->data.weight_sum.weights[l];
            curr_links[l].src_field_idx = 0;
            break;
          case(SUM_MF):
            curr_links[l].weight = 1.0;
            curr_links[l].src_field_idx =
              curr_stencil->data.sum_mf.field_indices[l];
            break;
          case(WEIGHT_SUM_MF):
            curr_links[l].weight = curr_stencil->data.weight_sum_mf.weights[l];
            curr_links[l].src_field_idx =
              curr_stencil->data.weight_sum_mf.field_indices[l];
            break;
        };
      }
      k += curr_stencil_size;
...
static MPI_Datatype get_wsum_mf_weight_mpi_datatype(MPI_Comm comm) {
  ...
  const MPI_Datatype array_of_types[] =
    {MPI_INT, YAC_MPI_SIZE_T, YAC_MPI_SIZE_T, MPI_DOUBLE};
  yac_mpi_call(
...
static struct interp_weight_stencils_wsum_mf * redist_wsum_mf_stencils(
  ...
  yac_alltoallv_packed_p2p(
    ...
    "redist_wsum_mf_stencils", __LINE__);
  yac_free_comm_buffers(sendcounts, recvcounts, sdispls, rdispls);
  free(send_buffer);
...
static Xt_redist * generate_halo_redists(
  ...
  size_t num_src_fields, MPI_Comm comm, Xt_config redist_config) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    size_t orig_pos = halo_points[i].data.orig_pos;
    YAC_ASSERT(
      orig_pos <= INT_MAX,
  ...
  yac_alltoallv_int_p2p(
    ...
    "generate_halo_redists", __LINE__);
...
static int compare_interp_weight_stencil_wsum_mf_tgt_orig_pos(
  ...
  YAC_ASSERT(
    ...
    "ERROR(compare_interp_weight_stencil_wsum_mf_tgt_orig_pos): invalid data")
...
static Xt_redist generate_redist_put_double(
  ...
  char const * routine = "generate_redist_put_double";
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    YAC_ASSERT_F(
      curr_count >= 1, "ERROR(%s): no owner found for global id", routine)
    for (int j = 0; j < curr_count; ++j)
      sendcounts[curr_point_infos[j].rank]++;
  }
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    for (int j = 0; j < curr_count; ++j) {
      ...
      size_t orig_pos = curr_point_infos[j].orig_pos;
      YAC_ASSERT_F(
        orig_pos <= INT_MAX, "ERROR(%s): offset not supported by MPI", routine)
      send_buffer[pos] = (int)orig_pos;
      reorder_idx[pos] = i;
    }
  }
  ...
  Xt_redist redist =
    xt_redist_single_array_base_custom_new(
      nsend, nrecv, send_msgs, recv_msgs, comm, redist_config);
...
static void interpolation_add_w_sum_mf(
  struct remote_point_info_reorder * remote_src_points, size_t halo_size,
  size_t num_src_fields, size_t tgt_count,
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
  enum yac_interp_weights_reorder_type reorder,
  struct yac_interpolation * interp,
  void (*interp_add_wsum_mf_at_src)(
    struct yac_interpolation *, Xt_redist *, size_t, size_t *, double *,
    size_t *, size_t *, size_t, Xt_redist),
  void (*interp_add_wsum_mf_at_tgt)(
    struct yac_interpolation *, Xt_redist *, size_t *, size_t, size_t *, double *,
    size_t *, size_t *, size_t), Xt_config redist_config) {

  // generate halo redists (one per source field)
  Xt_redist * halo_redists =
    generate_halo_redists(
      remote_src_points, halo_size, num_src_fields, comm, redist_config);
  ...
  if (reorder == YAC_MAPPING_ON_SRC) {
    ...
    struct remote_point_infos * tgt_infos =
      xmalloc(tgt_count * sizeof(*tgt_infos));
    for (size_t i = 0; i < tgt_count; ++i)
      tgt_infos[i] = tgt_stencils[i].tgt.data;
    Xt_redist result_redist =
      generate_redist_put_double(
        tgt_infos, tgt_count, comm, redist_config);
    free(tgt_infos);

    interp_add_wsum_mf_at_src(
      interp, halo_redists, tgt_count, num_src_per_tgt, weights,
    ...
    size_t * tgt_orig_pos = xmalloc(tgt_count * sizeof(*tgt_orig_pos));
    for (size_t i = 0; i < tgt_count; ++i) {
      YAC_ASSERT(
        tgt_stencils[i].tgt.data.count == 1,
        "ERROR(interpolation_add_w_sum_mf): currently unsupported target "
        "point distribution")
      tgt_orig_pos[i] =
        (size_t)(tgt_stencils[i].tgt.data.data.single.orig_pos);
    }

    interp_add_wsum_mf_at_tgt(
      interp, halo_redists, tgt_orig_pos, tgt_count,
      num_src_per_tgt, weights, src_field_idx, src_idx, num_src_fields);
    free(tgt_orig_pos);
...
static void interpolation_add_sum_at_src(
  ...
  yac_interpolation_add_sum_at_src(
    interp, halo_redists, tgt_count, num_src_per_tgt, src_field_idx,
    src_idx, num_src_fields, result_redist);
}

static void interpolation_add_sum_at_tgt(
  ...
  yac_interpolation_add_sum_at_tgt(
    interp, src_redists, tgt_pos, tgt_count, num_src_per_tgt,
    src_field_idx, src_idx, num_src_fields);
}
...
static void interpolation_add_sum_mf(
  ...
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
  enum yac_interp_weights_reorder_type reorder, void * interp,
  Xt_config redist_config) {

  interpolation_add_w_sum_mf(
    remote_src_points, halo_size, num_src_fields, tgt_count, tgt_stencils,
    num_src_per_tgt, weights, src_idx, src_field_idx, comm, reorder,
    (struct yac_interpolation *) interp,
    interpolation_add_sum_at_src, interpolation_add_sum_at_tgt,
    redist_config);
}

static void interpolation_add_sum(
  struct remote_point_info_reorder * remote_src_points, size_t halo_size,
  size_t num_src_fields, size_t tgt_count,
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
  ...
  interpolation_add_sum_mf(
    remote_src_points, halo_size, 1, tgt_count, tgt_stencils, num_src_per_tgt,
    weights, src_idx, src_field_idx, comm, reorder, interp, redist_config);
}

static void interpolation_add_wsum_mf(
  struct remote_point_info_reorder * remote_src_points, size_t halo_size,
  size_t num_src_fields, size_t tgt_count,
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
  ...
  interpolation_add_w_sum_mf(
    ...
    yac_interpolation_add_weight_sum_mvp_at_tgt, redist_config);
}

static void interpolation_add_wsum(
  struct remote_point_info_reorder * remote_src_points, size_t halo_size,
  size_t num_src_fields, size_t tgt_count,
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
...
static void yac_interp_weights_redist_w_sum_mf(
  ...
  struct yac_interpolation * interp,
  enum yac_interp_weights_reorder_type reorder,
  void (*interp_add_w_sum_mf)(
    struct remote_point_info_reorder *, size_t, size_t, size_t,
    struct interp_weight_stencil_wsum_mf *, size_t *, double *, size_t *,
    size_t *, MPI_Comm, enum yac_interp_weights_reorder_type, void *,
    Xt_config), Xt_config redist_config) {
  ...
  // add data to interpolation
  interp_add_w_sum_mf(
    remote_src_points, halo_size, num_src_fields, wsum_mf_count,
    wsum_mf_stencils, num_src_per_tgt, weights, src_idx, src_field_idx,
    comm, reorder, interp, redist_config);
...
static void yac_interp_weights_redist_w_sum_mf_raw(
  MPI_Comm comm, struct interp_weight_stencils_wsum_mf * wsum_mf_stencils_data,
  struct yac_interpolation * interp,
  void (*interp_add_w_sum_mf)(
    struct remote_point_info_reorder *, size_t, size_t, size_t,
    struct interp_weight_stencil_wsum_mf *, size_t *, double *, size_t *,
    size_t *, MPI_Comm, void *,
    Xt_config), Xt_config redist_config) {
  ...
  // redistribute stencils to respective owners
  struct interp_weight_stencils_wsum_mf * new_wsum_mf_stencils_data =
    redist_wsum_mf_stencils_tgt(comm, wsum_mf_stencils_data);
  ...
  size_t total_num_links = 0;
  for (size_t i = 0; i < wsum_mf_count; ++i)
    total_num_links += wsum_mf_stencils[i].count;

  // gather all source points and determine number of source fields
  struct remote_point_info_reorder * src_points =
    xmalloc(total_num_links * sizeof(*src_points));
  size_t num_src_fields = 0;
  for (size_t i = 0, k = 0; i < wsum_mf_count; ++i) {
    size_t curr_stencil_size = wsum_mf_stencils[i].count;
    struct interp_weight_stencil_wsum_mf_weight * curr_weights =
    ...
    for (size_t j = 0; j < curr_stencil_size; ++j, ++k) {
      ...
      if (curr_src_field_idx >= num_src_fields)
        num_src_fields = curr_src_field_idx + 1;
      src_points[k].data = curr_weights[j].src;
      src_points[k].field_idx = curr_src_field_idx;
      src_points[k].reorder_idx = i;
    }
  }
  yac_mpi_call(
    MPI_Allreduce(
  ...
  // sort remote points first by field_idx, second by rank, and
  // then by orig_pos
  qsort(src_points, total_num_links, sizeof(*src_points),
        compare_rank_pos_reorder_field_idx);

  // update stencils: set owner to -1; set orig_pos to position of respecitve
  // point in source field buffer
  // remove duplicated remote points
  struct remote_point_info * prev_src_point;
  size_t prev_field_idx;
  size_t num_src_points = 0;
  size_t src_field_buffer_size[num_src_fields];
  memset(
    src_field_buffer_size, 0,
    num_src_fields * sizeof(src_field_buffer_size[0]));

  if (total_num_links > 0) {
    prev_src_point = &(src_points[0].data);
    prev_field_idx = src_points[0].field_idx;
    src_field_buffer_size[src_points[0].field_idx] = 1;
    num_src_points = 1;
  }
  for (size_t i = 0; i < total_num_links; ++i) {
    struct remote_point_info * curr_src_point = &(src_points[i].data);
    size_t curr_field_idx = src_points[i].field_idx;
    if (compare_remote_point_info(prev_src_point, curr_src_point) ||
        (prev_field_idx != curr_field_idx)) {
      prev_src_point = curr_src_point;
      prev_field_idx = curr_field_idx;
      if (num_src_points != i) {
        src_points[num_src_points].data = *curr_src_point;
        src_points[num_src_points].field_idx = curr_field_idx;
      }
      src_field_buffer_size[curr_field_idx]++;
      num_src_points++;
    }
    struct interp_weight_stencil_wsum_mf * curr_stencil =
      wsum_mf_stencils + src_points[i].reorder_idx;
    size_t curr_stencil_size = curr_stencil->count;
    for (size_t j = 0; j < curr_stencil_size; ++j) {
      if ((!compare_remote_point_info(
             &(curr_stencil->data[j].src), curr_src_point)) &&
          (curr_stencil->data[j].src_field_idx == curr_field_idx)) {
        curr_stencil->data[j].src.rank = -1;
        curr_stencil->data[j].src.orig_pos =
          src_field_buffer_size[curr_field_idx] - 1;
      }
    }
  }
  ...
  // add data to interpolation
  interp_add_w_sum_mf(
    src_points, num_src_points, num_src_fields, wsum_mf_count,
    wsum_mf_stencils, num_src_per_tgt, weights, src_idx, src_field_idx,
    comm, interp, redist_config);
...
static Xt_config get_redist_config(
  char const * yaxt_exchanger_name, MPI_Comm comm) {

  Xt_config redist_config = xt_config_new();

  // if no exchanger has been defined yet -> check environment
  char * env_exchanger_name = NULL;
  if (yaxt_exchanger_name == NULL) {
    ...
    // to other processes
    size_t exchanger_name_len = 0;
    if (rank == 0) {

      // check if the user provided an exchanger name in the environment
      env_exchanger_name = getenv(YAC_YAXT_EXCHANGER_STR);
      exchanger_name_len =
        ((env_exchanger_name != NULL) && (env_exchanger_name[0] != '\0'))?
          strlen(env_exchanger_name):0;
    }

    // broadcast the length of the exchanger name provided by the user
    // through the environment
    yac_mpi_call(
      MPI_Bcast(&exchanger_name_len, 1, YAC_MPI_SIZE_T, 0, comm), comm);

    if (exchanger_name_len > 0) {

      if (rank == 0)
        env_exchanger_name = strdup(env_exchanger_name);
      else
        env_exchanger_name =
          xmalloc((exchanger_name_len + 1) * sizeof(*env_exchanger_name));

      // broadcast name of the exchanger
      yac_mpi_call(
        MPI_Bcast(
          env_exchanger_name, (int)(exchanger_name_len + 1), MPI_CHAR, 0, comm),
        comm);

      yaxt_exchanger_name = env_exchanger_name;
    }
  }
  ...
  if (yaxt_exchanger_name != NULL) {
    ...
    // set exchanger
    int exchanger_id = xt_exchanger_id_by_name(yaxt_exchanger_name);
    YAC_ASSERT_F(
      exchanger_id >= 0,
      "ERROR(get_redist_config): invalid yaxt exchanger name \"%s\"",
      yaxt_exchanger_name);
    xt_config_set_exchange_method(redist_config, exchanger_id);
  }

  free(env_exchanger_name);

  return redist_config;
}
...
struct yac_interpolation * yac_interp_weights_get_interpolation(
  ...
  int comm_size;
  yac_mpi_call(MPI_Comm_size(comm, &comm_size), comm);
  int * flag_buffer = xmalloc(2 * (size_t)comm_size * sizeof(*flag_buffer));
  int * rank_is_source = flag_buffer;
  int * rank_is_target = flag_buffer + comm_size;
  yac_mpi_call(
    MPI_Allgather(&is_source, 1, MPI_INT, rank_is_source, 1, MPI_INT, comm),
    comm);
  yac_mpi_call(
    MPI_Allgather(&is_target, 1, MPI_INT, rank_is_target, 1, MPI_INT, comm),
    comm);

  Xt_config redist_config = get_redist_config(yaxt_exchanger_name, comm);

  // sort stencils by type
  qsort(weights->stencils, weights->stencils_size, sizeof(*(weights->stencils)),
        compare_stencils);

  size_t local_stencil_counts[WEIGHT_STENCIL_TYPE_SIZE];
  ...
  if (global_stencil_counts[FIXED] > 0)
    yac_interp_weights_redist_fixed(
      ...
      weights->stencils + stencils_offsets[FIXED],
      (void*)interp, interp_add_fixed, rank_is_target);

  if (global_stencil_counts[DIRECT] > 0)
    yac_interp_weights_redist_direct(
      weights->comm, local_stencil_counts[DIRECT],
      weights->stencils + stencils_offsets[DIRECT], interp, interp_add_direct,
      redist_config, rank_is_source, rank_is_target);
  ...
  if (global_stencil_counts[SUM] > 0) {
    ...
    struct interp_weight_stencils_wsum_mf * wsum_stencils =
      generate_w_sum_mf_stencils(
        ...
        rank_is_source, rank_is_target);
    yac_interp_weights_redist_w_sum_mf(
      weights->comm, wsum_stencils, interp, reorder,
      interpolation_add_sum, redist_config);
  ...
  if (global_stencil_counts[WEIGHT_SUM] > 0) {
    ...
    yac_interp_weights_redist_w_sum_mf(
      weights->comm, wsum_stencils, interp, reorder,
      interpolation_add_wsum, redist_config);
    for (size_t i = 0; i < wsum_stencils->count; ++i)
      free_remote_point(wsum_stencils->data[i].tgt);
    free(wsum_stencils->data);
    free(wsum_stencils);
  ...
  if (global_stencil_counts[DIRECT_MF] > 0)
    yac_interp_weights_redist_direct_mf(
      ...
      weights->stencils + stencils_offsets[DIRECT_MF],
      interp, interp_add_direct_mf, redist_config,
      rank_is_source, rank_is_target);
  ...
  if (global_stencil_counts[SUM_MF] > 0) {
    ...
    struct interp_weight_stencils_wsum_mf * sum_mf_stencils =
      generate_w_sum_mf_stencils(
        ...
        rank_is_source, rank_is_target);
    yac_interp_weights_redist_w_sum_mf(
      weights->comm, sum_mf_stencils, interp, reorder,
      interpolation_add_sum_mf, redist_config);
  ...
  if (global_stencil_counts[WEIGHT_SUM_MF] > 0) {
    ...
    struct interp_weight_stencils_wsum_mf * wsum_mf_stencils =
      generate_w_sum_mf_stencils(
        ...
        rank_is_source, rank_is_target);
    yac_interp_weights_redist_w_sum_mf(
      weights->comm, wsum_mf_stencils, interp, reorder,
      interpolation_add_wsum_mf, redist_config);
...
static Xt_redist * generate_src_field_exchange_redists(
  struct yac_src_field_exchange_data * src_field_exchange_data,
  size_t num_src_fields, MPI_Comm comm,
  Xt_config redist_config) {
  ...
  for (size_t i = 0; i < num_src_fields; ++i) {
    nsends[i] = src_field_exchange_data[i].send.num_msg;
    nrecvs[i] = src_field_exchange_data[i].recv.num_msg;
    for (size_t j = 0; j < src_field_exchange_data[i].recv.num_msg; ++j) {
      if (max_recv_buffer_size < src_field_exchange_data[i].recv.msg[j].count)
        max_recv_buffer_size = src_field_exchange_data[i].recv.msg[j].count;
    }
  }
  ...
  int * pos_buffer =
    xmalloc((size_t)max_recv_buffer_size * sizeof(*pos_buffer));

  // generate yaxt send and receive messages
  for (size_t src_field_idx = 0; src_field_idx < num_src_fields;
        ++src_field_idx) {

    for (size_t send_idx = 0;
         send_idx < src_field_exchange_data[src_field_idx].send.num_msg;
         ++send_idx) {
      send_msgs[send_offsets[src_field_idx]].rank =
        src_field_exchange_data[src_field_idx].send.msg[send_idx].rank;
      send_msgs[send_offsets[src_field_idx]].datatype =
        xt_mpi_generate_datatype(
          src_field_exchange_data[src_field_idx].send.msg[send_idx].pos,
          src_field_exchange_data[src_field_idx].send.msg[send_idx].count,
          MPI_DOUBLE, comm);
      send_offsets[src_field_idx]++;
    }

    for (size_t recv_idx = 0;
         recv_idx < src_field_exchange_data[src_field_idx].recv.num_msg;
         ++recv_idx) {
      int count = 0;
      for (int i = 0;
          i < src_field_exchange_data[src_field_idx].recv.msg[recv_idx].count;
          ++i, ++count)
        pos_buffer[count] =
          src_field_exchange_data[src_field_idx].recv.msg[recv_idx].pos[i];

      recv_msgs[recv_offsets[src_field_idx]].rank =
        src_field_exchange_data[src_field_idx].recv.msg[recv_idx].rank;
      recv_msgs[recv_offsets[src_field_idx]].datatype =
        xt_mpi_generate_datatype(pos_buffer, count, MPI_DOUBLE, comm);
      recv_offsets[src_field_idx]++;
    }
  }
...
static void yac_interp_weights_data_init_(
  double frac_mask_fallback_value, double scaling_factor,
  double scaling_summand,
  struct yac_interp_weights_data * interp_weights_data) {

  interp_weights_data->frac_mask_fallback_value = frac_mask_fallback_value;
  interp_weights_data->scaling_factor = scaling_factor;
  interp_weights_data->scaling_summand = scaling_summand;

  interp_weights_data->num_fixed_values = 0;
  interp_weights_data->fixed_values = NULL;
  interp_weights_data->num_tgt_per_fixed_value = NULL;
  interp_weights_data->tgt_idx_fixed = NULL;

  interp_weights_data->num_wgt_tgt = 0;
  interp_weights_data->wgt_tgt_idx = NULL;
  interp_weights_data->num_src_per_tgt = NULL;
  interp_weights_data->weights = NULL;
  interp_weights_data->src_field_idx = NULL;
  interp_weights_data->src_idx = NULL;
  interp_weights_data->num_src_fields = 0;
  interp_weights_data->src_field_buffer_size = NULL;
}

static void yac_src_field_exchange_data_free(
  struct yac_src_field_exchange_data * src_field_exchange_data,
  size_t num_src_fields) {

  for (size_t i = 0; i < num_src_fields; ++i) {
    for (size_t j = 0; j < src_field_exchange_data[i].send.num_msg; ++j)
      free(src_field_exchange_data[i].send.msg[j].pos);
    free(src_field_exchange_data[i].send.msg);
    for (size_t j = 0; j < src_field_exchange_data[i].recv.num_msg; ++j)
      free(src_field_exchange_data[i].recv.msg[j].pos);
    free(src_field_exchange_data[i].recv.msg);
  }

  free(src_field_exchange_data);
}

static void interpolation_raw_add_w_sum_mf(
  struct remote_point_info_reorder * remote_src_points, size_t num_src_points,
  size_t num_src_fields, size_t tgt_count,
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
  struct yac_interpolation_raw * interp_raw, Xt_config redist_config) {
  ...
  // generate exchange information from remote_src_points
  ...
  for (size_t i = 0; i < num_src_points; ++i)
    sendcounts[remote_src_points[i].data.rank * num_src_fields +
               remote_src_points[i].field_idx]++;

  yac_generate_alltoallv_args(
    num_src_fields, sendcounts, recvcounts, sdispls, rdispls, comm);

  size_t total_sendcount = 0, total_recvcount = 0;
  for (int i = 0; i < comm_size; ++i) {
    total_sdispls[i] = total_sendcount;
    total_rdispls[i] = total_recvcount;
    total_sendcounts[i] = 0;
    total_recvcounts[i] = 0;
    for (size_t j = 0; j < num_src_fields; ++j) {
      total_sendcounts[i] += sendcounts[num_src_fields * i + j];
    ...
    total_sendcount += total_sendcounts[i];
    total_recvcount += total_recvcounts[i];
  }
  ...
  size_t * exchange_buffer =
    xmalloc((2 * num_src_points + recv_count) * sizeof(*exchange_buffer));
  size_t * send_buffer = exchange_buffer;
  size_t * reorder_idx = exchange_buffer + num_src_points;
  size_t * recv_buffer = exchange_buffer + 2 * num_src_points;
  ...
  memset(
    ...
    num_src_fields * sizeof(src_field_buffer_size[0]));
  for (size_t i = 0; i < num_src_points; ++i) {
    size_t curr_src_field_idx = (size_t)(remote_src_points[i].field_idx);
    size_t pos = sdispls[(size_t)(remote_src_points[i].data.rank) * num_src_fields +
                         curr_src_field_idx + 1]++;
    send_buffer[pos] = (size_t)remote_src_points[i].data.orig_pos;
    reorder_idx[pos] = src_field_buffer_size[curr_src_field_idx]++;
  }
  ...
  yac_alltoallv_size_t_p2p(
    send_buffer, total_sendcounts, total_sdispls,
    recv_buffer, total_recvcounts, total_rdispls, comm,
    "interpolation_raw_add_w_sum_mf", __LINE__);

  free(size_t_buffer);

  yac_src_field_exchange_data_add(
    interp_raw->src_field_exchange_data, num_src_fields, comm,
    recvcounts, recv_buffer, sendcounts, reorder_idx,
    interp_raw->interp_weights_data.src_field_buffer_size);

  yac_free_comm_buffers(sendcounts, recvcounts, sdispls, rdispls);
  free(exchange_buffer);

  size_t * tgt_idx = xmalloc(tgt_count * sizeof(*tgt_idx));

  for (size_t i = 0; i < tgt_count; ++i) {
    YAC_ASSERT(
      tgt_stencils[i].tgt.data.count == 1,
      "ERROR(interpolation_raw_add_w_sum_mf): currently unsupported "
      "target point distribution")
    tgt_idx[i] = (size_t)(tgt_stencils[i].tgt.data.data.single.orig_pos);
  }

  yac_interp_weights_data_set_wgt_tgt(
    &(interp_raw->interp_weights_data), num_src_fields, tgt_count,
    tgt_idx, num_src_per_tgt, weights, src_field_idx, src_idx,
    src_field_buffer_size);
...
static void interpolation_raw_add_wsum_mf(
  struct remote_point_info_reorder * src_points, size_t num_src_points,
  size_t num_src_fields, size_t tgt_count,
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
  void * interp_raw, Xt_config redist_config) {

  interpolation_raw_add_w_sum_mf(
    src_points, num_src_points, num_src_fields, tgt_count, tgt_stencils,
    num_src_per_tgt, weights, src_idx, src_field_idx, comm,
    (struct yac_interpolation_raw *)interp_raw, redist_config);
}

static void interpolation_raw_add_sum_mf(
  struct remote_point_info_reorder * src_points, size_t num_src_points,
  size_t num_src_fields, size_t tgt_count,
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
  ...
  interpolation_raw_add_w_sum_mf(
    src_points, num_src_points, num_src_fields, tgt_count, tgt_stencils,
    num_src_per_tgt, NULL, src_idx, src_field_idx, comm,
    (struct yac_interpolation_raw *)interp_raw, redist_config);
}
...
static void interpolation_raw_add_wsum(
  ...
  UNUSED(src_field_idx);

  interpolation_raw_add_w_sum_mf(
    src_points, num_src_points, num_src_fields, tgt_count, tgt_stencils,
    num_src_per_tgt, weights, src_idx, NULL, comm,
    (struct yac_interpolation_raw *)interp_raw, redist_config);
}

static void interpolation_raw_add_sum(
  struct remote_point_info_reorder * src_points, size_t num_src_points,
  size_t num_src_fields, size_t tgt_count,
  struct interp_weight_stencil_wsum_mf * tgt_stencils, size_t * num_src_per_tgt,
  double * weights, size_t * src_idx, size_t * src_field_idx, MPI_Comm comm,
  ...
  interpolation_raw_add_w_sum_mf(
    src_points, num_src_points, num_src_fields, tgt_count, tgt_stencils,
    num_src_per_tgt, NULL, src_idx, NULL, comm,
    (struct yac_interpolation_raw *)interp_raw, redist_config);
}
...
void yac_interp_weights_get_interpolation_raw(
  ...
  struct yac_interpolation_raw interpolation_raw;

  interpolation_raw.src_field_exchange_data = NULL;
  yac_interp_weights_data_init_(
    frac_mask_fallback_value, scaling_factor, scaling_summand,
    &interpolation_raw.interp_weights_data);
  ...
  { // check whether the collection_size is consistant across all processes
    ...
    YAC_ASSERT(
      (size_t)max_collection_size == collection_size,
      "ERROR(yac_interp_weights_get_interpolation_raw): "
      "mismatching collection sizes")
  }
  ...
  if (global_stencil_counts[FIXED] > 0)
    yac_interp_weights_redist_fixed(
      weights->comm, local_stencil_counts[FIXED],
      weights->stencils + stencils_offsets[FIXED],
      (void*)&interpolation_raw, interp_raw_add_fixed, rank_is_target);

  if (global_stencil_counts[DIRECT] > 0)
    yac_interp_weights_redist_direct(
      weights->comm, local_stencil_counts[DIRECT],
      weights->stencils + stencils_offsets[DIRECT],
      (void*)&interpolation_raw, interp_add_direct_raw,
      redist_config, rank_is_source, rank_is_target);
  ...
  if (global_stencil_counts[SUM] > 0) {
    ...
    yac_interp_weights_redist_w_sum_mf_raw(
      weights->comm, wsum_stencils, (void*)&interpolation_raw,
      interpolation_raw_add_sum, redist_config);
    for (size_t i = 0; i < wsum_stencils->count; ++i)
      free_remote_point(wsum_stencils->data[i].tgt);
    free(wsum_stencils->data);
    free(wsum_stencils);
  ...
  if (global_stencil_counts[WEIGHT_SUM] > 0) {
    ...
    yac_interp_weights_redist_w_sum_mf_raw(
      weights->comm, wsum_stencils, (void*)&interpolation_raw,
      interpolation_raw_add_wsum, redist_config);
    for (size_t i = 0; i < wsum_stencils->count; ++i)
      free_remote_point(wsum_stencils->data[i].tgt);
    free(wsum_stencils->data);
    free(wsum_stencils);
  ...
  if (global_stencil_counts[DIRECT_MF] > 0)
    yac_interp_weights_redist_direct_mf(
      weights->comm, local_stencil_counts[DIRECT_MF],
      weights->stencils + stencils_offsets[DIRECT_MF],
      (void*)&interpolation_raw, interp_add_direct_mf_raw, redist_config,
      rank_is_source, rank_is_target);
  ...
  if (global_stencil_counts[SUM_MF] > 0) {
    ...
    struct interp_weight_stencils_wsum_mf * sum_mf_stencils =
      generate_w_sum_mf_stencils(
        ...
        rank_is_source, rank_is_target);
    yac_interp_weights_redist_w_sum_mf_raw(
      weights->comm, sum_mf_stencils, (void*)&interpolation_raw,
      interpolation_raw_add_sum_mf, redist_config);
    for (size_t i = 0; i < sum_mf_stencils->count; ++i)
      free_remote_point(sum_mf_stencils->data[i].tgt);
    free(sum_mf_stencils->data);
    free(sum_mf_stencils);
  ...
  if (global_stencil_counts[WEIGHT_SUM_MF] > 0) {
    ...
    struct interp_weight_stencils_wsum_mf * wsum_mf_stencils =
      generate_w_sum_mf_stencils(
        ...
        rank_is_source, rank_is_target);
    yac_interp_weights_redist_w_sum_mf_raw(
      weights->comm, wsum_mf_stencils, (void*)&interpolation_raw,
      interpolation_raw_add_wsum_mf, redist_config);
    for (size_t i = 0; i < wsum_mf_stencils->count; ++i)
      free_remote_point(wsum_mf_stencils->data[i].tgt);
    free(wsum_mf_stencils->data);
    free(wsum_mf_stencils);
  ...
  Xt_redist * redists =
    generate_src_field_exchange_redists(
      interpolation_raw.src_field_exchange_data,
      interpolation_raw.interp_weights_data.num_src_fields, comm,
      redist_config);
  int with_frac_mask = YAC_FRAC_MASK_VALUE_IS_VALID(frac_mask_fallback_value);
  *interpolation_exchange =
    yac_interpolation_exchange_new(
      redists, interpolation_raw.interp_weights_data.num_src_fields,
      collection_size, with_frac_mask,
      "yac_interp_weights_get_interpolation_raw");
  *interp_weights_data = interpolation_raw.interp_weights_data;

  if (redists != NULL) {
    for (size_t i = 0; i < interpolation_raw.interp_weights_data.num_src_fields;
         ++i)
      xt_redist_delete(redists[i]);
    free(redists);
  }
  ...
  yac_src_field_exchange_data_free(
    interpolation_raw.src_field_exchange_data,
    interpolation_raw.interp_weights_data.num_src_fields);
...
struct yac_interpolation * yac_interp_weights_get_interpolation_f2c(
  ...
  return
    yac_interp_weights_get_interpolation(
      ...
      collection_size, frac_mask_fallback_value,
      scaling_factor, scaling_summand,
      ((yaxt_exchanger_name != NULL) && (yaxt_exchanger_name[0] != '\0'))?
        yaxt_exchanger_name:NULL, is_source, is_target);
}
...
static size_t stencil_get_num_weights_per_tgt(
  ...
  size_t num_weights_per_link_64_t = num_weights_per_link;
  yac_mpi_call(
    MPI_Allreduce(
      MPI_IN_PLACE, &num_weights_per_link_64_t, 1, YAC_MPI_SIZE_T,
      MPI_MAX, comm), comm);
...
static void stencil_xscan_offsets(
  ...
  size_t * global_counts = size_t_buffer + 0 * count;
  size_t * local_counts = size_t_buffer + 1 * count;
  size_t * offsets = size_t_buffer + 2 * count;

  for (size_t i = 0; i < num_fixed_values; ++i)
    local_counts[i] = num_tgt_per_fixed_value[i];
  for (size_t i = 0; i < num_src_fields; ++i)
    local_counts[num_fixed_values + i] = num_links_per_src_field[i];

  yac_mpi_call(
    MPI_Allreduce(local_counts, global_counts, (int)count, YAC_MPI_SIZE_T,
                  MPI_SUM, comm), comm);
  yac_mpi_call(
    MPI_Exscan(local_counts, offsets, (int)count, YAC_MPI_SIZE_T, MPI_SUM, comm),
    comm);
...
void yac_interp_weights_write_to_file(
  ...
  int weight_file_exists =
    (io_ranks[0] == comm_rank)?yac_file_exists(filename):0;
  yac_mpi_call(
    MPI_Bcast(&weight_file_exists, 1, MPI_INT, io_ranks[0], comm), comm);

  // return if the weight file already exists and is supposed to be kept
  if ((on_existing == YAC_WEIGHT_FILE_KEEP) && weight_file_exists) return;
  if ((on_existing == YAC_WEIGHT_FILE_ERROR) && weight_file_exists) {
    if (io_ranks[0] == comm_rank) {
      char const msg_fmt[] =
        "ERROR(yac_interp_weights_write_to_file): "
        "weight file already exists (%s)";
      char msg[strlen(msg_fmt) + strlen(filename)];
      sprintf(msg, msg_fmt, filename);
      yac_abort(comm, msg, __FILE__, __LINE__);
    }
    return;
  }
  ...
  size_t grid_sizes[2] = {src_grid_size, tgt_grid_size};
  yac_mpi_call(
    MPI_Allreduce(
      MPI_IN_PLACE, grid_sizes, 2, YAC_MPI_SIZE_T, MPI_MAX, comm), comm);
...
void yac_interp_weights_delete(struct yac_interp_weights * weights) {
  ...
  yac_mpi_call(MPI_Comm_free(&weights->comm), weights->comm);
...
void yac_interp_weights_data_init(
  ...
  yac_interp_weights_data_init_(
    YAC_FRAC_MASK_UNDEF, 1.0, 0.0, interp_weights_data);
}
...
{ \
  ...
  interp_weights_data_copy.frac_mask_fallback_value =
    interp_weights_data.frac_mask_fallback_value;
  interp_weights_data_copy.scaling_factor =
    interp_weights_data.scaling_factor;
  interp_weights_data_copy.scaling_summand =
    interp_weights_data.scaling_summand;

  size_t total_num_fixed_tgt = 0;
  for (size_t i = 0; i < interp_weights_data.num_fixed_values; ++i)
    total_num_fixed_tgt += interp_weights_data.num_tgt_per_fixed_value[i];
  interp_weights_data_copy.num_fixed_values =
    interp_weights_data.num_fixed_values;
  COPY_ARRAY(fixed_values, interp_weights_data.num_fixed_values)
  COPY_ARRAY(num_tgt_per_fixed_value, interp_weights_data.num_fixed_values)
  COPY_ARRAY(tgt_idx_fixed, total_num_fixed_tgt)

  size_t num_weights = 0;
  for (size_t i = 0; i < interp_weights_data.num_wgt_tgt; ++i)
    num_weights += interp_weights_data.num_src_per_tgt[i];
  interp_weights_data_copy.num_wgt_tgt = interp_weights_data.num_wgt_tgt;
  COPY_ARRAY(wgt_tgt_idx, interp_weights_data.num_wgt_tgt)
  COPY_ARRAY(num_src_per_tgt, interp_weights_data.num_wgt_tgt)
  COPY_ARRAY(weights, num_weights)
  COPY_ARRAY(src_field_idx, num_weights)
  COPY_ARRAY(src_idx, num_weights)
  interp_weights_data_copy.num_src_fields =
    interp_weights_data.num_src_fields;
  COPY_ARRAY(src_field_buffer_size, interp_weights_data.num_src_fields)

#undef COPY_ARRAY

  return interp_weights_data_copy;
}
...
void yac_interp_weights_data_free(
  ...
  free(interp_weights_data.src_field_buffer_size);
}
