...
static struct result_stencils * exchange_interp_results(
  ...
  char const * routine = "exchange_interp_results";
  ...
  for (int rank = 0; rank < comm.size; ++rank) {
    ...
    YAC_ASSERT_F(
      curr_sendcount <= INT_MAX, "ERROR(%s): pack size to big", routine)
...
static size_t do_search_creep (struct interp_method * method,
  ...
  char const * routine = "do_search_creep";
  ...
  YAC_ASSERT_F(
    ...
    "ERROR(%s): unsupported target field location "
    "(has to be YAC_LOC_CELL or YAC_LOC_CORNER)", routine)
  ...
  yac_alltoallv_int_p2p(
    ...
    interp_flag, sendcounts, sdispls+1, comm, routine, __LINE__);
