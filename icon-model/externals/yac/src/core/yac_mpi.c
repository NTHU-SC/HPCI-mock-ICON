...
void yac_alltoallv_p2p(
  ...
  size_t dt_size, MPI_Datatype dt, MPI_Comm comm, char const * caller, int line) {
...
#define USE_P2P_ALLTOALLV
    ...
    for (int i = lb; i < ub; ++i) {
      if (sendcounts[i] > 0) {
        YAC_ASSERT_F(
          ...
          "ERROR(%s(%d)::yac_alltoallv_p2p): "
          "sendcounts[%d] = %zu exceeds INT_MAX (%d)",
          caller, line, i, sendcounts[i], (int)INT_MAX)
        yac_mpi_call(
          MPI_Isend(
      ...
      if (recvcounts[i] > 0) {
        YAC_ASSERT_F(
          ...
          "ERROR(%s(%d)::yac_alltoallv_p2p): "
          "recvcounts[%d] = %zu exceeds INT_MAX (%d)",
          caller, line, i, recvcounts[i], (int)INT_MAX)
        yac_mpi_call(
          MPI_Irecv(
  ...
  for (int i = 0; i < comm_size; ++i) {
    ...
    YAC_ASSERT_F(
      sdispls[i] <= INT_MAX,
      "ERROR(%s(%d)::yac_alltoallv_p2p): "
      "sdispls[%d] = %zu exceeds INT_MAX (%d)",
      caller, line, i, sdispls[i], (int)INT_MAX)
    YAC_ASSERT_F(
      recvcounts[i] <= INT_MAX,
      "ERROR(%s(%d)::yac_alltoallv_p2p): "
      "recvcounts[%d] = %zu exceeds INT_MAX (%d)",
    ...
    YAC_ASSERT_F(
      rdispls[i] <= INT_MAX,
      "ERROR(%s(%d)::yac_alltoallv_p2p): "
      "rdispls[%d] = %zu exceeds INT_MAX (%d)",
      caller, line, i, rdispls[i], (int)INT_MAX)
...
void yac_allgather_size_t(
  const size_t * sendbuf, size_t * recvbuf, int count,
  ...
  size_t * temp = xmalloc((size_t)group_comm.size * (size_t)count * sizeof(*temp));
  ...
  for (int r = 0; r < lg2; ++r) {
    ...
    yac_mpi_call(
      MPI_Sendrecv(
        (void const*)temp, curr_len, YAC_MPI_SIZE_T, dst + group_comm.start, 0,
        (void *)(temp + (size_t)curr_len), curr_len, YAC_MPI_SIZE_T,
  ...
  yac_mpi_call(
    MPI_Sendrecv(
      (void const*)temp, rest, YAC_MPI_SIZE_T, dst + group_comm.start, 0,
      (void*)(temp + (size_t)curr_len), rest, YAC_MPI_SIZE_T,
      src + group_comm.start, 0, group_comm.comm, MPI_STATUS_IGNORE),
    group_comm.comm);
