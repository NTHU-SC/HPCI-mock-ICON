...
static void redist_weight_file_data(
  ...
  char const * routine = "redist_weight_file_data";
  ...
  for (size_t tgt_idx = 0, link_idx = 0, fixed_idx = 0;
    ...
    YAC_ASSERT_F(
      ...
      "ERROR(%s): inconsistent data in weight file;"
      "link and fixed data available for a target point", routine)
  ...
  yac_alltoallv_int_p2p(
    ...
    recv_pack_sizes, recvcounts, rdispls, comm, routine, __LINE__);
  ...
  yac_alltoallv_p2p(
    ...
    1, MPI_PACKED, comm, routine, __LINE__);
...
static size_t do_search_file(struct interp_method * method,
  ...
  // determine root io rank
  int local_is_io;
  int * io_ranks;
  int num_io_ranks;
  yac_get_io_ranks(comm, &local_is_io, &io_ranks, &num_io_ranks);
  int root_io_rank = io_ranks[0];
  free(io_ranks);

  YAC_ASSERT_F(
    (method_file->on_missing_file == YAC_INTERP_FILE_MISSING_ERROR) ||
    (method_file->on_missing_file == YAC_INTERP_FILE_MISSING_CONT),
    "ERROR(do_search_file): unsupported value for on_missing_file %d "
    "(has to be either YAC_INTERP_FILE_MISSING_ERROR(%d) or "
    "YAC_INTERP_FILE_MISSING_CONT (%d))",
    (int)(method_file->on_missing_file), YAC_INTERP_FILE_MISSING_ERROR,
    YAC_INTERP_FILE_MISSING_CONT);

  // determine if a file with the same name already exists
  int file_exists =
    (comm_rank == root_io_rank) &&
    yac_file_exists(method_file->weight_file_name);
  yac_mpi_call(
    MPI_Allreduce(
      MPI_IN_PLACE, &file_exists, 1, MPI_INT, MPI_MAX, comm), comm);

  // it is enough if one process calls abort, otherwise we would have a lot of
  // output
  if ((comm_rank == root_io_rank) && !file_exists &&
      (method_file->on_missing_file == YAC_INTERP_FILE_MISSING_ERROR)) {
    char const msg_fmt[] =
      "ERROR(do_search_file): weight file \"%s\" does not exist";
    char msg[strlen(msg_fmt) + strlen(method_file->weight_file_name)];
    sprintf(msg, msg_fmt, method_file->weight_file_name);
    yac_abort(comm, msg, __FILE__, __LINE__);
  }

  // if there is no weight file continue normally
  // (in case of error, another process will have called abort)
  ...
  YAC_ASSERT_F(
    (method_file->on_success == YAC_INTERP_FILE_SUCCESS_STOP) ||
    (method_file->on_success == YAC_INTERP_FILE_SUCCESS_CONT),
    "ERROR(do_search_file): unsupported value for on_success %d "
    "(has to be either YAC_INTERP_FILE_SUCCESS_STOP(%d) or "
    "YAC_INTERP_FILE_SUCCESS_CONT (%d))",
    (int)(method_file->on_success), YAC_INTERP_FILE_SUCCESS_STOP,
...
struct interp_method * yac_interp_method_file_new(
  ...
  method->on_missing_file = on_missing_file;
