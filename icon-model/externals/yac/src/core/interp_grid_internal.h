...
/**
 * Generates the edge_to_cell data for the current state of the local
 * target grid.
 * @param[in] interp_grid interpolation grid
 * @return edge_to_cell data of current state of local target grid
 * @remark use is responsible for freeing the edge_to_cell_array
 */
yac_size_t_2_pointer yac_interp_grid_generate_tgt_edge_to_cell(
  struct yac_interp_grid * interp_grid);
