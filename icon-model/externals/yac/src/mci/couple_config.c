...
static void yac_couple_config_field_couple_free(void * field_couple_) {
  ...
  free(field_couple->yaxt_exchanger_name);
}
...
static void yac_couple_config_field_merge(
  ...
  data_pairs[FRAC_MASK_IDX].len =
    (b != NULL) && YAC_FRAC_MASK_VALUE_IS_VALID(b->frac_mask_fallback_value);
  ...
  if (data_pairs[FRAC_MASK_IDX].len != 0) {
    ...
    YAC_ASSERT_F(
      (b == NULL) ||
      !YAC_FRAC_MASK_VALUE_IS_VALID(b->frac_mask_fallback_value) ||
      !memcmp(
...
static void yac_couple_config_field_couple_merge(
  ...
  YAC_ASSERT_F(
    ...
    "ERROR(yac_couple_config_field_couple_merge): "
    "inconsistent definition of enforce_write_weight_file (%d != %d)",
    a->enforce_write_weight_file, b->enforce_write_weight_file)
  YAC_ASSERT_F(
  ...
  YAC_ASSERT_F(
    !a->enforce_write_weight_file ||
    (a->weight_file_on_existing == b->weight_file_on_existing),
    "ERROR(yac_couple_config_field_couple_merge): "
    "inconsistent weight_file_on_existing (%d != %d)",
    (int)(a->weight_file_on_existing), (int)(b->weight_file_on_existing))
  YAC_ASSERT_F(
  ...
  YAC_ASSERT_F(
    (a->yaxt_exchanger_name != NULL) == (b->yaxt_exchanger_name != NULL),
    "ERROR(yac_couple_config_field_couple_merge): "
    "inconsistent availability of yaxt exchanger name (%s != %s)",
    (a->yaxt_exchanger_name != NULL)?"TRUE":"FALSE",
    (b->yaxt_exchanger_name != NULL)?"TRUE":"FALSE")
  YAC_ASSERT_F(
    (a->yaxt_exchanger_name == NULL) ||
    !strcmp(a->yaxt_exchanger_name, b->yaxt_exchanger_name),
    "ERROR(yac_couple_config_field_couple_merge): "
    "inconsistent yaxt exchanger name (\"%s\" != \"%s\")",
    a->yaxt_exchanger_name, b->yaxt_exchanger_name)
  YAC_ASSERT_F(
    a->use_raw_exchange == b->use_raw_exchange,
    "ERROR(yac_couple_config_field_couple_merge): "
    "inconsistent definition of use_raw_exchange (%d != %d)",
    a->use_raw_exchange, b->use_raw_exchange)
}
...
void yac_couple_config_field_enable_frac_mask(
  ...
  // (use memcmp for comparison, because it can be nan)
  YAC_ASSERT_F(
    !YAC_FRAC_MASK_VALUE_IS_VALID(old_frac_mask_fallback_value) ||
    !memcmp(
...
double yac_couple_config_get_frac_mask_fallback_value(
  ...
  YAC_ASSERT_F(
    YAC_FRAC_MASK_VALUE_IS_SET(field->frac_mask_fallback_value),
...
enum yac_weight_file_on_existing yac_couple_config_get_weight_file_on_existing(
  struct yac_couple_config * couple_config,
  size_t couple_idx, size_t field_couple_idx) {

  check_field_couple_idx(
    couple_config, couple_idx, field_couple_idx,
    "yac_couple_config_get_weight_file_on_existing", __LINE__);
...
char const * yac_couple_config_get_yaxt_exchanger_name(
  struct yac_couple_config * couple_config,
  size_t couple_idx, size_t field_couple_idx) {

  check_field_couple_idx(
    couple_config, couple_idx, field_couple_idx,
    "yac_couple_config_get_yaxt_exchanger_name", __LINE__);
...
int yac_couple_config_get_use_raw_exchange(
  struct yac_couple_config * couple_config,
  size_t couple_idx, size_t field_couple_idx) {

  check_field_couple_idx(
    couple_config, couple_idx, field_couple_idx,
    "yac_couple_config_get_use_raw_exchange", __LINE__);
...
static size_t yac_couple_config_get_field_couple_pack_size(
  ...
  int ints_pack_size;
  yac_mpi_call(MPI_Pack_size(12, MPI_INT, comm, &ints_pack_size), comm);
  ...
  return
    ...
    yac_couple_config_get_string_pack_size(
      field_couple->yaxt_exchanger_name, comm);
}
...
static void yac_couple_config_pack_field_couple(
  ...
  YAC_ASSERT(
    field_couple->weight_file_on_existing <= INT_MAX,
    "ERROR(yac_couple_config_pack_field_couple): "
    "weight_file_on_existing bigger than INT_MAX")
  YAC_ASSERT(
  ...
  int ints[] = {
    ...
    (int)(field_couple->coupling_period_operation),
    field_couple->enforce_write_weight_file,
    (int)(field_couple->weight_file_on_existing),
    field_couple->num_src_mask_names,
    field_couple->use_raw_exchange};
  enum {NUM_INTS = sizeof(ints) / sizeof(ints[0])};

  yac_mpi_call(
    MPI_Pack(
      ints, NUM_INTS, MPI_INT, buffer, buffer_size, position, comm), comm);
  ...
  yac_couple_config_pack_string(
    field_couple->yaxt_exchanger_name, buffer, buffer_size, position, comm);
}
...
static void yac_couple_config_unpack_field_couple(
  ...
  yac_mpi_call(
    MPI_Unpack(
      buffer, buffer_size, position, ints, 12, MPI_INT, comm), comm);
  ...
  YAC_ASSERT(
    ints[9] >= 0,
    "ERROR(yac_couple_config_unpack_field_couple): "
    "invalid weight_file_on_existing")
  YAC_ASSERT(
    ints[10] >= 0,
    "ERROR(yac_couple_config_unpack_field_couple): "
  ...
  field_couple->weight_file_on_existing =
    (enum yac_weight_file_on_existing)ints[9];
  field_couple->num_src_mask_names = (size_t)(ints[10]);
  field_couple->use_raw_exchange = (size_t)(ints[11]);
  ...
  field_couple->yaxt_exchanger_name =
    yac_couple_config_unpack_string(
      buffer, buffer_size, position, comm);
}
...
void yac_couple_config_def_couple(
  ...
  struct yac_interp_stack_config * interp_stack, int src_lag, int tgt_lag,
  ...
  YAC_ASSERT_F(isnormal(scale_summand) || (scale_summand == 0.0),
    ...
    scale_summand);
  YAC_ASSERT(
    (weight_file_on_existing == YAC_WEIGHT_FILE_ERROR) ||
    (weight_file_on_existing == YAC_WEIGHT_FILE_KEEP) ||
    (weight_file_on_existing == YAC_WEIGHT_FILE_OVERWRITE),
    "ERROR(yac_couple_config_def_couple): invalid parameter: weight_file_on_existing");
  ...
  field_couple.tgt_mask_name = string_dup(tgt_mask_name);
  field_couple.yaxt_exchanger_name = string_dup(yaxt_exchanger_name);
  field_couple.use_raw_exchange = use_raw_exchange != 0;
...
void yac_couple_config_get_field_source(
  struct yac_couple_config * couple_config,
  char const * tgt_component_name, char const * tgt_grid_name,
  char const * tgt_field_name, char const ** src_component_name,
  char const ** src_grid_name, char const ** src_field_name) {

  size_t tgt_comp_idx =
    yac_couple_config_get_component_idx(couple_config, tgt_component_name);
  size_t tgt_grid_idx =
    yac_couple_config_get_grid_idx(couple_config, tgt_grid_name);
  size_t tgt_field_idx =
    yac_couple_config_get_field_idx(
      couple_config, tgt_comp_idx, tgt_grid_idx, tgt_field_name);

  struct yac_couple_config_field_couple * field_couple = NULL;
  for (size_t couple_idx = 0; couple_idx < couple_config->num_couples;
       ++couple_idx) {
    ...
    if ((couple->component_indices[0] != tgt_comp_idx) &&
        (couple->component_indices[1] != tgt_comp_idx))
      continue;

    for (size_t temp_field_couple_idx = 0;
         temp_field_couple_idx < couple->num_field_couples;
         ++temp_field_couple_idx) {

      struct yac_couple_config_field_couple * temp_field_couple =
        couple->field_couples + temp_field_couple_idx;

      if ((temp_field_couple->target.component_idx == tgt_comp_idx) &&
          (temp_field_couple->target.field_idx == tgt_field_idx)) {

        YAC_ASSERT_F(
          field_couple == NULL,
          "ERROR(yac_couple_config_get_field_source): "
          "multiple couples with the same target field is not supported by "
          "this routine (target component: \"%s\" target grid: \"%s\""
          "target field: \"%s\")", tgt_component_name, tgt_grid_name,
          tgt_field_name);

        field_couple = temp_field_couple;
      }
    }
  }
  ...
  YAC_ASSERT_F(
    field_couple != NULL,
    "ERROR(yac_couple_config_get_field_source): "
    "provided field is not defined as a target in any coupling "
    "(target component: \"%s\" target grid: \"%s\" target field: \"%s\")",
    tgt_component_name, tgt_grid_name, tgt_field_name);

  struct yac_couple_config_component * src_component =
    couple_config->components + field_couple->source.component_idx;
  struct yac_couple_config_field * src_field =
    src_component->fields + field_couple->source.field_idx;
  struct yac_couple_config_grid * src_grid =
    couple_config->grids + src_field->grid_idx;

  *src_component_name = src_component->name;
  *src_grid_name = src_grid->name;
