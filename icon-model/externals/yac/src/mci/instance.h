 ...
 * @returns MPI communicator of the YAC instance
 */
 ...
 * @param[in] weight_file_on_existing specifies how YAC is supposed to handle
 *                                    the case of an already existing file with
 *                                    the same name
 * @param[in] mapping_on_source       side where the mapping is computed.
 *                                    Currently only source = 1 and target = 0
 *                                    are allowed
 ...
 *                                     provided)
 * @param[in] yaxt_exchanger_name     yaxt exchanger method to be used for the
 *                                    interpoolation
 * @param[in] use_raw_exchange        enables the use of raw exchange (YAC only
 *                                    transfers data required for interpolation
 *                                    to target process, afterwards user has to
 *                                    apply the interpolation himself)
 */
