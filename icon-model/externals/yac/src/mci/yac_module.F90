...
module yac
  ...
  double precision, parameter :: YAC_FRAC_MASK_NO_VALUE = 133713371337.0d0
  ...
  enum, bind(c)
    enumerator :: YAC_FILE_MISSING_ERROR = 0 !<  abort on missing file
    enumerator :: YAC_FILE_MISSING_CONT  = 1 !< continue on missing file
  end enum

  enum, bind(c)
    enumerator :: YAC_FILE_SUCCESS_STOP = 0 !< prevents following interpolation
                                            !! method from computating further
                                            !! weights
    enumerator :: YAC_FILE_SUCCESS_CONT = 1 !< continue weight computation with
  ...
  enum, bind(c)
   enumerator :: YAC_WGT_ON_EXISTING_ERROR     = 0
   enumerator :: YAC_WGT_ON_EXISTING_KEEP      = 1
   enumerator :: YAC_WGT_ON_EXISTING_OVERWRITE = 2
  end enum
 ...
 type :: yac_string
  ...
  interface
    function yac_fget_mpi_handshake_group_name () &
      result (mpi_handshake_group_name)
  ...
  interface yac_fdef_grid
     ...
     !! @param[in]  x_vertices            longitudes of vertices in radians
     !! @param[in]  y_vertices            latitudes of vertices in radians
     ...
     subroutine yac_fdef_grid_unstruct_edge_real ( grid_name,          &
       ...
       integer, intent(in)           :: nbr_cells
       integer, intent(in)           :: nbr_edges
       integer, intent(in)           :: nbr_edges_per_cell
       real, intent(in)              :: x_vertices(nbr_vertices)
       real, intent(in)              :: y_vertices(nbr_vertices)
       integer, intent(in)           :: cell_to_edge( &
                                          nbr_edges_per_cell,nbr_cells)
       integer, intent(in)           :: edge_to_vertex(2,nbr_edges)
       integer, intent(out)          :: grid_id
       logical, optional, intent(in) :: use_ll_edges
     ...
     !! @param[in]  x_points   longitudes of points in radians
     !! @param[in]  y_points   latitudes of points in radians
     !! @param[out] grid_id    grid identifier
     ...
     subroutine yac_fdef_grid_reg2d_rot_real ( grid_name,    &
       ...
       real, intent(in)             :: x_vertices(nbr_vertices(1))
       real, intent(in)             :: y_vertices(nbr_vertices(2))
       real, intent(in)             :: x_north_pole
       real, intent(in)             :: y_north_pole
       integer, intent(out)         :: grid_id
  ...
  interface yac_fdef_points
     ...
     subroutine yac_fdef_points_curve2d_real ( grid_id,       &
       ...
       integer, intent(in)  :: location             !< [IN]  location, one of center/edge/vertex
       real, intent(in)     :: &
         x_points_real(nbr_points(1),nbr_points(2)) !< [IN]  longitudes of points in radians
       real, intent(in)     :: &
         y_points_real(nbr_points(1),nbr_points(2)) !< [IN]  latitudes of points in radians
       integer, intent(out) :: point_id             !< [OUT] point identifier
     ...
     subroutine yac_fdef_points_curve2d_dble ( grid_id,       &
       ...
       integer, intent(in)  :: location        !< [IN]  location, one of center/edge/vertex
       double precision, intent(in) :: &
         x_points(nbr_points(1),nbr_points(2)) !< [IN]  longitudes of points in radians
       double precision, intent(in) :: &
         y_points(nbr_points(1),nbr_points(2)) !< [IN]  latitudes of points in radians
       integer, intent(out) :: point_id        !< [OUT] point identifier
     ...
     subroutine yac_fdef_points_unstruct_real ( grid_id,       &
       ...
       integer, intent(in)  :: location                  !< [IN]  location, one of center/edge/vertex
       real, intent(in)     :: x_points_real(nbr_points) !< [IN]  longitudes of points in radians
       real, intent(in)     :: y_points_real(nbr_points) !< [IN]  latitudes of points in radians
       integer, intent(out) :: point_id                  !< [OUT] point identifier
     ...
     subroutine yac_fdef_points_unstruct_dble ( grid_id,       &
       ...
       integer, intent(in)  :: nbr_points                   !< [IN]  number of points
       integer, intent(in)  :: location                     !< [IN]  location, one of center/edge/vertex
       double precision, intent(in) :: x_points(nbr_points) !< [IN]  longitudes of points in radians
       double precision, intent(in) :: y_points(nbr_points) !< [IN]  latitudes of points in radians
       integer, intent(out) :: point_id                     !< [OUT] point identifier
  ...
  !----------------------------------------------------------------------
  !>
  !!   Fortran interface for getting interpolation information for
  !!   raw data exchange
  !!
  !----------------------------------------------------------------------
  ...
  interface
    ...
    !!         processes can extract once all information required to execute
    !!         the interpolation by hand.
    !! @remark The user is responsible for deallocating the memory associated
    ...
    subroutine yac_fget_raw_interp_weights_data ( &
      ...
      integer, allocatable, intent(out) :: src_field_buffer_sizes(:)
  ...
  end interface yac_fget_raw_async
  ...
  !>
  !!   Fortran interface for exchanging coupling fields
  !!   using raw data exchange
  !!
  !----------------------------------------------------------------------

  interface yac_fexchange_raw
     ...
     subroutine yac_fexchange_raw_dble_ptr ( send_field_id,      &
       ...
       type(yac_dble_ptr), intent (in)    :: &
         send_field(send_nbr_pointsets, collection_size)        !< [IN] send field
       type(yac_dble_ptr), intent (inout) :: &
         src_field_buffer(send_nbr_pointsets, collection_size)  !< [INOUT] source field buffer
       integer, intent (out)              :: send_info          !< [OUT] returned send info
       integer, intent (out)              :: recv_info          !< [OUT] returned recv info
       integer, intent (out)              :: ierror             !< [OUT] returned error
  ...
  end interface yac_fexchange_raw
 ...
 interface
    ...
    subroutine yac_fget_spmap_overwrite_config_bnd_circle ( &
      center_lon, center_lat, inc_angle, spmap_overwrite_config_id, &
      ...
      integer, intent(out)                   :: spmap_overwrite_config_id
      double precision, optional, intent(in) :: spread_distance
      double precision, optional, intent(in) :: max_search_distance
      integer, optional, intent(in)          :: weight_type
    ...
    subroutine yac_ffree_spmap_overwrite_config ( &
      ...
      integer, intent(in) :: spmap_overwrite_config_id
    ...
    subroutine yac_fadd_interp_stack_config_spmap(interp_stack_config_id, &
      ...
      integer, optional, intent(in) :: spmap_overwrite_config_ids(:)
 ...
 interface yac_fdef_couple
  ...
  interface yac_fget_field_is_defined
  ...
  end interface yac_fget_field_is_defined
  ...
  end interface yac_fget_field_source
