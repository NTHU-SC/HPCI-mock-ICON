 ...
 */
void
CdiGrid::getValuesAsPointer(double *xvals, double *yvals)
{
  if (hasBounds)
    {
...
{
  for (int i = 0; i < xsize; i++) xvalsF[i] = (float) xvalues[i];
  for (int i = 0; i < ysize; i++) yvalsF[i] = (float) yvalues[i];
}
