...
main(int argc, char *argv[])
{
  ...
  if (argc > 2) table_fname = argv[2];

  // compute checksums from data file
  size_t nvars;
  struct cksum_table *file_vars = cksum_stream(fname, &nvars);
  if (!file_vars) exit(EXIT_FAILURE);
  // check checksums from table file
  int retcode;
  {
    size_t num_ref_entries;
    struct cksum_table *ref_var_table = read_table(table_fname, &num_ref_entries);
    if (num_ref_entries == (size_t) -1)
      {
        free(file_vars);
        exit(EXIT_FAILURE);
      }
    retcode = compare_checksums(file_vars, nvars, "file", ref_var_table, num_ref_entries, "reference table");
    free(file_vars);
    free(ref_var_table);
  }
