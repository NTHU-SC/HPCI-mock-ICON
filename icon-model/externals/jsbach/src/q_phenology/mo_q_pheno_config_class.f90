...
CONTAINS
  ...
  SUBROUTINE Init_q_pheno_config(config)
    ...
    ! Set defaults
    active      = .TRUE.
    lrestart_cont = .FALSE.      ! TRUE: Continue although PHENO variables are missing in restart file
    ...
    nml_unit = position_nml('lnd_q_pheno_nml', nml_handler, STATUS=istat)
    IF (istat == POSITIONED) READ(nml_unit, lnd_q_pheno_nml)
    ! ----------------------------------------------------------------------------------------------------- !
