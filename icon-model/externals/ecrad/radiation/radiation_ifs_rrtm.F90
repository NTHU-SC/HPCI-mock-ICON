...
!      if (.true.) then
  ...
  subroutine planck_function_atmos(nlev,istartcol,iendcol, &
    ...
    ! interval
    !$ACC DATA  CREATE(planck_store, frac, ind) PRESENT(config, thermodynamics, PFRAC, &
    !$ACC   planck_hl)

    !$ACC PARALLEL DEFAULT(PRESENT) ASYNC(1)
    !$ACC LOOP GANG VECTOR COLLAPSE(2) PRIVATE(temperature)
    do jlev = 1,nlev+1
      do jcol = istartcol,iendcol
    ...
    if (config%i_solver_lw == ISolverSpartacus) then
      ...
      !$ACC PARALLEL DEFAULT(PRESENT) ASYNC(1)
      !$ACC LOOP GANG VECTOR TILE(4,1,32) PRIVATE(iband)
      do jcol = istartcol,iendcol
        do jlev = 1,nlev+1
          do jg = 1,config%n_g_lw
  ...
  subroutine planck_function_surf(istartcol, iendcol, config, temperature, PFRAC, &
    ...
    !$ACC PARALLEL DEFAULT(PRESENT) CREATE(planck_store, frac, ind) ASYNC(1)
