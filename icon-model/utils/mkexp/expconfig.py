...
class ExpConfig(ConfigObj):
    ...
    def __init__(self, experiment_config_name,
        ...
        setup_environment = ""
        if setup_config_name:
            map_config_name = get_config_name(
                [our.env_lib_path, our.env_lib_dir], 'MAP.config')
            if map_config_name:
                env_map = Map(map_config_name)
                setup_environment = env_map.get_environment(pre_config)

        experiment_type = pre_config.get('EXP_TYPE', '')
        feedback.debug(f"{experiment_type=}")
        # Empty environment should load default
        environment = pre_config.get('ENVIRONMENT', setup_environment)
        feedback.debug(f"{environment=}")
        # Options should always be treated as a list
        setup_options = pre_config.get('SETUP_OPTIONS', '')
        if not isinstance(setup_options, (list, tuple)):
            if setup_options:
                setup_options = [setup_options]
            else:
                setup_options = []
        exp_options = pre_config.get('EXP_OPTIONS', '')
        ...
        # Read default environment settings from library
        ...
        if environment:
            lib_config_name = get_config_name([our.env_lib_path,
                our.env_lib_dir], environment+'.config')
            if lib_config_name:
                pre_config.merge(ConfigObj(lib_config_name))
                split_shared_sections(pre_config)
                list_assign(pre_config)
                ...
                feedback.warning("cannot find environment config for '%s', "
                                 "using default environment only", environment)
        ...
        # Ensure consistent config setting if setup environment was used
        pre_config['ENVIRONMENT'] = environment or ""
