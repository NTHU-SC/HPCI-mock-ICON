...
* Allow list assignments (+=/-=/>=) and shared sections on command line
* Report file name and text line with config file syntax errors

Config
------

* Respect command line and user config when determining environment,
  allowing list assignments and shared sections
...
* Using ``QUEUE_TYPE`` instead of ``ENVIRONMENT`` is no longer supported
* Infer ``ENVIRONMENT`` from setup config, using ``environments/MAP.config``
* Always read default environment before loading specific environments

Tools
-----

* Debug (verbose) option for mkexp
* Import: simplify use

  * Ignore script errors by default
  * Suppress creation of jobs section by default
  * Allow duplicates in files section
  * Allow multiple land namelists (nested setups)

* setconfig: simplify use, add clean-up features

  * Make deletion of missing keys non-fatal for setconfig
  * Add option to remove empty sections
  * Allow replacement of header comment from file

* getconfig: handle command line consistent with mkexp
...
* Make experiment type, script dir, namelists section, and default config
  optional to allow use without standard library
