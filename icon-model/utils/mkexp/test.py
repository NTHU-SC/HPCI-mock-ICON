...
from environment_mapping import Map

# Uncomment only if needed badly
### feedback.setLevel(feedback.DEBUG) ###
...
def is_exec(filename):
    return bool(os.stat(filename).st_mode &
        (stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH))

def writeconfig(exp_id, string):
    filename = join("test", exp_id+".config")
...
def _config(text):
    return expconfig.ConfigObj(text.split("\n"))
...
class RunningTestCase(MkexpTestCase):
    ...
    def test_executable_flag(self):
        result = output(script("mkexp test0001.config "
            "jobs.nox...extends=run jobs.nox...exec=false"))
    ...
    def test_legacy_library(self):
        ...
        result = output(script("""
            mkexp -p legacy legacy/test0001.config EXP_OPTIONS=option1
        """))
        self.assertMultiLineEqual(expected, result)
...
class CommandLineTestCase(MkexpTestCase):
    ...
    def test_list_assign(self):
        command = script(
            "mkexp test0001.config EXP_OPTIONS=option2 EXP_OPTIONS+=option1"
            " 'EXP_OPTIONS>=2>3' EXP_OPTIONS-=option3"
        )
        expected = "default_output = .false."
    ...
    def test_shared_section(self):
        ignored = output(script(
            "mkexp test0001.config jobs.dummy...extends=run"
            " jobs.run.time_limit=4 jobs.dummy.time_limit=2"
            " jobs.dummy,run.time_limit=42"
        ))
        for (job, expected) in {
            "run": "time_limit = 42",
            "dummy": "time_limit = 42"
        }.items():
            result = readfile('test/experiments/test0001/test0001.'+job)
            self.assertIn(expected, result)
...
class EnvironmentTestCase(MkexpSimpleTestCase):
...
class ExpConfigConfigObjTestCase(unittest.TestCase):
...
class ExpConfigTestCase(MkexpTestCase):

    def setUp(self):
        self.exp_id = self.id().split('.')[-1]
        MkexpTestCase.setUp(self)

    def set_config(self, contents):
        self.config = expconfig.ExpConfig(writeconfig(self.exp_id, contents),
            config_roots=['test'])

    def test_default_config(self):
        self.set_config(u"""
        """)
        self.assertEqual(self.config["MODEL_DIR"], ".")
...
class EnvironmentMappingMapTextCase(unittest.TestCase):

    def setUp(self):
        self.map = Map("""
            [levante_gpu]
              host_fqdn = l*.lvt.dkrz.de
              gpu_enabled = true
            [levante]
              host_fqdn = l*.lvt.dkrz.de
            [mpimet]
              host_fqdn = mpipc*.mpimet.mpg.de, breeze*.mpimet.mpg.de
            [macOS]
              host_os = darwin*
        """.split("\n"))
    ...
    def test_no_match(self):
        self.assertIsNone(
            self.map.get_environment(_config("""
                host_os = linux-gnu
                host_fqdn = vinson
                #gpu_enabled = true
            """))
    ...
    def test_first_match(self):
        self.assertEqual(
            ...
            self.map.get_environment(_config("""
                host_os = linux-gnu
                host_fqdn = levante4.lvt.dkrz.de
                gpu_enabled = true
            """))
        )
    ...
    def test_match_list(self):
        self.assertEqual(
            ...
            self.map.get_environment(_config("""
                host_os = linux-gnu
                host_fqdn = breeze1.mpimet.mpg.de
                #gpu_enabled = true
            """))
        )
